...
description: "최대 50,000명의 사용자 또는 1000 RPS에 대한 고가용성, 데이터베이스 복제 및 인프라 크기 조정을 갖춘 GitLab Self-Managed 인스턴스를 배포합니다."
title: '참조 아키텍처: 최대 1000 RPS 또는 50,000명의 사용자'
---
...
이 페이지는 초당 1000개 요청(RPS)의 피크 로드, 실제 데이터를 기반으로 한 최대 50,000명의 사용자의 일반적인 피크 로드를 대상으로 설계된 GitLab 참조 아키텍처를 설명합니다.
...
- **Target load**:  API:  1000 RPS, 웹:  100 RPS, Git(Pull):  100 RPS, Git(Push):  20 RPS
...
|------------------------------------------|-------|-------------------------|------------------|---------------|-----------|
| 외부 로드 밸런서<sup>4</sup>       | 1     | 16 vCPU, 14.4GB 메모리 | `n1-highcpu-16`  | `c5.4xlarge`  | `F16s v2` |
| Consul<sup>2</sup>                       | 3     | 2 vCPU, 1.8GB 메모리   | `n1-highcpu-2`   | `c5.large`    | `F2s v2`  |
| PostgreSQL<sup>2</sup>                   | 3     | 32vCPU, 120GB 메모리  | `n1-standard-32` | `m5.8xlarge`  | `D32s v3` |
| PgBouncer<sup>2</sup>                    | 3     | 2 vCPU, 1.8GB 메모리   | `n1-highcpu-2`   | `c5.large`    | `F2s v2`  |
| 내부 로드 밸런서<sup>4</sup>       | 1     | 16 vCPU, 14.4GB 메모리 | `n1-highcpu-16`  | `c5.4xlarge`  | `F16s v2` |
...
| Gitaly<sup>6</sup><sup>7</sup>           | 3     | 64 vCPU, 240GB 메모리  | `n1-standard-64` | `m5.16xlarge` | `D64s v3` |
...
| GitLab Rails<sup>8</sup>                 | 12    | 32vCPU, 28.8GB 메모리 | `n1-highcpu-32`  | `c5.9xlarge`  | `F32s v2` |
...
1000 RPS / 50k 사용자 참조 아키텍처는 대부분의 일반적인 워크플로우를 수용하도록 설계되었습니다. GitLab은 다음 엔드포인트 처리량 목표에 대해 정기적으로 스모크 및 성능 테스트를 수행합니다:
...
GitLab 및 해당 구성 요소를 최대 1000 RPS 또는 50,000명의 사용자를 수용하도록 설정하려면:
...
1. 선택한 Linux 패키지를 [다운로드 및 설치](../../install/package/_index.md#supported-platforms)하세요. 선택한 운영 체제에 대해 GitLab 패키지 리포지토리만 추가하고 GitLab을 설치해야 합니다. 현재 설치와 동일한 버전 및 유형(Community 또는 Enterprise 에디션)을 선택합니다.
1. `/etc/gitlab/gitlab.rb`을(를) 편집하고 이전 섹션의 주 노드와 동일한 내용을 추가하고 `redis_master_node`을(를) `redis_replica_node`로 바꿉니다:
...
이 섹션은 새로운 Redis 큐 인스턴스를 설치하고 설정하는 곳입니다.
...
> [!note]
> Praefect는 3개 노드의 홀수 개 또는 이상으로 배포되어야 합니다. 이는 노드가 쿼럼의 일부로 투표할 수 있도록 하기 위함입니다.
...
- `<praefect_internal_token>`:  Gitaly 클러스터(Praefect) 내 복제 트래픽에 사용됩니다. 이는 `praefect_external_token`와는 다릅니다. Gitaly 클라이언트가 Gitaly Cluster (Praefect)의 내부 노드에 직접 액세스할 수 없어야 하기 때문입니다. 이는 데이터 손실로 이어질 수 있습니다.
...
|----------------------|-------------------------|-----------------|--------------|
| Webservice           | 308 vCPU<br/>385 GB 메모리 (요청)<br/>539 GB 메모리 (제한) | 11 x `n1-standard-32` | 11 x `c5.9xlarge` |
...
| PostgreSQL<sup>2</sup>                   | 3     | 32vCPU, 120GB 메모리 | `n1-standard-32` | `m5.8xlarge`  |
| PgBouncer<sup>2</sup>                    | 3     | 2 vCPU, 1.8GB 메모리  | `n1-highcpu-2`   | `c5.large`    |
| 내부 로드 밸런서<sup>4</sup>       | 1     | 16 vCPU, 14.4GB 메모리 | `n1-highcpu-16` | `c5.4xlarge`  |
...
| Gitaly<sup>6</sup><sup>7</sup>           | 3     | 64 vCPU, 240GB 메모리 | `n1-standard-64` | `m5.16xlarge` |
...
1000 RPS 또는 50,000명의 사용자를 위해 약 308개의 Puma 워커 총 개수를 권장하므로 최소 77개의 Webservice Pod를 실행하는 것이 좋습니다.
...
이러한 지원 배포에는 클라우드 공급자의 구현과 관련된 다양한 배포 및 [GitLab Shell](https://docs.gitlab.com/charts/charts/gitlab/gitlab-shell/)과 같은 GitLab 배포를 지원하는 배포가 포함됩니다.
...
1000 RPS 또는 50,000 사용자 참조 아키텍처 구성을 대상으로 하는 GitLab Helm 차트에 대한 예제는 [차트 프로젝트에서 찾을 수 있습니다](https://gitlab.com/gitlab-org/charts/gitlab/-/blob/master/examples/ref/50k.yaml).
