...
description: "최대 10,000명의 사용자 또는 200 RPS를 위한 GitLab Self-Managed 인스턴스 배포 및 확장(고가용성, 데이터베이스 복제, 인프라 크기 조정 포함)."
title: '참조 아키텍처: 최대 200 RPS 또는 10,000명의 사용자'
---
...
이 페이지는 초당 200개 요청(RPS)의 피크 로드, 최대 10,000명의 사용자(수동 및 자동)의 일반적인 피크 로드를 대상으로 설계된 GitLab 참조 아키텍처를 설명합니다. 이는 실제 데이터를 기반으로 합니다.
...
- **Target load**:  API:  200 RPS, 웹:  20 RPS, Git(풀):  20 RPS, Git(푸시):  4 RPS
...
| PostgreSQL<sup>2</sup>                   | 3     | 8 vCPU, 30GB 메모리    | `n1-standard-8`  | `m5.2xlarge`   | `D8s v3`  |
...
| Gitaly<sup>6</sup><sup>7</sup>           | 3     | 16vCPU, 60GB 메모리   | `n1-standard-16` | `m5.4xlarge`   | `D16s v3` |
...
| GitLab Rails<sup>8</sup>                 | 3     | 32vCPU, 28.8GB 메모리 | `n1-highcpu-32`  | `c5.9xlarge`   | `F32s v2` |
...
3. 평판이 좋은 타사 외부 PaaS Redis 솔루션에서 선택적으로 실행할 수 있습니다. 자세한 내용은 [자신의 Redis 인스턴스 제공](#provide-your-own-redis-instances) 과 [권장 클라우드 제공자 및 서비스](_index.md#recommended-cloud-providers-and-services)를 참조하세요.
   - Redis는 주로 단일 스레드이며 CPU 코어 증가의 이점을 크게 얻지 못합니다. 이 규모의 아키텍처의 경우 최적의 성능을 달성하기 위해 명시된 대로 별도의 캐시 및 지속 인스턴스를 보유하는 것이 강력히 권장됩니다.
4. HA 기능을 제공할 수 있는 평판이 좋은 타사 로드 밸런서 또는 서비스(LB PaaS)를 사용하여 실행하는 것이 권장됩니다. 크기 조정은 선택한 로드 밸런서와 네트워크 대역폭 같은 추가 요인에 따라 달라집니다. 자세한 정보는 [로드 밸런서](_index.md#load-balancers)를 참조하세요.
...
200 RPS / 10,000명 사용자 참조 아키텍처는 대부분의 일반적인 워크플로를 수용하도록 설계되었습니다. GitLab은 다음 엔드포인트 처리량 목표에 대해 정기적으로 스모크 및 성능 테스트를 수행합니다:
...
최대 200 RPS 또는 10,000명의 사용자를 수용하도록 GitLab 및 해당 구성 요소를 설정하려면 다음을 수행합니다:
...
1. 선택한 Linux 패키지를 [다운로드 및 설치](../../install/package/_index.md#supported-platforms)하세요. 선택한 운영 체제에 대해 GitLab 패키지 리포지토리만 추가하고 GitLab을 설치해야 합니다. 현재 설치와 동일한 버전 및 유형(Community 또는 Enterprise 에디션)을 선택합니다.
1. `/etc/gitlab/gitlab.rb`를 편집하고 `redis_master_node`를 `redis_replica_node`로 바꾸는 이전 섹션의 기본 노드와 동일한 내용을 추가합니다:
...
구성 지침은 [서비스 검색](../gitaly/praefect/configure.md#service-discovery)을(를) 참조하세요.
...
> [!note]
> 타사 설정을 통해 Praefect의 데이터베이스를 Geo를 사용하지 않는 한 주 [GitLab](#provide-your-own-postgresql-instance) 데이터베이스와 동일한 서버에 배치할 수 있습니다. Geo를 사용하는 경우에는 복제를 올바르게 처리하기 위해 별도의 데이터베이스 인스턴스가 필요합니다. 이 설정에서는 주 데이터베이스 설정의 사양을 변경할 필요가 없습니다. 영향이 미미하기 때문입니다.
...
|----------------------|-------------------------|-----------------|--------------|
| Webservice           | 80 vCPU<br/>100GB 메모리(요청)<br/>140GB 메모리(제한) | 3 x `n1-standard-32` | 3 x `c5.9xlarge` |
...
| PostgreSQL<sup>2</sup>                   | 3     | 8 vCPU, 30GB 메모리  | `n1-standard-8`  | `m5.2xlarge` |
...
| Gitaly<sup>6</sup><sup>7</sup>           | 3     | 16vCPU, 60GB 메모리 | `n1-standard-16` | `m5.4xlarge` |
...
200 RPS 또는 10,000 사용자의 경우, 총 Puma 워커 수를 약 80으로 권장하므로 차례로 최소 20개의 웹 서비스 Pod을 실행하는 것이 권장됩니다.
...
Gateway API 또는 Ingress 컨트롤러 포드를 Webservice 노드에 DaemonSet으로 배포하는 것도 권장됩니다. 이는 컨트롤러가 제공하는 웹 서비스 Pod과 동적으로 확장되고 더 큰 머신 유형이 일반적으로 가지는 더 높은 네트워크 대역폭을 활용할 수 있도록 하기 위함입니다.
...
200 RPS 또는 10,000 사용자 참조 아키텍처 구성을 대상으로 하는 GitLab Helm 차트의 예 [차트 프로젝트에서 찾을 수 있습니다](https://gitlab.com/gitlab-org/charts/gitlab/-/blob/master/examples/ref/10k.yaml).
