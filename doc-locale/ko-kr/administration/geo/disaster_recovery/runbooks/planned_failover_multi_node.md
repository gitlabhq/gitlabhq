...
## 다중 노드 구성을 위한 Geo 계획된 장애 조치 {#geo-planned-failover-for-a-multi-node-configuration}
...
이 런북은 1개의 세컨더리가 있는 다중 노드 Geo 사이트의 계획된 장애 조치를 안내합니다. 다음 [40 RPS / 2,000 사용자 참조 아키텍처](../../../reference_architectures/2k_users.md)를 가정합니다:
...
> [!note]
> 다음 단계를 따르기 전에 `root` 액세스 권한이 **세컨더리**에 있는지 확인하세요. Geo 복제본을 승격하고 장애 조치를 수행하는 자동화된 방법이 제공되지 않습니다.

**세컨더리** 사이트에서:

1. 오른쪽 위 모서리에서 **운영자**를 선택합니다.
1. 왼쪽 사이드바에서 **Geo** > **사이트**를 선택하여 상태를 확인하세요. 복제된 객체 (녹색으로 표시)는 100%에 가까워야 하며 실패 (빨간색으로 표시)가 없어야 합니다. 많은 비율의 객체가 복제되지 않은 경우(회색으로 표시), 사이트에 완료할 시간을 더 제공하는 것을 고려하세요.
...
복제 실패의 일반적인 원인은 **프라이머리** 사이트에서 누락된 데이터입니다. 백업에서 데이터를 복원하거나 누락된 데이터에 대한 참조를 제거하여 이러한 실패를 해결할 수 있습니다.

유지 보수 기간은 Geo 복제 및 확인이 완전히 완료될 때까지 끝나지 않습니다. 기간을 최대한 짧게 유지하려면 활성 사용 중에 이러한 프로세스가 최대 100%에 가까운지 확인해야 합니다.

**세컨더리** 사이트가 여전히 **프라이머리** 사이트에서 데이터를 복제하고 있는 경우 불필요한 데이터 손실을 방지하려면 다음 단계를 따르세요:

1. **프라이머리** 사이트에서 [maintenance mode](../../../maintenance_mode/_index.md) 를 활성화하고 모든 [background jobs](../../../maintenance_mode/_index.md#background-jobs)을 중지했는지 확인하세요.
...
1. 이 마지막 단계에서 **프라이머리** 사이트를 영구적으로 비활성화해야 합니다.
   ...
   - **프라이머리** 사이트에 SSH 액세스 권한이 없는 경우 머신을 오프라인 상태로 만들고 재부팅되지 않도록 방지합니다. 이를 수행할 수 있는 많은 방법이 있으므로 단일 권장 사항을 피합니다. 다음을 수행해야 할 수 있습니다:
...
1. **세컨더리** 사이트의 모든 Sidekiq, PostgreSQL 및 Gitaly 노드에 SSH로 접속하여 다음 명령 중 하나를 실행합니다:
   ...
   - 추가 확인 없이 세컨더리 사이트를 프라이머리로 승격하려면:
