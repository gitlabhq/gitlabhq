...
description: Debian에 Linux 패키지 설치
title: Debian에 Linux 패키지 설치
---
...
> [!note]
> [지원되는 플랫폼](_index.md#supported-platforms)을 확인하여 지원되는 배포판과 아키텍처의 전체 목록을 알아봅니다.
...
설치 전에 다음 선택적 환경 변수를 설정하여 GitLab 설치를 사용자 지정할 수 있습니다. **이 변수들은 첫 번째 설치 중에만 작동**하며 후속 재구성 실행에는 영향을 주지 않습니다. 기존 설치의 경우 `/etc/gitlab/initial_root_password`의 비밀번호를 사용하거나 [루트 비밀번호를 재설정](../../security/reset_user_password.md)합니다.
