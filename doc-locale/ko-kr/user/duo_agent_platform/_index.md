...
- GitLab 18.9 이하에서는 Agent Platform을 GitLab Duo Enterprise 추가 기능과 함께 사용할 수 없습니다. Agent Platform을 GitLab Duo Enterprise와 함께 사용하려면 GitLab 18.10 이상으로 업그레이드하세요.
- GitLab 버전에 따라:
...
- GitLab Self-Managed의 경우 [인스턴스를 구성](../../administration/gitlab_duo/configure/_index.md)하세요.
...
Free 티어에서 사용 가능한 기능을 위해서는 [GitLab Credits](../../subscriptions/gitlab_credits.md#for-the-free-tier) 구매가 필요합니다.
...
[Free 티어의 사용자](../../subscriptions/gitlab_credits.md#for-the-free-tier)의 경우 베타 및 실험 기능은 크레딧을 소비하지 않지만, 해당 기능에 액세스하려면 Monthly Commitment Pool의 크레딧이 필요합니다.

> [!warning]
> 기능이 일반적으로 사용 가능하게 되면 모든 GitLab 버전 및 모든 제공 서비스에서 해당 기능의 사용이 GitLab Credits를 소비하기 시작합니다. 베타 기능은 언제든지 사용 요금이 부과되는 일반 사용 가능으로 변경될 수 있습니다.

| 기능 | Free | Premium | Ultimate |
|---------|---|---|---|
| [Agent 도구 거버넌스](agents/tool-governance.md) <br /> 도구 수준의 승인 정책을 구성하여 실행 시 인적 승인으로 민감한 AI 에이전트 작업을 제어합니다. | {{< yes >}} | {{< yes >}} | {{< yes >}} |
| [AI 감사 이벤트 보고서](ai-audit-events.md) <br /> 규정 준수 및 거버넌스 목적으로 GitLab Duo AI 에이전트 활동의 통합 기록을 찾아보고 필터링합니다. | {{< no >}} | {{< yes >}} | {{< yes >}} |
| [사용자 지정 플로우](flows/custom.md) <br /> 여러 에이전트를 결합하여 비즈니스 문제를 해결합니다. | {{< yes >}} | {{< yes >}} | {{< yes >}} |
| [프로젝트 컨텍스트 초기화](onboarding.md#initialize-project-context) <br /> `AGENTS.md` 파일을 자동으로 생성하여 AI 에이전트에서 사용할 프로젝트 규칙을 문서화합니다. | {{< yes >}} | {{< yes >}} | {{< yes >}} |
| [CI/CD 설정 개선](onboarding.md#improve-cicd-setup) <br /> CI/CD 구성을 분석하고 제안된 개선 사항을 포함한 머지 리퀘스트를 생성합니다. | {{< yes >}} | {{< yes >}} | {{< yes >}} |
...
| [머지 충돌 해결](../project/merge_requests/conflicts.md#resolve-conflicts-with-gitlab-duo) <br /> 머지 충돌을 자동으로 분석하고 충돌 파일을 편집한 후 해결 커밋을 푸시합니다. | {{< no >}} | {{< yes >}} | {{< yes >}} |
