...
GitLab Flavored Markdown (GLFM)는 GitLab 사용자 인터페이스에서 텍스트 형식을 지정하는 마크업 언어입니다. GLFM:
...
1. 마크다운 미리보기를 지원하는 GitLab의 어딘가에 마크다운을 붙여 넣습니다. 예를 들어 이슈 또는 머지 리퀘스트 댓글이나 설명, 또는 새 마크다운 파일이 있습니다.
...
- [CommonMark 사양](https://spec.commonmark.org/current/)을 기반으로 하는 핵심 마크다운 기능.
- [GitHub Flavored Markdown](https://github.github.com/gfm/)의 확장 기능.
- GitLab을 위해 특별히 만들어진 확장 기능.

모든 표준 마크다운 형식은 GitLab에서 예상대로 작동합니다. 일부 표준 기능은 표준 사용에 영향을 주지 않으면서 추가 기능으로 확장됩니다.
...
| [제목](#headings)                 | [연결 가능한 제목 앵커](#heading-anchors) |
| [이미지](#images)                     | [내장 비디오](#videos) 및 [오디오](#audio) |
| [링크](#links)                       | [자동으로 URL 연결](#url-auto-linking) |

## 마크다운과 접근성 {#markdown-and-accessibility}

GitLab Flavored Markdown을 사용하면 디지털 콘텐츠를 만드는 것입니다. 이 콘텐츠는 청중이 최대한 접근할 수 있도록 해야 합니다. 다음 목록은 완전하지 않지만, GitLab Flavored Markdown 스타일 중 특히 주의를 기울일 필요가 있는 것에 대한 지침을 제공합니다.
...
테이블을 접근 가능하고 스캔 가능하게 유지하려면 테이블에 빈 셀이 없어야 합니다. 셀에 특별한 의미 있는 값이 없으면 "해당 없음"의 의미인 **N/A**를 입력하거나 **없음**을 입력하는 것을 고려하세요.
...
- 코드 스팬 (`` `code` ``), [추가 백틱 사용](#use-additional-backticks) 또는 이스케이프 지원 없음.
- 이모지(`:emoji:` 단축코드 및 사용자 지정 이모지).
- 자동 연결된 URL.
- [GitLab 특정 참조](#gitlab-specific-references)(예: `#123`, `@user` 및 `!456`).

굵게, 기울임꼴, 링크, 제목, 목록 및 기타 블록 수준 형식 지정과 같은 표준 Markdown 구문은 제목에서 처리되지 않습니다. 예를 들어 제목 `` **Merge request title** ``은 굵게 표시되지 않으며 별표로 표시됩니다.
...
GitLab은 모든 Markdown 제목에 자동으로 앵커를 추가하므로 링크할 수 있습니다.

마우스를 가져가면 해당 앵커로의 링크가 표시되어 제목 링크를 쉽게 복사하여 다른 곳에서 사용할 수 있습니다.

앵커는 다음 규칙에 따라 제목의 내용으로부터 생성됩니다:

1. 모든 텍스트가 소문자로 변환됩니다.
1. 문자, 숫자, 하이픈 및 언더스코어를 제외한 모든 문자가 제거됩니다.
...
스니펫에서 제목은 파일명에서 파생된 접두사도 가져오므로 여러 파일 간 앵커 충돌을 방지합니다. 예를 들어, `## TL;DR` 제목이 `README.md` 파일에 있으면 `#readme-tldr` 앵커를 가져오고, `#tldr` 대신 사용됩니다.
...
> 이 더 긴 줄은 위의 줄과 두 개의 줄 바꿈으로 분리되어 있으므로 *별도 단락*입니다.
>
> 이 줄도 별도 단락이지만... 이 줄들은 단일 줄 바꿈으로만 분리되어 있으므로 *줄 바꿈되지 않으며* 이전 줄을 따릅니다. *같은 단락*에서.
...
> 강조 또는 기울임꼴은 *별표* 또는 _밑줄_을 사용합니다.
>
> 강한 강조 또는 굵은 글자는 이중 **별표** 또는 **밑줄**을 사용합니다.
>
> **별표와 _밑줄_**을 결합한 강조입니다.
>
> 이중 물결표를 사용한 취소선입니다. ~~이를 지우세요.~~
...
단어의 일부만 강조하려면 여전히 별표를 사용하면 됩니다.
...
정렬된 목록의 경우 목록을 시작할 번호(예: `1.`)를 추가한 다음 각 줄의 시작 부분에 공백을 추가합니다. 첫 번째 번호 이후에는 사용하는 번호가 중요하지 않습니다. 정렬된 목록은 세로 순서로 자동으로 번호가 매겨지므로 같은 목록의 모든 항목에 대해 `1.`을 반복하는 것이 일반적입니다. `1.`이 아닌 숫자로 시작하면 해당 번호를 첫 번째 번호로 사용하고 거기서 계산합니다.
...
완료 및 미완료 외에 작업도 **적용 불가**할 수 있습니다. 이슈, 머지 리퀘스트, 에픽 또는 댓글에서 적용 불가 체크박스를 선택해도 아무 영향이 없습니다.
...
- 이 줄은 [제목 텍스트도 있는 링크](https://example.com "This link takes you to Example!")를 보여줍니다.
```
...
제목 앵커를 사용하여 페이지의 특정 섹션으로 링크합니다:

```markdown
- 이 줄은 [다른 Markdown 페이지의 섹션으로, `#`과 제목 앵커를 사용하여](permissions.md#project-permissions) 링크합니다.
- 이 줄은 [같은 페이지의 다른 섹션으로, `#`과 제목 앵커를 사용하여](#heading-anchors) 링크합니다.
```
...
- 이슈, 에픽 및 작업 항목을 `[work_item:123]` 구문으로 참조하는 옵션:
  - [도입됨](https://gitlab.com/gitlab-org/gitlab/-/issues/352861) GitLab 18.1에서 [기능 플래그](../administration/feature_flags/_index.md) 이름 `extensible_reference_filters`로. 기본적으로 비활성화되어 있습니다.
  - GitLab 18.2에서 [일반 사용 가능](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/197052)합니다. 기능 플래그 `extensible_reference_filters`가 제거되었습니다.
- 에픽을 `[epic:123]` 구문으로 참조하는 옵션이 GitLab 18.4에서 [도입](https://gitlab.com/gitlab-org/gitlab/-/issues/352864)되었습니다.
- 개인 스니펫을 참조하는 기능:
  - `personal_snippet_reference_filters`라는 이름의 [기능 플래그](../administration/feature_flags/_index.md)와 함께 GitLab 19.0에 [도입됨](https://gitlab.com/gitlab-org/gitlab/-/work_items/217306). 기본적으로 비활성화되어 있습니다.
  - GitLab 19.2에서 [일반적으로 사용 가능](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/241935)합니다. `personal_snippet_reference_filters` 기능 플래그가 제거되었습니다.

{{< /history >}}

GitLab Flavored Markdown은 GitLab 특정 참조를 렌더링합니다. 예를 들어 이슈, 커밋, 팀 구성원 또는 전체 프로젝트 팀을 참조할 수 있습니다. GitLab Flavored Markdown은 해당 참조를 링크로 변환하여 이들 사이를 탐색할 수 있습니다. 프로젝트에 대한 모든 참조는 프로젝트 이름이 아닌 **프로젝트 슬러그**를 사용해야 합니다.
...
| 스니펫 <sup>3</sup>                                                                              | `$123`                                                | `namespace/project$123`                        | `project$123`                      |
| 개인 스니펫 <sup>3</sup>                                                                     | `$123`                                                |                                                |                                    |
...
1. 레이블 또는 마일스톤의 경우 `namespace/project` 앞에 `/`를 추가하여 정확한 레이블 또는 마일스톤을 지정하고 모든 가능한 모호성을 제거합니다.
1. 스니펫 ID는 개인 및 프로젝트 스니펫 전체에서 고유하므로 주어진 ID는 항상 단일 스니펫을 식별합니다.
...
- 이슈에 대한 댓글: `"https://gitlab.com/gitlab-org/gitlab/-/issues/1234#note_101075757"`, 렌더링: `#1234 (comment 101075757)`
- 이슈 디자인 탭: `"https://gitlab.com/gitlab-org/gitlab/-/issues/1234/designs"`, 렌더링: `#1234 (designs)`
- 개별 디자인에 대한 링크: `"https://gitlab.com/gitlab-org/gitlab/-/issues/1234/designs/layout.png"`, 렌더링: `#1234[layout.png]`
...
- 에픽이 GitLab 18.1에서 일반 사용 가능합니다. `work_item_epics` 기능 플래그가 제거되었습니다.
...
요약에는 작업 항목 유형별로 해당하는 대로 참조된 항목의 **담당자**, **마일스톤** 및 **상태** 정보가 포함됩니다.
...
- [도입됨](https://gitlab.com/gitlab-org/gitlab/-/issues/29663) GitLab 17.3에서 [기능 플래그](../administration/feature_flags/_index.md) 이름 `comment_tooltips`로. 기본적으로 비활성화되어 있습니다.
...
GitLab Observability UI 대시보드 설명 및 댓글을 포함할 수 있습니다. 예를 들어 에픽, 이슈 및 MR 등이 있습니다.

Observability 대시보드 URL을 포함하려면:
...
- 세 번째 줄과 그 이후의 모든 줄에는 셀 값이 포함됩니다.
  - 마크다운에서 셀을 여러 줄로 구분할 수 **없습니다**. 한 줄로 유지해야 하지만 매우 길 수 있습니다. `<br>` 태그를 사용하여 필요한 경우 줄 바꿈을 강제할 수도 있습니다.
  - 셀 크기가 서로 일치하지 **않아도** 됩니다. 유연하지만 파이프 기호(`|`)로 구분해야 합니다.
  - 빈 셀이 **있을 수 있습니다**.
...
`items`의 모든 요소가 `fields`에 해당 값을 가져야 하는 것은 아닙니다.
...
> 인용문 중단.
>
> > 이 매우 긴 줄은 줄 바꿈할 때도 올바르게 인용됩니다. 이 줄이 모든 사람을 위해 실제로 줄 바꿈할 수 있을 만큼 길다는 것을 확인하기 위해 계속 쓰세요. 인용문에서 **마크다운**을 *사용*할 수도 있습니다.
...
LaTeX 문법으로 작성된 수학은 [KaTeX](https://github.com/KaTeX/KaTeX)로 렌더링됩니다. KaTeX는 LaTeX의 [부분집합](https://katex.org/docs/supported.html)만 지원합니다. 이 문법은 `:stem: latexmath`을 사용하는 AsciiDoc 위키 및 파일에서도 작동합니다. 자세한 내용은 [Asciidoctor 사용자 설명서](https://asciidoctor.org/docs/user-manual/#activating-stem-support)를 참조하세요.

악의적인 활동을 방지하기 위해 GitLab은 처음 1000개의 인라인 수학 인스턴스만 렌더링합니다. 이 한계는 [특정 그룹](../api/graphql/reference/_index.md#mutationgroupupdate) 또는 전체 [GitLab Self-Managed 인스턴스](../administration/instance_limits.md#math-rendering-limits)에 대해 비활성화할 수 있습니다.
...
이 태그 중 하나를 지원되는 콘텐츠 유형의 **설명** 필드의 줄에 추가합니다.
...
- 참고: 사용자가 문서를 대충 훑어볼 때도 참고해야 하는 정보:
...
```markdown
:monkey:가 주위를 맴돌고 싶고 :star2:를 추가하고 싶을 때가 있습니다.
...
전면 사항은 마크다운 문서의 시작 부분, 즉 본문 내용 앞에 포함되는 메타데이터입니다. 이 데이터는 [Jekyll](https://jekyllrb.com/docs/front-matter/), [Hugo](https://gohugo.io/content-management/front-matter/) 및 많은 다른 애플리케이션과 같은 정적 사이트 생성기에서 사용할 수 있습니다.
...
각 `::include`는 줄의 시작 부분에서 시작해야 하며 `file=`에 파일 또는 URL을 지정합니다. 지정된 파일(또는 URL)의 콘텐츠는 `::include`의 위치에 포함되고 나머지 마크다운과 함께 처리됩니다.

포함된 파일 내의 포함 지시문은 무시됩니다. 예를 들어, `file1`이 `file2`를 포함하고 `file2`가 `file3`을 포함하면 `file1`이 처리될 때 `file3`의 콘텐츠는 없습니다.
...
- [도입됨](https://gitlab.com/gitlab-org/gitlab/-/issues/14389) GitLab 18.2에서 [기능 플래그](../administration/feature_flags/_index.md) 이름 `markdown_placeholders`로. 기본적으로 비활성화되어 있습니다.
...
\**굵은 글씨 아님, 일부 별표 사이에 배치된 이탤릭 텍스트*\*
```
...
이 경우 `]`에 `&#93;`와 같은 동등한 HTML 엔터티를 사용해야 할 수 있습니다.
...
HTML 태그 내에서 마크다운을 사용할 수 있지만 마크다운을 포함하는 줄이 자체 줄로 분리된 경우에만 가능합니다.
...
<dl>
  ...
  **아주** 잘 작동하지는 *않습니다*. HTML 태그는 대부분의 경우 잘 작동합니다.
...
>   <dt>HTML의 마크다운</dt>
>
>   <b>아주</b> 잘 작동하지는 <em>않습니다</em>. HTML 태그는 대부분의 경우 잘 작동합니다.
>
>   </dd>
> </dl>
...
HTML의 [`<details>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/details) 및 [`<summary>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/summary) 태그를 사용하여 콘텐츠를 축소할 수 있습니다. 예를 들어, 긴 로그 파일을 축소하여 더 적은 화면 공간을 차지하도록 합니다.

```html
<details>
<summary>확장하려면 클릭</summary>

이러한 세부 정보는 확장될 때까지 <strong>숨겨진 상태</strong>로 <em>유지</em>됩니다.
...
이러한 세부 정보는 확장될 때까지 **숨겨진 상태**로 _remain_됩니다.
...
> <details>
> <summary><em>확장</em>하려면 클릭</summary>
>
> 이러한 세부 정보는 확장될 때까지 <b>숨겨진 상태</b>로 <em>유지</em>됩니다.
>
> <pre><code>로그 붙여넣기</code></pre>
>
> </details>
