...
이 API를 사용하여 프로젝트 [배지](../user/project/badges.md)를 관리합니다.

배지는 링크와 이미지 URL 모두에서 실시간으로 대체되는 자리표시자를 지원합니다. 사용 가능한 자리표시자는 다음과 같습니다:
...
## 프로젝트의 모든 배지 나열 {#list-all-badges-of-a-project}

프로젝트의 모든 배지를 나열합니다(그룹 배지 포함).
...
| `name`    | 문자열         | 아니요  | 반환할 배지의 이름(대소문자 구분). |
...
## 프로젝트의 배지 검색 {#retrieve-a-badge-of-a-project}
...
## 프로젝트에 대한 배지 생성 {#create-a-badge-for-a-project}
...
## 프로젝트의 배지 업데이트 {#update-a-badge-of-a-project}
...
## 프로젝트에서 배지 삭제 {#delete-a-badge-from-a-project}

프로젝트에서 배지를 삭제합니다. 그룹 배지를 삭제하려면 [그룹 배지 API](group_badges.md)를 대신 사용합니다.
...
## 프로젝트에서 배지 미리보기 {#preview-a-badge-from-a-project}

`link_url` 및 `image_url` 최종 URL이 자리표시자 보간을 해결한 후 어떻게 될지를 반환합니다.
