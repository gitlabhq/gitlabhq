...
description: アプリケーション用のカスタム機能フラグを作成および維持します。
title: 機能フラグ
---
...
<i class="fa-youtube-play" aria-hidden="true"></i>機能フラグの実際の動作例については、[機能フラグによるリスクの排除](https://www.youtube.com/watch?v=U9WqoK9froI)を参照してください。
...
新しい[機能フラグを作成する](#create-a-feature-flag)際、または作成後に既存の機能フラグを編集する際に、**デプロイ** > **機能フラグ**に移動し、**編集** ({{< icon name="pencil" >}}) を選択することで、機能フラグに戦略を追加できます。
...
> [!warning]
> **ランダム**を選択すると、個々のユーザーに対してアプリケーションの動作が一貫しなくなります。
...
**ユーザーID**に基づいて一貫性のある[ロールアウト率](#percent-rollout)は、同じ動作をします。ロールアウト率は、ユーザーのパーセンテージよりも柔軟性が高いため、ロールアウト率を使用する必要があります。

> [!warning]
> ユーザーのパーセンテージ戦略が選択されている場合、機能フラグを有効にするには、UnleashクライアントにユーザーIDを指定する**must**があります。以下の[Rubyの例](#ruby-application-example)を参照してください。
...
> [!warning]
> 対象ユーザー向けに機能フラグを有効にするには、UnleashクライアントにユーザーIDを指定する**must**があります。以下の[Rubyの例](#ruby-application-example)を参照してください。
...
これらのフィールドの意味は、時間の経過とともに変わる可能性があります。たとえば、**インスタンスID**は単一のトークンである場合も、**環境**に割り当てられた複数のトークンである場合もあります。また、**アプリケーション名**は、実行環境ではなくアプリケーションのバージョンを記述している可能性があります。
...
Unleashクライアントには、**ロールアウト率（ログインユーザー）**のロールアウト戦略または**ターゲットユーザー**のリストで使用するユーザーIDが付与されます。
...
- 1分あたり1リクエストで、約500クライアント（8 RPS）をサポートします。
- 15秒あたり1リクエストで、約125クライアントをサポートします。
...
スケーラビリティに関しては、GitLabインスタンスの仕様次第です。GitLab.comは、多数の同時リクエストを処理するために、高度にスケールしたアーキテクチャを使用しています。

しかしながら、[リファレンスアーキテクチャ](../administration/reference_architectures/_index.md#additional-workloads)に基づいて容量が不十分なGitLab Self-Managedインスタンスは、同等のパフォーマンスを発揮せず、機能フラグのトラフィックによってオーバーロードされることさえあります。GitLabユーザー_に加えて_、デプロイされたアプリケーションのユーザー数も考慮してください。
