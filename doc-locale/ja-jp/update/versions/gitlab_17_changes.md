...
Helmチャートのインストールの詳細については、[Helmチャート8.0アップグレードノート](https://docs.gitlab.com/charts/releases/8_0/)を参照してください。

## アップグレード時に注意すべきイシュー {#issues-to-be-aware-of-when-upgrading}

特定のGitLabバージョンからアップグレードする際は、アップグレードに影響を与える可能性のあるいくつかのイシューに注意してください。

### GitLab 16.11から {#from-gitlab-1611}
...
- Gitalyストレージは、この例にあるパスと同じパスを共有できなくなりました。
  ...
  ストレージがノードの`gitlab.rb`ファイルから削除された場合、それに関連付けられているすべてのプロジェクトは、GitLabデータベースでストレージを更新する必要があります。Railsコンソールを使用してストレージを更新できます。例: 
...
- PostgreSQL 13のバイナリが削除されました。
  ...
  アップグレードする前に、インストールで[PostgreSQL 14](https://docs.gitlab.com/omnibus/settings/database/#upgrade-packaged-postgresql-server)が使用されていることを確認してください。
...
### GitLab 17.0と以前のバージョンから {#from-gitlab-170-and-earlier}
...
### GitLab 17.1と以前のバージョンから {#from-gitlab-171-and-earlier}

GitLab 17.1と以前のバージョンからアップグレードする場合:

- GitLab Duoを使用しており、GitLab 17.2.3または以前のバージョンにアップグレードする場合は、以下の両方を行う必要があります:
  - ライセンスを再同期してください。
  - アップグレード後にサーバーを再起動します。
- GitLab Duoを使用しており、GitLab 17.2.4または以降のバージョンにアップグレードする場合は、以下のいずれかを行う必要があります:
  - ライセンスを再同期してください。
  - 24時間ごとに実行される次のスケジュールされたライセンス同期まで待ちます。

GitLab 17.2.4または以降のバージョンにアップグレードした後は、これらの手順は今後のアップグレードでは必要ありません。

詳細については、[イシュー480328](https://gitlab.com/gitlab-org/gitlab/-/issues/480328)を参照してください。

### GitLab 17.3から {#from-gitlab-173}
...
### GitLab 17.4から {#from-gitlab-174}

17.4から17.5にアップグレードする際のバックグラウンドジョブの移行失敗。
...
このエラーはいずれ自然に解消されますが、Railsコンソールで以下のスクリプトを実行してエラーの表示を停止することもできます:
...
## 17.11.0へのアップグレード {#upgrades-to-17110}
...
## 17.10.0へのアップグレード {#upgrades-to-17100}
...
## 17.9.0へのアップグレード {#upgrades-to-1790}
...
## 17.8.0へのアップグレード {#upgrades-to-1780}
...
## 17.7.0へのアップグレード {#upgrades-to-1770}
...
> [!note]
> GitLab 17.7にアップグレードする前に、[OpenSSL 3ガイド](https://docs.gitlab.com/omnibus/settings/ssl/openssl_3/)を使用して、外部インテグレーションの互換性を特定し、評価してください。
...
Linuxパッケージに同梱されているすべてのコンポーネントは、OpenSSL 3と互換性があります。したがって、GitLabパッケージに含まれていない、[「外部」](https://docs.gitlab.com/omnibus/settings/ssl/openssl_3/#identifying-external-integrations)のサービスとインテグレーションのみを確認する必要があります。
...
## 17.5.0へのアップグレード {#upgrades-to-1750}

> [!note]
> OpenSSL 3へのアップグレードはGitLab 17.7.0に延期されました。

- GitLab Runner分散キャッシュのS3オブジェクトストレージアクセスは、MinIOクライアントではなく、[Go言語向けAWS SDK v2](https://gitlab.com/gitlab-org/gitlab-runner/-/merge_requests/4987)で処理されるようになりました。`FF_USE_LEGACY_S3_CACHE_ADAPTER`[GitLab Runner機能フラグ](https://docs.gitlab.com/runner/configuration/feature-flags/)を`true`に設定することで、MinIOクライアントを再度有効にできます。
...
## 17.4.0へのアップグレード {#upgrades-to-1740}
...
## 17.3.0へのアップグレード {#upgrades-to-1730}
...
## 17.2.1へのアップグレード {#upgrades-to-1721}
...
## 17.1.0へのアップグレード {#upgrades-to-1710}
...
1. [タイムアウトした操作の残りを完了します](../package/package_troubleshooting.md#error-command-timed-out-after-3600s)。このプロセスの最後に、SidekiqとPumaが再起動すると、`500`エラーが修正されます。
...
## 17.0.0へのアップグレード {#upgrades-to-1700}
...
[共有のシークレットチャート](https://docs.gitlab.com/charts/charts/shared-secrets/)を無効にした場合は、[これらのシークレットを手動で作成](https://docs.gitlab.com/charts/releases/8_0/)する必要があります。
