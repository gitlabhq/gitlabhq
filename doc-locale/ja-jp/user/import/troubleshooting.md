...
GitLabに移行する際、次の問題が発生する可能性があります。
...
1. GitLab APIを使用して、影響を受けるプロジェクトの[インポート状態](../../api/project_import_export.md#retrieve-the-status-of-a-project-import)を確認します。
...
1. `RepositoryImportWorker`に関連するエントリと、[インポートステータスの確認](#check-import-status)からの相関IDを検索します。
...
[レビューログ](#review-logs)で収集した情報を、以下の一般的な問題と照らし合わせて確認します:
...
## サポートナレッジベース {#support-knowledge-base}

問題が引き続き発生する場合は、[GitLabサポートナレッジベース](https://support.gitlab.com/hc/en-us/)を参照してください。
