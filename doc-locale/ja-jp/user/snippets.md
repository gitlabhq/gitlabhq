...
- GitLabユーザーインターフェース。
- The [GitLab for VS Code拡張機能](../editor_extensions/visual_studio_code/projects.md#create-a-snippet)。
...
- パーソナルスニペット: どのプロジェクトからも独立しています。[表示レベル](public_access.md)をパブリックまたはプライベートに設定できます。
- プロジェクトスニペット: 特定のプロジェクトに関連付けられています。表示レベルをパブリック、またはプロジェクトメンバーのみに表示されるように設定できます。

GitLab.comでは、グループオーナーは[個人スニペットの作成を制限](group/manage.md#restrict-personal-snippets-for-enterprise-users)できます（[エンタープライズユーザー](enterprise_user/_index.md)向け）。

> [!note]
> 2019年7月以降、`Internal`表示レベル設定は、GitLab.com上の新規プロジェクト、グループ、スニペットで無効になっています。`Internal`表示レベル設定を使用している既存のスニペットは、この設定を保持します。変更の詳細については、[関連するイシュー](https://gitlab.com/gitlab-org/gitlab/-/issues/12388)を参照してください。
...
1. 作成するスニペットの種類を選択します。
   - パーソナルスニペットを作成するには、次のいずれかを実行します。
     ...
     - その他のページから作成する場合: 右上隅で、**新規作成**（{{< icon name="plus" >}}）を選択し、次に**新しいスニペット**を選択します。
     ...
     - [GitLab for VS Code拡張機能](../editor_extensions/visual_studio_code/_index.md)をインストールしている場合は、[`Gitlab: Create snippet`コマンド](https://marketplace.visualstudio.com/items?itemName=GitLab.gitlab-workflow#create-snippet)を使用します。
...
1. **ファイル**に、`example.rb`や`index.html`など、適切なファイル名と拡張子を入力します。適切な拡張子を持つファイル名の場合、[構文ハイライト](#filenames)で表示されます。ファイル名を追加しないと、既知の[コピーアンドペーストのバグ](https://gitlab.com/gitlab-org/gitlab/-/issues/22870)が発生する可能性があります。ファイル名を指定しない場合、GitLabが[名前を作成](#filenames)します。
...
スニペットは、指定されたファイル名と拡張子に基づいて、構文ハイライトをサポートします。ファイル名と拡張子を指定せずにスニペットを送信することもできますが、リポジトリにファイルとしてコンテンツを作成するには、有効なファイル名が必要です。
