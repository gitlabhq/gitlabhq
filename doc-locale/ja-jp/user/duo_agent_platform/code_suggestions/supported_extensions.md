...
| Eclipse                                                          | [Eclipse用GitLab](../../../editor_extensions/eclipse/setup.md) |
...
1. VS Codeで、**GitLab**の拡張機能設定を開きます:
   1. 上部のバーで、**Code** > **Settings** > **Extensions**に移動します。
   1. リストで**GitLab**を検索し、**管理**（{{< icon name="settings" >}}）を選択します。
   1. **Extension Settings**を選択します。
1. **User**設定で、**AI Assisted Code Suggestions: Enabled Supported Languages**という名前のセクションを探します。
1. 言語のコード提案を有効にするには、その言語のチェックボックスをオンにします。
1. 言語のコード提案を無効にするには、その言語のチェックボックスをオフにします。
...
- インストールして[GitLab for VS Code](../../../editor_extensions/visual_studio_code/_index.md)を有効にしました。
- [VS Code拡張機能の設定](https://gitlab.com/gitlab-org/gitlab-vscode-extension/#setup)の手順を完了し、GitLabアカウントにアクセスするための拡張機能を承認している。

これを行うには、次の手順を実行します:

1. [言語の識別子](https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#textDocumentItem)のリストで、目的の言語を見つけます。後の手順で、その言語の**識別子**が必要となります。
1. VS Codeで、**GitLab**の拡張機能設定を開きます:
   1. 上部のバーで、**Code** > **Settings** > **Extensions**に移動します。
   1. リストで**GitLab**を検索し、**管理**（{{< icon name="settings" >}}）を選択します。
   1. **Extension Settings**を選択します。
   1. **User**設定で、**GitLab › Ai Assisted Code Suggestions: Additional Languages**を選択し、**Add Item**を選択します。
...
- [JetBrains IDE用GitLabプラグイン](../../../editor_extensions/jetbrains_ide/_index.md)をインストールして有効にしている。
...
1. [言語の識別子](https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#textDocumentItem)のリストで、目的の言語を見つけます。後の手順で、言語の識別子が必要です。
1. IDEの上部のバーで、IDE名を選択し、次に**Settings**を選択します。
1. 左側のサイドバーで、**Tools** > **GitLab Duo**を選択します。
...
- [GitLab for Eclipse plugin](../../../editor_extensions/eclipse/_index.md)をインストールして有効にしている。
- [Eclipse設定](../../../editor_extensions/eclipse/setup.md)の手順を完了し、GitLabアカウントにアクセスするための拡張機能を承認している。
...
1. **Additional Languages**に、コンマで区切られた言語の識別子のリストを追加します。識別子に先頭のピリオドを追加しないでください。たとえば、`html`、`md`、`powershell`を使用します。
