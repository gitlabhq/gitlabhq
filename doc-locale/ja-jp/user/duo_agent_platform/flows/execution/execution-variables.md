...
description: フローを実行するジョブで、どの事前定義済み環境変数、IDトークン変数が利用可能で、どれが利用できないかについて学習します。
title: フロー実行変数
---
...
| `CI_WORKLOAD_REF` | フロー実行のワークロード参照（例: `refs/workloads/c727f70ba7f`）。これらは内部Git参照であり、パイプラインジョブが終了または失敗した場合に自動的に削除されます。|
...
宣言した[IDトークン](../../../../ci/secrets/id_token_authentication.md)は、フロージョブ内で環境変数として利用できます。各変数は、宣言したトークンの名前を使用します。

IDトークンは以下で宣言します:

- CI/CDを使用するフローの場合、`agent-config.yml`ファイル。詳細については、[IDトークンを設定](_index.md#configure-id-tokens)を参照してください。
- [カスタム外部エージェント](../../agents/external.md#authenticate-with-id-tokens)の設定。

例えば、`id_tokens`ブロックで`VAULT_ID_TOKEN`トークンを宣言すると、フローは`$VAULT_ID_TOKEN`を使用できます。
...
**設定** > **CI/CD** > **変数**で定義されたプロジェクト、グループ、またはインスタンスのカスタムCI/CD変数は利用できません。
