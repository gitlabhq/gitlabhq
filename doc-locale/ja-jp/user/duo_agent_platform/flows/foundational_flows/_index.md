...
- [デベロッパー](developer.md): イシューから実行可能なマージリクエストを作成するか、GitLab Duo Agentic Chatで異なるタスクを完了します。
...
- [セキュリティレビュー](security_review.md): マージリクエストの変更におけるビジネスロジックのセキュリティ脆弱性を検出します。
...
- [エージェントによる破壊的変更の解決](agentic-breaking-change-resolution.md): 依存のバンプマージリクエストにおける破壊的変更を自動的に解決します。

## デベロッパー向け {#for-developers}

GitLabに新しい基本フローを作成して追加する方法については、[基本フロー開発ガイド](../../../../development/ai_features/foundational_flows.md)を参照してください。
...
詳細については、[フロー実行を設定する](../execution.md)を参照してください。
...
- イメージレジストリの完全なイメージ参照がGitLab 19.0で[導入されました](https://gitlab.com/gitlab-org/gitlab/-/work_items/594208)。
...
1. **設定の変更**を選択します。
1. **フローの実行**で、**フロー実行を許可**チェックボックスと**基本フローを許可**チェックボックスを選択します。
1. オプション。**イメージレジストリ**テキストボックスに、次のいずれかを入力します:

   - そのレジストリからデフォルトイメージを使用するレジストリホスト名。
   - イメージを完全に上書きするための完全なイメージ参照（たとえば、`registry.example.com/group/project/image:tag`）。

   デフォルトの`registry.gitlab.com`を使用するには、空白のままにします。
1. **変更を保存**を選択します。
...
1. **フローの実行**で、**フロー実行を許可**チェックボックスと**基本フローを許可**チェックボックスを選択します。
1. トップレベルグループのみ、有効にしたい各基本フローのチェックボックスを選択します。
1. **変更を保存**を選択します。
