...
- 機能フラグ`ai_catalog_flows`はGitLab 18.8で[デフォルトで有効](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/216969)になりました。
- パイプラインイベントトリガーは、GitLab 18.9で`ai_flow_trigger_pipeline_hooks`という名前の[フラグ](../../../administration/feature_flags/_index.md)とともに[実験的機能](../../../policy/development_stages_support.md)として[導入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/212797)されました。デフォルトでは無効になっています。
...
- [ベータ版および実験的機能が有効になっている](../turn_on_off.md#turn-on-beta-and-experimental-features)こと。
...
- 閲覧できるのは以下のみです:
  - 管理プロジェクトのメンバーで、ゲスト、プランナー、レポーター、デベロッパー、メンテナー、またはオーナーロールを付与されているユーザー。
...
- フローは、そのプロジェクトのトップレベルグループで同時に有効になります。
- どのイベントがフローをトリガーするかを指定する[トリガー](../triggers/_index.md)を追加します。一部のトリガーイベントには、サービスアカウントユーザーが関与します。詳細については、[複合ID](../composite_identity.md)を参照してください。
...
1. オプション。**作業アイテム**を選択した場合は、**作業アイテムイベント設定**セクションの**トリガー条件**ドロップダウンリストから次のいずれかのアクションを1つ以上選択します: **作成日**、**ステータスの変更**。
1. **有効化**を選択します。
...
フローは、グループとプロジェクトの**AI** > **フロー**ページに表示されます。トップレベルグループ内の任意のプロジェクトのメンバーは、そのプロジェクトでフローを有効にできます。
...
フローは、プロジェクトの**AI** > **フロー**リストに表示されます。
...
> [!warning]
> サービスアカウントは、次の両方のプロジェクトにアクセスできます:
>
> - ユーザーがアクセスできる。
> - フローが追加されている。
...
デフォルトでは、カスタムフローは有効になっています。トップレベルグループまたはインスタンスで、それらをオンまたはオフにすることができます。

カスタムフローがオフの場合:

- ユーザーはカスタムフローを作成、有効化、無効化、または実行できません。
- 既存のカスタムフローは、プロジェクトの**AI** > **フロー** > **有効**に表示されなくなります。
- プロジェクトで作成されたカスタムフローは、**AI** > **フロー** > **管理中**に表示されますが、実行することはできません。
- [基本フローは引き続き利用可能です。](foundational_flows/_index.md)
...
1. **設定の変更**を選択します。
1. **Custom and external agents and flows**で、**Allow custom flows**チェックボックスをオンまたはオフにします。
1. **変更を保存**を選択します。

この設定は、グループ内のすべてのサブグループに引き継がれます。
...
インスタンスレベルの設定がオフの場合、グループレベルの設定でこれを上書きすることはできません。
