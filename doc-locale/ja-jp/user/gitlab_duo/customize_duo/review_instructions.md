...
マージリクエストのレビュー時にGitLab Duoが参照する標準を提供する、カスタムレビュー手順を作成します。

たとえば、GitLab DuoにRubyファイルはRubyのスタイル規約に、GoファイルはGoのスタイル規約に注目するよう指示できます。

> [!note]
> カスタムレビュー手順は、AIレビュアーへのヒントであり、強制されるポリシーではありません。GitLab Duoは、これらをレビューを形成するためのコンテキストとして使用しますが、すべての手順がすべての場合に適用されることを保証するものではありません。セキュリティ管理、コンプライアンス義務、または一貫した適用が必要なその他の要件のために、カスタム手順に依存しないでください。
...
- 指示は、強制ではなくヒントとして記述してください。指示は、レビューの動作を形成するヒントであり、GitLab Duoが従うことを義務付けられているポリシーではありません。「常にフラグを立てる」や「決して許可しない」のような表現は避けてください。この表現は、共同作業者をその動作が保証されていると誤解させる可能性があります。
...
`fileFilters`では、globパターンを使用して特定のファイルをターゲットにします。
