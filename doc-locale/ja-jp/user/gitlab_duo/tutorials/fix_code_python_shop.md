...
[GitLab Duo Chat（エージェント）](../../gitlab_duo_chat/agentic_chat.md)と[GitLab Duoコード提案](../../duo_agent_platform/code_suggestions/_index.md)を使用すると、次のことができます:
...
1. Web IDEを開きます:
   1. GitLab UIの上部のバーで、**検索または移動先**を選択して、プロジェクトを見つけます。
...
詳細については、[VS CodeでGitLab Duo Chatを使用する](../../gitlab_duo_chat/agentic_chat.md#use-gitlab-duo-chat-in-vs-code)を参照してください。
...
詳細については、[コード提案](../../duo_agent_platform/code_suggestions/_index.md)のドキュメントを参照してください。
...
1. 必要に応じて、生成されたコードをレビューして調整します。
   ...
   調整する内容をChatに質問する場合は、GitLab UIではなく、IDEの`setup.py`ファイルで質問してください。これにより、作成したばかりの`setup.py`ファイルなど、[作業中のコンテキスト](../../duo_agent_platform/context.md#gitlab-duo-chat-agentic)がChatに提供されます。
