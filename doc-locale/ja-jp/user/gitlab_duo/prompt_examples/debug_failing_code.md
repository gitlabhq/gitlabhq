...
description: 失敗したコードまたはテストのバグを特定して修正します。
title: 失敗したコードをデバッグする
---
...
- 時間の目安: 10～25分
- レベル: 初心者
- 前提条件: エラーメッセージまたは失敗したコードが利用可能、IDEでGitLab Duo Chatが利用可能

## 課題 {#the-challenge}

デバッグに手動で何時間も費やすことなく、バグやテスト失敗の根本原因を迅速に特定し、効果的な修正を実装します。

## アプローチ {#the-approach}

エラーを分析し、原因を特定し、GitLab Duo Chatを使用して修正を実装します。

### ステップ1: 分析 {#step-1-analyze}

エラーメッセージと関連コードをコピーします。次に、GitLab Duo Chatにエラーの説明を依頼します。
...
期待される結果: 特定された問題を修正する動作中のコード。

### ステップ3: 防止 {#step-3-prevent}

同様の問題を回避する方法についてガイダンスを求めます。
...
- まず、失敗している特定のコードセクションのみをコピーします。Chatがより多くのコンテキストを必要とする場合は、ファイルからより多くのコードを追加します。
- 根本的な問題を理解できるように、Chatに修正の説明を依頼します。
- 最初の提案がうまくいかない場合は、試してみたときに何が起こったかをChatに伝えます。
