...
## GitLab Duoをオンにロック {#lock-gitlab-duo-on}
...
GitLab Duoの可用性を**Always on**に設定すると、実験およびベータ機能は自動的にオンになりません。実験およびベータ機能を使用するには、[個別にオンにする](#turn-on-beta-and-experimental-features)必要があります。
...
トップレベルグループでGitLab Duoをロックするには:

1. 上部のバーで**検索または移動先**を選択し、トップレベルグループを見つけます。
1. 左側のサイドバーで、**設定** > **GitLab Duo**を選択します。
1. **設定の変更**を選択します。
1. **GitLab Duoの可用性**の下で、**Always on**を選択します。
1. **変更を保存**を選択します。

GitLab Duoは、すべてのサブグループとプロジェクトでロックされています。サブグループまたはプロジェクトのオーナーロールを持つユーザーは、GitLab Duoをオフにすることはできません。
...
GitLab Duoは、すべてのグループ、サブグループ、プロジェクトでロックされています。グループ、サブグループ、またはプロジェクトのオーナーロールを持つユーザーは、GitLab Duoをオフにすることはできません。
...
- 管理者アクセス権。
- [ネットワーク接続](../../administration/gitlab_duo/configure/_index.md)が有効です。
