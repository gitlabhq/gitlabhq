...
title: GitLab.comの設定
description: GitLab.comのインスタンスの設定。
---
...
これらの設定はGitLab.comで使用され、[GitLab.com](https://about.gitlab.com/pricing/)の顧客が利用できます。
...
> [!note]
> `git push`とGitLabプロジェクトのインポートは、Cloudflareを介した各リクエストにつき5 GiBに制限されています。ファイルアップロード以外のインポートは、この制限の影響を受けません。リポジトリの制限は、パブリックプロジェクトとプライベートプロジェクトの両方に適用されます。
...
- [API経由](../../api/project_import_export.md#export-a-project)。APIを使用して、エクスポートをAmazon S3などのストレージプラットフォームにプログラムでアップロードすることもできます。
...
|----------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------|-------------------------------|
| アーティファクトの最大サイズ（圧縮）                                              | 1 GB                                                                                                       | [アーティファクトの最大サイズ](../../administration/cicd/limits.md#maximum-artifacts-size)を参照してください。 |
...
| スケジュールされたパイプラインCron                                                          | `*/5 * * * *`                                                                                              | [パイプラインスケジュールの詳細設定](../../administration/cicd/limits.md#maximum-scheduled-pipeline-frequency)を参照してください。 |
| パイプライン毎の最大ジョブ数                                                | Freeプランの場合は`500`、すべてのトライアルプランの場合は`1000`、Premiumの場合は`1500`、Ultimateの場合は`2000`              | [パイプライン毎の最大ジョブ数](../../administration/cicd/limits.md#maximum-number-of-jobs-in-a-pipeline)を参照してください。 |
| アクティブなパイプラインの最大ジョブ数                                                 | Freeプランの場合は`500`、すべてのトライアルプランの場合は`1000`、Premiumの場合は`20000`、Ultimateの場合は`60000`            | [アクティブなパイプライン内のジョブ数](../../administration/cicd/limits.md#number-of-jobs-in-active-pipelines)を参照してください。 |
| プロジェクトに対するCI/CDサブスクリプションの最大数                                         | `2`                                                                                                        | [プロジェクトに対するCI/CDサブスクリプションの数](../../administration/cicd/limits.md#number-of-cicd-subscriptions-to-a-project)を参照してください。 |
| プロジェクト内のパイプライントリガーの最大数                                 | `25000`                                                                                                    | [パイプライントリガー数を制限する](../../administration/cicd/limits.md#limit-the-number-of-pipeline-triggers)を参照してください。 |
| プロジェクト内のパイプラインスケジュールの最大数                                           | Freeプランの場合は`10`、すべての有料プランの場合は`50`                                                                | [パイプラインスケジュール数](../../administration/cicd/limits.md#number-of-pipeline-schedules)を参照してください。 |
| 各スケジュールに対するパイプラインの最大数                                              | Freeプランの場合は`24`、すべての有料プランの場合は`288`                                                               | [1日にパイプラインスケジュールによって作成できるパイプラインの数を制限する](../../administration/cicd/limits.md#limit-the-number-of-pipelines-created-by-a-pipeline-schedule-each-day)を参照してください。 |
| 各セキュリティポリシープロジェクトに対して定義されたスケジュールルールの最大数        | すべての有料プランで無制限                                                                               | [セキュリティポリシープロジェクトに対して定義されたスケジュールルールの数](../../administration/cicd/limits.md#limit-the-number-of-schedule-rules-defined-for-security-policy-project)を参照してください。 |
| [パイプラインのアーカイブ](../../administration/settings/continuous_integration.md#archive-pipelines) | 1年                                                                                       | なし。2020年6月22日より前に作成されたジョブは、2020年9月22日以降にアーカイブされました。 |
| 各[単体テストレポート](../../ci/testing/unit_test_reports.md)の最大テストケース数 | `500000`                                                                                              | 無制限。                    |
| 登録済みのRunnerの最大数                                                       | Freeプラン: 各グループに対して`50`、各プロジェクトに対して`50`<br/>すべての有料プラン: 各グループに対して`1000`、各プロジェクトに対して`1000` | [スコープごとの登録Runner数](../../administration/cicd/limits.md#number-of-registered-runners-for-groups-and-projects)を参照してください。 |
| dotenv変数の制限                                                        | Freeプラン: `50`<br>Premiumプラン: `100`<br>Ultimateプラン: `150`                                             | [dotenv変数を制限する](../../administration/cicd/limits.md#limit-dotenv-variables)を参照してください。 |
| ダウンストリームパイプラインの最大トリガーレート（特定のプロジェクト、ユーザー、コミットの場合） | 毎分`350`                                                                                          | [ダウンストリームパイプラインの最大トリガーレート](../../administration/cicd/limits.md#limit-downstream-pipeline-trigger-rate)を参照してください。 |
| パイプラインの階層ツリー内のダウンストリームパイプラインの最大数            | `1000`                                                                                                     | [パイプライン階層サイズを制限する](../../administration/cicd/limits.md#limit-pipeline-hierarchy-size)を参照してください。 |
| プロジェクト、コミット、ユーザーごとのパイプライン作成リクエストの最大数（1分あたり） | `25`                                                                                                | [パイプライン作成のレート制限](../../administration/cicd/limits.md#pipeline-creation-rate-limits)を参照してください。 |
...
- 画像をダウンロードするためのGoogle Cloud Storage（[Googleがインターネットユーザーに提供するIP範囲](https://knowledge.workspace.google.com/admin/security/obtain-google-ip-address-ranges)を参照）またはGoogle Cloud Content Delivery Network。
...
詳細については、[GitLabホストRunner](../../ci/runners/_index.md)を参照してください。
...
### コンテナレジストリのホスト名リスト {#container-registry-hostname-list}

`registry.gitlab.com`から画像をプルする場合、これらの追加のホスト名を許可リストに追加してください:
...
[インポート元](../import/_index.md)（移行ツール）は、使用するGitLabによってデフォルトで利用できるものが異なります:
...
GitLab.comへのインポートでは、各トップレベルネームスペースの[プレースホルダユーザー](../import/mapping/post_migration_mapping.md#placeholder-users)数が制限されます。制限は、プランとシート数によって異なります。詳細については、[GitLab.comのプレースホルダーユーザー制限の表](../import/mapping/post_migration_mapping.md#placeholder-user-limits)を参照してください。
...
- GitLab 17.10の[GitLab.com](https://gitlab.com/gitlab-org/gitlab/-/issues/521970)で有効になりました。
...
- GitLab 19.0で[一般提供](https://gitlab.com/gitlab-org/gitlab/-/issues/537446)になりました。機能フラグ`merge_requests_diff_commits_limit`は削除されました。
...
- GitLab 18.0で[導入](https://gitlab.com/gitlab-org/gitlab/-/issues/506673)。
- GitLab 18.5でGitLab SaaS管理者の例外を[導入](https://gitlab.com/gitlab-org/gitlab/-/issues/560840)。

{{< /history >}}

GitLab.comでは、[トップレベルグループの作成](../../api/groups.md#create-a-group)はUIを介してのみ許可されています。管理者のみがAPIを使用してトップレベルグループを作成できます。
...
> [!note]
> 管理者のドキュメントについては、[レート制限](../../security/rate_limits.md)を参照してください。
...
| プロジェクトの認証されていないrawエンドポイントトラフィック               | 1分あたり800リクエスト        |
| IPアドレスからの認証されていないトラフィック                       | 毎分500件のリクエスト        |
| ユーザーの認証済みAPIトラフィック                             | 毎分2,000件のリクエスト      |
| ユーザーの認証済み非API HTTPトラフィック                    | 毎分1,000件のリクエスト      |
| ユーザーの認証されたGit HTTPSトラフィック                       | 1分あたり10,000リクエスト     |
| IPアドレスからの認証されていないGit HTTPSトラフィック             | 1分あたり15,000リクエスト     |
...
| GitLabパッケージホスティングシステムのIPアドレスに対するリクエスト（GitLab 16.11で導入） | 毎分3,000件のリクエスト      |
...
| RunnerジョブのRunnerトークンを使用したリクエスト (`/api/v4/jobs/request`) | 毎分2,000件のリクエスト      |
| Runnerジョブのトレースパッチリクエスト (`/api/v4/jobs/trace`) | 毎分200件のリクエスト      |
| Runnerジョブのトークンを使用したリクエスト (`/api/v4/jobs/*`)          | 毎分200件のリクエスト        |
| プロジェクト内の全プロジェクトメンバーをリスト表示                              | 毎分200件のリクエスト        |
...
- [プロジェクトのインポート/エクスポートレート制限](../project/settings/import_export.md#rate-limits)。
- [グループのインポート/エクスポートレート制限](../project/settings/import_export.md#rate-limits-1)。
...
- [ブロックされたリクエストへの応答に関するヘッダーのリスト](../../administration/settings/user_and_ip_rate_limits.md#response-headers)。
...
プロジェクト、グループ、スニペットには、[GitLab.comで無効](https://gitlab.com/gitlab-org/gitlab/-/issues/12388)になっている[内部表示レベル](../public_access.md#internal-projects-and-groups)設定があります。
...
GitLab.comには、`git+ssh`に対して[別のSSHポート](https://about.gitlab.com/blog/gitlab-dot-com-now-supports-an-alternate-git-plus-ssh-port/)を使用してアクセスできます。
...
トップレベルネームスペース内のWebhookが1分あたりに呼び出せる回数。ネームスペース内のすべてのプロジェクトおよびグループWebhookがこの制限を共有します。この制限は、プランとサブスクリプションのシート数によって異なります。
...
レート制限に達すると、ネームスペース内のすべてのWebhookは一時的に無効になり、次の1分で自動的に再有効化されます。

レート制限された場合、WebhookリストのWebhookの横にバッジが表示されます。Webhook編集ページには、ネームスペースの制限が1分あたりに何回超過したか、およびWebhookがいつ再有効化されるかを示すメッセージが表示されます。
...
- [並列ページデプロイ](../../administration/instance_limits.md#number-of-parallel-pages-deployments)。
