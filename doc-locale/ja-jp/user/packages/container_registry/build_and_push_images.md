...
description: コンテナイメージをビルドし、DockerコマンドまたはCI/CDパイプラインを使用してGitLabレジストリにプッシュします。
---
...
1. Dockerコマンドを実行して、ビルドまたはプッシュします。例: 
...
[GitLab CI/CD](../../../ci/_index.md)を使用して、コンテナイメージをレジストリからビルド、プッシュ、テスト、デプロイします。
...
独自のDocker-in-Docker（DinD）コンテナイメージを、レジストリまたは依存プロキシで使用できます。

DinDを使用して、CI/CDパイプラインからコンテナ化されたアプリケーションをビルド、テスト、デプロイします。
...
GitLabレジストリに保存されているイメージを使用する場合は、このアプローチを使用します。

あなたの`.gitlab-ci.yml`ファイルで:

- `image`と`services`をあなたのレジストリを指すように更新してください。
...
Docker Hubなどの外部レジストリからのイメージをキャッシュして、ビルドを高速化し、レート制限を回避するには、このアプローチを使用します。

あなたの`.gitlab-ci.yml`ファイルで:

- `image`と`services`を依存プロキシのプレフィックスを使用するように更新してください。
- サービス[エイリアス](../../../ci/services/_index.md#available-settings-for-services)を追加します。
...
RunnerでDinDを使用している場合、あなたの`.gitlab-ci.yml`ファイルは次のようになります:
...
- `$CI_REGISTRY_IMAGE`は、このプロジェクトに紐付けられたレジストリのアドレスに解決されます。
- `$IMAGE_TAG`は、レジストリアドレスと`$CI_COMMIT_REF_SLUG`（イメージタグ）を組み合わせたカスタム変数です。The [`$CI_COMMIT_REF_NAME`事前定義変数](../../../ci/variables/predefined_variables.md#predefined-variables)は、ブランチ名またはタグ名に解決され、スラッシュを含むことができます。イメージタグにはスラッシュを含めることはできません。代わりに`$CI_COMMIT_REF_SLUG`を使用してください。

次の例では、CI/CDタスクを4つのパイプラインステージに分割しており、そのうち2つのテストは並行して実行されます。

`build`は、コンテナレジストリに保存され、必要に応じてコンテナイメージをダウンロードする後続のパイプラインステージで使用されます。`main`ブランチにプッシュすると、パイプラインはイメージを`latest`としてタグ付けし、アプリケーション固有のデプロイスクリプトを使用してデプロイします:
