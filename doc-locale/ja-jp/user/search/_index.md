...
拡大するコードベースや組織で必要なものを見つけましょう。特定のコード作業アイテム、マージリクエスト、およびその他のコンテンツをプロジェクト全体で検索することで、時間を節約できます。ニーズに応じて、**基本的な検索**、[**高度な検索**](advanced_search.md)、[**完全一致コードの検索**](exact_code_search.md)の3種類の検索から選択できます。
...
| コミット        |     {{< no >}}      | {{< no >}}  | {{< yes >}} |
| 作業アイテム     |     {{< yes >}}     | {{< yes >}} | {{< yes >}} |
| マージリクエスト |     {{< yes >}}     | {{< yes >}} | {{< yes >}} |
| マイルストーン<sup>2</sup> |     {{< yes >}}     | {{< yes >}} | {{< yes >}} |
| プロジェクト       |     {{< yes >}}     | {{< yes >}} | {{< no >}} |
| ユーザー          |     {{< yes >}}     | {{< yes >}} | {{< yes >}} |
| Wiki          |     {{< no >}}      | {{< no >}}  | {{< yes >}} |
...
1. グローバルベーシック検索では、プロジェクトのマイルストーンのみが返され、グループのマイルストーンは返されません。
...
- プロジェクトまたはグループの[表示レベルを制限](../../administration/settings/visibility_and_access_controls.md#restrict-visibility-levels)します。パブリックプロジェクトが制限されている場合、匿名のグローバル検索はGitLabのサインインページにリダイレクトされます。
- **管理者**エリアでアクセスを制限します。

  1. 右上隅で、**管理者**を選択します。
  1. **設定** > **検索**を選択します。
  1. **高度な検索**を展開します。
  1. **未承認ユーザーに検索を使えるように許可する**チェックボックスをオフにします。
  1. **変更を保存**を選択します。
...
## デフォルトの検索スコープを設定します {#configure-a-default-search-scope}
...
1. 左サイドバーで、**設定** > **検索**を選択します。
1. **表示レベルとアクセス制御**を展開します。
1. **デフォルトの検索スコープ**ドロップダウンリストから、スコープを選択します。自動選択の場合は、**システムデフォルト(自動)**を選択します。
1. **変更を保存**を選択します。
...
- 最近表示したマージリクエスト
- 最近表示した作業アイテム
- プロジェクト内の作業アイテムの[GitLab Flavored Markdown参照](../markdown.md#gitlab-specific-references)
...
1. 検索ページの左サイドバーで、**アーカイブを含む**チェックボックスを選択します。
1. 左サイドバーで、**適用**を選択します。
...
1. コード検索ページの左サイドバーで、1つまたは複数の言語を選択します。
1. 左サイドバーで、**適用**を選択します。
