...
また、グループ内のプロジェクトのすべてのイシューとマージリクエスト、およびグループのアクティビティに関する分析を表示することもできます。
...
## グループを探す {#explore-groups}

すべての公開グループまたは内部グループを確認するには:

1. 上部のバーで、**検索または移動先**を選択します。
1. ドロップダウンから**検索**を選択します。
1. 左のサイドバーで**グループ**を選択します。
...
1. 上部のバーで、**検索または移動先**を選択します。
1. ドロップダウンから**すべてのグループを表示**を選択します。
...
- GitLab 18.2で[導入](https://gitlab.com/groups/gitlab-org/-/epics/13781)された**非アクティブ**タブは、[フラグ](../../administration/feature_flags/_index.md) `your_work_groups_vue`とともに提供されています。デフォルトでは無効になっています。
- GitLab 18.3で[一般提供](https://gitlab.com/gitlab-org/gitlab/-/issues/542790)になりました。機能フラグ`your_work_groups_vue`は削除されました。
- GitLab 18.8の**グループを探す**ページに[導入](https://gitlab.com/groups/gitlab-org/-/epics/13781)された**非アクティブ**タブは、[フラグ](../../administration/feature_flags/_index.md) `explore_groups_vue`とともに提供されています。デフォルトでは無効になっています。
- GitLab 18.11で[一般提供](https://gitlab.com/gitlab-org/gitlab/-/issues/585432)されました。機能フラグ`explore_groups_vue`は削除されました。

{{< /history >}}

グループは、削除保留中であるか、アーカイブされている場合に非アクティブになります。

すべての非アクティブなグループを表示するには:

1. 上部のバーで、**検索または移動先**を選択します。
1. ドロップダウンからいずれかを選択します:
   - メンバーであるグループをフィルタリングするには、**すべてのグループを表示**。
   - 公開または内部グループをフィルタリングするには、**検索**。
1. **無効**タブを選択します。

リスト内の各非アクティブなグループには、グループがアーカイブされているか削除保留中であるかを示すバッジが表示されます。

グループが削除保留中の場合、リストには以下も表示されます:

- グループが最終削除のためにスケジュールされている時間。
- **復元**アクション。グループを復元すると:
  - **削除保留中**ラベルは削除されます。グループは削除のためにスケジュールされなくなります。
  - グループは**非アクティブ**タブから削除されます。
...
1. 右上の**新規作成** ({{< icon name="plus" >}}) と**新しいグループ**を選択します。
1. **グループを作成**を選択します。
...
<i class="fa-youtube-play" aria-hidden="true"></i>グループの詳細については、[GitLab Namespaces (users, groups and subgroups)](https://youtu.be/r0sJgjR2f5A)（GitLabネームスペース（ユーザー、グループ、サブグループ））をご覧ください。
...
- GitLab 16.0で[GitLab.comGitLab、GitLab Self-Managed、およびGitLab Dedicatedで有効化されました](https://gitlab.com/gitlab-org/gitlab/-/issues/389557)。PremiumおよびUltimateのみです。
- GitLab 18.0で、GitLab PremiumからGitLab Freeに[移行](https://gitlab.com/groups/gitlab-org/-/epics/17208)しました。

{{< /history >}}

デフォルトでは、初めてグループを削除すると、削除保留状態になります。グループを再度削除すると、すぐに削除されます。

前提条件: 

- グループのオーナーロールが必要です。
- グループにプロジェクトが含まれている場合、オーナーは[プロジェクトの削除が許可されている](../../administration/settings/visibility_and_access_controls.md#restrict-project-deletion-to-administrators)必要があります。
...
1. グループの概要ページの右上隅で、**アクション**（{{< icon name="ellipsis_v" >}}）を選択します。
1. **削除**を選択します。
1. 確認ダイアログで、グループのパスを入力し、**はい、グループを削除します**を選択します。

グループダッシュボードからグループを削除することもできます。

1. トップバーで、**検索または移動先** > **すべてのグループを表示**を選択します。
1. 削除するグループの（{{< icon name="ellipsis_v" >}}）を選択します。
1. **削除**を選択します。
1. 確認ダイアログで、グループのパスを入力し、**はい、グループを削除します**を選択します。

このアクションにより、グループの削除をスケジュールするバックグラウンドジョブが追加されます。GitLab.comでは、グループは30日後に削除されます。GitLab Self-Managedでは、[インスタンスの設定](../../administration/settings/visibility_and_access_controls.md#deletion-protection)から保持期間を変更できます。

グループが削除のためにスケジュールされると、スケジュールされたCI/CDパイプラインは停止します。

グループの削除をスケジュールしたユーザーが、削除が行われる前にグループへのアクセスを失った場合（例えば、グループを離脱する、ロールがダウングレードされる、またはグループからBANされるなど）、削除ジョブは代わりにグループを復元するため、グループは削除のためにスケジュールされなくなります。

> [!warning]
> グループ削除をスケジュールしたユーザーが、ジョブの実行前にオーナーロールまたは管理者アクセスを再取得した場合、ジョブはグループを完全に削除します。
...
- グループのオーナーロールが必要です。
- グループの[削除をスケジュールしました](#schedule-a-group-for-deletion)。

削除がスケジュールされているグループを完全に削除するには:
...
削除がスケジュールされているグループを復元するには:

1. 上部のバーで、**検索または移動先**を選択して、グループを見つけます。
1. グループの概要ページの右上隅で、**アクション**（{{< icon name="ellipsis_v" >}}）を選択します。
1. **復元**を選択します。

## アクションメニューでグループを管理する {#manage-groups-with-the-actions-menu}

すべてのグループのリストを表示し、**アクション**メニューで管理できます。

前提条件: 

- グループを離れること以外の利用可能なすべてのアクションには、オーナーロールが必要です。

グループの**アクション**メニューにアクセスするには:

1. トップバーで、**検索または移動先** > **すべてのグループを表示**を選択します。
1. **グループ**ページで、自分のグループを見つけて**アクション**メニュー ({{< icon name="ellipsis_v" >}}) を選択します。
1. アクションを選択します。

グループの状態に応じて、以下のアクションが利用可能です:

| グループの状態      | 利用可能なアクション       |
| ---------------- | ----------------------- |
| アクティブ           | **編集**、**アーカイブ**、**転送**、**このグループから離脱する**、**削除** |
| アーカイブ済み         | **アーカイブ解除**、**このグループから離脱する**、**削除** |
| 削除保留中 | **復元**、**このグループから離脱する** |
...
1. **検索**テキストボックスに、参加したいグループの名前を入力します。
1. 検索結果で、グループの名前を選択します。
1. グループページで、グループ名の下にある**アクセスをリクエストする**を選択します。
...
> [!note]
> グループメンバーの**ソース**の表示は一貫性がない場合があります。詳細については、[イシュー23020](https://gitlab.com/gitlab-org/gitlab/-/issues/23020)を参照してください。

すべてのネームスペースメンバー（およびそれぞれの占有シート）を表示するには、トップレベルのネームスペースで、[**使用量割り当て**ページを表示](../../subscriptions/manage_seats.md#view-seat-usage)します。
...
- GitLab Self-Managedインスタンスの場合:
  - [新規ユーザーアカウントの作成が無効になっている](../../administration/settings/sign_up_restrictions.md#disable-new-user-account-creation)場合、管理者がユーザーを追加する必要があります。
...
1. オプション。**アクセス有効期限**に、日付を入力または選択します。その日付以降、ユーザーはプロジェクトにアクセスできなくなります。
   ...
   > [!warning]
   > メンテナーは、自分のアクセス有効期限を延長する機能を含め、ロールの有効期限が切れるまで完全な権限を持ちます。
...
1. オプション。**メンバーを削除**確認ダイアログで、次のチェックボックスの一方または両方を選択します。
   ...
   - **関連するイシューとマージリクエストからもこのユーザーの割り当てを解除する**
1. **メンバーを削除**を選択します。
...
- プロジェクトの作成中に、ドロップダウンリストからグループを選択します。
  ...
  ![グループを選択する](img/select_group_dropdown_v13_10.png)
