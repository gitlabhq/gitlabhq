...
1. 上部のバーで、**検索または移動先**を選択し、あなたのグループを見つけてください。
1. **設定** > **一般**を選択します。
...
グループのオーナーを変更できます。各グループには、オーナーロールを持つ少なくとも1人のメンバーが常に必要です。内部ユーザー（「ボット」）とサービスアカウントは、グループの唯一のオーナーになることはできません。
...
- [導入](https://gitlab.com/gitlab-org/gitlab/-/issues/499732) GitLab 17.9のプロジェクトおよびグループアクセストークンWebhookへの60日および30日のトリガー[フラグ付き](../../administration/feature_flags/_index.md) `extended_expiry_webhook_execution_setting`という名前が付けられています。デフォルトでは無効になっています。
...
これらのWebhookがトークンの有効期限が切れる60日前と30日前にもトリガーされるように設定するには:
...
## エンタープライズユーザーの個人スニペットを制限する {#restrict-personal-snippets-for-enterprise-users}
...
グループ内の[enterprise users](../enterprise_user/_index.md)が個人ネームスペースに[スニペット](../snippets.md)を作成できないようにすることができます。無効にすると、エンタープライズユーザーは引き続きプロジェクトスニペットを作成できます。

前提条件: 

- グループのオーナーロールが必要です。

エンタープライズユーザーの個人スニペットを制限するには:
...
GitLab.comでは、ユーザーが新しいメンバーをトップレベルグループのサブグループまたはプロジェクトに招待する機能を削除できます。これにより、グループのオーナーも招待を送信できなくなります。ユーザーを再度招待するには、この設定を無効にする必要があります。
...
1. 上部のバーで、**検索または移動先**を選択し、あなたのグループまたはサブグループを見つけてください。
1. **管理** > **メンバー**を選択します。
...
### グループのユーザー制限を設定する {#set-a-user-cap-for-a-group}
...
1. 上部のバーで、**検索または移動先**を選択し、あなたのグループを見つけてください。トップレベルグループでのみキャパシティを設定できます。
1. **設定** > **一般**を選択します。
1. **権限とグループ機能**を展開します。
...
グループ、サブグループ、またはプロジェクトが外部で共有されている場合にユーザー上限が適用されるようにするには、[最上位ネームスペースでのみグループ共有を制限する](../../user/project/members/sharing_projects_groups.md#prevent-inviting-groups-outside-the-group-hierarchy)。トップレベルでネームスペースを制限することで、同じネームスペースでの招待の許可、外部共有からの新しいユーザー（シート）の追加の防止が可能となります。
...
- GitLab 18.7で、ユーザーデータの表示を許可するネームスペース設定が`usage_billing_dev`[フラグ](../../administration/feature_flags/_index.md)とともに[導入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/215371)されました。
