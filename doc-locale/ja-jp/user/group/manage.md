...
> [!warning]
> プロジェクトを移動できないため、[Container Registry](../packages/container_registry/_index.md)タグを持つプロジェクトがネームスペースに含まれている場合、そのネームスペースの名前を変更することはできません。
>
...
- GitLab 18.9で[一般提供](https://gitlab.com/gitlab-org/gitlab/-/issues/526771)。機能フラグ`archive_group`は削除されました。
...
アーカイブされたグループからのイシューは、[issue 585677](https://gitlab.com/gitlab-org/gitlab/-/work_items/585677)が解決されるまで、イシューボードに表示され続けます。
...
- ターゲットグループが、グループ内のいずれかのプロジェクト、またはそのサブグループに、[NPM](../packages/npm_registry/_index.md#naming-convention)命名規則に従うnpmパッケージが存在するトップレベルグループの場合、転送は失敗します。
...
- GitLab 18.9で[一般提供](https://gitlab.com/gitlab-org/gitlab/-/work_items/583564)。機能フラグ`allow_personal_snippets_setting`は削除されました。

{{< /history >}}

グループ内の[エンタープライズユーザー](../enterprise_user/_index.md)が個人のネームスペースに[スニペット](../snippets.md)を作成できないようにすることができます。無効にすると、エンタープライズユーザーは引き続きプロジェクトスニペットを作成できます。
...
> [!note]
> [共有](../project/members/sharing_projects_groups.md)や[移行](../import/_index.md)などの機能を使用すると、これらのサブグループやプロジェクトへのアクセスが許可される場合があります。
...
> [!note]
> 保留中のメンバーがいるグループに対して[ユーザー上限](#user-cap-for-groups)が有効になっている場合、制限付きアクセスを有効にすると、保留中のすべてのメンバーがグループから自動的に削除されます。
...
最小アクセスロールを持つユーザーは、認証してグループにアクセスできますが、[制限された権限](../../user/permissions.md#users-with-minimal-access)があります。コード行が利用可能になると、意図したアクセスレベルにプロモートできます。請求対象ロールを持つ既存のユーザーは、この動作の影響を受けません。
...
グループ、サブグループ、またはプロジェクトが外部で共有されている場合にユーザーキャパシティが適用されるようにするには、[トップレベルネームスペースでのみグループ共有を制限](../../user/project/members/sharing_projects_groups.md#prevent-inviting-groups-outside-the-group-hierarchy)します。トップレベルでネームスペースを制限することで、同じネームスペースでの招待の許可、外部共有からの新しいユーザー（シート）の追加の防止が可能となります。
...
[GitLabクレジットダッシュボード](../../subscriptions/gitlab_credits.md#gitlab-credits-dashboard)にユーザーデータを表示するには:
