...
> [!warning]
> [コンテナレジストリタグ](../packages/container_registry/_index.md)を持つプロジェクトが含まれている場合、プロジェクトを移動できないため、ネームスペースの名前を変更することはできません。
>
...
- **無効**タブが**あなたの作業**ページと**検索**ページに表示されます
- 別のネームスペースに転送できません
...
**あなたの作業**リストビューから直接グループをアーカイブするには、次の手順を実行します:
...
**あなたの作業**リストビューから直接グループをアーカイブ解除するには、次の手順を実行します:
...
- GitLab 18.5で`allow_personal_snippets_setting`[フラグ](../../administration/feature_flags/_index.md)とともに[導入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/200575)されました。デフォルトでは無効になっています。
...
> [!note]
> [共有](../project/members/sharing_projects_groups.md)や[移行](../import/_index.md)などの機能を使用すると、これらのサブグループおよびプロジェクトへのアクセスが許可される場合があります。
...
> [!note]
> 保留中のメンバーがいるグループに対して[ユーザーキャパシティ](#user-cap-for-groups)が有効になっている場合、制限付きアクセスを有効にすると、保留中のメンバーはすべてグループから自動的に削除されます。
...
- [GitLab](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/225777) 18.10でデフォルトで有効化されました。
...
最小アクセスロールを持つユーザーは、認証してグループにアクセスできますが、[制限された権限](../permissions.md#users-with-minimal-access)があります。コード行が利用可能になると、意図したアクセスレベルにプロモートできます。請求対象ロールを持つ既存のユーザーは、この動作の影響を受けません。

[コード行の使用状況を表示](../../subscriptions/manage_seats.md#view-seat-usage)し、最小アクセスを持つユーザーを管理できます。
...
- シート数は、次の場合はまだ超過する可能性があります: 
  - SAML、SCIM、またはLDAPを使用して新しいメンバーを追加し、サブスクリプションのシート数を超えた場合。[最小アクセスのフォールバック](#provisioning-behavior-with-saml-scim-and-ldap)機能が有効になっている場合、ユーザーはブロックされる代わりに最小アクセスを割り当てられます。
...
### 休眠状態のエンタープライズユーザー再アクティブ化 {#dormant-enterprise-user-reactivation}

制限付きアクセスが有効で利用可能なシートがない場合、休眠状態の[エンタープライズユーザー](../enterprise_user/_index.md)が再度サインインしようとすると、再アクティブ化される代わりに[pending approval](#approve-pending-members-for-a-group)に設定されます。既存のグループメンバーシップとプロジェクトメンバーシップは保持されます。グループオーナーは、シートが利用可能になったときにユーザーを承認できます。

非エンタープライズ休眠メンバーは、非アクティブ化される代わりにグループメンバーシップが削除されます。SAML、SCIM、またはLDAP同期を通じて再参加すると、[プロビジョニング](#provisioning-behavior-with-saml-scim-and-ldap)動作が適用され、利用可能なシートがない場合は最小アクセスロールが付与されます。

詳細については、[休眠メンバーの自動削除](moderate_users.md#automatically-remove-dormant-members)を参照してください。
...
GitLab Self-Managedの[ユーザーキャパシティ](../../administration/settings/sign_up_restrictions.md#user-cap)については、こちらをご覧ください。
...
グループ、サブグループ、またはプロジェクトが外部で共有されている場合にユーザーキャパシティが適用されるようにするには、[トップレベルネームスペースでのみグループ共有を制限](../project/members/sharing_projects_groups.md#prevent-inviting-groups-outside-the-group-hierarchy)します。トップレベルでネームスペースを制限することで、同じネームスペースでの招待の許可、外部共有からの新しいユーザー（シート）の追加の防止が可能となります。
...
- GitLab 18.7で、ユーザーデータの表示を許可するネームスペース設定が`usage_billing_dev`[フラグ](../../administration/feature_flags/_index.md)とともに[導入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/215371)されました。[デフォルトで有効](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/215714)。
