...
- **非アクティブ**タブが**あなたの作業**ページと**検索**ページに表示されます
- 別のネームスペースに転送できません
...
- グループとそのサブグループおよびプロジェクトは、グループリストの**アクティブ**または**メンバー**タブに戻ります。
...
1. 左側のサイドバーで、**検索または移動先**を選択します。[新しいナビゲーションをオン](../interface_redesign.md#turn-new-navigation-on-or-off)にしている場合、このフィールドは上部のバーにあります。
1. **すべてのグループを表示**を選択します。
1. **非アクティブ**タブで、アーカイブ解除するグループを見つけて({{< icon name="ellipsis_v" >}})を選択します。
1. **アーカイブ解除**を選択します。
...
- GitLab 17.9で、`extended_expiry_webhook_execution_setting`という[フラグ](../../administration/feature_flags/_index.md)とともに、60日と30日のトリガーがプロジェクトとグループアクセストークンWebhookに[導入](https://gitlab.com/gitlab-org/gitlab/-/issues/499732)されました。デフォルトでは無効になっています。
...
GitLabは、グループトークンの有効期限が切れる前に、複数の[有効期限メール](settings/group_access_tokens.md#group-access-token-expiry-emails)を送信し、関連する[Webhook](../project/integrations/webhook_events.md#project-and-group-access-token-events)をトリガーします。デフォルトでは、これらのWebhookは、トークンの有効期限が切れる7日前にトリガーされます。
...
1. **設定** > **一般**を選択します。
1. **権限とグループ機能**セクションを展開します。
1. **Disable Group/Project members invitation**（グループ/プロジェクトメンバーの招待を無効にする）を選択します。
1. **変更を保存**を選択します。
...
1. 左側のサイドバーで、**検索または移動先**を選択して、グループまたはサブグループを見つけます。[新しいナビゲーションをオン](../interface_redesign.md#turn-new-navigation-on-or-off)にしている場合、このフィールドは上部のバーにあります。
1. **管理** > **メンバー**を選択します。
...
1. 左側のサイドバーで、**検索または移動先**を選択して、グループを見つけます。[新しいナビゲーションをオン](../interface_redesign.md#turn-new-navigation-on-or-off)にしている場合、このフィールドは上部のバーにあります。トップレベルグループでのみキャパシティを設定できます。
1. **設定** > **一般**を選択します。
1. **権限とグループ機能**を展開します。
...
テンプレートソースとしてグループ内のプロジェクトのみを選択できます。これにはグループと共有されているプロジェクトが含まれますが、構成されているグループのサブグループまたは親グループ内のプロジェクトは**excludes**（除外）されます。
