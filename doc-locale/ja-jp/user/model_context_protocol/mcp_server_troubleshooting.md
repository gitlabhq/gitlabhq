...
description: GitLab MCPサーバーで発生する一般的な問題のトラブルシューティングを行う。
title: GitLab MCPサーバーのトラブルシューティング
---
...
GitLab MCPサーバーを操作しているときに、次の問題が発生する可能性があります。

## GitLab MCPサーバー起動時の`404 Not Found`  {#404-not-found-when-starting-the-gitlab-mcp-server}

GitLab MCPサーバーの起動時に`404 Not Found`エラーが発生する可能性があります。このエラーは、MCPサーバーへのアクセスが無効になっている場合に発生します。

この問題を解決するには、[GitLab MCPサーバーの前提条件](mcp_server.md#prerequisites)をすべて満たしていることを確認してください。

## `/api/v4/mcp`は`404 Not Found`を返します。 {#apiv4mcp-returns-404-not-found}
...
GitLab 18.6以前では、MCPクライアントライブラリがGitLab MCPサーバープロトコル仕様をサポートしていない場合に、このエラーが発生する可能性があります。

この問題を解決するには、AIツールプロバイダーにクライアント実装の更新を依頼してください。
