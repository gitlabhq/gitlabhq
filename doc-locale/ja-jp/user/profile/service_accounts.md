...
> [!note] [複合ID](../duo_agent_platform/composite_identity.md)に関連付けられたサービスアカウントのユーザー名を更新することはできません。
...
> [!warning]これは元に戻せません。ローテーションされたトークンに依存するサービスは動作を停止します。
...
> [!warning]これは元に戻せません。失効されたトークンに依存するサービスは動作を停止します。
