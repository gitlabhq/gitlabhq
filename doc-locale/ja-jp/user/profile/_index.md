...
各GitLabアカウントには、ユーザープロファイルがあり、ユーザー自身とユーザーのGitLabアクティビティに関する情報が含まれています。
...
1. プロフィールページの右上隅で、**アクション** ({{< icon name="ellipsis_v" >}}) を選択します。
1. **ユーザーIDをコピー**を選択します。
...
以前にサポートPINを作成した場合、そのPINはプロフィールでアクセス可能で、作成後7日間有効です。
...
> [!note] [メールを非公開にしても](#set-your-public-email) 、コミットのマッチングや[グループとプロジェクトのインポート](../import/_index.md)に利用されることを防ぐことはできません。
...
> [!note] [イシュー438600](https://gitlab.com/gitlab-org/gitlab/-/issues/438600)のため、グループの通知は削除されたメールアドレスに引き続き送信されます。
...
> [!note] GitLabの管理者は、この[設定](../../administration/settings/account_and_limit_settings.md#prevent-users-from-making-their-profiles-private)を無効にして、すべてのプロフィールを公開にすることができます。

プロファイルを非公開にするには:

1. 右上隅で、アバターを選択します。
1. **プロファイルを編集**を選択します。
1. **非公開プロファイル**チェックボックスをオンにします。
...
- Atomフィード
- アカウントの作成日
- アクティビティ、グループ、コントリビュートしたプロジェクト、パーソナルプロジェクト、Star付きプロジェクト、スニペットのタブ

> [!note]ユーザープロフィールページを非公開にしても、RESTまたはGraphQL APIからすべての公開リソースが隠されるわけではありません。たとえば、[自動生成されたプライベートコミットメールを使用](#use-an-automatically-generated-private-commit-email)する場合を除いて、コミット署名に関連付けられているメールアドレスにアクセスできます。
...
GitLabは、コントリビュートグラフの下にREADMEの内容を表示します。
...
DiscordやX（旧Twitter）など、あなたが持っている特定の外部アカウントへのリンクを追加できます。他のユーザーが他のプラットフォームであなたとつながるのに役立ちます。
...
1. **主要設定**セクションで、以下を追加します。
   ...
   - GitHubユーザー名。
   - LinkedInのプロフィール名。
   - [Mastodonハンドル](#add-a-mastodon-handle)。
...
### Mastodonハンドルを追加する {#add-a-mastodon-handle}
...
- GitLab 18.8で、GitLabのユーザープロフィールを使用してMastodonアカウントを検証する機能が[一般公開されました](https://gitlab.com/gitlab-org/gitlab/-/work_items/504194)。機能フラグ`verify_mastodon_user`は削除されました。

{{< /history >}}

Mastodonハンドルを追加するには:

1. GitLabプロフィールにMastodonハンドルを追加します。
   1. GitLabにサインインします。
   1. 右上隅で、アバターを選択します。
   1. **プロファイルを編集**を選択します。
   1. **ソーシャルアカウント**セクションで、**Mastodon**に移動し、ハンドルを入力します。例: `@alex.garcia@exampleServer`。
   1. **プロファイル設定を更新**を選択します。
1. 検証URIを取得します。
   1. 右上隅で、アバターを選択します。
   1. 自分の名前またはユーザー名を選択します。
   1. 右側でMastodonハンドルを選択します。1.開いたページからURIをコピーします。
1. Mastodonに検証を追加します。
   1. Mastodonプロフィールの設定に移動します。
   1. **Extra fields**セクションで、以前のURIを入力します。
   1. Mastodonプロフィールの変更を保存します。

MastodonのGitLabサイトの追加フィールドの横に緑色のチェックマークが表示されていることを確認します。緑色のチェックマークが表示されない場合は、追加のトラブルシューティングを実行する必要があるかもしれません。

## ユーザープロファイルページにプライベートコントリビュートを表示する {#show-private-contributions-on-your-user-profile-page}

ユーザーコントリビュートカレンダーグラフと最近のアクティビティリストでは、プライベートプロジェクトに対する[コントリビュートアクション](contributions_calendar.md#user-contribution-events)を確認できます。
...
1. 右上隅で、アバターを選択します。
1. **ステータスを設定**を選択します。以前にステータスを設定した場合は、代わりに**ステータスを編集**を選択します。
...
プライマリメールは、ログイン、コミットメール、通知メールのデフォルトメールアドレスです。プライマリメールが変更された場合、元のプライマリメールはセカンダリメールとして追加されます。この機能により、元のプライマリメールで行われたコミットがあなたのアカウントに関連付けられたままになります。
...
> [!note]この機能が無効になると、現在のすべてのフォロー/フォロワー接続が削除されます。

## ユーザーのアクティビティを表示する {#view-a-users-activity}

GitLabは、[ユーザーのコントリビュートアクティビティ](contributions_calendar.md)を追跡します。ユーザーのアクティビティを表示するには:

1. ユーザーのプロフィールに移動します。
1. GitLabメニューで、**アクティビティ**を選択します。

**最新のアクティビティ**コントリビュートのリストが表示されます。

## 自分のアクティビティを表示する {#view-your-activity}
...
1. **あなたの作業**を選択します。
1. **アクティビティ**を選択します。
1. オプション。コントリビュートの種類でアクティビティをフィルタリングするには、**あなたのアクティビティ**タブで、次のタブを選択します。
...
ブラウザを閉じると、通常、`_gitlab_session`および`gitlab_user` Cookieはクライアント側でクリアされます。Cookie が有効期限切れになったり利用できなくなったりすると、GitLabは次のようになります。
...
> [!note]
> 
> セッションがサインアウトされた場合、または[アクティブなセッションリスト](active_sessions.md)からセッションが失効する場合、すべての**ログイン情報を記憶する**トークンが失効するされます。他のセッションがアクティブなままになっているときに、ブラウザが閉じられたり、既存のセッションが有効期限切れになったりすると、**ログイン情報を記憶する**機能でセッションを復元できなくなります。
