...
- GitLab 19.1で[一般提供](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/239045)になりました。機能フラグ`compliance_framework_templates`は削除されました。
...
1. オプション。**名前**、**説明**、または**色**を上書きして、フレームワークをカスタマイズします。
1. オプション。これをグループのデフォルトフレームワークにするには、**デフォルトとして設定**を選択します。
1. **フレームワークの作成**を選択します。
...
GitLab Ultimateでは、コンプライアンスフレームワークに特定の**要件**を定義できます。要件は1つまたは複数のコントロールで構成されており、フレームワークが割り当てられたプロジェクトの設定または動作に対するチェックです。各要件には最大5つのコントロールがあります。
...
| 管理者よりも多くのメンバー                                 | `more_members_than_admins`                                 | プロジェクトに割り当てられている管理者[（オーナーまたはメンテナー）](../../project/members/_index.md)の数が、メンバー総数よりも少ないことを保証します。 |
...
#### 外部コントロールライフサイクル {#external-control-lifecycle}
 ...
 外部コントロールには**非同期**のワークフローがあります。[コンプライアンススキャン](../compliance_center/compliance_status_report.md#scan-timing-and-triggers)が実行されると、外部サービスにペイロードを送信します。
