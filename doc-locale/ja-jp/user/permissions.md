...
title: ロールと権限
description: GitLabでの各ユーザーロールが利用できる権限と機能について理解します。
---
...
[管理者アクセス](../administration/_index.md)を持つユーザーはすべての権限を持ち、任意のアクションを実行できます。
...
ユーザーをグループまたはプロジェクトに追加する際、ロールを割り当てます。ロールはユーザーの権限を決定します。[デフォルトロール](#default-roles)または[カスタムロール](custom_roles/_index.md)のいずれかを割り当てます。

ユーザーは各グループおよびプロジェクトで異なるロールを持つことができます。ユーザーは常に、その最高ロールの権限を保持します。たとえば、ユーザーが以下の場合:
...
割り当てられたロールを表示するには、**メンバー**ページ ([グループ](group/_index.md#view-group-members)または[プロジェクト](project/members/_index.md#view-project-members)) にアクセスします。
...
| ロール           | 説明 |
| -------------- | ----------- |
| 最小アクセス | プロジェクトへのアクセスなしで、限定されたグループ情報を表示します。詳細については、[最小アクセスを持つユーザー](#users-with-minimal-access)を参照してください。 |
| ゲスト          | イシューおよびエピックの表示とコメント。コードをプッシュしたり、リポジトリにアクセスしたりすることはできません。このロールは、[プライベートおよび内部プロジェクト](public_access.md)にのみ適用されます。 |
| プランナー        | イシュー、エピック、マイルストーン、イテレーションを作成および管理します。プロジェクトの計画と追跡に焦点を当て、コード変更の表示と共同作業が可能です。 |
| レポーター       | コードの表示、イシューの作成、レポートの生成。コードをプッシュしたり、保護ブランチを管理したりすることはできません。 |
| デベロッパー      | 非保護ブランチにコードをプッシュし、マージリクエストを作成し、CI/CDパイプラインを実行します。プロジェクトの設定を管理することはできません。 |
| メンテナー     | ブランチ、マージリクエスト、CI/CD設定、およびプロジェクトメンバーを管理します。プロジェクトを削除することはできません。 |
| オーナー          | 削除および表示レベル設定を含む、プロジェクトまたはグループに対する完全な制御。 |

デフォルトでは、すべてのユーザーがトップレベルグループを作成し、ユーザー名を変更できます。ユーザーは、[管理者アクセス](../administration/user_settings.md)を使用してこの動作を変更できます。
...
| グループを削除                                                                                |       |         |          |           |            |   ✓   |
| グループの転送                                                                              |       |         |          |           |            |   ✓   |
| [サブスクリプション、ストレージ、コンピューティング時間](../subscriptions/manage_users_and_seats.md#gitlabcom-billing-and-usage)を管理 |       |         |          |           |            |   ✓   |
...
1. デベロッパーとメンテナーは、個々のアクションに基づくイベントのみを表示できます。詳細については、[前提条件](compliance/audit_events.md#prerequisites)を参照してください。
...
1. ユーザーは、自身のアクションに基づくイベントのみを表示できます。詳細については、[前提条件](compliance/audit_events.md#prerequisites)を参照してください。
...
[パッケージとコンテナレジストリ](packages/_index.md)のグループ権限:
...
1. [親エピックを表示](group/epics/manage_epics.md#who-can-view-an-epic)する権限が必要です。
...
[保護ブランチ設定](project/repository/branches/protection_rules.md)を構成すると、ロールを選択することで、そのロールおよびそれより上位のすべてのロールを持つユーザーにアクセス権が付与されます。たとえば、**メンテナー**を保護ブランチ設定で選択した場合、メンテナーロールとオーナーロールの両方を持つユーザーがアクションを実行できます。
...
2. 非メンバー: プロジェクトがパブリックであり、**プロジェクトベースのパイプラインの表示レベル**が有効な場合のみ。
   <br>ゲスト: **プロジェクトベースのパイプラインの表示レベル**が有効な場合のみ。
3. 非メンバー: プロジェクトがパブリックであり、**プロジェクトベースのパイプラインの表示レベル**が有効で、[`artifacts:public: false`](../ci/yaml/_index.md#artifactspublic)がジョブに設定されていない場合のみ。
   <br>ゲスト: **プロジェクトベースのパイプラインの表示レベル**が有効で、`artifacts:public: false`がジョブに設定されていない場合のみ。<br>レポーター: `artifacts:public: false`がジョブに設定されていない場合のみ。<br>この`artifacts:public`設定は、GitLab UIおよびAPIアクセスにのみ影響します。CI/CDジョブトークンは、Runner APIを使用してアーティファクトに引き続きアクセスできます。
...
12. デベロッパー: ユーザーがマージ権限を持つブランチの場合のみ。保護ブランチの場合、ターゲットブランチに対するマージ権限が必要です。保護タグの場合、ユーザーは保護タグを作成する権限が必要です。これらの権限要件は、スケジュールを作成または編集する際に適用され、ブランチ保護ルールが時間とともに変更される可能性があるため、動的にチェックされます。
13. 手動で実行する場合、パイプラインは、スケジュールオーナーの権限ではなく、トリガーしたユーザーの権限で実行されます。
...
1. GitLab Self-Managedでは、ゲストロールを持つユーザーは、公開および内部プロジェクトでのみこのアクションを実行できます（非公開プロジェクトでは実行できません）。[外部ユーザー](../administration/external_users.md)は、プロジェクトが内部であっても、レポーター、デベロッパー、メンテナー、またはオーナーロールが必要です。GitLab.comのゲストロールを持つユーザーは、内部の表示レベルが利用できないため、公開プロジェクトでのみこのアクションを実行できます。
...
| [マージリクエストとコメントを検索<sup>1</sup><sup>2</sup>](search/_index.md)           |   ✓   |    ✓    |    ✓     |     ✓     |     ✓      |   ✓   |
...
1. プランナーおよびレポーターロールからの承認は、[プロジェクトで有効になっている](project/merge_requests/approvals/rules.md#enable-approval-permissions-for-additional-users)場合にのみ利用可能です。
...
1. 外部メンバーからのコントリビュートを受け入れるプロジェクトでは、ユーザーは自分のマージリクエストを作成、編集、および完了できます。担当者、レビュアー、ラベル、マイルストーンなどの一部のフィールドは編集できません。
...
| [機密イシュー](search/_index.md)とコメントを検索<sup>6</sup>          |       |    ✓    |    ✓     |     ✓     |     ✓      |   ✓   |
...
| [マイルストーン](search/_index.md)を検索<sup>6</sup>                                |       |    ✓    |    ✓     |     ✓     |     ✓      |   ✓   |
...
1. プランナーまたはオーナーロールを持たないユーザーは、自身が作成したイシューのみを削除できます。
...
1. ゲストユーザーは、作成したイシューまたは割り当て済みのイシューのタイトルと説明を変更できます。
1. プランナーまたはオーナーロールを持たないユーザーは、自身が作成したタスクのみを削除できます。
...
| [プロジェクトコード](search/_index.md)を検索<sup>1</sup><sup>2</sup>                  |   ✓   |    ✓    |    ✓     |     ✓     |     ✓      |   ✓   |
| [コミット](search/_index.md)とコメントを検索<sup>1</sup><sup>2</sup>          |   ✓   |    ✓    |    ✓     |     ✓     |     ✓      |   ✓   |
...
1. GitLab Self-Managedでは、ゲストロールを持つユーザーは、公開および内部プロジェクトでのみこのアクションを実行できます（非公開プロジェクトでは実行できません）。[外部ユーザー](../administration/external_users.md)は、プロジェクトが内部であっても、明示的なアクセス権（少なくとも**プランナー**ロール）を付与されている必要があります。GitLab.comのゲストロールのユーザーは、内部表示レベルを利用できないため、公開プロジェクトでのみこのアクションを実行できます。GitLab 15.9以降では、Ultimateプランのライセンスを持つゲストロールのユーザーは、管理者（GitLab Self-ManagedまたはGitLab Dedicated）またはグループオーナー（GitLab.com）が許可した場合、非公開リポジトリのコンテンツを表示できます。管理者またはグループオーナーは、APIまたはUIから[カスタムロール](custom_roles/_index.md)を作成し、そのロールをユーザーに割り当てることができます。GitLab 18.7以降では、プランナーロールを持つユーザーはプライベートリポジトリのコンテンツを表示できます。
1. プランナーロールを持つユーザーは、プライベートプロジェクトで完全一致コードの検索やコード、コミット、コミットに対するコメントの高度な検索を使用できません。詳細については、[エピック &17674](https://gitlab.com/groups/gitlab-org/-/work_items/17674)を参照してください。
...
- ユーザーがインスタンスまたはGitLab.comのネームスペースで他のロールを持たない場合、ライセンスシートとしてカウントされます。

最小アクセスロールを持つユーザーが、いずれかのプロジェクトまたはサブグループで[請求対象ロール](../subscriptions/manage_users_and_seats.md#billable-users)を付与された場合、そのユーザーは最高のロールに基づいてライセンスシートを消費します。
...
この問題を回避するには、これらのユーザーにゲスト、プランナー、レポーター、デベロッパー、メンテナー、またはオーナーロールを親グループ内の任意のプロジェクトまたはサブグループに付与します。ゲストユーザーは、Premiumプランではライセンスシートを消費しますが、Ultimateプランでは消費しません。
