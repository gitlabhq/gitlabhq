...
- **パブリックデプロイキー**: GitLabインスタンス内の任意のプロジェクトにアクセスを許可できます。各プロジェクトへのアクセスは、少なくともメンテナーロールを持つユーザーによって[許可](#grant-project-access-to-a-public-deploy-key)される必要があります。
...
- デプロイキーのオーナーがブロックされたり、インスタンスから削除された場合、デプロイキーは[却下されます](#deploy-key-is-rejected)。リポジトリ操作（プルやプッシュを含む）には使用できません。
- デプロイキーが保護ブランチルールで指定されている場合、デプロイキーの作成者は:
  - 保護ブランチ、およびデプロイキー自体へのアクセスを獲得します。
...
[GitLab CLI](../../../editor_extensions/gitlab_cli/_index.md)は`glab deploy-key list`コマンドを提供します。
...
[GitLab CLI](../../../editor_extensions/gitlab_cli/_index.md)は`glab deploy-key add`コマンドを提供します。
...
- GitLab CLIの[デプロイキー用コマンド](https://gitlab.com/gitlab-org/cli/-/tree/main/docs/source/deploy-key)

## トラブルシューティング {#troubleshooting}

### デプロイキーが却下されました {#deploy-key-is-rejected}

デプロイキーは以下のいずれかの場合に却下されます:

- キーのオーナーがブロックされている場合。
- キーの有効期限が切れている場合。

デプロイキーが却下されると、プルやプッシュを含むすべてのリポジトリ操作に使用できなくなります。

この問題を解決するには、次の手順に従います:

- キーのオーナーがブロックされている場合、ユーザーのブロックを解除するか、[キーのオーナーを](#set-the-owner-of-a-deploy-key)アクティブユーザーに変更します。
- キーの有効期限が切れている場合は、新しいデプロイキーを作成します。
...
- デプロイキーに関連付けられているオーナーの[プロジェクトメンバーシップ権限](../../permissions.md#project-permissions)が、**プロジェクトコードの表示**に必要な権限よりも低くなっています。
...
1. [サービスアカウントユーザーをプロジェクトに招待](../../profile/service_accounts.md#add-a-service-account-to-a-group-or-project)します。
...
デプロイキーは特定のユーザーに属しており、そのユーザーがブロックされたり、インスタンスから削除されたりすると却下されます。ユーザーが削除されたときにデプロイキーが動作し続けるようにするには、そのオーナーをアクティブユーザーに変更します。
