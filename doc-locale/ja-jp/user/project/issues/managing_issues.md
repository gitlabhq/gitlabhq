...
description: 編集、移動、クローズ、一括操作、アサイン、ヘルスステータス、自動化などのさまざまなイシュー機能の使用など、GitLabのイシューを管理する方法について説明します。
title: イシューを管理する
---
...
- プロジェクトに対してプランナー、レポーター、デベロッパー、メンテナー、またはオーナーのロールを持っているか、イシューの作成者であるか、イシューに割り当てられている必要があります。

イシューを編集するには、次の手順に従います:

1. 上部のバーで、**検索または移動先**を選択して、プロジェクトを見つけます。
1. **計画** > **イシュー**を選択し、表示するイシューのタイトルを選択します。
...
<i class="fa-youtube-play" aria-hidden="true"></i>[概要を見る](https://www.youtube.com/watch?v=-BWBQat7p5M)
...
[イシュー409844](https://gitlab.com/gitlab-org/gitlab/-/issues/409844)で、この実験的機能に関するフィードバックをお寄せください。

**データ使用量**: この機能を使用すると、入力したテキストが大規模言語モデルに送信されます。

## イシューを一括編集する {#bulk-edit-issues}

{{< history >}}

- GitLab 17.7で、イシューの一括編集に必要な最小ロールが、レポーターからプランナーに[変更されました](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/169256)。
- GitLab 18.7で、より多くの一括編集属性が[追加されました](https://gitlab.com/gitlab-org/gitlab/-/issues/520791)。

{{< /history >}}

グループまたはプロジェクト内で複数のイシューを一度に編集できます。

前提条件: 

- グループまたはプロジェクトに対してプランナー、レポーター、デベロッパー、メンテナー、またはオーナーのロールを持っている必要があります。
...
1. 右上隅にある**その他のアクション**（{{< icon name="ellipsis_v" >}}） > **移動**を選択します。
1. イシューの移動先のプロジェクトを検索します。
1. **移動**を選択します。

コメントまたは説明で[`/move`クイックアクション](../quick_actions.md#move)を使用することもできます。
...
1. **移動**ドロップダウンリストから、移動先のプロジェクトを選択します。
1. **アイテムを移動**を選択します。
...
- ドラッグアンドドロップですべてのリスト項目を並べ替えます。
- タスクリスト項目を削除します。
- [タスクリスト項目をタスク作業アイテムに変換します](../../tasks.md#from-a-task-list-item)。

### タスクリスト項目を削除する {#delete-a-task-list-item}

前提条件: 

- プロジェクトに対してレポーター、デベロッパー、メンテナー、またはオーナーのロールを持っているか、イシューの作成者であるか、イシューに割り当てられている必要があります。
...
- GitLab UIの他のページから:
  1. 上部のバーで、**検索または移動先**を選択して、プロジェクトを見つけます。
  1. **計画** > **イシュー**を選択し、表示するイシューを選択します。
  1. 右上隅で、**その他のアクション**（{{< icon name="ellipsis_v" >}}）を選択し、**イシューをクローズ**を選択します。

コメントまたは説明で[`/close`クイックアクション](../quick_actions.md#close)を使用することもできます。
...
クローズされたイシューを再度開くには、右上隅で、**その他のアクション**（{{< icon name="ellipsis_v" >}}）を選択し、**イシューを再オープン**を選択します。再度オープンされたイシューは、他のオープンイシューと変わりません。

コメントまたは説明で[`/reopen`クイックアクション](../quick_actions.md#reopen)を使用することもできます。
...
マージリクエストをマージするときは、対象のイシューをクローズすることが適切かどうかを確認する責任があります。ユーザーは、マージリクエストの説明と、コミットメッセージの本文にイシューのクローズパターンを含めることができます。コミットメッセージ内のクローズメッセージは見落としがちです。どちらの場合も、マージリクエストウィジェットには、マージ時にクローズするイシューに関する情報が表示されます:
...
- GitLab 15.4で[変更](https://gitlab.com/gitlab-org/gitlab/-/issues/240922)されました。コミットまたはマージリクエストのプロジェクトではなく、参照されているイシューのプロジェクト設定がチェックされます。
...
- プロジェクトに対してメンテナーまたはオーナーのロールを持っている必要があります。
...
イシューの説明が長い場合、GitLabはその一部のみを表示します。説明全体を表示するには、**続きを読む**を選択する必要があります。この切り詰める機能により、長文をスクロールせずに、ページ上の他の要素を簡単に見つけることができます。
...
1. 上部のバーで、**検索または移動先**を選択して、プロジェクトを見つけます。
1. **計画** > **イシュー**を選択し、表示するイシューを選択します。
1. 右上隅で、**その他のアクション**（{{< icon name="ellipsis_v" >}}）を選択します。
1. **イシューを削除**を選択します。
...
- GitLab 18.7で、イシューから主な成果、目的、タスクへの変更が[導入](https://gitlab.com/gitlab-org/gitlab/-/issues/520791)されました。

{{< /history >}}

前提条件: 

- イシューの作成者であるか、プロジェクトに対してプランナー、レポーター、デベロッパー、メンテナー、またはオーナーのロールを持っているか、イシューの作成者であるか、イシューに割り当てられている必要があります。
...
1. 右上隅で、**その他のアクション**（{{< icon name="ellipsis_v" >}}）を選択します。
1. **種類の変更**を選択します
1. **タイプ**ドロップダウンリストで新しいタイプを選択します:

   - 主な成果
   - 目標
   - タスク
   - エピック（イシューを親グループに移動します）。詳細については、[promote an issue to an epic](#promote-an-issue-to-an-epic)を参照してください。

1. **種類の変更**を選択します。

イシューをインシデントにプロモートするには、[promote an issue to an incident](#promote-an-issue-to-an-incident)を参照してください。
...
- プロジェクトの直近の親グループに対してプランナー、レポーター、デベロッパー、メンテナー、またはオーナーのロールを持っている必要があります。
- 次のいずれかの条件を満たす必要があります。
  - プロジェクトに対してプランナー、レポーター、デベロッパー、メンテナー、またはオーナーのロールを持っている必要があります。
  - イシューの作成者である。
  - イシューに割り当てられている。
...
1. 右上隅で、**その他のアクション**（{{< icon name="ellipsis_v" >}}）を選択します。
1. **種類の変更**を選択します
1. **タイプ**ドロップダウンリストから、**エピック**を選択します。
1. **種類の変更**を選択します。

または、[`/promote_to Epic`クイックアクション](../quick_actions.md#promote_to)を使用することもできます。

### イシューをインシデントにプロモートする {#promote-an-issue-to-an-incident}

[`/promote_to Incident`クイックアクション](../quick_actions.md#promote_to)を使用して、イシューを[インシデント](../../../operations/incident_management/incidents.md)にプロモートできます。
...
- [`/iteration`クイックアクション](../quick_actions.md#iteration)を使用します。
- イシューをボード内のイテレーションリストにドラッグします。
...
- 右上隅で、**アサインされたイシュー**（{{< icon name="issues" >}}）を選択します。

## 子アイテムの表示設定を設定する {#configure-child-item-display-preferences}

{{< history >}}

- 子アイテムの[展開された](https://gitlab.com/gitlab-org/gitlab/-/work_items/551231)GitLab 18.10での表示オプション。

{{< /history >}}

**子アイテム**セクションに表示される情報を設定して、ワークフローにとって最も重要なものに集中できます。

> [!note]
> 1つのイシューで表示される情報を変更すると、グループとプロジェクトのすべての作業アイテムの情報が変更されます。
...
イシューリストでエピックに表示する属性を設定するには、[表示設定を構成](../../work_items/_index.md#configure-list-display-preferences)します。
...
- ORフィルタリングが、GitLab 15.6で`or_issuable_queries`[フラグ](../../../administration/feature_flags/_index.md)とともに[導入](https://gitlab.com/gitlab-org/gitlab/-/issues/23532)されました。デフォルトでは無効になっています。
- GitLab 15.9で、ORフィルタリングが[GitLab.comおよびGitLab Self-Managedで有効](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/104292)になりました。
- GitLab 17.0で、ORフィルタリングが[一般提供](https://gitlab.com/gitlab-org/gitlab/-/issues/296031)になりました。機能フラグ`or_issuable_queries`は削除されました。
- カスタムステータスまたは親アイテムによるイシューのリストのフィルタリングが、GitLab 18.7で[導入](https://gitlab.com/gitlab-org/gitlab/-/issues/520791)されました。
...
1. 表示されるドロップダウンリストから、フィルタリングする属性を選択します。次のフィルタリングを使用できます:
   - 担当者
   - 作成者
   - 機密
   - [連絡先](../../crm/_index.md)
   - [ヘルス](managing_issues.md#health-status)
   - イテレーション
   - ラベル
   - マイルストーン
   - 自分のリアクション
   - [組織](../../crm/_index.md)
   - [親](../../group/epics/_index.md)
   - リリース
   - 検索対象（タイトルまたは説明）
   - ステータス
   - サブスクライブ
   - 型
   - ウェイト
   - [カスタムフィールド](../../work_items/custom_fields.md)
1. 属性のフィルタリングに使用する演算子を選択または入力します。次の演算子を使用できます。
   - `=`: 等しい
   - `!=`: 次のいずれでもない
   - `||`: いずれか1つ（担当者、作成者、ラベル、タイプの場合）。包括的なORのように機能します。たとえば、`Assignee is one of Sidney Jones`と`Assignee is one of Zhang Wei`でフィルタリングすると、GitLabは、`Sidney`、`Zhang`、またはその両方が担当者に割り当てられているイシューを表示します。
...
たとえば、`I am securing information for M&A`を検索すると、GitLabはタイトルまたは説明に`securing`、`secured`、または`information`が含まれる結果を返すことができます。ただし、GitLabは文章または`I`、`am`、`M&A`などの単語を正確に一致させません。これらは語彙的に意味があるまたは重要とは見なされないためです。これは、PostgreSQL全文検索の制限事項です。

#### IDでイシューをフィルタリングする {#filter-issues-by-id}

1. 上部のバーで、**検索または移動先**を選択して、プロジェクトを見つけます。
1. **計画** > **イシュー**を選択します。
1. イシューリストの上にある**結果を検索またはフィルタリング**テキストボックスに、`#`に続けてイシューIDを入力します。たとえば、`#362255`と入力すると、イシュー362255のみが返されます。
1. **このテキストを検索**を選択します。
1. <kbd>Enter</kbd>を押すか、検索アイコン（{{< icon name="search" >}}）を選択します。

### パネルでイシューを開く {#open-issues-in-a-panel}
...
- GitLab 18.6で[一般提供](https://gitlab.com/gitlab-org/gitlab/-/issues/463829)になりました。機能フラグ`issues_list_drawer`は削除されました。
...
- パネルが画面の右側に表示されます。
- パネルでエピックを直接編集できます。
- パネルを閉じるには、閉じるアイコン（{{< icon name="close" >}}）を選択するか、**Escape**キーを押します。

#### 全ページ表示でイシューを開く {#open-an-issue-in-full-page-view}

イシューをフルビューで開くには:

- 新しいタブでイシューを開きます。イシューのリストから、次のいずれかを実行します:
  - イシューを右クリックして、新しいブラウザータブで開きます。
  - <kbd>Command</kbd>キーまたは<kbd>Control</kbd>キーを押しながら、イシューを選択します。
- イシューを選択し、パネルから次のいずれかの操作を行います:
  - 左上隅で、イシューの参照（例: `my_project#123`）を選択します。
  - 右上隅で、**全ページ表示**（{{< icon name="maximize" >}}）を選択します。

イシューを常に全ページ表示で開くには、[configure your list display preferences](../../work_items/_index.md#configure-list-display-preferences)を設定します。

## イシュー参照をコピーする {#copy-issue-reference}

GitLabの別の場所でイシューを参照するには、完全なURLまたは短い参照（`namespace/project-name#123`のようなもの。ここで、`namespace`はグループまたはユーザー名のいずれか）を使用できます。

イシュー参照をクリップボードにコピーするには:

1. 上部のバーで、**検索または移動先**を選択して、プロジェクトを見つけます。
1. **計画** > **イシュー**を選択し、表示するイシューを選択します。
1. 右上隅で、**その他のアクション**（{{< icon name="ellipsis_v" >}}） > **参照をコピー**を選択します。

これで、この参照を別の説明またはコメントに貼り付けることができます。

詳細については、[GitLab固有の参照](../../markdown.md#gitlab-specific-references)をご覧ください。

## イシューのメールアドレスをコピーする {#copy-issue-email-address}

メールを送信してイシューにコメントを作成できます。このアドレスにメールを送信すると、メール本文を含むコメントが作成されます。

メールを送信してコメントを作成する方法、および必要な設定の詳細については、[reply to a comment by sending email](../../discussions/_index.md#reply-to-a-comment-by-sending-email)を参照してください。

イシューのメールアドレスをコピーするには:

1. 上部のバーで、**検索または移動先**を選択して、プロジェクトを見つけます。
1. **計画** > **イシュー**を選択し、表示するイシューを選択します。
1. 右上隅で、**その他のアクション**（{{< icon name="ellipsis_v" >}}） > **イシューのメールアドレスをコピー**を選択します。
...
[`/health_status`](../quick_actions.md#health_status)および[`/clear_health_status`](../quick_actions.md#clear_health_status)のクイックアクションを使用して、ヘルスステータスを設定およびクリアすることもできます。
...
イシューにステータスを割り当ると、ワークフローの進行状況を追跡できます。ステータスは、基本的なオープン/クローズステータスよりも詳細な追跡を提供し、**進行中**、**完了**、または**実施しない**のような特定のステージを使用できます。

カスタムステータスを設定する方法の詳細については、[status section](../../work_items/status.md)を参照してください。
...
[`/status`クイックアクション](../quick_actions.md#status)を使用して、ステータスを設定することもできます。

## 参加者 {#participants}

参加者とは、イシューを操作したユーザーのことです。参加者の表示については、[参加者](../../participants.md)を参照してください。
...
ステータスページアプリケーションがプロジェクトに関連付けられている場合は、[`/publish`クイックアクション](../quick_actions.md#publish)を使用してイシューを公開できます。
...
一部のアクションには、まだ対応するUIボタンがありません。次の操作は、**クイックアクションを使用する場合にのみ**実行できます:

- [Zoomミーティングを追加または削除する](associate_zoom_meeting.md)（[`/zoom`および`/remove_zoom`](../quick_actions.md#zoom)）。
- [イシューを公開する](#publish-an-issue)（[`/publish`](../quick_actions.md#publish)）。
- イシューを同じプロジェクトまたは別のプロジェクトにクローンする（[`/clone`](../quick_actions.md#clone)）。
- イシューをクローズし、別のイシューの複製としてマークする（[`/duplicate`](../quick_actions.md#duplicate)）。
- プロジェクト内の別のマージリクエストまたはイシューからラベルとマイルストーンをコピーする（[`/copy_metadata`](../quick_actions.md#copy_metadata)）。
