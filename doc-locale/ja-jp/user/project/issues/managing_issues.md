...
1. **Plan** > **イシュー**を選択し、表示するイシューのタイトルを選択します。
...
## イシューの一括編集 {#bulk-edit-issues}

{{< history >}}

- イシューの一括編集に必要な最小限のロールが、[変更](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/169256)され、レポーターからプランナーになりました（GitLab 17.7）。
- GitLab 18.7で、一括編集[できる属性がさらに追加](https://gitlab.com/gitlab-org/gitlab/-/issues/520791)されました。
...
1. 上部のバーで、**検索または移動先**を選択して、プロジェクトを見つけます。
1. **Plan** > **イシュー**を選択し、表示するイシューを選択します。
1. 右上隅で、**追加のアクション** ({{< icon name="ellipsis_v" >}}) > **移動**を選択します。
1. イシューの移動先のプロジェクトを検索します。
1. **移動**を選択します。

コメントまたは説明で[`/move`クイックアクション](../quick_actions.md#move)を使用することもできます。

### 親イシューの移動時に子アイテムを移動する {#moving-child-items-when-the-parent-issue-is-moved}
...
1. **移動**ドロップダウンリストから、宛先プロジェクトを選択します。
1. **Move items**を選択します。
...
- タスクリストアイテムを削除します。
- [タスクリストアイテムをタスク作業アイテムに変換](../../tasks.md#from-a-task-list-item)。
...
- GitLab UIの他のページから:
  1. 上部のバーで、**検索または移動先**を選択して、プロジェクトを見つけます。
  1. **Plan** > **イシュー**を選択し、表示するイシューを選択します。
  1. 右上隅で、**追加のアクション** ({{< icon name="ellipsis_v" >}}) を選択し、**イシューをクローズ**を選択します。
...
クローズされたイシューを再度開くには、右上隅で、**追加のアクション** ({{< icon name="ellipsis_v" >}}) を選択し、**イシューを再オープン**を選択します。再度オープンされたイシューは、他のオープンイシューと変わりません。
...
## **続きを読む**で説明が切り捨てられるのを防ぐ {#prevent-truncating-descriptions-with-read-more}
...
1. 上部のバーで、**検索または移動先**を選択して、プロジェクトを見つけます。
1. **Plan** > **イシュー**を選択し、表示するイシューを選択します。
1. 右上隅で、**追加のアクション** ({{< icon name="ellipsis_v" >}}) を選択します。
1. **イシューを削除**を選択します。
...
- イシューから主な成果、目的、タスクへの種類の変更が[導入](https://gitlab.com/gitlab-org/gitlab/-/issues/520791)されました（GitLab 18.7）。
...
- ORフィルターが、GitLab 15.6で`or_issuable_queries`という名前の[フラグ付きで](../../../administration/feature_flags/_index.md) [導入](https://gitlab.com/gitlab-org/gitlab/-/issues/23532)されました。デフォルトでは無効になっています。
- GitLab 15.9で、ORフィルターが[GitLab.comおよびGitLabセルフマネージドで有効化](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/104292)されました。
- ORフィルターが[一般提供](https://gitlab.com/gitlab-org/gitlab/-/issues/296031)されるようになりました（GitLab 17.0）。機能フラグ`or_issuable_queries`は削除されました。
...
1. 表示されるドロップダウンリストから、フィルターする属性を選択します。次のフィルターを使用できます:
   - 担当者
   - 作成者
   - 機密
   - [連絡先](../../crm/_index.md)
...
1. 上部のバーで、**検索または移動先**を選択して、プロジェクトを見つけます。
1. **計画** > **イシュー**を選択します。
1. イシューリストの上にある**結果を検索またはフィルタリング**テキストボックスに、`#`の後にイシューIDを入力します。たとえば、`#362255`と入力すると、イシュー362255のみが返されます。
1. **このテキストを検索**を選択します。
1. <kbd>Enter</kbd>を押すか、検索アイコン（{{< icon name="search" >}}）を選択します。
...
- パネルが画面の右側に表示されます。
- パネルでエピックを直接編集できます。
- パネルを閉じるには、閉じるアイコン ({{< icon name="close" >}}) を選択するか、**エスケープ**を押します。

#### イシューを全ページ表示で開く {#open-an-issue-in-full-page-view}

イシューをフルビューで開くには:

- 新しいタブでイシューを開きます。イシューのリストから、次のいずれかを実行します:
  - イシューを右クリックして、新しいブラウザータブで開きます。
  - <kbd>Command</kbd>または<kbd>Control</kbd>を押しながら、イシューを選択します。
- イシューを選択し、パネルから次のいずれかの操作を行います:
  - 左上隅で、イシューの参照（例：`my_project#123`）を選択します。
  - 右上隅で、**全ページ表示** ({{< icon name="maximize" >}}) を選択します。

イシューを常に全ページ表示で開くには、[アイテムをドロワーで開くかどうかを設定する設定](../../work_items/_index.md#configure-list-display-preferences)を参照してください。
...
1. 上部のバーで、**検索または移動先**を選択して、プロジェクトを見つけます。
1. **Plan** > **イシュー**を選択し、表示するイシューを選択します。
1. 右上隅で、**追加のアクション** ({{< icon name="ellipsis_v" >}}) > **Copy Reference**を選択します。
...
1. 上部のバーで、**検索または移動先**を選択して、プロジェクトを見つけます。
1. **Plan** > **イシュー**を選択し、表示するイシューを選択します。
1. 右上隅で、**追加のアクション** ({{< icon name="ellipsis_v" >}}) > **Copy issue email address**を選択します。
...
[`/health_status`](../quick_actions.md#health_status)および[`/clear_health_status`](../quick_actions.md#clear_health_status)クイックアクションを使用して、ヘルスステータスを設定およびクリアすることもできます。
...
イシューにステータスを割り当てて、ワークフローの進行状況を追跡できます。ステータスは、基本的なオープン/クローズ状態よりも詳細な追跡を提供するため、**進行中**、**完了**、または**Won't do**などの特定のステージを使用できます。
...
参加者とは、イシューを操作したユーザーのことです。参加者の表示については、[participants](../../participants.md)を参照してください。
...
一部のアクションには、まだ対応するUIボタンがありません。次の操作は、**クイックアクションを使用する**場合にのみ実行できます:

- [Zoomミーティングを追加または削除](associate_zoom_meeting.md) ([`/zoom`および`/remove_zoom`](../quick_actions.md#zoom))。
- [イシューの公開](#publish-an-issue) ([`/publish`](../quick_actions.md#publish))。
- イシューを同じプロジェクトまたは別のプロジェクトにクローンします ([`/clone`](../quick_actions.md#clone))。
- イシューをクローズし、別のイシューの複製としてマークします ([`/duplicate`](../quick_actions.md#duplicate))。
- プロジェクト内の別のマージリクエストまたはイシューからラベルとマイルストーンをコピーします ([`/copy_metadata`](../quick_actions.md#copy_metadata))。
