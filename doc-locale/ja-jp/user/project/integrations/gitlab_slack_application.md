...
description: "GitLab for Slackアプリを構成してスラッシュコマンドを使用し、SlackワークスペースでGitLabからの通知を受信します。"
---
...
[Slackスラッシュコマンド](slack_slash_commands.md)または[Mattermostスラッシュ（/）コマンド](mattermost_slash_commands.md)を代わりに使用する場合:
...
Slackワークスペースでエイリアスの競合が発生した場合（たとえば、複数のプロジェクトまたはグループが同じエイリアスを使用しようとする場合）、GitLabは自動的に次の形式でフォールバックエイリアスを割り当てます:

- プロジェクトの場合: `p-<project_id>` (例: `p-12345`)
- グループの場合: `g-<group_id>` (例: `g-67890`)

優先されるエイリアスが利用できない場合、スラッシュコマンドでこれらのフォールバックエイリアスを使用できます。
...
1. **トリガー**セクションで、次のことを行います。
   ...
   - オンにしたチェックボックスごとに、通知を受信するSlackチャンネルの名前を入力します。コンマで区切られた最大10個のチャンネル名を入力できます（例: `#channel-one, #channel-two`）。
     ...
     > [!note]
     > Slackチャンネルがプライベートの場合、GitLab for Slackアプリをそのチャンネルに[追加する](#receive-notifications-to-a-private-channel)必要があります。

1. オプション。**通知設定**セクションで、次のことを行います。
   - **失敗したパイプラインのみを通知**チェックボックスをオンにして、失敗したパイプラインの通知のみを受信します。
   - **Notify only when status changes**チェックボックスを選択すると、refのパイプラインステータスが変更された場合にのみ通知を受信します。
   - **通知を送信するブランチ**ドロップダウンリストから、通知を受信するブランチを選択します。

     これらのブランチから作成されたタグによってトリガーされたパイプラインに対しても通知が送信されます。
