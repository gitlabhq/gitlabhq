...
> [!note]
> プロジェクトアナリティクスを無効にしても、**分析**ナビゲーション項目が削除されるだけで、データは引き続き計算され、対応するAPIエンドポイントを通じて利用できます。
...
1. **設定** > **一般**を選択します。
1. **表示レベル、プロジェクトの機能、権限**セクションを展開します。
1. **プロジェクトアクセストークンの有効期限に関する追加のWebhookトリガーを追加する**チェックボックスを選択します。
1. **変更を保存**を選択します。

## プロジェクトのCI/CDを無効にする {#turn-off-cicd-for-a-project}

プロジェクトでCI/CDを使用しない場合は、無効にできます。これにより、マージリクエストからCI/CDバナーが削除されます。
