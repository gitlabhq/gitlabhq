...
GitLab Pagesはデフォルトでブランチ/タグに依存せず、そのデプロイは`.gitlab-ci.yml`で指定したもののみに依存することに注意してください。新しいコミットがページ専用のブランチにプッシュされるたびに、[`rules:if`](../../../ci/yaml/_index.md#rulesif)を使用して`pages`ジョブを制限できます。
...
> [!note]
> トップレベルの`publish`キーワードは、GitLab 17.9で[非推奨](https://gitlab.com/gitlab-org/gitlab/-/issues/519499)になり、現在は`pages`キーワードの下にネストする必要があります。
...
`FF_USE_FASTZIP`変数は、[`ARTIFACT_COMPRESSION_LEVEL`](../../../ci/runners/configure_runners.md#artifact-and-cache-settings)に必要な[機能フラグ](https://docs.gitlab.com/runner/configuration/feature-flags/#available-feature-flags)を有効にします。
