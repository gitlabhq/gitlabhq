...
> [!warning]
> ネームスペースが一般的なドメイン名とHTTPSで提供される場合の制限については、[Subdomains of subdomains](introduction.md#subdomains-of-subdomains)を参照してください。

Pagesドメインを明確に理解するには、以下の例をお読みください。

> [!note]
> 以下の例では、**一意のドメインを使用**設定を無効にしたことを意味します。無効にしていない場合は、前のテーブルを参照して、`example.io`を`gitlab.io`で置き換えてください。
...
> [!note]
> 一部の静的サイトジェネレーターでは、`baseurl`オプションの名前が異なる場合があります。
...
[デフォルトの例](https://gitlab.com/pages)のいずれかをフォークすることでウェブサイトをデプロイする場合、`baseurl`はすでにこのように設定されています。すべての例はプロジェクトウェブサイトです。ユーザーまたはグループのWebサイトを作成する場合は、プロジェクトからこの設定を削除する必要があります。Jekyllの例では、Jekyllの`_config.yml`を以下のように変更します:
