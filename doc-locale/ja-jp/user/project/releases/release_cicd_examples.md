...
> [!note]
> UIでGitタグを作成する際にリリースノートを提供しないでください。リリースノートを入力するとリリースが作成され、パイプラインが失敗します。
...
> [!note]
> `before_script`または`script`で設定された環境変数は、同じジョブ内で展開することができません。[変数を展開するために利用可能にする可能性](https://gitlab.com/gitlab-org/gitlab-runner/-/issues/6400)について詳しくはこちらをご覧ください。
