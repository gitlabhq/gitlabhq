...
- [GitLab Premium](https://about.gitlab.com/pricing/)と[GitLab Ultimate](https://about.gitlab.com/pricing/)では、次の点でより柔軟に対応できます。
  ...
  - 特定のファイルについて、[コードオーナー](../../codeowners/_index.md)のリストを作成します。
  ...
  - [グループマージリクエスト承認の設定](../../../group/manage.md#group-merge-request-approval-settings)を設定する。
    ...
    > [!note]
    > グループマージリクエスト承認設定のサポートは、[エピック4367](https://gitlab.com/groups/gitlab-org/-/epics/4367)で追跡されています。
...
- プロジェクトに必要な監視とセキュリティのレベルをより詳細に制御するための、さらなる[マージリクエスト承認設定](settings.md)。
- [マージリクエスト承認API](../../../../api/merge_request_approvals.md)を使用して、マージリクエスト承認ルールを設定する。

ルールの設定に関する詳細は、[承認ルール](rules.md)を参照してください。
...
- [`CODEOWNERS`ファイル](../../codeowners/_index.md#codeowners-file)を使用してレビュアーを決定します。
...
マージリクエストの承認ステータスを表示するには、マージリクエスト自体、またはプロジェクトやグループのマージリクエストのリストを確認します。
...
承認がコードオーナーの要件を満たしているかどうかを確認するには、**適格な承認者を展開**（{{< icon name="chevron-lg-down" >}}）を選択します。
...
- マージリクエストウィジェットで、**承認する**を選択します。
- コメントで[`/approve`クイックアクション](../../quick_actions.md#approve)を使用します。
...
- [開いているスレッド](../_index.md#prevent-merge-unless-all-threads-are-resolved)
- [CI/CDパイプラインの失敗](../auto_merge.md)

### マージリクエスト作成者による承認を防止する {#prevent-merge-request-creator-approval}

マージリクエスト作成者が自分の作業を承認するのを防ぐには、[**マージリクエストの作成者による承認を防止する**](settings.md#prevent-approval-by-merge-request-creator)設定を有効にします。
...
- [プランナーまたはレポーターロールを持つユーザーの承認権限を有効にする](rules.md#enable-approval-permissions-for-additional-users)
