...
![マージリクエスト差分は、追加および削除されたコード行を示しています。](img/mr_diff_example_v16_9.png)
...
- **Lines changed**: このファイルで追加および削除された行の数で、`+2 -2`の形式です。
...
1. そのリンクにアクセスすると、選択したファイルが一覧の一番上に表示されます。ファイルブラウザには、ファイル名の横にリンクアイコン（{{< icon name="link" >}}）が表示されます。
   ...
   ![選択されたYAMLファイルが上部に表示された、ファイルをリスト表示しているマージリクエスト。](img/linked_file_v17_4.png)
...
- ファイルの末尾までスクロールし、**前へ**または**次へ**のいずれかを選択します。
...
1. **設定**（{{< icon name="preferences" >}}）を選択します。**並べて表示**または**インライン**のいずれかを選択します。この例は、GitLabが同じ変更をインラインモードと並べて表示モードの両方でどのようにレンダリングするかを示しています。
   ...
   ![サイドバイサイドモードでのマージリクエストのコード変更。](img/changes-sidebyside_v17_10.png)
...
- <i class="fa-youtube-play" aria-hidden="true"></i>[概要を見る](https://youtu.be/1izKaLmmaCA?si=O2HDokLLujRro_3O)
...
1. 左側で、疑問符（{{< icon name="question" >}}）を選択します。表示するには、選択した最初の行までスクロールする必要がある場合があります。
   ...
   ![マージリクエストでGitLab Duoを使用して選択したコードスニペットを説明するアイコン。](img/explain_code_v17_1.png)

GitLab Duo Chatがコードを説明します。説明の生成には時間がかかる場合があります。

必要に応じて、説明の品質に関するフィードバックを提供できます。

GitLabは、大規模言語モデルが正しい結果を生成することを保証できません。生成された説明は、注意して使用してください。
...
1. コメントが添付されている行までスクロールします。ガターマージンで、**折りたたむ**（{{< icon name="collapse" >}}）を選択します: ![マージリクエスト差分でコメントを折りたたむアイコン。](img/collapse-comment_v17_1.png)
...
1. コメントが添付されている行までスクロールします。ガターマージンで、ユーザーアバターを選択します: ![マージリクエスト差分でコメントを展開するアイコン。](img/expand-comment_v17_10.png)
...
1. **ホワイトスペースの変更を表示する**を選択またはクリアします。
   ...
   ![環境設定メニューが展開され、「ホワイトスペースの変更を表示する」オプションが選択されたマージリクエスト差分。](img/merge_request_diff_v17_10.png)
...
ターゲットブランチにすでに存在する変更が表示されないように、GitLabはマージリクエストのソースブランチをターゲットブランチの`HEAD`と比較します。

ソースブランチとターゲットブランチが競合する場合、GitLabは競合するファイルごとにマージリクエスト差分にアラートを表示します:

![マージリクエスト差分における競合アラート。](img/conflict_ui_v15_6.png)
...
- [コード品質の調査結果](../../../ci/testing/code_quality.md#merge-request-changes-view)
- [静的な解析の調査結果](../../application_security/sast/_index.md#merge-request-changes-view)

## マージリクエストの変更をダウンロードする {#download-merge-request-changes}

マージリクエストに含まれる変更を、GitLabの外部で使用するためにダウンロードできます。
...
`git am`は`-p1`オプションをデフォルトで使用します。詳細については、[`git-apply`](https://git-scm.com/docs/git-apply)を参照してください。

### 以前の差分バージョンをダウンロードする {#download-older-diff-versions}

{{< history >}}

- GitLab 18.7で[導入](https://gitlab.com/gitlab-org/gitlab/-/issues/373246)されました。

{{< /history >}}

以前の差分バージョンをパッチまたは差分ファイルとしてダウンロードするには:

1. ダウンロードしたい[差分バージョンを比較](versions.md#compare-diff-versions)します。
1. URLパスに`.diff`または`.patch`を追加します。
...
- [ブランチの比較をダウンロードする](../repository/branches/_index.md#download-branch-comparisons)
- [マージリクエストのレビュー](reviews/_index.md)
- [マージリクエストバージョン](versions.md)
