...
1. 変更したいコードの行を見つけます。
   ...
   - 複数行を選択するには、次の手順に従います。
     1. 行番号にカーソルを合わせ、**この行にコメントを追加**（{{< icon name="comment" >}}）を選択します。

        ![差分ファイルの任意の行にコメントします。](img/comment_on_any_diff_line_v16_6.png)

     1. 選択してドラッグし、目的のすべての行を含めます。詳細については、[複数行のサジェスチョン](#multi-line-suggestions)を参照してください。
   - 特定の行ではなくファイル全体にコメントするには、ファイルのヘッダーで**このファイルにコメントする** ({{< icon name="comment" >}}) を選択します。
...
- [サジェスチョンの作成](#create-suggestions)で説明されているように、選択してドラッグします。GitLabが、提案ブロックを作成します。
...
- マージリクエストの作成者であるか、プロジェクトのデベロッパー、メンテナー、またはオーナーロールが必要です。
...
## サジェスチョンを却下する {#reject-suggestions}

前提条件:

- マージリクエストの作成者であるか、プロジェクトのデベロッパー、メンテナー、またはオーナーロールが必要です。

マージリクエストから直接サジェスチョンの変更を却下するには:

1. 上部のバーで、**検索または移動先**を選択して、プロジェクトを見つけます。
1. 左側のサイドバーで、**コード** > **マージリクエスト**を選択して、マージリクエストを見つけます。
1. 却下するサジェスチョンを含むコメントを見つけます。
1. オプション。サジェスチョンを却下する理由を記載した返信を追加します。
1. **スレッドを解決にする**を選択します。
...
1. **マージ提案**までスクロールし、ニーズに合わせてテキストを変更します。このメッセージで使用できるプレースホルダーのリストについては、[サポートされている変数](#supported-variables)を参照してください。
...
1. 目的の提案を追加したら、**提案を適用**を選択します。
   ...
   > [!warning]
   > 複数の作成者からの変更を含むサジェスチョンのバッチを適用すると、結果として生成されるコミットでは、あなたが作成者としてクレジットされます。[コミットを追加するユーザーからの承認を防止する](../approvals/settings.md#prevent-approvals-by-users-who-add-commits)ようにプロジェクトを設定すると、このマージリクエストの適格な承認者ではなくなります。
