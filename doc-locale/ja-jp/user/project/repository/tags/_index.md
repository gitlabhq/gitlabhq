...
デフォルトでは、CI/CDジョブに特定のルールが設定されていない場合、それらのルールは新しく作成されたタグのタグパイプラインに含まれます。タグのパイプラインは、タグがコミットをターゲットとする場合にのみ作成されます。
