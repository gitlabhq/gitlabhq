...
1. VS Codeで、**GitLab**の拡張機能設定を開きます:
   ...
   1. リストで**GitLab**を検索し、**Manage**（{{< icon name="settings" >}}）を選択します。
   1. **Extension Settings**を選択します。
1. **User**設定で、**AI Assisted Code Suggestions: Enabled Supported Languages**というタイトルのセクションを探します。
1. 言語のコード提案を有効にするには、その言語のチェックボックスをオンにします。
1. 言語のコード提案を無効にするには、その言語のチェックボックスをオフにします。
1. 変更は自動的に保存され、すぐに反映されます。

言語に対してコード提案を無効にすると、GitLab Duoアイコンが変化し、この言語では提案が無効になっていることを示します。
...
1. **Item**で、サポートする各言語の識別子を追加します。識別子は、`html`や`powershell`のように小文字にする必要があります。各識別子には、ファイル拡張子の先頭のピリオドを付けないでください。
1. **OK**を選択します。
...
1. IDEのトップバーで、IDE名を選択し、次に**設定**を選択します。
