...
1. 左サイドバーで、**Extensions** > **GitLab**を選択します。
1. **Manage**（{{< icon name="settings" >}}）を選択し、**Settings**を選択します。
1. **GitLab** > **Duo Code Suggestions**で、**GitLab Duo Code Suggestions**チェックボックスをオンにします。

#### コード提案ログを表示する {#view-code-suggestions-logs}

コード提案がIDEに対して有効になっているにもかかわらず提案がまだ表示されない場合、以下の手順に従います:

1. IDEで、GitLab for VS Codeの**Extension Settings**で、**GitLab: Debug**を有効にします。
...
1. GitLab for VS Codeの**Extension Settings**で、**GitLab Duo Code Suggestions**チェックボックスをオフにしてから再度オンにします。
...
1. VS Codeで、トップバーの**コード** > **設定** > **設定**へ移動します。
...
1. IDEで、トップバーからIDE名を選択し、次に**設定**を選択します。
