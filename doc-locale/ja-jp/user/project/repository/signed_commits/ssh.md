...
- コミットへの署名に使用するSSHキーを、GitLabアカウントに、[使用目的](../../../ssh.md#add-an-ssh-key-to-your-gitlab-account)が**認証と署名**または**署名**のSSHキーとして追加する必要があります。
- Git設定のコミッターメールアドレスは、GitLabアカウントに関連付けられている[検証済みのメールアドレス](../../../../user/profile/_index.md#change-your-primary-email)と一致する必要があります。
...
- OpenSSH 8.1以降。
  ...
  > [!note] OpenSSH 8.7には、署名機能が壊れています。OpenSSH 8.7を使用している場合は、OpenSSH 8.8にアップグレードしてください。
...
- Gitの`user.email`が、GitLabアカウントに関連付けられている[検証済みのメールアドレス](../../../../user/profile/_index.md#change-your-primary-email)と一致している必要があります。
