...
1. **GitリポジトリのURL**を入力します。`ssh://gitlab.com/gitlab-org/gitlab.git`の形式でURLを指定してください。
   ...
   > [!note] SSH URLは、SCP形式のURL（`git@host:path/to/repo.git`）ではなく、`ssh://host/path/to/repo.git`の形式を使用する必要があります。コロン（`:`）をスラッシュ（`/`）に置き換え、`ssh://`プレフィックスを追加します。
...
> [!note] [GitLab Silent Mode](../../../../administration/silent_mode/_index.md)は、プッシュおよびプルの両方の更新を無効にします。
...
[プロジェクトアクセストークン](../../settings/project_access_tokens.md)または[グループアクセストークン](../../../group/settings/group_access_tokens.md)の場合、ユーザー名には空白でない値を、パスワードにはトークンを使用します。
...
> [!note]
> 生成されたキーは、GitLabデータベースに保存され、ファイルシステムには保存されません。そのため、ミラー用のSSH公開キーの認証は、事前受信フックでは使用できません。
