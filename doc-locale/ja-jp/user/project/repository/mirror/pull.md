...
> [!note]
> ダウンストリームプルミラーリポジトリから削除されたアイテムでも、アップストリームリポジトリに残っているものは、次回のプル時に復元されます。例: ミラーリングされたリポジトリのみで削除されたブランチは、次回のプル後に再び表示されます。
...
> [!warning]
> ブランチをミラーしている場合、このオプションを有効にするとローカルの変更が失われます。
...
> [!warning]
> この機能は、自身のプロジェクト、または信頼できるメンテナーがいるプロジェクトでのみ有効にしてください。
