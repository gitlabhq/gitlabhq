...
1. ブランチ名の横に、ファイル名と拡張子を入力します。例: `my_file.md`。
1. ファイルにコンテンツを追加します。
1. **変更をコミットする**を選択します。
...
事前にコンテンツが入力された状態で新しいファイルエディタを開くには、URLの`content`クエリパラメータを使用します。例: 
...
1. 新しいファイルを作成したいディレクトリに移動します。
1. ディレクトリ名の横にあるプラスアイコン（{{< icon name="plus" >}}）> **新しいファイル**を選択します。
1. **Filename**（ファイル名）に、GitLabが標準で用意しているテンプレートの名を入力します。
   - `.gitignore`
   - `.gitlab-ci.yml`
   - `LICENSE`
   - `Dockerfile`
...
Webエディタ内の単一行または複数行にリンクするには、URLのファイル名部分にハッシュ情報を追加します。例: 
...
フォークされたプロジェクトからのマージリクエストを使って作業している場合は、ファイルを編集して変更をコミットできます。これを行うには、次の手順を実行します:

1. マージリクエストに移動します。
1. 編集したいファイルに移動します。
1. **編集** > **単一のファイルを編集**を選択します。
1. **変更をコミットする**を選択します。
1. **コミットメッセージ**に、コミットの理由を入力します。次の情報が提供されます: `Your changes can be committed to <branch-name> because a merge request is open.`
1. **変更をコミットする**を選択します。
...
1. GitLabでファイルを開きます。
1. ファイル名の右上にある**アクション**（{{< icon name="ellipsis_v" >}}）を選択します。
1. 目的の操作を選択します。
...
Webエディタは[Monaco Editor](https://github.com/microsoft/monaco-editor)をベースにしており、いくつかの[アクセシビリティ機能](https://github.com/microsoft/monaco-editor/wiki/Monaco-Editor-Accessibility-Guide)が用意されています:
...
- [ファイルツリーブラウザー](files/file_tree_browser.md)
