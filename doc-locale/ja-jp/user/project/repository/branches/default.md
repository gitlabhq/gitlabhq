...
[プロジェクトAPI](../../../../api/projects.md)の`default_branch`属性も使用できます。APIでプロジェクトを作成し、`initialize_with_readme`を`true`に設定すると、`default_branch`パラメータを次のように指定できます:

- ブランチ名。例: `main`。
- 完全修飾された参照。例: `refs/heads/main`。

完全修飾された参照を指定すると、APIは`refs/heads/`プレフィックスを削除します。
...
1. グループオーナーがインスタンスのデフォルトブランチ保護をオーバーライドできるようにするには、[**オーナーがグループごとにデフォルトのブランチ保護を管理できるようにする**](#prevent-overrides-of-default-branch-protection)を選択します。
1. **変更を保存**を選択します。
...
> [!note]
> GitLabの管理者は、引き続きグループのデフォルトブランチ保護を更新できます。
...
> [!warning]
> デフォルトブランチの名前を変更すると、テスト、CI/CDの設定、サービス、ヘルパーユーティリティ、およびリポジトリが使用するあらゆるインテグレーションが破損する可能性があります。このブランチ名を変更する前に、プロジェクトオーナーとメンテナーに相談してください。この変更のスコープには、関連するコードやスクリプト内での古いブランチ名の参照も含まれていることを、プロジェクトオーナーおよびメンテナーが理解していることを確認してください。
...
- [2021年3月のブログ投稿: The new Git default branch name](https://about.gitlab.com/blog/new-git-default-branch-name/)（新しいGitのデフォルトブランチ名）
