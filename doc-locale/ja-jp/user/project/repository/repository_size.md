...
description: リポジトリのサイズがどのように計算および表示されるか、サイズ制限、およびGitリポジトリのリポジトリストレージを削減する方法を理解する。
title: リポジトリサイズ
---
...
リポジトリサイズは、リポジトリ内のすべてのファイルの累積サイズです。この計算は、リポジトリの[ハッシュ](../../../administration/repository_storage_paths.md)化されたストレージパスで`du --summarize --bytes`を実行するのと似ています。サイズはリポジトリが変更されたときに再計算されますが、15分に1回を超えることはありません。

リポジトリサイズは、プロジェクトサイズのサブセットです。**プロジェクト情報**の下のプロジェクト概要ページに表示されるプロジェクトサイズには、リポジトリのほか、LFSオブジェクト、ジョブアーティファクト、パッケージ、およびWikiが含まれます。

### リポジトリサイズを表示 {#view-repository-size}

前提条件: 

- プロジェクトのメンテナーまたはオーナーロール、あるいはネームスペースのオーナーロール。
...
1. **ストレージ**タブを選択します。
1. 使用量の内訳で、**リポジトリ**行の値を表示します。

プロジェクトが使用するストレージの詳細については、[ストレージ](../../storage_usage_quotas.md)を参照してください。

## サイズとストレージの制限 {#size-and-storage-limits}

管理者は、GitLab Self-Managedの[リポジトリサイズ制限](../../../administration/settings/account_and_limit_settings.md#repository-size-limit)を設定できます。GitLab.comの場合、サイズ制限は[事前に定義されています](../../gitlab_com/_index.md#account-and-limit-settings)。
...
### ローカルクローンを変更する必要がある{#local-clones-must-be-changed}
...
### 実行中のパイプラインがクリーンアップを失敗させる可能性がある {#running-pipelines-can-cause-cleanup-to-fail}
...
### フォークしたプロジェクトでは履歴を書き換えることはできない {#history-cannot-be-rewritten-on-forked-project}
...
> [!note]
> コミットに関する情報（ファイルコンテンツを含む）はデータベースにキャッシュされ、リポジトリから削除された後も表示されたままになります。
...
> [!warning]
> 内部参照を削除すると、関連するマージリクエストコミット、パイプライン、および変更の詳細が利用できなくなります。
...
> [!note]
> 文字列を置換文字列`***REMOVED***`に置き換えることもできます。詳細については、[リポジトリからテキストを削除する](#redact-text-from-repository)を参照してください。
...
> [!note]
> blob削除を成功させるには、プロセス中にリポジトリへのアクセスを一時的に制限することを検討してください。blobの消去中に新しいコミットがプッシュされると、操作が失敗する可能性があります。
...
> [!note]
> 機密情報を含むプロジェクトがフォークした場合、ハウスキーピングタスクはこのプロセスを完了せずに成功する可能性があります。ハウスキーピングは、フォークされたデータを含む[特別なオブジェクトプールリポジトリ](../../../administration/housekeeping.md#object-pool-repositories)の整合性を保持する必要があります。サポートが必要な場合は、GitLabサポートにお問い合わせください。
...
> [!warning]
> このアクションは元に戻せません。履歴を書き換え、ハウスキーピングを実行すると、変更は永続的になります。
...
> [!note]
> 機密情報を含むプロジェクトがフォークした場合、ハウスキーピングタスクは、特殊なオブジェクトプールリポジトリ[にフォークされたデータが含まれている](../../../administration/housekeeping.md#object-pool-repositories)の整合性を維持するために、この削除プロセスを失敗する可能性があります。サポートが必要な場合は、GitLabサポートにお問い合わせください。
...
> [!note]
> 新しいコミットでファイルを削除しても、以前のコミットとblobが存在するため、リポジトリサイズはすぐに減少しません。サイズを効果的に縮小するには、[`git filter-repo`](https://github.com/newren/git-filter-repo)のようなツールを使用して、履歴を書き換える必要があります。
