...
description: Gitリポジトリのサイズ計算、制限、およびGitリポジトリのリポジトリストレージを削減する方法について説明します。
title: リポジトリサイズ
---
...
## Gitの履歴を書き換える前に {#before-you-rewrite-git-history}

Gitの履歴を書き換えてリポジトリからデータを削除する前に、以下のセクションの情報を考慮する必要があります。

### ローカルクローンを変更する必要があります {#local-clones-must-be-changed}

リポジトリのクローンを持っているユーザーは全員、次のいずれかの操作を行う必要があります:

- ローカルコピーを削除し、リポジトリの新しいコピーをクローンします。
- すべてのリモート変更をフェッチし、進行中のすべての作業がその上にリベースされていることを確認します。

適切に行われないと、ローカルの変更をプッシュするユーザーは、削除されるはずだったファイルを復活させてしまう可能性があります。

### 実行中のパイプラインによってクリーンアップが失敗する可能性があります {#running-pipelines-can-cause-cleanup-to-fail}

クリーンアップ中にパイプラインがまだ実行されている場合、それらはリポジトリと相互作用し、クリーンアップが正しく機能しない可能性があります。

### ガベージコレクションの猶予期間 {#garbage-collection-grace-period}

GitLabは30分の猶予期間をもってGitのガベージコレクションを実行します。このプロセスは、以下の両方のオブジェクトをクリーンアップします:

- いずれの参照からも到達不能。
- 少なくとも30分経過している。

削除したいデータがいずれかのコミットから到達可能であるか、30分未満のものである場合、Gitのガベージコレクションでは削除されません。

その結果、ハウスキーピングを実行した後、**到達不能オブジェクトを排除する**を選択するまで、少なくとも30分待つ必要があります。

### フォークしたプロジェクトでは履歴を書き換えることはできません {#history-cannot-be-rewritten-on-forked-project}

ビルトインメソッドは、フォークしたリポジトリでは使用できません。GitLabオブジェクトがフォーク間で保存される方法では、Gitがフォーク間で共有されているオブジェクトをガベージコレクションすることは不可能です。

### 回避策: プロジェクトをアーカイブする {#workaround-archive-the-project}

クリーンアップを行う前に[アーカイブ](../working_with_projects.md#archive-a-project)すると、リポジトリは読み取り専用になります。これにより、クリーンアップ中に誰も変更を加えることがなくなります。

リポジトリをアーカイブすると、フォーク関係も削除されます。これにより、データをクリーンアップすることができます。しかし、データがフォークにプルされた場合、そこでもクリーンアップを行う必要があります。
...
> [!note]
> ファイルコンテンツを含むコミットに関する情報はデータベースにキャッシュされ、リポジトリから削除された後も表示されたままになります。
...
機密情報または秘密データをリポジトリから削除するには、以下のいずれかの方法を使用します:

- ファイルを完全に削除するには、[blobを削除](#remove-blobs)します。
- ファイルを保持しつつ、機密テキストを置換テキスト`***REMOVED***`に置き換えるには、[テキストを削除する](#redact-text-from-repository)を使用します。
...
> [!note]
> 置換文字列`***REMOVED***`で文字列を置き換えることもできます。詳細については、[リポジトリからテキストを削除する](#redact-text-from-repository)を参照してください。
...
> [!note] blobの削除を成功させるために、プロセス中は一時的にリポジトリへのアクセスを制限することを検討してください。blobの消去中に新しいコミットがプッシュされると、操作が失敗する可能性があります。
...
1. blobの削除が完了するまで待ってから続行してください:
   - もし[メールの通知](../../../user/profile/notifications.md)が有効になっている場合、リポジトリの履歴の書き換えが完了したことを示すメールが届くまで待機してください。
   - もしメールの通知が有効になっていない場合、少なくとも5分間待機してください。
...
> [!note]
> 機密情報を含むプロジェクトがフォークしたものである場合、ハウスキーピングタスクはこのプロセスを完了せずに成功する可能性があります。ハウスキーピングは、フォークされたデータを含む[特別なオブジェクトプールリポジトリ](../../../administration/housekeeping.md#object-pool-repositories)の整合性を保持する必要があります。サポートが必要な場合は、GitLabサポートにお問い合わせください。

#### オブジェクトIDのリストを取得する {#get-a-list-of-object-ids}

blobを消去するには、消去するオブジェクトのリストが必要です。これらのIDを取得するには、`ls-tree`コマンドを使用するか、[リポジトリAPIリポジトリツリーのリスト](../../../api/repositories.md#list-all-repository-trees-in-a-project)エンドポイントを使用します。次の手順では、`ls-tree`コマンドを使用します。
...
- GitLab 17.9の機密イシュー`https://gitlab.com/gitlab-org/gitlab/-/issues/472018`で一般提供開始。機能フラグ`rewrite_history_ui`は削除されました。

{{< /history >}}

誤ってコミットされた機密情報や社外秘の情報を完全に削除し、リポジトリの履歴からアクセスできないようにします。文字列のリストを`***REMOVED***`に置き換えます。

> [!warning]このアクションは元に戻すことができません。履歴を書き換え、ハウスキーピングを実行すると、変更は永続的になります。

GitLabでファイルを削除すると、公開されたシークレットが削除されますが、同時に以下のことも行われます:

- Gitの履歴を書き換えます。古いコミット履歴に基づく履歴タグとブランチが正しく機能しない場合があります。
- 破壊的です。既存のローカルクローンは、更新されたリポジトリとの互換性がないため、再度クローンする必要があります。
- 削除によってコンテンツが更新されるため、コミットハッシュが更新されます。
- 書き換えプロセス中にコミット署名が破棄されます。
- コミットハッシュに依存する次のような機能が破損します。
  - マージリクエストを開く。未完了のマージリクエストはマージに失敗し、手動でのリベースが必要になる場合があります。
...
代替手段として、リポジトリから特定のファイルを完全に削除するには、[blobを削除する](#remove-blobs)を参照してください。
...
1. **リポジトリの保守**を展開します。
1. **テキストを削除する**を選択します。
1. ドロワーで、削除するテキストを入力します。正規表現とグロブパターンが使用できます。
1. **一致する文字列を削除**を選択します。
1. 確認ダイアログで、プロジェクトパスを入力します。
...
> [!note] 機密情報を含むプロジェクトがフォークされている場合、ハウスキーピングタスクは、この削除処理を完了できなくなり、[フォークされたデータを含む](../../../administration/housekeeping.md#object-pool-repositories)特別なオブジェクトプールリポジトリの整合性が維持されなくなる可能性があります。サポートが必要な場合は、GitLabサポートにお問い合わせください。

## トラブルシューティング {#troubleshooting}

これらのセクションでは、発生する可能性のある問題に対する解決策が示されています。
...
> [!note]
> 新しいコミットでファイルを削除しても、以前のコミットやblobがまだ存在するため、リポジトリサイズはすぐに削減されません。サイズを効果的に縮小するには、[`git filter-repo`](https://github.com/newren/git-filter-repo)のようなツールを使用して、履歴を書き換える必要があります。
