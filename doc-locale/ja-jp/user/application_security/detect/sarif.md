...
1. 以下のCWEはシークレット関連です:
   ...
   - [CWE-321（ハードコードされた暗号キー）](https://cwe.mitre.org/data/definitions/321.html)。
...
- 文字列フィールドが[文字数制限](#limits)を超過しています。
...
ファイルごとの実行制限については、[制限](#limits)を参照してください。
...
| 実行あたりの最大結果数                | 5,000                                                         | {{< no >}}   |
| 実行あたりの最大ルール数                  | 25,000                                                        | {{< no >}}   |
| ルールあたりの最大タグ数                  | 10                                                            | {{< no >}}   |
| `rule.name`の最大長             | 255文字                                                | {{< no >}}   |
| `shortDescription.text`の最大長 | 1,024文字                                              | {{< no >}}   |
| `fullDescription.text`の最大長  | 1,024文字。発見タイトルとして使用される場合は255文字に切り詰められます。 | {{< no >}}   |
| `message.text`の最大長          | 1,024文字。発見タイトルとして使用される場合は255文字に切り詰められます。 | {{< no >}}   |
| `helpUri`の最大長               | 2,048文字                                              | {{< no >}}   |
| サポートされているSARIFバージョン               | 2.1.0のみ                                                    | {{< no >}}   |

実行あたりのカウントが制限を超えると、GitLabは最初のN個のエントリを処理し、警告を記録します。結果に文字数制限を超える文字列フィールドがある場合、結果全体がスキップされ、[ドロップ率](#ingestion-behavior)にカウントされます。
