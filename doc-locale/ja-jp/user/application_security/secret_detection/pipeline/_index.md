...
<i class="fa-youtube-play" aria-hidden="true"></i> このパイプラインシークレット検出ドキュメントのインタラクティブな読み取りおよびハウツーデモについては、以下をご覧ください。

- [How to enable secret detection in GitLab Application Security Part 1/2（GitLabアプリケーションセキュリティでシークレット検出を有効にする方法: パート1/2）](https://youtu.be/dbMxeO6nJCE?feature=shared)
- [How to enable secret detection in GitLab Application Security Part 2/2（GitLabアプリケーションセキュリティでシークレット検出を有効にする方法: パート2/2）](https://youtu.be/VL-_hdiTazo?feature=shared)
...
| マージリクエストウィジェットで新しい発見を確認する                            | {{< no >}}        | {{< yes >}} |
...
- ブランチの場合:
  ...
  - 新規の**new, non-default branch**では、最新のコミットのコンテンツのみがスキャンされます。ブランチの以前のコミットはスキャンに含まれません。
  - 既存の**existing, non-default branch**では、最後にプッシュされたコミットから最新のコミットまでのすべてのコミットのコンテンツがスキャンされます。
  - 分岐点からのすべてのコミットを常にスキャンするには、[merge request pipelines](../../detect/security_configuration.md#use-security-scanning-tools-with-merge-request-pipelines)を有効にします。
...
デフォルトでは、GitLabが最初にリポジトリをクローンするときは、最新のコミットのみをフェッチします（「シャロークローン」）。この初期クローンを超えて追加のコミットが必要な場合、アナライザーは最適化された戦略を使用してそれらを自動的にフェッチします:

- マージリクエストの場合、アナライザーはマージベースより後にコミットされた変更のみを取得し、データ転送を最小限に抑えます。
- 例えば`--since`や`--max-count`のようなログオプションが指定されている場合、アナライザーは必要なコミットのみをフェッチします。
- 履歴スキャン中、アナライザーはリポジトリの完全な履歴をフェッチします。もしリポジトリがシャロークローンされていた場合、アナライザーは`--unshallow`オプションを使用します。

アナライザーが必要なコミットをフェッチできない場合、利用可能なデータをスキャンするフォールバックを行います:

- 強制プッシュ後、アナライザーはリポジトリの現在の状態のみをスキャンします。
- ネットワーク障害が発生した場合、アナライザーは初期クローン後に利用可能なコミットをスキャンします。
- タイムアウトが発生した場合、アナライザーは部分的なコミット履歴でスキャンを継続します。
...
制限されたネットワーク環境で、不足しているコミットに関する永続的な問題が発生する場合は、トラブルシューティングの回避策を参照してください。
...
- 特定のシークレットは、シークレット値全体ではなく、そのプレフィックスを検索することによって検出されます。これらのシークレットタイプでは、同じファイル内にある同じタイプの検出すべてが、単一の発見としてレポートされます。
...
シークレットが検出されると、そのシークレットに対して脆弱性が作成されます。スキャンされたファイルからシークレットが削除され、パイプラインシークレット検出が再度実行された場合でも、脆弱性は「検出されたまま」になります。これは、流出したシークレットは、失効するまでセキュリティ上のリスクであり続けるためです。削除されたシークレットもGit履歴に残り続けます。Gitリポジトリの履歴からシークレットを削除するには、[リポジトリからテキストを削除する](../../../project/repository/repository_size.md#redact-text-from-repository)をご覧ください。
...
1. 周囲のコードを見て、検出されたパターンが実際にシークレットであるかどうかを判断します。
...
これについては、[general application security troubleshooting section](../../../../ci/jobs/job_artifacts_troubleshooting.md#error-message-no-files-to-upload)を参照してください。

#### エラー: `Couldn't run the gitleaks command: exit status 2` {#error-couldnt-run-the-gitleaks-command-exit-status-2}

このエラーは、アナライザーが必要なコミットにアクセスできないことを示します。ほとんどの場合、アナライザーは不足しているコミットを自動的にフェッチしますが、制限された環境では問題が発生する可能性があります。

問題を診断するには、[debug-level logging](../../troubleshooting_application_security.md#debug-level-logging)を有効にして以下を探してください:
...
#### `GIT_DEPTH`を調整しても、スキャンされる内容は変更されません {#adjusting-git_depth-doesnt-change-what-gets-scanned}

これは予期された動作です。`GIT_DEPTH`は初期クローン用のRunner変数です。アナライザーの動作は変更されません。

シークレット検出アナライザーは、以下に基づいてスキャンする内容を決定します:

- パイプラインタイプ（プッシュ、マージリクエスト、スケジュール済み）
- ブランチコンテキスト（デフォルト、新規、既存）
- あなたの設定（`SECRET_DETECTION_LOG_OPTIONS`、`SECRET_DETECTION_HISTORIC_SCAN`）

たとえば、30個のコミットのみをスキャンするには:
...
オプションの全リストについては、[Git log options](https://git-scm.com/docs/git-log)ドキュメントを参照してください。

#### 強制プッシュ検出 {#force-push-detection}

強制プッシュ後、次のようなメッセージが表示される場合があります:

```plaintext
Failed to retrieve all the commits from the last Git push event due to a force push
```

これは予期された動作です。スキャンは現在のリポジトリの状態を使用して続行されます。
...
これはコンテナ化された環境における一般的なセキュリティ設定を示しています。アクションは不要です。
