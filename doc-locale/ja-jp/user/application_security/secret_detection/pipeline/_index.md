...
- それらを[マージリクエストレポート](../../../project/merge_requests/reports.md)、[パイプラインセキュリティレポート](../../detect/security_scanning_results.md)、および[脆弱性レポート](../../vulnerability_report/_index.md)で確認できます。
- 承認ワークフローで結果を使用する。
...
<i class="fa-youtube-play" aria-hidden="true"></i>このパイプラインシークレット検出ドキュメントのインタラクティブな読み方とハウツーデモについては、以下を参照してください:

- [GitLabアプリケーションセキュリティでシークレット検出を有効にする方法パート1/2](https://youtu.be/dbMxeO6nJCE?feature=shared)
- [GitLabアプリケーションセキュリティでシークレット検出を有効にする方法パート2/2](https://youtu.be/VL-_hdiTazo?feature=shared)

<i class="fa-youtube-play" aria-hidden="true"></i> その他のインタラクティブな読み取りおよびハウツーデモについては、[Get Started With GitLab Application Security Playlist](https://www.youtube.com/playlist?list=PL05JrBw4t0KrUrjDoefSkgZLx5aJYFaF9)（GitLabアプリケーションセキュリティ入門プレイリスト）をご覧ください。
...
|:------------------------------------------------------------------------|:------------------|:------------|
| [アナライザーの動作をカスタマイズする](configure.md#customize-analyzer-behavior) | {{< yes >}}       | {{< yes >}} |
| [出力](#secret-detection-results)をダウンロードする                            | {{< yes >}}       | {{< yes >}} |
| マージリクエストレポートで新しい検出結果を表示します。                               | {{< no >}}        | {{< yes >}} |
| パイプラインの**セキュリティ**タブで特定されたシークレットを表示する              | {{< no >}}        | {{< yes >}} |
| [脆弱性を管理する](../../vulnerability_report/_index.md)          | {{< no >}}        | {{< yes >}} |
| [セキュリティダッシュボード](../../security_dashboard/_index.md)にアクセスします。     | {{< no >}}        | {{< yes >}} |
| [アナライザールールセットをカスタマイズする](configure.md#customize-analyzer-rulesets) | {{< no >}}        | {{< yes >}} |
| [セキュリティポリシーを有効にする](../../policies/_index.md)                    | {{< no >}}        | {{< yes >}} |
...
1. 左サイドバーで、**ビルド** > **パイプラインエディタ**を選択します。
...
- ブランチの場合:
  - **デフォルトブランチ**上:
    - アナライザーバージョン`v7.38.1`以降で、有効な以前のコミットが[事前定義されたCI/CD変数](../../../../ci/variables/predefined_variables.md)`CI_COMMIT_BEFORE_SHA`を介して利用可能な場合、以前のコミットから現在のコミットまでのすべてのコミットのコンテンツがスキャンされます。
    - 有効な以前のコミットが利用できない場合（たとえば、新しいリポジトリに最初にプッシュされたコミット）、Gitワークツリーの内容がスキャンされます。
  - **feature branch**上:
    - アナライザーバージョン`v7.35.0`以降では、マージベースから最新のコミットまでのすべてのコミットの内容（分岐後にブランチに固有のすべてのコミット）がスキャンされます。この動作は、マージベースが利用可能な場合のすべてのフィーチャーブランチパイプラインに適用されます。
    - GitLab 19.1以降では、[事前定義されたCI変数](../../../../ci/variables/predefined_variables.md) `CI_COMMIT_DEFAULT_BRANCH_BASE_SHA`を介してマージベースのSHAが公開されます。
    - GitLabバージョン19.1以前、またはマージベースが利用できない場合、アナライザーは以前の動作にフォールバックします:
      - 新しいフィーチャーブランチでは、最新のコミットのみがスキャンされます。ブランチ上の以前のコミットはスキャンに含まれません。
      - 既存のフィーチャーブランチでは、最後にプッシュされたコミットから最新のコミットまでのすべてのコミットがスキャンされます。

    > [!note]
    > ブランチの分岐点からのすべてのコミットを毎回スキャンするには、[マージリクエストパイプライン](../../../../ci/pipelines/merge_request_pipelines.md)を有効にします。
...
### アナライザーがコミットをフェッチする方法 {#how-the-analyzer-fetches-commits}

デフォルトでは、GitLabが最初にリポジトリをクローンする際、最新のコミットのみをフェッチする（「シャロークローン」）です。この初期クローンを超えて追加のコミットが必要な場合、アナライザーは最適化された戦略を使用してそれらを自動的にフェッチします:

- マージリクエストの場合、アナライザーはマージベース以降にコミットされた変更のみを取得するし、データ転送を最小限に抑えるます。
- ログオプション（`--since`や`--max-count`など）が指定されている場合、アナライザーは必要なコミットのみをフェッチします。
- 履歴スキャン中、アナライザーはリポジトリの完全な履歴をフェッチします。リポジトリがシャロークローンされた場合、アナライザーは`--unshallow`オプションを使用します。

アナライザーが必要なコミットをフェッチできない場合、利用可能なデータをスキャンすることにフォールバックします:

- 強制プッシュ後、アナライザーはリポジトリの現在の状態のみをスキャンします。
- ネットワーク失敗がある場合、アナライザーは初期クローン後に利用可能なコミットをスキャンします。
- タイムアウトが発生した場合、アナライザーは部分的なコミット履歴でスキャンを続行します。

これらのフォールバックにより、制限されたコンテナ化された環境でもパイプラインが正常に完了することが保証されます。

### 初期リポジトリクローン深度 {#initial-repository-clone-depth}

Runnerの[`GIT_DEPTH`](../../../../ci/runners/configure_runners.md#shallow-cloning)は、最初にクローンされるコミットの数を制御します。パイプラインシークレット検出は、必要に応じて追加のコミットを自動的にフェッチするため、通常この設定を調整する必要はありません。

制限されたネットワークコンテナ化された環境でコミットが見つからないという永続的な問題が発生する場合は、回避策についてトラブルシューティングを参照してください。
...
- 特定のシークレットは、シークレット値全体ではなく、そのプレフィックスを検索することで検出されます。これらのシークレットタイプでは、同じファイル内にある同じタイプの検出すべてが、単一の発見としてレポートされます。

  たとえば、SSH秘密キーは、プレフィックス`-----BEGIN OPENSSH PRIVATE KEY-----`によって検出されます。同じファイルに複数のSSH秘密キーがある場合、パイプラインシークレット検出は1つの発見のみを作成します。
- 履歴スキャンを実行する場合、または既存のコミットでパイプラインシークレット検出を有効にする場合、1つのコミットでシークレットが導入され、同じスキャン中に以降のコミットで変更された場合、最新のシークレット値のみが脆弱性レポートに表示されます。
...
シークレットが検出されると、それに対して脆弱性が作成されます。スキャンされたファイルからシークレットが削除され、パイプラインシークレット検出が再度実行された場合でも、脆弱性は「検出されたまま」になります。これは、流出したシークレットは、失効するまでセキュリティ上のリスクであり続けるためです。削除されたシークレットもGit履歴に残り続けます。Gitリポジトリの履歴からシークレットを削除するには、[リポジトリからテキストを削除する](../../../project/repository/repository_size.md#redact-text-from-repository)をご覧ください。
...
- [マージリクエストレポート](../../../project/merge_requests/reports.md): マージリクエストで導入された新しい検出結果を表示します。
- [パイプラインセキュリティレポート](../../detect/security_scanning_results.md): 最新のパイプライン実行から得られたすべての発見を表示します。
- [脆弱性レポート](../../vulnerability_report/_index.md): すべてのセキュリティ検出結果の一元管理を提供します。
...
1. 検出されたパターンが実際にシークレットであるかどうかを判断するには、周囲のコードを参照してください。
...
これについては、[一般的なアプリケーションセキュリティのトラブルシューティングセクション](../../../../ci/jobs/job_artifacts_troubleshooting.md#error-message-no-files-to-upload)を参照してください。

#### エラー: `Couldn't run the gitleaks command: exit status 2` {#error-couldnt-run-the-gitleaks-command-exit-status-2}

このエラーは、アナライザーが必要なコミットにアクセスできないことを示しています。アアナライザーはほとんどの場合、欠落しているコミットを自動的にフェッチしますが、制限されたコンテナ化された環境では問題が発生する可能性があります。

問題を診断するには、[デバッグレベルのロギング](../../troubleshooting_application_security.md#turn-on-debug-level-logging)を有効にして、以下を検索します:
...
- ほとんどの場合、アクションは不要です。アナライザーにフェッチを自動的に処理させます。
- 制限されたネットワークの場合は、初期クローン深度を増やします:
...
パイプラインシークレット検出は、リポジトリのデフォルトブランチが、ジョブがトリガーされたブランチと関連がない場合、`ERR fatal: ambiguous argument`というメッセージで失敗する可能性があります。詳細については、イシュー[!352014](https://gitlab.com/gitlab-org/gitlab/-/issues/352014)を参照してください。
...
#### `GIT_DEPTH`の調整では、スキャンされる内容は変更されません {#adjusting-git_depth-doesnt-change-what-gets-scanned}

これは予期される動作です。`GIT_DEPTH`は初期クローン用の変数です。アナライザーの動作は変更されません。

シークレット検出アナライザーは、以下に基づいてスキャンする内容を決定します:

- パイプラインの種類（プッシュ、マージリクエスト、スケジュール済み）
- ブランチのコンテキスト（デフォルト、新規、既存）
- あなたの設定（`SECRET_DETECTION_LOG_OPTIONS`、`SECRET_DETECTION_HISTORIC_SCAN`）

たとえば、30コミットのみをスキャンするには:
...
`HEAD~10`から`HEAD`までのコミットのみをスキャンするには:
...
オプションの完全なリストについては、[Gitログオプション](https://git-scm.com/docs/git-log)のドキュメントを参照してください。

#### 強制プッシュ検出 {#force-push-detection}
...
これは予期される動作です。スキャンは現在のリポジトリの状態を使用して続行されます。

#### リポジトリ信頼設定 {#repository-trust-configuration}
...
これは、コンテナ化された環境における一般的なセキュリティ設定を示しています。アクションは不要です。
