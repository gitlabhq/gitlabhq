...
シークレット検出は、アクティビティをモニタリングして、次の両方を行います。
...
## 誤検出を減らすGitLab Duo {#reducing-false-positives-with-gitlab-duo}

シークレット検出のスキャナーは、誤検出を生成し、脆弱性レポートにノイズを発生させる可能性があります。[GitLab Duo誤検出判定](../vulnerabilities/secret_false_positive_detection.md)機能は、シークレット検出の結果を自動的に分析し、誤検出の可能性が高いものを特定します。これにより、セキュリティチームは本物のシークレットに集中し、手動でのトリアージにかかる時間を削減できます。

GitLab Duoアドオンをご利用のUltimateのお客様の場合、誤検出判定は、各セキュリティスキャンの後に自動的に実行され、各評価の説明とともに信頼度スコアが提供されます。
