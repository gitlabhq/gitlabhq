...
SASTスキャナーは、脆弱性レポートにノイズを発生させる誤検出を生成する可能性があります。GitLab Duoは、脆弱性管理を支援します。

### 誤検出の検出 {#false-positive-detection}

[GitLab Duo false positive detection](../vulnerabilities/false_positive_detection.md)は、重大度が高いSASTの脆弱性を自動的に分析して、可能性のある誤検出を特定します。これにより、セキュリティチームは真の脆弱性に集中し、手動トリアージに費やす時間を削減できます。

GitLab Duoアドオンをご利用のUltimateのお客様の場合、誤検出の検出は、各セキュリティスキャンの後に自動的に実行され、各評価の説明とともに信頼性スコアが提供されます。

### エージェント型のSAST脆弱性の解決 {#agentic-sast-vulnerability-resolution}

[Agentic SAST Vulnerability Resolution](../vulnerabilities/agentic_vulnerability_resolution.md)は、高度および重大の重大度のSAST脆弱性に対するコンテキスト認識型コード修正を含むマージリクエストを自動的に生成します。このエージェント型アプローチでは、複数回の推論を使用して、人間の介入を最小限に抑えて脆弱性を解決します。
...
| [GitLab Duo false positive detection](../vulnerabilities/false_positive_detection.md)（GitLab Duoアドオンが必要です）               | {{< no >}}        | {{< yes >}} |
| [Agentic SAST Vulnerability Resolution](../vulnerabilities/agentic_vulnerability_resolution.md)（GitLab Duo Enterpriseが必要です） | {{< no >}}        | {{< yes >}} |
...
- DockerまたはKubernetes executorを使用した、LinuxベースのGitLab Runner。GitLab.com用のホストRunnerを使用している場合は、デフォルトで有効になっています。
...
> [!note]デフォルトでは、SASTはブランチパイプラインでのみ実行されます。マージリクエストパイプラインでSASTを実行するには、[use security scanning tools with merge request pipelines](../detect/security_configuration.md#use-security-scanning-tools-with-merge-request-pipelines)を参照してください。
...
> [!note] UI設定方法は、既存の`.gitlab-ci.yml`ファイルが最小限である場合に最適に機能します。設定が複雑な場合、ツールが解析に失敗する可能性があります。その場合は、代わりに[CI/CDファイルを編集](#enable-sast-by-editing-the-cicd-file)してください。

カスタマイズしてSASTを有効化および設定するには:

1. 上部のバーで、**検索または移動先**を選択して、プロジェクトを見つけます。
1. **セキュリティ** > **セキュリティ設定**を選択します。
1. プロジェクトのデフォルトブランチの最新のパイプラインが完了し、有効なSASTアーティファクトが生成された場合は、**Configure SAST**を選択します。それ以外の場合は、静的アプリケーションセキュリティテスト（SAST）行の**Enable SAST**を選択します。
1. SASTのカスタム値を入力します。

   カスタム値は`.gitlab-ci.yml`ファイルに保存されます。SAST設定ページにないCI/CD変数の場合、それらの値はGitLab SASTテンプレートから継承されます。
...
## 結果の把握 {#understanding-the-results}
...
> [!note]検出結果はフィーチャーブランチで生成されます。その発見がデフォルトブランチにマージされると、脆弱性になります。この区別は、セキュリティ対策状況を評価する上で重要です。
...
- [リモート設定ファイルを指定](customize_rulesets.md#remote-ruleset-file)して、中央ルールセットを共有および再利用します。
...
SASTにおける言語サポート計画の詳細については、[category direction page](https://about.gitlab.com/direction/application_security_testing/static-analysis/sast/#language-support)を参照してください。
...
これらの言語は、高度なSAST（Ultimate）と標準アナライザー（全プラン）の両方でサポートされています: 
...
1. 参照されているエピックでは、これらの言語に対する高度なSASTのサポートが提案されています。
...
高度な脆弱性の追跡のサポートは、言語と使用されるアナライザーによって異なります。

GitLab Advanced SASTアナライザーとSemgrepベースのアナライザーの両方でサポートされている言語:
...
Semgrepベースのアナライザーのみでサポートされている言語:
...
GitLab SASTは、デフォルト設定で使用するように設計されています。ただし、必要に応じて、[change configuration variables](#available-cicd-variables)または[customize detection rules](customize_rulesets.md)を実行できます。

### 安定版と最新版のSASTテンプレート {#stable-vs-latest-sast-templates}

SASTには、本番環境で使用されるデフォルトの[`stable`](https://gitlab.com/gitlab-org/gitlab/-/blob/master/lib/gitlab/ci/templates/Jobs/SAST.gitlab-ci.yml)テンプレートと、最先端の機能をテストするための[`latest`](https://gitlab.com/gitlab-org/gitlab/-/blob/master/lib/gitlab/ci/templates/Jobs/SAST.latest.gitlab-ci.yml)テンプレートが用意されています。違いとそれぞれの使用時期の詳細については、[template editions](../detect/security_configuration.md#template-editions)を参照してください。
...
自動更新の動作をオーバーライドするには、`SAST.gitlab-ci.yml`テンプレートをインクルードした後、CI/CD設定ファイルで`SAST_ANALYZER_IMAGE_TAG` CI/CD変数を設定します。
...
SemgrepベースのSASTアナライザーをカスタマイズして、GitLab管理ルールセットでサポートされていない言語をスキャンできます。ただし、GitLabはこれらの他の言語のルールセットを提供しないため、それらをカバーするには、[replace or add to the default rules](customize_rulesets.md#replace-or-add-to-the-default-rules)する必要があります。また、関連ファイルが変更されたときにジョブが実行されるように、`semgrep-sast` CI/CDジョブの`rules`も変更する必要もあります。
...
1. Rust用のカスタムルールセットを提供します。リポジトリのルートにある`.gitlab/`ディレクトリに、`sast-ruleset.toml`という名前のファイルを作成します。次の例では、SemgrepレジストリのRust用デフォルトルールセットを使用しています。
   ...
   詳細については、[Replace or add to the predefined rules](customize_rulesets.md#replace-or-add-to-the-default-rules)を参照してください。
...
カスタム認証局を信頼するには、SAST環境で信頼するCA証明書のバンドルを`ADDITIONAL_CA_CERT_BUNDLE`変数に設定します。`ADDITIONAL_CA_CERT_BUNDLE`の値には、[X.509 PEM公開キー証明書のテキスト表現](https://www.rfc-editor.org/rfc/rfc7468#section-5.1)が含まれている必要があります。たとえば、`.gitlab-ci.yml`ファイルでこの値を設定するには、以下のように記述します。
...
<table class="sast-table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        GitLab高度なSASTアナライザーが同時に実行される場合に、Semgrepアナライザー専用で除外されるパスのカンマ区切りリスト。これにより、高度なSASTによってすでにスキャンされたファイルを除外することで、脆弱性の重複を防ぎます。このリストは<code>SAST_EXCLUDED_PATHS</code>とマージされます。
      </td>
      <td>なし</td>
      <td>
...
SASTアナライザーは、内部的にOSSのセキュリティスキャナーを使用して分析を実行します。GitLabは、セキュリティスキャナーの推奨設定を設定するため、調整について心配する必要はありません。ただし、まれですが、デフォルトのスキャナー設定が要件に合わない場合があります。
...
<table class="sast-table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        マルチコアスキャンはデフォルトで有効になっており、使用可能なCPUコアを自動検出します（セルフホストRunnerでは4つに制限）。<code>--multi-core <number of cores></code>（たとえば、<code>--multi-core 12</code>）でオーバーライドします。マルチコア実行には、比例してより多くのメモリが必要です。コアあたり4GBを割り当てる必要があります。<code>DISABLE_MULTI_CORE</code>を設定して、無効にします。使用可能なリソースを超えると、パフォーマンスの問題が発生する可能性があります。
      </td>
    </tr>
    <tr>
      <td rowspan="3">
...
- [`docker`](https://docs.gitlab.com/runner/executors/docker.html)または[`kubernetes`](https://docs.gitlab.com/runner/install/kubernetes.html) executorを備えたGitLab Runner。詳細については、[前提条件](#getting-started)を参照してください。
