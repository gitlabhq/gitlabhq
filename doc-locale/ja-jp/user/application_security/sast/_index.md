...
> [!note]
> デフォルトでは、SASTはブランチパイプライン内でのみ実行されます。マージリクエストパイプラインでSASTを実行する場合は、[マージリクエストパイプラインでセキュリティスキャンツールを使用する](../detect/security_configuration.md#use-security-scanning-tools-with-merge-request-pipelines)を参照してください。
...
> [!note]
> UIによる設定方法は、既存の`.gitlab-ci.yml`ファイルがない、または最小限の構成の場合に最も適しています。複雑な設定ファイルがある場合、ツールが正しく解析できないことがあります。その場合は、代わりに[CI/CDファイルを直接編集](#enable-sast-by-editing-the-cicd-file)してください。
...
> [!note]
> Semgrepアナライザーは`.gitignore`ファイルを尊重しません。`.gitignore`に記載されているファイルでも、`.semgrepignore`または`SAST_EXCLUDED_PATHS`を使用して明示的に除外されない限り、分析対象となります。
