...
description: AlmaLinuxおよびRHEL互換ディストリビューションへのLinuxパッケージのインストール
title: AlmaLinuxおよびRHEL互換ディストリビューションへのLinuxパッケージのインストール
---
...
- OS要件:
  ...
  - Oracle Linux 8
  - Oracle Linux 9
  - サポートされているRed Hat Enterprise Linuxバージョンと互換性のあるディストリビューション
...
- `https://` URLの場合、GitLabは自動的に[Let's Encryptで証明書をリクエストします](https://docs.gitlab.com/omnibus/settings/ssl/#enable-the-lets-encrypt-integration)。これには、受信HTTPアクセスと有効なホスト名が必要です。[独自の証明書](https://docs.gitlab.com/omnibus/settings/ssl/#configure-https-manually)を使用することも、`http://`（`s`なし）を暗号化されていないURLに使用することもできます。
...
GitLabは、ルート管理者アカウントのランダムなパスワードとメールアドレスを生成し、24時間`/etc/gitlab/initial_root_password`に保存します。24時間後、このファイルはセキュリティ上の理由から自動的に削除されます。
...
インスタンスをカスタマイズするには、インストール前に次のオプションの環境変数を設定します。**これらの変数は、最初のインストール中にのみ機能します**。また、後続の再構成の実行には影響しません。既存のインストールでは、`/etc/gitlab/initial_root_password`のパスワードを使用するか、[ルートパスワードをリセット](../../security/reset_user_password.md)してください。
...
`gitlab_rails['initial_root_password']`を設定して`/etc/gitlab/gitlab.rb`で最初のパスワードを設定することもできますが、推奨されません。パスワードがクリアテキストであるため、セキュリティ上のリスクがあります。これを構成した場合は、インストール後に必ず削除してください。

{{< /alert >}}

上記の環境変数を使用して、GitLabエディションを選択してカスタマイズします:
...
コミュニケーションをとる時期をお知らせいただくために、[メールサブスクリプション設定センター](https://about.gitlab.com/company/preference-center/)にアクセスしてください。明示的なメールオプトインポリシーがあるため、メールの送信内容と頻度を完全に制御できます。

月に2回、開発チームからの新機能、インテグレーション、ドキュメント、舞台裏のストーリーなど、知っておく必要のあるGitLabニュースをお届けします。バグとシステムパフォーマンスに関連する重要なセキュリティアップデートについては、専用のセキュリティニュースレターにサインアップしてください。
