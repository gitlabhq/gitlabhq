...
復元プロセスでは、バックアップと同じバージョンの既存のGitLabインストールが必要です。本番環境で使用する前に、[前提条件](#restore-prerequisites)に従い、復元プロセス全体をテストしてください。

## 復元の前提条件 {#restore-prerequisites}

### 復元先のGitLabインスタンスがすでに動作している必要がある {#the-destination-gitlab-instance-must-already-be-working}

復元を実行するには、正常に動作しているGitLabインストールが必要です。これは、復元操作を実行するシステムユーザー（`git`）には、通常、データをインポートするために必要なSQLデータベース（`gitlabhq_production`）の作成や削除を行う権限がないためです。

### 宛先のGitLabインスタンスに既存のデータがあってはなりません {#the-destination-gitlab-instance-must-not-have-existing-data}

復元プロセスは、データ型に応じて既存のデータを異なる方法で処理します:

- PostgreSQLのデータは、復元プロセス中に自動的に消去されます。
- Gitリポジトリ: 同じ名前のリポジトリが既に存在する場合、復元は「リポジトリは既に存在します」というエラーで失敗します。詳細については、[イシュー118459](https://gitlab.com/gitlab-org/gitlab/-/issues/118459)を参照してください。
- ファイルシステムデータは、復元前に別のディレクトリに移動が試行されます。
- オブジェクトストレージデータは自動的にクリアされません。孤立したデータが残らないように、復元する前にオブジェクトストレージバケットを手動でクリアする必要があります。

信頼性の高い復元プロセスを実現するため、例えば本番環境からステージングへの復元を自動化する場合、バックアップと同じバージョンの新しいGitLabインストールを使用してください。

SQLデータの復元では、PostgreSQL拡張機能が所有しているビューはスキップされます。
...
バックアップを復元するには、GitLabシークレットも復元する必要があります。新しいGitLabインスタンスに移行する場合は、旧サーバーからGitLabのシークレットファイルをコピーしなくてはなりません。これらには、データベース暗号化キー、CI/CD変数、および2要素認証に使用される変数が含まれます。キーがないと、2要素認証が有効なユーザーのアクセス喪失や、GitLab Runnerがサインインできないなど、複数のイシューが発生します。

> [!warning] 
> **異なるFQDNに復元すると、WebAuthnデバイスは無効になります:** WebAuthn登録（YubiKeyなど）は、作成されたorigin（ドメイン/ホスト名）に暗号技術的にバインドされています。バックアップを元のインスタンスとは異なるFQDNを持つGitLabインスタンスに復元すると、すべてのWebAuthnデバイスが無効になります。復元が完了した後、ユーザーはWebAuthnデバイスを再登録する必要があります。
>
> WebAuthnとホスト名の要件の詳細については、[2要素認証](../../user/profile/account/two_factor_authentication.md#information-for-gitlab-administrators)を参照してください。

インストール方法に基づいて、以下を復元してください:
...
[シークレットを復元する](https://docs.gitlab.com/charts/backup-restore/restore/#restoring-the-secrets)。

必要に応じて、[GitLab HelmチャートのシークレットをLinuxパッケージ形式に変換](https://docs.gitlab.com/charts/installation/migration/helm_to_package/)できます。

{{< /tab >}}

{{< tab title="Docker" >}}

`/etc/gitlab`を`/srv/gitlab/config`の下にマウントしている場合:
...
- [シークレットのイシューのトラブルシューティング](troubleshooting_backup_gitlab.md#when-the-secrets-file-is-lost)

### 特定のGitLabの設定がバックアップ元の環境と一致している必要がある {#certain-gitlab-configuration-must-match-the-original-backed-up-environment}

以前の`/etc/gitlab/gitlab.rb` （Linuxパッケージインストールの場合）または`/home/git/gitlab/config/gitlab.yml` （セルフコンパイルインストールの場合）と、TLSまたはSSHキー、証明書を個別に復元することをお勧めします。

特定の設定は、PostgreSQLのデータに結び付いています。例: 
...
次に、[復元の前提条件](#restore-prerequisites)の手順を完了し、元のインストールからGitLabのシークレットファイルをコピーした後、`gitlab-ctl reconfigure`を実行したことを確認します。

続いて、復元するバックアップのIDを指定してバックアップを復元します。

> [!warning]
> 次のコマンドは、GitLabデータベースの内容を上書きするので注意してください！
...
正しいGitLabバージョンをインストールしてから、再試行してください。

> [!warning]
> インストールがPgBouncerを使用している場合、パフォーマンス上の理由またはPatroniクラスターで使用する場合に、復元コマンドは[追加のパラメータ](backup_gitlab.md#back-up-and-restore-for-installations-using-pgbouncer)を必要とします。
...
特に`/etc/gitlab/gitlab-secrets.json`が復元された場合、または別のサーバーが復元ターゲットである場合、データベース値が復号化できることを確認してください。

```shell
sudo gitlab-rake gitlab:doctor:secrets
```

さらに確実にするために、アップロードされたファイルの整合性チェックを実行できます:
...
1. [データベースコンソール](https://docs.gitlab.com/omnibus/settings/database/#connecting-to-the-postgresql-database)に入ります。
1. 次を実行します。
...
- [GitLabの設定を確認](../raketasks/maintenance.md#check-gitlab-configuration)
- [データベース値が復号化できることを確認](../raketasks/check.md#verify-database-values-can-be-decrypted-using-the-current-secrets)
- [アップロードされたファイルの整合性チェック](../raketasks/check.md#uploaded-files-integrity):
...
GitLab Helmチャートでは、[GitLab Helmチャートインストールを復元する](https://docs.gitlab.com/charts/backup-restore/restore/#restoring-a-gitlab-installation)に記載された手順に従います。
...
> [!warning]
> GitLab 17.1以前は、[競合状態の影響を受け](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/158412)、データ損失を引き起こす可能性があります。この問題は、GitLabの[オブジェクトプール](../repository_storage_paths.md#hashed-object-pools)を使用しており、フォークされているリポジトリに影響します。データ損失を回避するには、GitLab 17.2以降のみを使用してバックアップを復元してください。
...
> [!note] 
> `REPOSITORIES_PATHS`および`SKIP_REPOSITORIES_PATHS`オプションは、Gitリポジトリにのみ適用されます。プロジェクトまたはグループのデータベースエントリには適用されません。`SKIP=db`を指定して作成されたリポジトリのバックアップだけでは、新しいインスタンスに特定のリポジトリを復元することはできません。
