...
description: gitlab-backupコマンドを使用して、データベース、リポジトリ、および設定ファイルを含むセルフマネージドGitLabインスタンスをバックアップします。
title: GitLabをバックアップする
description: GitLabインスタンスのバックアップに関するガイドで、バックアップ戦略、データタイプ、コマンドオプション、スケーリングの考慮事項を網羅しています。
---
...
- セキュアファイルがGitLab 16.1で[導入されました](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/121142)。
- 外部マージリクエストの差分がGitLab 17.1で[導入されました](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/154914)。
...
> [!warning] [設定ファイルの保存](#storing-configuration-files)について読み、それらを個別にバックアップすることを強くお勧めします。
...
大まかなガイドラインとして、1kのリファレンスアーキテクチャで100 GB未満のデータを使用している場合は、次の手順に従ってください:
...
- [オブジェクトストレージ設定](#object-storage)
- [設定ファイルガイド](#storing-configuration-files)

## バックアップをスケールする {#scaling-backups}

GitLabデータのボリュームが増加するにつれて、バックアップコマンドの実行時間が長くなります。Gitリポジトリの同時バックアップや増分リポジトリバックアップなどのバックアップオプションは、実行時間を短縮するのに役立ちます。状況によっては、バックアップコマンド自体が現実的でなくなることがあります。24時間以上かかる場合があるためです。
...
- [並行してGitリポジトリをバックアップ](#back-up-git-repositories-concurrently)。
- [大規模なリファレンスアーキテクチャのバックアップと復元](backup_large_reference_architectures.md)。
- [代替バックアップ戦略](#alternative-backup-strategies)。
- [GitLabリポジトリバックアップ時間の短縮に関するブログ記事](https://about.gitlab.com/blog/how-we-decreased-gitlab-repo-backup-times-from-48-hours-to-41-minutes/)。

## バックアップする必要のあるデータ {#what-data-needs-to-be-backed-up}

次のデータをバックアップする必要があります。
...
バックアップコマンドは、各リポジトリに対してGitバンドルを生成し、それらすべてをtarアーカイブにします。これにより、プールリポジトリデータがすべてのフォークに複製されます。私たちのテストでは、100 GBのGitリポジトリをバックアップしてS3にアップロードするのに2時間強かかりました。Gitデータが約400 GBに達する場合、バックアップコマンドを定期バックアップに使用するのは現実的ではないと考えられます。詳細については、[代替バックアップ戦略](#alternative-backup-strategies)を参照してください。
...
- [Google Cloud Storageオブジェクトバージョニング](https://cloud.google.com/storage/docs/object-versioning)

こちらも参照してください。

- [バックアップコマンドの詳細](#backup-command)
- [オブジェクトストレージ設定](../object_storage.md)
...
> [!warning] GitLabが提供するバックアップRakeタスクは、設定ファイルを保存しません。その主な理由は、データベースには、2要素認証やCI/CDセキュア変数の暗号化情報を含むアイテムが保存されているためです。暗号化キーと同じ場所に暗号化情報を保存すると、そもそも暗号化を行う意味がなくなります。たとえば、シークレットファイルにはデータベースの暗号化キーが含まれています。このファイルを失うと、GitLabアプリケーションはデータベース内の暗号化された値を復号化できなくなります。
>
...
GitLabは、キャッシュストアとして、およびバックグラウンドジョブシステムSidekiqの永続データの保存先として、Redisを使用します。提供されるバックアップコマンドはRedisデータをバックアップしません。これは、バックアップコマンドで一貫性のあるバックアップを取得するためには、保留中または実行中のバックグラウンドジョブがないことを意味します。
...
- セルフコンパイルインストールを使用している場合は、`rsync`がインストールされているか確認し、インストールされていない場合はインストールしてください。

### バックアップコマンド {#backup-command}

- バックアップコマンドは、Linuxパッケージ（Omnibus）/Docker/セルフコンパイルインストールのオブジェクトストレージ内の項目をバックアップしません。
- バックアップコマンドは、インストールでPgBouncerを使用している場合（パフォーマンス上の理由、またはPatroniクラスターとの併用時）に、追加のパラメータを必要とします。
...
> [!warning]カスタムバックアップファイル名を使用した場合、[バックアップのライフタイムを制限する](#limit-backup-lifetime-for-local-files-prune-old-backups)ことはできません。
...
- Blob、例えばアップロード、ジョブアーティファクト、外部マージリクエストの差分。
...
> [!warning] `COMPRESS_CMD`と`DECOMPRESS_CMD`を使用してデフォルトのGzip圧縮ライブラリをオーバーライドすることはサポートしていますが、デフォルトオプションのGzipライブラリのみを定期的にテストしています。バックアップの実行可能性については、ユーザー自身がテストおよび検証する必要があります。圧縮コマンドをオーバーライドするかどうかにかかわらず、一般的なバックアップのベストプラクティスとして、ユーザー自身でテストと検証を行うことを強くおすすめします。別の圧縮ライブラリで問題が発生した場合は、デフォルトのライブラリに戻すことをおすすめします。代替ライブラリを使用した場合のトラブルシューティングやエラーの修正は、GitLabにとって優先度が低くなります。
...
> [!note]
> `pigz`はGitLabLinuxパッケージには含まれていません。ユーザー自身でインストールする必要があります。
...
> [!note] 
> `zstd`はGitLabLinuxパッケージには含まれていません。ユーザー自身でインストールする必要があります。
...
> [!note]
> バックアップのために[オブジェクトストレージ](#upload-backups-to-a-remote-cloud-storage)を使用している場合、tarアーカイブの作成をスキップすることはできません。
...
> [!note]
> リポジトリのみが増分バックアップをサポートしています。そのため、`INCREMENTAL=yes`を使用した場合でも、タスクは自己完結型のバックアップtarアーカイブを作成します。これは、リポジトリ以外のすべてのサブタスクが依然としてフルバックアップを作成している（既存のフルバックアップを上書きする）ためです。すべてのサブタスクに対する増分バックアップのサポートに関する機能リクエストについては、[イシュー19256](https://gitlab.com/gitlab-org/gitlab/-/issues/19256)を参照してください。
...
新しいGitLabインスタンス（既存データなし）に増分バックアップを復元するには、完全バックアップから増分バックアップを作成する必要があります。ベースバックアップを作成する際に、バックアップコンポーネントをスキップしないでください。
...
> [!note]
> バックアップのためにオブジェクトストレージを使用している場合、[tarアーカイブの作成をスキップする](#skipping-tar-creation)ことはできません。

バックアップスクリプトで、作成した`.tar`ファイルをリモートストレージにアップロードさせることができます。次の例では、Amazon S3をストレージに使用していますが、Google Cloud StorageやAzureなどの他のクラウドプロバイダー、またはローカルマウントされた共有を使用することもできます。
...
- [GitLabでのオブジェクトストレージの使用](../object_storage.md)
...
> [!warning]次のcronジョブは、GitLab設定ファイルまたはSSHホストキーをバックアップしません。

**重要:**次の項目もバックアップすることを忘れないでください:
...
> [!warning]このセクションで説明されているプロセスは、バックアップにカスタムファイル名を使用した場合機能しません。

定期的なバックアップがディスク容量を使い切ってしまわないように、バックアップのライフタイムを制限しておくとよいでしょう。それにより、次回のバックアップタスクの実行時に、`backup_keep_time`より古いバックアップは削除されます。

この設定オプションで管理できるのは、ローカルファイルのみです。GitLabは、ユーザーがファイルをリストしたり削除したりする権限を持たない可能性があるため、サードパーティのオブジェクトストレージに保存されている古いファイルを削除しません。オブジェクトストレージに適切な保持ポリシーを設定することをお勧めします。

こちらも参照してください。

- [カスタムファイル名設定](#backup-filename)
...
これは、このタスクが`pg_dump`を使用し、nullの検索パスを設定してすべてのSQLクエリにスキーマを明示的に含めることで、CVE-2018-1058に対処しているために発生します。
...
各デプロイメントには異なる機能があるため、それらをどのように活用できるかをよりよく理解するために、まずバックアップする必要があるデータをレビューする必要があります。

例えば、Amazon RDSを使用している場合、その組み込みのバックアップと復元機能を使用してGitLabPostgreSQLデータベースデータを処理し、バックアップコマンドを使用する際にPostgreSQLデータベースデータを除外することを選択できます。
...
> [!warning] Gitalyクラスター（Praefect）は、[スナップショットのバックアップをサポートしていません](../gitaly/praefect/_index.md#snapshot-backup-and-recovery)。
