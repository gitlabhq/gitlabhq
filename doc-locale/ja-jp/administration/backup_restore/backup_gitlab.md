...
> [!warning]
> [設定ファイルの保存](#storing-configuration-files)について読み、それらを個別にバックアップすることを強くお勧めします。
...
> [!warning]
> カスタムバックアップファイル名を使用した場合、[バックアップのライフタイムを制限する](#limit-backup-lifetime-for-local-files-prune-old-backups)ことはできません。
...
> [!warning]
> 次のcronジョブは、GitLab設定ファイルまたはSSHホストキーをバックアップしません。
...
> [!warning]
> このセクションで説明されているプロセスは、バックアップにカスタムファイル名を使用した場合機能しません。
