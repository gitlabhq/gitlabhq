...
- 削除がスケジュールされているグループまたはプロジェクトの[インスタンス設定](#immediate-deletion)で[導入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/205556)された即時削除をGitLab 18.5で許可します。デフォルトでは有効になっています。GitLab.comおよびDedicatedでは無効になっています。
...
1. **設定** > **一般**を選択します。
1. **表示レベルとアクセス制御**を展開します。
1. **Deletion protection**（削除保護）までスクロールし、保持期間を`1`日から`90`日の間の値に設定します。
1. **変更を保存**を選択します。
...
- 削除がスケジュールされているグループまたはプロジェクトの即時削除を許可するインスタンス設定は、`allow_immediate_namespaces_deletion`という名前の[フラグ](../feature_flags/_index.md)とともにGitLab 18.5で[導入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/205556)されました。セルフマネージドではデフォルトで有効になっていますが、GitLab.comおよびDedicatedでは無効になっています。
...
1. **設定** > **一般**を選択します。
1. **表示レベルとアクセス制御**を展開します。
1. **即時削除**までスクロールして、チェックボックスをオフにします。
1. **変更を保存**を選択します。
...
1. **設定** > **一般**を選択します。
1. **表示レベルとアクセス制御**を展開します。
1. **Default snippet visibility**（デフォルトのスニペットの表示レベル）で、目的の表示レベルを選択します:
   - **プライベート**
...
1. **設定** > **一般**を選択します。
1. **表示レベルとアクセス制御**を展開します。
1. **Default group visibility**（デフォルトのグループの表示レベル）で、目的の表示レベルを選択します:
...
1. **設定** > **一般**を選択します。
1. **表示レベルとアクセス制御**を展開します。
1. **RSA SSH keys**（RSA SSHキー）に移動します。
...
1. **設定** > **一般**を選択します。
1. **表示レベルとアクセス制御**を展開します。
1. **Prevent group member invitations**（グループメンバーの招待を禁止する）チェックボックスをオンにします。
1. **変更を保存**を選択します。
