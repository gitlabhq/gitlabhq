...
- [インスタンス設定](#immediate-deletion)は、削除がスケジュールされたグループまたはプロジェクトの即時削除をGitLab 18.5で[導入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/205556)できるようにします。デフォルトでは有効になっています。GitLab.comおよびDedicatedでは無効。
...
### 即時削除 {#immediate-deletion}

{{< history >}}

- GitLab 18.5で、削除予定のグループまたはプロジェクトの即時削除を許可するインスタンス設定が`allow_immediate_namespaces_deletion`[フラグ](../feature_flags/_index.md)とともに[導入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/205556)されました。Self-Managedではデフォルトで有効になっていますが、GitLab.comとDedicatedでは無効になっています。
...
デフォルトでは、即時削除は、削除対象としてマークされたグループとプロジェクトに対して許可されています。これにより、ユーザーは構成された保持期間を効果的にバイパスし、グループまたはプロジェクトをすぐに削除できます。

これは無効にできるため、グループとプロジェクトは、構成された保持期間の後にのみ自動的に削除されます:

1. 右上隅で**管理者**を選択します。
1. **設定** > **一般**を選択します。
1. **表示レベルとアクセス制御**を展開します。
1. **即時削除**までスクロールし、チェックボックスをオフにします。
1. **変更を保存**を選択します。

{{< alert type="note" >}}

管理者は、管理者ページから常にグループとプロジェクトをすぐに削除できます。
...
[共有](../../user/project/members/sharing_projects_groups.md)や[移行](../../user/import/_index.md)などの機能を使用すると、これらのグループやプロジェクトへのアクセスが許可される場合があります。
...
- GitLab 18.7で、ユーザーデータの表示を許可するインスタンス設定が`usage_billing_dev`[フラグ](../feature_flags/_index.md)とともに[導入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/214538)されました。
...
GitLabクレジットダッシュボードにユーザーデータの表示をオンにするには:
