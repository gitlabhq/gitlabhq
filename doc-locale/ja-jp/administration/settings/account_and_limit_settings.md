...
description: GitLab Self-Managedでユーザーが作成できるプロジェクトの最大数を設定します。添付ファイル、プッシュ、リポジトリのサイズ制限を設定します。
title: アカウントと制限の設定
---
...
新しいユーザーのパーソナルネームスペース内のプロジェクトの最大数を設定するには:
...
**デフォルトのプロジェクトの制限**を0に設定した場合、ユーザーは個人のネームスペースにプロジェクトを作成できません。ただし、グループ内でのプロジェクトの作成は引き続き可能です。
...
GitLabのコメントと返信における添付ファイルの最大サイズは、100 MBです。添付ファイルの最大サイズを変更するには:
...
1. プロジェクトで[Git LFS](../../topics/git/lfs/_index.md)を有効にしているにもかかわらず、ストレージが大幅に増加しました。
...
### リポジトリサイズを確認する {#check-repository-size}

プロジェクトが、設定されたリポジトリのサイズ制限に近づいているかどうかを確認するには:

1. [ストレージ使用量を表示](../../user/storage_usage_quotas.md#view-storage)します。**リポジトリ**のサイズには、Gitリポジトリファイルと[Git LFS](../../topics/git/lfs/_index.md)オブジェクトの両方が含まれます。
...
ユーザーがアクティビティなしでサインインしたままでいられる時間を変更できます。

1. 右上隅で、**管理者**を選択します。
1. 左側のサイドバーで、**設定** > **一般**を選択します。
1. **アカウントと制限**を展開します。
1. **セッション時間(分)** フィールドに入力します。
   > [!warning]
   > **セッション時間(分)** を`0`に設定すると、GitLabインスタンスが正常に動作しなくなります。詳細については、[イシュー19469](https://gitlab.com/gitlab-org/gitlab/-/issues/19469)を参照してください。
1. **変更を保存**を選択します。
1. 変更を反映させるため、GitLabを再起動します。
   > [!note]
   > GitLab Dedicatedの場合は、[サポートチケット](https://support.gitlab.com/hc/en-us/requests/new?ticket_form_id=4414917877650)を送信して、インスタンスの再起動をリクエストしてください。

[**ログイン情報を記憶する**オプション](#configure-the-remember-me-option)が有効になっている場合、ユーザーのセッションは無期限にアクティブなままになります。
...
デフォルトでは、セッションが非アクティブになってから一定時間が経過すると、セッションの有効期限が切れます。代わりに、セッションが作成されてから一定時間が経過すると有効期限切れになるように設定できます。

セッション継続時間に達すると、次の場合でもセッションが終了し、ユーザーはサインアウトされます。

- ユーザーがセッションをまだアクティブに使用している。
- ユーザーがサインイン時に[**ログイン情報を記憶する**](#configure-the-remember-me-option)を選択した。
...
セッションが終了すると、ユーザーに再度サインインするように求めるウィンドウが表示されます。

### ログイン情報を記憶するオプションを設定する {#configure-the-remember-me-option}

ユーザーはサインイン時に**ログイン情報を記憶する**チェックボックスをオンにできます。その特定のブラウザからアクセスする限り、そのセッションは無期限にアクティブなままになります。セキュリティやコンプライアンスの目的でセッションを期限切れにするには、この設定をオフにします。この設定をオフにすると、[セッション時間をカスタマイズ](#customize-the-default-session-duration)した際に指定した非アクティブ時間が経過した時点で、ユーザーのセッションが確実に期限切れになります。
...
トップレベルグループのオーナーにサービスアカウントの作成を許可するには:
...
## 非アクティブなプロジェクトおよびグループアクセストークンの保持期間 {#inactive-project-and-group-access-token-retention-period}
...
デフォルトでは、GitLabは、グループアクセストークンとプロジェクトアクセストークン、およびそれらの[トークンファミリー](../../api/personal_access_tokens.md#automatic-reuse-detection)を、そのトークンファミリー内で最後にアクティブだったトークンが非アクティブになってから30日後に削除します。この削除により、トークンファミリー内のすべてのトークンと関連付けられたボットユーザーが削除され、ボットのコントリビュートは[ゴーストユーザー](../../user/profile/account/delete_account.md#associated-records)に移動されます。
...
1. **非アクティブなプロジェクトやグループアクセストークンの保持期間**テキストボックスで、保持期間を変更します。
   - 数値を指定した場合、すべてのグループアクセストークンとプロジェクトアクセストークンは、指定した日数にわたって非アクティブな状態が続いた後に削除されます。
   - フィールドを空白のままにした場合、非アクティブなトークンは削除されません。
1. **変更を保存**を選択します。

[アプリケーション設定API](../../api/settings.md)を使用して`inactive_resource_access_tokens_delete_after_days`属性を修正することもできます。

## パーソナルアクセストークンのプレフィックス {#personal-access-token-prefix}

パーソナルアクセストークンのプレフィックスを指定できます。カスタムプレフィックスを使用すると、次のようなメリットがあります:

- トークンを明確に区別して識別できる。
- セキュリティスキャン中に、漏洩したトークンをより簡単に識別できる。
- 異なるインスタンス間でのトークンの混同のリスクが低減されます。
...
> [!warning]
> デフォルトでは、クライアントサイドのシークレット検出、シークレットプッシュ保護、パイプラインシークレット検出は、カスタムプレフィックスを持つトークンを検出しません。これにより、検出漏れが増加する可能性があります。ただし、これらのトークンを検出するために[パイプラインシークレット検出をカスタマイズ](../../user/application_security/secret_detection/pipeline/configure.md#customize-analyzer-rulesets)できます。

### プレフィックスを設定する {#set-a-prefix}

デフォルトのグローバルプレフィックスを変更するには:
...
- GitLab 17.10で[導入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/179852)され、[機能フラグ](../feature_flags/_index.md) `custom_prefix_for_all_token_types`が付けられました。デフォルトでは無効になっています。
...
インスタンスで生成されるすべてのトークンの先頭に付加するカスタムプレフィックスを設定できます。カスタムプレフィックスを使用すると、次のようなメリットがあります:
...
- [SCIMトークン](../../security/tokens/_index.md#token-prefixes)
- [ワークスペーストークン](../../security/tokens/_index.md#workspace-token)

前提条件: 

- インスタンスへの管理者アクセス権が必要です。

カスタムトークンのプレフィックスを設定するには:
...
`gl`は予約済みのプレフィックスです。インスタンスが、GitLabがデフォルトのインスタンストークンプレフィックスを`gl`からプレフィックスなしに変更する前に設定を保存した場合でも、`gl`が値として保存されています。GitLabは、保存された`gl`の値をプレフィックスなしと同じように扱います。そのため、インスタンストークンプレフィックスを`gl`に設定することはできません。
...
- [変更](https://gitlab.com/gitlab-org/gitlab/-/issues/461901)：最大許容ライフタイム制限がGitLab 17.6で400日に引き上げられました。[機能フラグを伴って](../feature_flags/_index.md)、`buffered_token_expiration_limit`という名前で。デフォルトでは無効になっています。

{{< /history >}}

> [!flag]
> 延長された最大許容ライフタイム制限の利用可否は、機能フラグによって制御されます。詳細については、履歴を参照してください。この機能フラグはGitLab Dedicatedでは利用できません。

ユーザーは、アクセストークンの最大ライフタイム（日数）をオプションで指定できます。これには、[パーソナルアクセストークン](../../user/profile/personal_access_tokens.md)、[グループ](../../user/group/settings/group_access_tokens.md)アクセストークン、および[プロジェクトアクセストークン](../../user/project/settings/project_access_tokens.md)が含まれます。このライフタイムは必須ではなく、0より大きく次の最大値以下の任意の値を設定できます:
...
- 新しいパーソナルアクセストークンにライフタイムを適用し、ユーザーは有効期限と、許可されたライフタイム以降の日付を設定する必要があります。
- 3時間後、有効期限がないか、許可されたライフタイムよりも長いライフタイムを持つ古いトークンは失効されます。失効する前に、管理者が許可されたライフタイムを変更または削除できるように、3時間の猶予が与えられます。
...
**ユーザーOAuthアプリケーション**設定をオンまたはオフにするには:
...
### OAuthアクセストークンのライフタイムを制限する {#limit-the-lifetime-of-oauth-access-tokens}

{{< history >}}

- GitLab 19.1で[導入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/237354)されました。

{{< /history >}}

デフォルトでは、OAuthアクセストークンは2時間（7200秒）後に期限切れになります。インスタンス管理者は、OAuthアクセストークンのカスタムライフタイムを設定できます。

最小値は300秒（5分）です。未設定の場合、デフォルトの7200秒（2時間）が適用されます。この設定は、インスタンスによって発行されたすべての新しいOAuthアクセストークンに適用されます。

前提条件: 

- 管理者である必要があります。

OAuthアクセストークンのライフタイムを設定するには:

1. 右上隅で、**管理者**を選択します。
1. 左側のサイドバーで、**設定** > **一般**を選択します。
1. **アカウントと制限**セクションを展開します。
1. **OAuthアクセストークンライフタイム(秒)** フィールドに入力します。
1. **変更を保存**を選択します。

[アプリケーション設定API](../../api/settings.md)を使用して、`oauth_access_token_expires_in`属性でこれを設定することもできます。

### OAuth動的クライアント登録を無効にする {#turn-off-oauth-dynamic-client-registration}
...
- GitLab 19.3で[導入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/248615)されました。

{{< /history >}}

OAuthクライアント（MCPクライアントやAIツールなど）は、`POST /oauth/register`にリクエストを送信することで、OAuth動的クライアント登録（DCR）を使用してアプリケーションを自動的に登録します。デフォルトでは、DCRは有効です。

DCRを無効にすると、OAuthクライアントには以下の結果が適用されます:

- OAuthクライアントはアプリケーションを自動的に登録できません。クライアントは代わりに事前に登録されたOAuthアプリケーションを使用する必要があります。
- GitLabは、動的に登録されたOAuthアプリケーションを削除し、そのアクセストークンと付与を失効します。

前提条件: 

- 管理者である必要があります。

OAuth動的クライアント登録を無効にするには、[アプリケーション設定API](../../api/settings.md)を使用して`dynamic_client_registration_enabled`属性を`false`に設定します。
...
- GitLab 16.7で[導入](https://gitlab.com/gitlab-org/gitlab/-/issues/423302)され、[機能フラグ](../feature_flags/_index.md) `ui_for_organizations`が付けられました。デフォルトでは無効になっています。

{{< /history >}}

> [!flag]
> GitLab Self-Managedでは、デフォルトではこの機能は利用できません。管理者が`ui_for_organizations`[機能フラグを有効にする](../feature_flags/_index.md)と、この機能を使用できるようになります。GitLab.comとGitLab Dedicatedでは、この機能は使用できません。この機能は本番環境での使用には対応していません。
...
1. 左側のサイドバーで、**設定** > **一般**を選択します。
1. **アカウントと制限**を展開します。
1. **新規ユーザーによるトップレベルグループの作成を許可します**チェックボックスをオフにします。

> [!note]
> この設定は、設定をオフにした後に追加されたユーザーにのみ適用されます。既存のユーザーは、引き続きトップレベルグループを作成できます。

## メンバー以外のユーザーがプロジェクトとグループを作成できないようにする {#prevent-non-members-from-creating-projects-and-groups}

デフォルトでは、ゲストロールを持つユーザーはプロジェクトとグループを作成できます。GitLab管理者は、この動作を防ぐことができます:
...
- GitLab 17.1で`disallow_private_profiles`[機能フラグ](../feature_flags/_index.md)とともに[導入](https://gitlab.com/gitlab-org/gitlab/-/issues/421310)されました。デフォルトでは無効になっています。
...
ユーザーが自分のプロファイルを非公開にできないようにするには:
...
- [新規ユーザーのプロファイルをデフォルトで非公開にする](#set-profiles-of-new-users-to-private-by-default)オプションもオフになります。
...
1. 左側のサイドバーで、**設定** > **一般**を選択します。
1. **アカウントと制限**を展開します。
1. **新規ユーザーのプロファイルをデフォルトで非公開にします**チェックボックスをオンにします。
1. **変更を保存**を選択します。
...
## すべてのユーザーの達成を自動的に承認する {#automatically-accept-achievements-for-all-users}
...
- GitLab 19.4で[導入](https://gitlab.com/gitlab-org/gitlab/-/work_items/607750)されました。

{{< /history >}}

デフォルトでは、ユーザーは自身のユーザープロフィールに[達成](../../user/profile/achievements.md)が表示される前に、それを明示的に承認する必要があります。代わりに、インスタンス上のすべてのユーザーに対し、新たに授与された達成を自動的に承認する設定を構成できます。

これをオンにすると、新しい達成はすぐに承認され、受け取ったユーザーのプロフィールに表示されます。GitLabは引き続き達成に関するメール通知を送信しますが、そのメールには承認リンクが含まれなくなります。この設定は、設定がオンになる前に授与された達成には適用されません。

受け取ったユーザーは、引き続き[プロフィールから達成を非表示にできます](../../user/profile/achievements.md#change-visibility-of-specific-achievements)。

すべてのユーザーの達成を自動的に承認するには:

1. 右上隅で、**管理者**を選択します。
1. 左側のサイドバーで、**設定** > **一般**を選択します。
1. **アカウントと制限**を展開します。
1. **Automatically accept achievements for all users**チェックボックスを選択します。
1. **変更を保存**を選択します。
...
[Linuxパッケージ](https://docs.gitlab.com/omnibus/)インストール環境で添付ファイルの最大サイズを200 MBに増やすには:
...
### This repository has exceeded its size limit {#this-repository-has-exceeded-its-size-limit}
