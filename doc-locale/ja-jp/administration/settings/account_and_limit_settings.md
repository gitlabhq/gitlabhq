...
> [!note]
> ウェブUIを介して[リポジトリにファイルを追加する](../../user/project/repository/web_editor.md#create-a-file)場合、最大添付ファイルサイズが制限要因となります。その理由は、GitLabがコミットを生成する前に、Webサーバーがファイルを受信する必要があるためです。大きなファイルをリポジトリに追加する場合は、[Git LFS](../../topics/git/lfs/_index.md)を使用してください。この設定は、Git LFSオブジェクトをプッシュする際には適用されません。
...
1. **セッション時間(分)** フィールドに入力します。
   > [!warning]
   > **セッション時間(分)** に`0`を設定すると、GitLabインスタンスが機能しなくなります。詳細については、[イシュー19469](https://gitlab.com/gitlab-org/gitlab/-/issues/19469)を参照してください。
1. **変更を保存**を選択します。
1. 変更を反映させるため、GitLabを再起動します。
   > [!note]
   > GitLab Dedicatedをご利用の場合は、インスタンスの再起動をリクエストするために[サポートチケット](https://support.gitlab.com/hc/en-us/requests/new?ticket_form_id=4414917877650)を提出してください。
...
- ユーザーがサインイン時に[**ログイン情報を記憶する**](#configure-the-remember-me-option)を選択しました。
...
サービスアカウントのパーソナルアクセストークンについては、[アプリケーション設定API](../../api/settings.md)の`service_access_tokens_expiration_enforced`設定を使用します。
...
## 非アクティブなプロジェクトやグループアクセストークンの保持期間 {#inactive-project-and-group-access-token-retention-period}
...
デフォルトでは、GitLabは、グループアクセストークンとプロジェクトアクセストークン、およびそれらの[トークンファミリー](../../api/personal_access_tokens.md#automatic-reuse-detection)を、そのトークンファミリー内の最後にアクティブだったトークンが非アクティブになってから30日後に削除します。この削除により、トークンファミリー内のすべてのトークン、関連付けられたボットユーザーが削除され、ボットのコントリビュートは[ゴーストユーザー](../../user/profile/account/delete_account.md#associated-records)に移動されます。
...
1. **非アクティブなプロジェクトやグループアクセストークンの保持期間**テキストボックスで、保持期間を変更します。
   - 数値が定義されている場合、すべてのグループアクセストークンとプロジェクトアクセストークンは、指定された日数の間非アクティブになった後に削除されます。
   - フィールドが空白の場合、非アクティブなトークンは決して削除されません。
1. **変更を保存**を選択します。
...
> [!warning]
> デフォルトでは、クライアント側のシークレット検出、シークレットプッシュ保護、およびパイプラインシークレット検出は、カスタムプレフィックスを持つトークンを検出しません。これにより、検出漏れが増加する可能性があります。ただし、これらのトークンを検出するために[パイプラインシークレット検出のカスタマイズ](../../user/application_security/secret_detection/pipeline/configure.md#customize-analyzer-rulesets)を行うことができます。
...
> [!flag]
> 延長された最大許容ライフタイム制限の利用可能性は、機能フラグによって制御されます。詳細については、履歴を参照してください。機能フラグはGitLab Dedicatedでは利用できません。
...
> [!note]
> ユーザーのSSHキーが無効になった場合、同じキーを削除して再度追加することができます。
...
[アプリケーション設定API](../../api/settings.md)を使用して`oauth_access_token_expires_in`属性でこの設定を構成することもできます。
...
> [!flag]
> GitLab Self-Managedでは、デフォルトではこの機能は利用できません。管理者が`ui_for_organizations`という名前の[機能フラグを有効にする](../feature_flags/_index.md)と、この機能を使用できるようになります。GitLab.comとGitLab Dedicatedでは、この機能は使用できません。この機能は本番環境での使用には対応していません。
