...
> [!note]`ModelMapper`クラスは、[GitLab 18.3](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/196293)で追加されました。以前のバージョンでは、[Geoデータ型モデルクラス](#geo-data-type-model-classes)のリストを手動で指定する必要があります。
...
> [!warning]推奨される対応は、8時間のリース有効期限が切れるのを待つことです。手動でのリース解除は、即時の同期が不可欠であり、かつ同期ジョブが実行中でないことを確認できている場合にのみ実施してください。
...
> [!note]リースをリリースした後、通常Geoの同期スケジュールに従ってリポジトリ同期が再試行されます。または、上記のように手動で同期をトリガーすることもできます。
...
どこから始めればよいかわからない場合は、[コマンドラインで`git fsck`コマンドを手動で実行する](../../../repository_checks.md#run-a-check-using-the-command-line)することで、プライマリサイトのソースリポジトリで整合性チェックを実行できます。
...
**重要:**現在の`.gitmodules`ファイルを編集しても、この問題は解決**しません**。問題のデータは、ファイルの現在のバージョンではなく、リポジトリのGit履歴内に存在しているためです。
...
1. [Start a Rails console session](../../../operations/rails_console.md#starting-a-rails-console-session)を**プライマリ**サイトで実行します。
...
> [!warning]ページデプロイのレコードを削除する前に、最新のバックアップが機能していることを確認してください。これらのデプロイを安全に削除できることをチームと連携して確認してください。
...
> [!warning]ジョブアーティファクトのレコードを削除する前に、最新のバックアップが機能していることを確認してください。これらのアーティファクトを安全に削除できることをチームと連携して確認してください。
...
> [!warning]パッケージファイルのレコードを削除する前に、最新のバックアップが機能していることを確認してください。これらのパッケージファイルを安全に削除できることをチームと連携して確認してください。
...
> [!warning]パイプラインアーティファクトのレコードを削除する前に、最新のバックアップが機能していることを確認してください。これらのアーティファクトを安全に削除できることをチームと連携して確認してください。
...
### タイムアウトにより同期していないLFSオブジェクト {#lfs-objects-out-of-sync-due-to-timeout}

大規模なファイルがデフォルトの8時間blobダウンロードタイムアウトを超過すると、LFSオブジェクトは`Sync timed out after 28800`で同期に失敗する可能性があります。
...
#### タイムアウトしたLFSオブジェクトを特定して検証する {#identify-and-validate-timed-out-lfs-objects}

タイムアウトを増やした後もLFSオブジェクトが失敗し続ける場合は、影響を受けるオブジェクトを特定し、ファイルがプライマリサイトに存在することを確認してください。

1. **セカンダリ**サイトで影響を受けるオブジェクトを特定します:
...
1. 前の手順の`lfs_object_id`の値を使用して、ファイルが**プライマリ**サイトに存在することを確認します:
...
#### LFSオブジェクトを同期済みとしてマークする {#mark-lfs-objects-as-synced}
...
- GitLab 16.0から必須となった[新しい設定構造を使用](../../../../update/versions/gitlab_16_changes.md#gitaly-configuration-structure-change)します。
...
> [!note]すべてのリポジトリデータ型は、GitLab 16.3でGeo Self-Service Frameworkに移行されました。[この機能をGitLab Geo Self-Service Frameworkに再実装するためのイシュー](https://gitlab.com/gitlab-org/gitlab/-/issues/426659)が作成されています。
...
> [!warning]この手順はリスクが高く、強制的な方法です。他のトラブルシューティング手段で問題が解決しなかった場合の最終手段としてのみ実行してください。この手順を実行すると、リポジトリが再同期されるまで一時的にデータが失われます。
...
> [!warning]ファイルが実際にセカンダリサイトに存在し、アクセス可能であることを確認した場合にのみ、オブジェクトを同期済みとしてマークしてください。
