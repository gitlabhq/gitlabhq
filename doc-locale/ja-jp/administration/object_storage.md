...
以下のプロバイダーは動作することが知られていますが、このリストがすべてを網羅しているわけではありません:
...
上記のプロバイダー以外にも、Fogライブラリでサポートされている他のサービスも互換性がある場合があります。ご自身の判断で使用してください。あらゆるサポートは、商業的に合理的な努力の対象となります。
...
> [!note] 
> 
> GitLab Helm Chartについては、[統合された形式](https://docs.gitlab.com/charts/charts/globals/#consolidated-object-storage)を設定する方法を参照してください。
...
ストレージ固有形式では、オブジェクトごとに個別のオブジェクトストレージの接続と設定を定義します。ただし、統合形式でサポートされていないストレージタイプを除き、[統合形式を使用](#transition-to-consolidated-form)することをおすすめします。GitLab Helmチャートを使用する場合は、チャートが[オブジェクトストレージの統合形式](https://docs.gitlab.com/charts/charts/globals/#consolidated-object-storage)をどのように扱うのかを参照してください。

統合形式以外では、[暗号化されたS3バケット](#encrypted-s3-buckets)の使用はサポートされていません。使用すると、[ETagの不一致エラー](#etag-mismatch)が発生する可能性があります。

> [!note]
> ストレージ固有のフォームの場合、共有フォルダーが不要なため、[直接アップロード](https://gitlab.com/gitlab-org/gitlab/-/issues/27331)がデフォルトになる可能性があります。
...
| [オートスケールRunnerのキャッシュ](https://docs.gitlab.com/runner/configuration/autoscale/#distributed-runners-caching)（パフォーマンスを向上させるためのオプション） | {{< icon name="dotted-circle" >}}いいえ |
...
| `endpoint`                                  | S3互換サービスを設定する際に、`http://127.0.0.1:9000`のようなURLを入力することで使用できます。これは、`host`よりも優先されます。統合形式では必ず`endpoint`を使用してください。 | （オプション） |
| `path_style`                                | `true`に設定すると、`bucket_name.host/object`ではなく、`host/bucket_name/object`形式のパスを使用します。S3互換サービスでパス形式のアドレス指定が必要な場合は、`true`に設定します。AWS S3の場合は`false`のままにします。 | `false` |
...
- GitLabが[インターネットプロキシを使用するように設定されている](https://docs.gitlab.com/omnibus/settings/environment-variables/)場合、エンドポイントのIPアドレスを`no_proxy`リストに追加する必要があります。
...
- `universe_domain`設定はGitLab 18.9で[導入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/221401)されました。
...
| `universe_domain`            | Google Cloudのリクエストに使用するユニバースドメイン。[Google Cloud Dedicated](https://cloud.google.com/sovereign-cloud)またはその他の非デフォルトユニバースドメインへの接続に使用します。 | `googleapis.com` |
...
> [!note]
> バケットの暗号化を[顧客管理の暗号化キー](https://cloud.google.com/storage/docs/encryption/using-customer-managed-keys)で使用するには、[統合された形式](#configure-a-single-storage-connection-for-all-object-types-consolidated-form)を使用します。
...
1. `/etc/gitlab/gitlab.rb`を編集し、必要な値に置き換えて次の行を追加します。
   ...
   デフォルト以外のユニバースドメイン（例: [Google Cloud Dedicated](https://cloud.google.com/sovereign-cloud)）を使用するには:
...
1. 次の内容を`object_storage.yaml`ファイルに記述し、[Kubernetesシークレット](https://docs.gitlab.com/charts/charts/globals/#connection)として使用します。
...
> [!note] 
> 
> Storj Gatewayはマルチスレッドコピーを[サポートしていません](https://github.com/storj/gateway-st/blob/4b74c3b92c63b5de7409378b0d1ebd029db9337d/docs/s3-compatibility.md)（表の`UploadPartCopy`を参照）。実装は[計画](https://github.com/storj/roadmap/issues/40)されていますが、それが完了するまでは[マルチスレッドコピーを無効にする](#multi-threaded-copying)必要があります。
...
> [!note] 
> 
> HCPへの接続で`SignatureDoesNotMatch - The request signature we calculated does not match the signature you provided. Check your HCP Secret Access key and signing method.`のようなエラーが返される場合があります。これらの場合、`endpoint`をネームスペースではなくテナントのURLに設定し、バケットのパスを`<namespace_name>/<bucket_name>`のように設定してください。
...
[Ceph RGW](https://docs.ceph.com/en/reef/cephadm/services/rgw/)はCeph用のS3互換APIです。次の設定例を使用してください。
...
Ceph RGWで[サーバーサイド暗号化](#server-side-encryption-headers)を有効にするには、HTTPSを使用して接続する必要があります。Cephは非セキュアな接続での暗号化リクエストを拒否します。
...
1. `/etc/gitlab/gitlab.rb`を編集し、必要な値に置き換えて次の行を追加します。
   ...
   [AWS IAMプロファイル](#use-amazon-instance-profiles)を使用している場合は、AWSアクセスキーおよびシークレットアクセスキーまたはバリューペアを省略します。例: 
...
1. [`git`ユーザーのホームディレクトリ](https://docs.gitlab.com/omnibus/settings/configuration/#move-the-home-directory-for-a-user)がローカルディスク上にあることを確認します。
...
デフォルトのGitLabの設定を使用している場合、[Alibaba](https://gitlab.com/gitlab-org/charts/gitlab/-/issues/1564)などの一部のS3互換オブジェクトストレージバックエンドは`ETag mismatch`エラーを生成する可能性があります。
...
統合された形式はS3互換サービスに推奨されます。一部のサービスでは、ETag不一致エラーを解決するために、互換性モードの有効化など、追加のサーバーサイド設定が必要な場合もあります。
...
- プライマリで[オブジェクトストレージに移行しました](#migrate-to-object-storage)。
  ...
  - オプション「このセカンダリサイトによるオブジェクトストレージへのコンテンツの複製を許可します」が有効になっています。
