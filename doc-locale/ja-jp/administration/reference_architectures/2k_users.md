...
description: "マルチノードコンポーネントと推奨されるサイジングで、最大2,000ユーザーまたは40 RPSのGitLab Self-Managedインスタンスをデプロイします。"
...
- **Unsure which Reference Architecture to use**？[詳細については、こちらのガイドをご覧ください](_index.md#deciding-which-architecture-to-start-with)。
...
40 RPS/2,000ユーザーのリファレンスアーキテクチャは、最も一般的なワークフローに対応するように設計されています。GitLabは、次のエンドポイントスループットの目標に対して、定期的にスモークテストとパフォーマンステストを実施しています。
...
> [!note]
> Redisは主にシングルスレッドであり、CPUコア数の増加による恩恵は大きくありません。詳細については、[スケーリングに関するドキュメント](_index.md#scaling-an-environment)を参照してください。
...
> [!note]
> Gitalyドキュメント全体で参照されているトークンは、管理者によって選択された任意のパスワードです。このトークンは、GitLab APIまたはその他の同様のWeb APIトークン用に作成されたトークンとは関係ありません。
...
1. Gitalyサーバーノードの`/etc/gitlab/gitlab.rb`ファイルを編集して、ストレージパスを設定し、ネットワークリスナーを有効にして、トークンを設定します。
   ...
   > [!note]
   > [GitLabが必要とする](../gitaly/configure_gitaly.md#gitlab-requires-a-default-repository-storage)ため、`default`エントリを`gitaly['configuration'][:storage]`から削除することはできません。
...
> - 2,000のリファレンスアーキテクチャは、高可用性設定ではありません。HAを実現するには、変更された[3,000または60 RPSのリファレンスアーキテクチャ](3k_users.md#cloud-native-hybrid-reference-architecture-with-helm-charts-alternative)を参照してください。
...
ゲートウェイAPIまたはIngressコントローラーポッドをWebserviceノード全体にDaemonSetとしてデプロイすることも推奨されます。これにより、コントローラーはサービスを提供するWebserviceポッドとともに動的にスケールでき、大きなマシンタイプが通常備えている、より大きなネットワーク帯域幅を利用できます。
