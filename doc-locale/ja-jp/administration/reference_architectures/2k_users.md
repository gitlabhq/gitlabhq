...
1. Gitalyサーバーノードの`/etc/gitlab/gitlab.rb`ファイルを編集して、ストレージパスを設定し、ネットワークリスナーを有効にして、トークンを設定します。
   ...
   > [!note] `gitaly['configuration'][:storage]`から`default`エントリを削除することはできません。これは[GitLabが必要とするため](../gitaly/configure_gitaly.md#gitlab-requires-a-default-repository-storage)です。
