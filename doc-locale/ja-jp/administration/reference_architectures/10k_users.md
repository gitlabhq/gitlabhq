...
description: "最大10,000ユーザーまたは200 RPS向けのGitLab Self-Managedインスタンスを、高可用性、データベースレプリケーション、インフラストラクチャサイジングでデプロイおよびスケールする。"
title: 'リファレンスアーキテクチャ: 最大200 RPSまたは10,000ユーザー'
---
...
このドキュメントでは、実際のデータに基づき、ピーク負荷を1秒あたり200リクエスト（RPS）、手動および自動の両方で最大10,000ユーザーの典型的なピーク負荷を対象とするように設計されたGitLabのリファレンスアーキテクチャについて説明します。

リファレンスアーキテクチャの完全なリストについては、[利用可能なリファレンスアーキテクチャ](_index.md#available-reference-architectures)を参照してください。

> [!note]
> このアーキテクチャをデプロイする前に、まず[main documentation](_index.md) 、特に[Before you start](_index.md#before-you-start)および[Deciding which architecture to use](_index.md#deciding-which-architecture-to-start-with)のセクションを読み通すことをお勧めします。

- **Target load**: API: 200 RPS、Web: 20 RPS、Git（プル）: 20 RPS、Git（プッシュ）: 4 RPS
- **HA**: はい（[Praefect](#configure-praefect-postgresql)にはHAのためにサードパーティのPostgreSQLソリューションが必要です）
- **クラウドネイティブハイブリッドの代替**: [はい](#cloud-native-hybrid-reference-architecture-with-helm-charts-alternative)
...
| PostgreSQL<sup>2</sup>                   | 3     | 8 vCPU、30 GBメモリ    | `n1-standard-8`  | `m5.2xlarge`   | `D8s v3`  |
...
| Redis/Sentinel - キャッシュ<sup>3</sup>       | 3     | 4 vCPU、15 GBメモリ    | `n1-standard-4`  | `m5.xlarge`    | `D4s v3`  |
| Redis/Sentinel - 永続<sup>3</sup>  | 3     | 4 vCPU、15 GBメモリ    | `n1-standard-4`  | `m5.xlarge`    | `D4s v3`  |
| Gitaly<sup>6</sup><sup>7</sup>           | 3     | 16 vCPU, 60 GBメモリ   | `n1-standard-16` | `m5.4xlarge`   | `D16s v3` |
...
| Sidekiq<sup>8</sup>                      | 4     | 4 vCPU、15 GBメモリ    | `n1-standard-4`  | `m5.xlarge`    | `D4s v3`  |
| GitLab Rails<sup>8</sup>                 | 3     | 32 vCPU, 28.8 GBメモリ | `n1-highcpu-32`  | `c5.9xlarge`   | `F32s v2` |
...
3. 定評のあるサードパーティの外部PaaS Redisソリューションでオプションで実行できます。詳細については、[Provide your own Redis instances](#provide-your-own-redis-instances)および[Recommended cloud providers and services](_index.md#recommended-cloud-providers-and-services)を参照してください。
   - Redisは主にシングルスレッドであり、CPUコアを増やしても大きなメリットは得られません。この規模のアーキテクチャでは、最適なパフォーマンスを達成するために、指定されたキャッシュインスタンスと永続インスタンスを個別に持つことを強くお勧めします。
4. HA機能を提供できる定評のあるサードパーティのロードバランサーまたはサービス（LB PaaS）で実行することをおすすめします。サイジングは、選択されたロードバランサーと、ネットワーク帯域幅などの追加要因に依存します。詳細については、[ロードバランサー](_index.md#load-balancers)を参照してください。
5. 定評のあるクラウドプロバイダーまたはSelf-Managedソリューションで実行する必要があります。詳細については、[オブジェクトストレージを設定する](#configure-the-object-storage)を参照してください。
6. Gitaly Cluster (Praefect)は、耐障害性の利点を提供しますが、セットアップと管理がさらに複雑になります。[Gitaly Cluster (Praefect)をデプロイする前に、既存の技術的な制限事項と考慮事項](../gitaly/praefect/_index.md#before-deploying-gitaly-cluster-praefect)を確認してください。シャーディングされたGitalyが必要な場合は、上記の表にリストされている`Gitaly`と同じ仕様を使用してください。
...
200 RPS / 10kユーザーのリファレンスアーキテクチャは、最も一般的なワークフローに対応するように設計されています。GitLabは、次のエンドポイントスループットの目標に対して、定期的にスモークテストとパフォーマンステストを実施しています。
...
これらの目標は、CIパイプラインやその他のワークロードを含む、指定されたユーザー数に対する環境負荷の合計を反映した、実際の顧客データに基づいています。これは一般的なワークロード構成を表します。異常なワークロードパターンに関するガイダンスについては、[Understanding RPS composition](sizing.md#understanding-rps-composition-and-workload-patterns)を参照してください。
...
- クラウドネイティブハイブリッド向けのゲートウェイAPIまたはIngress実装を備えたクラウドプロバイダー相当の環境

## コンポーネントをセットアップする {#set-up-components}

GitLabとそのコンポーネントを最大200 RPSまたは10,000ユーザーに対応させるように設定するには:
...
1. Gitリポジトリへのアクセスを提供する[Gitaly Cluster (Praefect)を設定](#configure-gitaly-cluster-praefect)します。
...
> [!note]
> Consulは3ノード以上の奇数でデプロイする必要があります。これは、ノードがクォーラムの一部として投票できるようにするためです。
...
このセクションでは、GitLabで使用する高可用性のPostgreSQLクラスターを設定する方法を説明します。
...
高可用性やデータベースのロードバランシングに関するガイダンスを含め、詳細については以下を参照してください:
...
1. 必要な[users and databases](../postgresql/external.md)を設定します。
1. [GitLab Railsを設定する](#configure-gitlab-rails)に従い、適切な接続の詳細を指定してGitLabアプリケーションサーバーを設定します。
...
まず、**各ノードに**Linux GitLabパッケージを[インストール](../../install/package/_index.md#supported-platforms)してください。必ずGitLabパッケージリポジトリのみを追加し、選択したオペレーティングシステム用にGitLabをインストールしてください。ただし、`EXTERNAL_URL`値は指定しないでください。

#### PostgreSQLノード {#postgresql-nodes}

1. いずれかのPostgreSQLノードにSSHで接続します。
1. PostgreSQLのユーザー名/パスワードのペアのパスワードハッシュを生成します。これは、`gitlab`のデフォルトのユーザー名を使用することを前提としています（推奨）。コマンドは、パスワードと確認を要求します。次のステップで、このコマンドによって出力された値を`<postgresql_password_hash>`の値として使用します。

   ```shell
   sudo gitlab-ctl pg-password-md5 gitlab
   ```

1. PgBouncerのユーザー名/パスワードのペアのパスワードハッシュを生成します。これは、`pgbouncer`のデフォルトのユーザー名を使用することを前提としています（推奨）。コマンドは、パスワードと確認を要求します。次のステップで、このコマンドによって出力された値を`<pgbouncer_password_hash>`の値として使用します。

   ```shell
   sudo gitlab-ctl pg-password-md5 pgbouncer
   ```

1. PostgreSQLレプリケーションのユーザー名/パスワードのペアのパスワードハッシュを生成します。これは、`gitlab_replicator`のデフォルトのユーザー名を使用することを前提としています（推奨）。コマンドはパスワードと確認を要求します。次のステップで、このコマンドによって出力された値を`<postgresql_replication_password_hash>`の値として使用します。

   ```shell
   sudo gitlab-ctl pg-password-md5 gitlab_replicator
   ```

1. Consulデータベースのユーザー名/パスワードのペアのパスワードハッシュを生成します。これは、`gitlab-consul`のデフォルトのユーザー名を使用することを前提としています（推奨）。コマンドは、パスワードと確認を要求します。次のステップで、このコマンドによって出力された値を`<consul_password_hash>`の値として使用します。
...
PostgreSQLは、Patroniがそのフェイルオーバーを管理し、デフォルトで`pg_rewind`を使用して競合を処理します。ほとんどのフェイルオーバー処理方法と同様に、これによりデータ損失が発生する可能性がわずかにあります。詳細については、さまざまな[Patroniレプリケーション方法](../postgresql/replication_and_failover.md#selecting-the-appropriate-patroni-replication-method)を参照してください。
...
> [!note]
> PgBouncerはシングルスレッドであり、CPUコアの増加から大きな恩恵を受けることはありません。詳細については、[スケーリングに関するドキュメント](_index.md#scaling-an-environment)を参照してください。
...
1. 変更を反映するために[GitLabを再設定](../restart_gitlab.md#reconfigure-a-linux-package-installation)します。
   ...
   エラー`execute[generate databases.ini]`が発生した場合、これは既存の[known issue](https://gitlab.com/gitlab-org/omnibus-gitlab/-/issues/4713)によるものです。次のステップの後に2回目の`reconfigure`を実行すると、解決されます。
...
1. 前のステップからの潜在的なエラーを解決するために、もう一度[Reconfigure GitLab](../restart_gitlab.md#reconfigure-a-linux-package-installation)を実行します。
1. 各ノードが現在のプライマリと通信していることを確認します:
...
> - Redisは主にシングルスレッドであり、CPUコアを増やしても大きなメリットは得られません。この規模のアーキテクチャでは、最適なパフォーマンスを達成するために、指定されたキャッシュインスタンスと永続インスタンスを個別に持つことを強くお勧めします。詳細については、[スケーリングに関するドキュメント](_index.md#scaling-an-environment)を参照してください。
...
### 独自のRedisインスタンスを提供する {#provide-your-own-redis-instances}

以下のガイダンスに従って、[third party external service for the Redis Cache and Persistence instances](../redis/replication_and_failover_external.md#redis-as-a-managed-service-in-a-cloud-provider)をオプションで使用できます:
...
### Redisキャッシュクラスターを設定する {#configure-the-redis-cache-cluster}

このセクションでは、新しいRedisキャッシュインスタンスのインストールとセットアップについて説明します。
...
#### プライマリRedisキャッシュノードを設定する {#configure-the-primary-redis-cache-node}
...
#### レプリカRedisキャッシュノードを設定する {#configure-the-replica-redis-cache-nodes}

1. **レプリカ**RedisサーバーにSSHで接続します。
1. 利用したいLinuxパッケージを[ダウンロードしてインストール](../../install/package/_index.md#supported-platforms)します。必ずGitLabパッケージリポジトリのみを追加し、選択したオペレーティングシステム用にGitLabをインストールしてください。現在のインストールと同じバージョンとタイプ（Community EditionまたはEnterprise Edition）を選択します。
1. `/etc/gitlab/gitlab.rb`を編集し、`redis_master_node`を`redis_replica_node`に置き換えて、前のセクションのプライマリノードと同じコンテンツを追加します:
...
### Redis永続クラスターを設定する {#configure-the-redis-persistent-cluster}

このセクションでは、新しいRedis永続インスタンスのインストールとセットアップについて説明します。

プライマリRedisおよびレプリカRedisノードの両方には、`redis['password']`で定義した同じパスワードが必要です。フェイルオーバー中、Sentinelはノードを再設定し、その状態をプライマリからレプリカ（またはその逆）に変更できます。

#### プライマリRedis永続ノードを設定する {#configure-the-primary-redis-persistent-node}
...
#### レプリカRedis永続ノードを設定する {#configure-the-replica-redis-persistent-nodes}

1. **replica** Redis永続サーバーにSSHで接続します。
1. 利用したいLinuxパッケージを[ダウンロードしてインストール](../../install/package/_index.md#supported-platforms)します。必ずGitLabパッケージリポジトリのみを追加し、選択したオペレーティングシステム用にGitLabをインストールしてください。現在のインストールと同じバージョンとタイプ（Community EditionまたはEnterprise Edition）を選択します。
1. `/etc/gitlab/gitlab.rb`を編集し、次の内容を追加します。
...
> [!warning]
> Gitalyの仕様は、正常に稼働する環境での利用パターンとリポジトリサイズの上位パーセンタイルに基づいています。ただし、数ギガバイトを超える）[大規模なモノレポ](_index.md#large-monorepos)や[追加のワークロード](_index.md#additional-workloads)がある場合、環境のパフォーマンスに大きく影響することがあり、さらなる調整が必要になる場合があります。これがあてはまると思われる場合は、必要に応じて追加のガイダンスについてお問い合わせください。

Gitaly Cluster (Praefect)は、耐障害性の利点を提供しますが、セットアップと管理がさらに複雑になります。[Gitaly Cluster (Praefect)をデプロイする前に、既存の技術的な制限事項と考慮事項](../gitaly/praefect/_index.md#before-deploying-gitaly-cluster-praefect)を確認してください。
...
- ロードバランシング: Praefectノードへのトラフィックの均等な分散。高度な設定には、[TCP load balancer](../gitaly/praefect/configure.md#load-balancer) （ほとんどのセットアップで推奨）または[service discovery DNS](../gitaly/praefect/configure.md#service-discovery)を使用できます。詳細については、[load balancing for Praefect](#load-balancing-for-praefect)を参照してください。

このセクションでは、推奨される標準セットアップを順番に設定する方法について詳しく説明します。より高度な設定については、[スタンドアロンGitaly Cluster (Praefect)のドキュメント](../gitaly/praefect/_index.md)を参照してください。

### Praefectのロードバランシング {#load-balancing-for-praefect}

TCPロードバランサーまたはサービスディスカバリDNSを使用して、Praefectノードにトラフィックを分散できます。TCPロードバランサーは、すべてのデプロイシナリオで機能するため、ほとんどのセットアップで推奨されます。

#### TCPロードバランサー {#tcp-load-balancer}

従来のTCPロードバランサー（HAProxyまたはAWS ELBなど）は、Praefectノード全体にトラフィックを分散します。このアプローチは次のことを可能にします。

- すべてのデプロイメソッドで機能します
- 簡単なセットアップと運用管理を提供します。
- TLSおよび非TLSの設定の両方をサポートします。
- 接続が特定のノードに集中する可能性があるため、トラフィックの分散が不均一になる場合があります。
- ノードの再起動後、トラフィックの再バランスに時間がかかる場合があります。

設定手順については、[load balancer](../gitaly/praefect/configure.md#load-balancer)を参照してください。

#### サービスディスカバリDNS {#service-discovery-dns}

サービスディスカバリはDNSを使用してPraefectノードアドレスを取得し、クライアントがすべての利用可能なノードにリクエストを均等に分散できるようにします。このアプローチは次のことを可能にします。

- すべてのPraefectノードにトラフィックを均等に分散します。
- ノードが追加または再起動されたときに、トラフィックを自動的に再バランスします。
- DNSインフラストラクチャ（Consul、CoreDNSなど）が必要です。
- TLSを使用する場合、GitLab 18.9以降が必要です。
- Linuxパッケージ（Omnibus）インストールでのみ利用可能です。

設定手順については、[service discovery](../gitaly/praefect/configure.md#service-discovery)を参照してください。

### Praefect PostgreSQLを設定する {#configure-praefect-postgresql}

Gitalyクラスター（Praefect）のルーティングおよびトランザクションマネージャーであるPraefectは、クラスターステータスデータを保存するために独自のデータベースサーバーを必要とします。
...
まず、Praefect PostgreSQLノードにLinuxパッケージを[インストール](../../install/package/_index.md#supported-platforms)してください。必ずGitLabパッケージリポジトリのみを追加し、選択したオペレーティングシステム用にGitLabをインストールしてください。ただし、`EXTERNAL_URL`値は指定しないでください。
...
> [!note]
> サードパーティのセットアップでは、Geoを使用している場合を除き、便宜上Praefectのデータベースをメインの[GitLab](#provide-your-own-postgresql-instance)データベースと同じサーバーに配置することができます。Geoを使用している場合は、レプリケーションを正しく処理するために個別のデータベースインスタンスが必要です。このセットアップでは、影響は最小限であるはずなので、メインデータベースセットアップの仕様を変更する必要はありません。
...
Praefectは、Gitaly Cluster (Praefect)のルーターおよびトランザクションマネージャーであり、Gitalyへのすべての接続はPraefectを通過します。このセクションでは、その設定方法について詳しく説明します。
...
- `<praefect_external_token>`: Gitaly Cluster (Praefect)でホスティングされているリポジトリに使用され、このトークンを持つGitalyクライアントのみがアクセスできます。
...
Gitaly Cluster (Praefect) ノードは、Praefectで`virtual storage`を使用して設定されます。各ストレージには、クラスターを構成する各Gitalyノードの詳細が含まれています。各ストレージには名前も付けられ、この名前は設定のいくつかの領域で使用されます。このガイドでは、ストレージの名前は`default`になります。また、このガイドは新規インストールを対象としています。既存の環境をアップグレードしてGitaly Cluster (Praefect)を使用する場合は、別の名前を使用する必要がある場合があります。詳細については、[Gitaly Cluster (Praefect)のドキュメント](../gitaly/praefect/configure.md#praefect)を参照してください。
...
1. `/etc/gitlab/gitlab.rb`ファイルを編集して、Praefectを設定します。
   ...
   > [!note]
   > [GitLabが必要とする](../gitaly/configure_gitaly.md#gitlab-requires-a-default-repository-storage)ため、`default`エントリを`virtual_storages`から削除することはできません。
...
1. Praefectでは、メインのGitLabアプリケーションと同様に、いくつかのデータベース移行を実行する必要があります。そのためには、**移行を実行するPraefectノードを1つだけ**選択する必要があります。これは、_デプロイノード_とも呼ばれます。このノードは、次の手順に従って、他のノードよりも先に設定する必要があります。
   ...
   1. 変更を反映し、Praefectデータベースのマイグレーションを実行するために、[GitLabを再設定](../restart_gitlab.md#reconfigure-a-linux-package-installation)します。

1. 他のすべてのPraefectノードで、変更を反映するために[GitLabを再設定](../restart_gitlab.md#reconfigure-a-linux-package-installation)します。
...
1. 利用したいLinuxパッケージを[ダウンロードしてインストール](../../install/package/_index.md#supported-platforms)します。必ずGitLabパッケージリポジトリのみを追加し、選択したオペレーティングシステム用にGitLabをインストールしてください。ただし、`EXTERNAL_URL`値は指定しないでください。
...
- 内部ロードバランサーはTLS接続を処理するように設定する必要があります。ロードバランサーが暗号化されたトラフィックをバックエンドに終端せずに転送するTLSパススルーをサポートするようにロードバランサーを設定します。パススルー/ダイレクトサーバーリターン (DSR) ロードバランサーは使用しないでください。ロードバランサーは、適切なロードバランシングとヘルスチェックを維持するために、接続をアクティブにプロキシする必要があります。
...
コンテナレジストリ、SAML、LDAPなどの追加のGitLab機能を設定する場合は、Rails設定に加えて、Sidekiq設定も更新します。詳細については、[external Sidekiq documentation](../sidekiq/_index.md)を参照してください。

以下のSidekiqノードが例として使用されます:
...
> [!note]
> [Because it's recommended to use Object storage](../object_storage.md)がデータオブジェクトのNFSの代わりに推奨されるため、以下の例にはオブジェクトストレージの設定が含まれています。
...
1. 利用したいLinuxパッケージを[ダウンロードしてインストール](../../install/package/_index.md#supported-platforms)します。必ずGitLabパッケージリポジトリのみを追加し、選択したオペレーティングシステム用にGitLabをインストールしてください。
1. `/etc/gitlab/gitlab.rb`を編集し、以下の設定を使用します。ノード間のリンクの一貫性を維持するため、アプリケーションサーバーの`external_url`は、ユーザーがGitLabへのアクセスに使用する外部URLを指す必要があります。これは、GitLabアプリケーションサーバーへのトラフィックをルーティングする[外部ロードバランサー](#configure-the-external-load-balancer)のURLになります。
...
1. ノードがGitalyに接続できることを確認します:

   ```shell
   sudo gitlab-rake gitlab:gitaly:check
   ```

   次に、ログをテールしてリクエストを確認します:

   ```shell
   sudo gitlab-ctl tail gitaly
   ```

1. オプションで、Gitalyサーバーから、Gitalyが内部APIへのコールバックを実行できることを確認します:
...
Linuxパッケージを使用して、[Prometheus](../monitoring/prometheus/_index.md)を実行するスタンドアロンモニタリングノードを設定できます。
...
> [!note]
> これは**advanced**なセットアップです。Kubernetesでサービスを実行することは、複雑であることがよく知られています。Kubernetesに関する十分な実務知識と経験がある場合にのみ、**この設定が推奨**されます。このセクションの残りの部分では、このことを前提としています。

Kubernetes上のGitalyの可用性、制限、およびデプロイの考慮事項については、[Kubernetes上のGitaly](../gitaly/kubernetes.md)を参照してください。
...
|----------------------|-------------------------|-----------------|--------------|
| Webservice           | 80 vCPU<br/>100 GBメモリ（リクエスト）<br/>140 GBメモリ（制限） | 3 x `n1-standard-32` | 3 x `c5.9xlarge` |
| Sidekiq              | 12.6 vCPU<br/>28 GBメモリ（リクエスト）<br/>56 GBメモリ（制限） | 4 x `n1-standard-4` | 4 x `m5.xlarge`  |
| サポートサービス  | 8 vCPU<br/>30 GBメモリ | 2 x `n1-standard-4` | 2 x `m5.xlarge`   |
...
| PostgreSQL<sup>2</sup>                   | 3     | 8 vCPU、30 GBメモリ  | `n1-standard-8`  | `m5.2xlarge` |
...
| Redis/Sentinel - キャッシュ<sup>3</sup>       | 3     | 4 vCPU、15 GBメモリ  | `n1-standard-4`  | `m5.xlarge`  |
| Redis/Sentinel - 永続<sup>3</sup>  | 3     | 4 vCPU、15 GBメモリ  | `n1-standard-4`  | `m5.xlarge`  |
| Gitaly<sup>6</sup><sup>7</sup>           | 3     | 16 vCPU, 60 GBメモリ | `n1-standard-16` | `m5.4xlarge` |
...
3. 定評のあるサードパーティの外部PaaS Redisソリューションでオプションで実行できます。詳細については、[Provide your own Redis instances](#provide-your-own-redis-instances)および[Recommended cloud providers and services](_index.md#recommended-cloud-providers-and-services)を参照してください。
   - Redisは主にシングルスレッドであり、CPUコアを増やしても大きなメリットは得られません。この規模のアーキテクチャでは、最適なパフォーマンスを達成するために、指定されたキャッシュインスタンスと永続インスタンスを個別に持つことを強くお勧めします。
4. HA機能を提供できる定評のあるサードパーティのロードバランサーまたはサービス（LB PaaS）で実行することをおすすめします。また、サイジングは選択したロードバランサーと、ネットワーク帯域幅などの追加要因によって異なります。詳細については、[ロードバランサー](_index.md#load-balancers)を参照してください。
5. 定評のあるクラウドプロバイダーまたはSelf-Managedソリューションで実行する必要があります。詳細については、[オブジェクトストレージを設定する](#configure-the-object-storage)を参照してください。
6. Gitaly Cluster (Praefect)は、耐障害性の利点を提供しますが、セットアップと管理がさらに複雑になります。[Gitaly Cluster (Praefect)をデプロイする前に、既存の技術的な制限事項と考慮事項](../gitaly/praefect/_index.md#before-deploying-gitaly-cluster-praefect)を確認してください。シャーディングされたGitalyが必要な場合は、上記の表にリストされている`Gitaly`と同じ仕様を使用してください。
7. Gitalyの仕様は、正常に稼働する環境での利用パターンとリポジトリサイズの上位パーセンタイルに基づいています。ただし、（数ギガバイトを超える）[大規模なモノレポ](_index.md#large-monorepos)または[追加のワークロード](_index.md#additional-workloads)がある場合、これらはGitおよびGitalyのパフォーマンスに大幅に影響を与えることがあり、さらなる調整が必要になる場合があります。
<!-- markdownlint-enable MD029 -->
...
200 RPSまたは10,000ユーザーの場合、合計Pumaワーカー数は約80を推奨するため、少なくとも20のWebserviceポッドを実行することをお勧めします。
...
##### ゲートウェイAPI / Ingress {#gateway-api--ingress}

ゲートウェイAPIまたはIngressコントローラーポッドをWebserviceノード全体にDaemonSetとしてデプロイすることも推奨されます。これは、コントローラーがWebserviceポッドとともに動的にスケールすることを可能にし、より大きなマシンタイプが通常持つより高いネットワーク帯域幅を活用するためです。

これは厳密な要件ではありません。ゲートウェイAPIまたはIngressコントローラーポッドは、ウェブトラフィックを処理するのに十分なリソースがある限り、必要に応じてデプロイできます。
...
以前に文書化された標準のデプロイと同様に、ここでは初期目標として14個のSidekiqワーカーが使用されています。特定のワークフローによっては、追加のワーカーが必要になる場合があります。
...
200 RPSまたは10,000ユーザーを対象とするGitLab Helm Chartのリファレンスアーキテクチャ設定の例は、[Chartsプロジェクトで見つけることができます](https://gitlab.com/gitlab-org/charts/gitlab/-/blob/master/examples/ref/10k.yaml)。
...
> [!note]
> 環境と要件に応じて、必要に応じて追加機能をセットアップするために、追加のハードウェア要件または調整が必要になる場合があります。詳細については、個別のページを参照してください。
