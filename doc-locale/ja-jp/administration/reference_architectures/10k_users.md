...
description: "高可用性、データベースのレプリケーション、およびインフラストラクチャのサイジングにより、最大10,000ユーザーまたは200 RPSに対応するGitLab Self-Managedインスタンスをデプロイし、スケールするします。"
...
このページでは、実際のデータに基づいて、手動および自動で最大10,000ユーザーの一般的なピーク負荷である、200 RPS（1秒あたりのリクエスト数）のピーク負荷をターゲットとするように設計されたGitLabリファレンスアーキテクチャについて説明します。

リファレンスアーキテクチャの完全なリストについては、[利用可能なリファレンスアーキテクチャ](_index.md#available-reference-architectures)を参照してください。

> [!note]
> 
> このアーキテクチャをデプロイする前に、まず[主要ドキュメント](_index.md)、特に[はじめに](_index.md#before-you-start)と[どのアーキテクチャを使用するかを決定する](_index.md#deciding-which-architecture-to-start-with)セクションを読んでおくことをおすすめします。

- **Target load**: API: 200 RPS、Web: 20 RPS、Git（プル）: 20 RPS、Git（プッシュ）: 4 RPS
- **HA**: はい（[Praefect](#configure-praefect-postgresql)にはHA用のサードパーティ製PostgreSQLソリューションが必要です）
- **クラウドネイティブハイブリッドの代替**: [はい](#cloud-native-hybrid-reference-architecture-with-helm-charts-alternative)
- **Unsure which Reference Architecture to use**?[詳細については、このガイドを参照してください](_index.md#deciding-which-architecture-to-start-with)
...
3. 定評のあるサードパーティの外部PaaS Redisソリューションでオプションで実行できます。詳細については、[独自のRedisインスタンスを提供する](#provide-your-own-redis-instances)および[推奨されるクラウドプロバイダーとサービス](_index.md#recommended-cloud-providers-and-services)を参照してください。
   - Redisは主にシングルスレッドであり、CPUコアを増やしても大きなメリットは得られません。この規模のアーキテクチャでは、最適なパフォーマンスを達成するために、指定されたように個別のキャッシュインスタンスと永続インスタンスを設けることが強く推奨されます。
4. HA機能を提供できる定評のあるサードパーティのロードバランサーまたはサービス（LB PaaS）で実行することをおすすめします。サイジングは、選択されたLoad BalancerおよびNetwork Bandwidthなどの追加要素に依存します。詳細については、[ロードバランサー](_index.md#load-balancers)を参照してください。
...
> [!note] 
> 
> PaaSソリューションでインスタンスの設定を行う場合、回復力のあるクラウドアーキテクチャのプラクティスに合わせるため、3つの異なるアベイラビリティゾーンに最低3つのノードを実装することをお勧めします。
...
200 RPS / 10,000ユーザーのリファレンスアーキテクチャは、ほとんどの一般的なワークフローに対応するように設計されています。GitLabは、次のエンドポイントスループットの目標に対して、定期的にスモークテストとパフォーマンステストを実施しています。
...
これらの目標は、CIパイプラインやその他のワークロードを含む、指定されたユーザー数に対する環境負荷の合計を反映した、実際の顧客データに基づいています。これは一般的なワークロード構成を示します。非典型的なワークロードパターンについては、[Understanding RPS composition](sizing.md#understanding-rps-composition-and-workload-patterns)を参照してください。
...
GitLabとそのコンポーネントが最大200 RPSまたは10,000ユーザーに対応するように設定するには:
...
> [!note] 
> 
> Consulは、3ノード以上の奇数でデプロイする必要があります。これは、ノードがクォーラムの一部として投票できるようにするためです。
...
- [Recommendedクラウドプロバイダーs and services](_index.md#recommended-cloud-providers-and-services)。
...
1. 必要な[ユーザーとデータベース](../postgresql/external.md)を設定します。
1. [configure GitLab Rails](#configure-gitlab-rails)に従って、GitLabアプリケーションサーバーに適切な接続詳細を設定します。
...
PostgreSQLは、Patroniがそのフェイルオーバーを管理し、競合を処理するためにデフォルトで`pg_rewind`を使用します。ほとんどのフェイルオーバー処理方法と同様に、これによりデータ損失が発生する可能性がわずかにあります。詳細については、さまざまな[Patroniレプリケーション方法](../postgresql/replication_and_failover.md#selecting-the-appropriate-patroni-replication-method)を参照してください。
...
> [!note] 
> 
> PgBouncerはシングルスレッドであり、CPUコアの増加による恩恵はあまりありません。詳細については、[スケーリングに関するドキュメント](_index.md#scaling-an-environment)を参照してください。
...
1. 変更を有効にするため、[GitLabを再設定](../restart_gitlab.md#reconfigure-a-linux-package-installation)します。
   ...
   エラー`execute[generate databases.ini]`が発生した場合、これは既存の[既知のイシュー](https://gitlab.com/gitlab-org/omnibus-gitlab/-/issues/4713)が原因です。次のステップの後に2回目の`reconfigure`を実行すると、解決されます。
...
1. 前のステップからの潜在的なエラーを解決するために、再度[GitLabを再設定](../restart_gitlab.md#reconfigure-a-linux-package-installation)してください。
...
> - Redisは主にシングルスレッドであり、CPUコアを増やしても大きなメリットは得られません。このアーキテクチャでは、単一の統合Redisクラスター（3ノード）を使用しますが、最適なパフォーマンスのために、Redisを個別のキャッシュと永続インスタンス（合計6ノード）に分割できます。RedisのCPUが飽和状態になった場合、これが強く推奨されます。詳細については、[スケーリングに関するドキュメント](_index.md#scaling-an-environment)を参照してください。
...
このセクションでは、GitLabで使用する外部Redisクラスターを設定する方法を説明します。次のIPを例として使用します。
...
次のガイダンスに従って、Redisキャッシュおよび永続インスタンスに[サードパーティの外部サービス](../redis/replication_and_failover_external.md#redis-as-a-managed-service-in-a-cloud-provider)を任意で使用できます:
...
### Redisキャッシュクラスターの設定 {#configure-the-redis-cache-cluster}

このセクションでは、新しいRedisキャッシュインスタンスをインストールして設定します。
...
#### プライマリRedisキャッシュノードの設定 {#configure-the-primary-redis-cache-node}
...
#### レプリカRedisキャッシュノードの設定 {#configure-the-replica-redis-cache-nodes}

1. **レプリカ**RedisサーバーにSSHで接続します。
1. 利用したいLinuxパッケージを[ダウンロードしてインストール](../../install/package/_index.md#supported-platforms)します。必ずGitLabパッケージリポジトリのみを追加し、選択したオペレーティングシステム用にGitLabをインストールしてください。現在のインストールと同じバージョンとタイプ（Community EditionまたはEnterprise Edition）を選択します。
1. `/etc/gitlab/gitlab.rb`を編集し、前のセクションのプライマリノードと同じ内容を`redis_master_node`を`redis_replica_node`に置き換えて追加します:
...
### Redis永続クラスターの設定 {#configure-the-redis-persistent-cluster}

このセクションでは、新しいRedis永続インスタンスをインストールして設定します。

プライマリRedisおよびレプリカRedisノードの両方には、`redis['password']`で定義した同じパスワードが必要です。フェイルオーバー中、Sentinelはノードを再設定し、その状態をプライマリからレプリカ（またはその逆）に変更できます。

#### プライマリRedis永続ノードの設定 {#configure-the-primary-redis-persistent-node}
...
#### レプリカRedis永続ノードの設定 {#configure-the-replica-redis-persistent-nodes}
...
> [!warning] 
> 
> Gitalyの仕様は、良好な状態での使用パターンとリポジトリサイズの高いパーセンタイルに基づいています。ただし、[大規模なモノレポ](_index.md#large-monorepos) （数ギガバイトを超えるもの）または[追加のワークロード](_index.md#additional-workloads)がある場合、これらは環境のパフォーマンスに大きな影響を与え、さらなる調整が必要になる可能性があります。これがあてはまると思われる場合は、必要に応じて追加のガイダンスについてお問い合わせください。
...
- ロードバランシング: Praefectノードへのトラフィックを均等に分散します。ほとんどの設定で推奨される[TCPロードバランサー](../gitaly/praefect/configure.md#load-balancer) 、または高度な設定には[サービスディスカバリDNS](../gitaly/praefect/configure.md#service-discovery)を使用できます。詳細については、[Praefectのロードバランシング](#load-balancing-for-praefect)を参照してください。
...
TCPロードバランサーまたはサービスディスカバリDNSを使用して、Praefectノードにトラフィックを分散できます。TCPロードバランサーは、すべてのデプロイシナリオで機能するため、ほとんどの設定で推奨されます。

#### TCPロードバランサー {#tcp-load-balancer}

従来のTCPロードバランサー（HAProxyやAWS ELBなど）は、Praefectノードにトラフィックを分散します。このアプローチは次のことを可能にします。

- すべてのデプロイ方法で機能します
- 簡単な設定と運用管理を提供します
- TLSと非TLS両方の設定をサポートします
- 接続が特定のノードに集中するため、トラフィックの不均一な分散が発生する可能性があります
- ノードの再起動後、トラフィックの再調整に時間がかかる場合があります

設定手順については、[load balancer](../gitaly/praefect/configure.md#load-balancer)を参照してください。

#### サービスディスカバリDNS {#service-discovery-dns}

サービスディスカバリはDNSを使用してPraefectノードのアドレスを取得するため、クライアントはすべての利用可能なノードにリクエストを均等に分散できます。このアプローチは次のことを可能にします。

- すべてのPraefectノードにトラフィックを均等に分散します
- ノードが追加または再起動されたときに、トラフィックを自動的に再調整します
- DNSインフラストラクチャ（Consul、CoreDNSなど）が必要です
- TLSを使用する場合、GitLab 18.9以降が必要です
- Linuxパッケージ（Omnibus）インストールでのみ利用可能です

設定手順については、[サービスディスカバリ](../gitaly/praefect/configure.md#service-discovery)を参照してください。

### Praefect PostgreSQLを設定する {#configure-praefect-postgresql}

Gitaly Cluster (Praefect)のルーティングおよびトランザクションマネージャーであるPraefectは、クラスターステータスデータを保存するために独自のデータベースサーバーを必要とします。
...
> [!note]
> 
> サードパーティのセットアップでは、メインの[GitLab](#provide-your-own-postgresql-instance)データベースと同じサーバーにPraefectのデータベースを併置することが可能です。ただし、Geoを使用している場合は、レプリケーションを正しく処理するために個別のデータベースインスタンスが必要です。この設定では、メインデータベース設定のspecsを変更する必要はありません。影響は最小限に抑えられるはずです。
...
Gitaly Cluster (Praefect)ノードは、`virtual storage`を経由してPraefectで設定されます。各ストレージには、クラスターを構成する各Gitalyノードの詳細が含まれています。各ストレージには名前も付けられ、この名前は設定のいくつかの領域で使用されます。このガイドでは、ストレージの名前は`default`になります。また、このガイドは新規インストールを対象としています。既存の環境をアップグレードしてGitaly Cluster (Praefect)を使用する場合は、別の名前を使用する必要がある場合があります。詳細については、[Gitaly Cluster (Praefect)のドキュメント](../gitaly/praefect/configure.md#praefect)を参照してください。
...
1. `/etc/gitlab/gitlab.rb`ファイルを編集して、Praefectを設定します。
   ...
   > [!note] 
   > 
   > `default`エントリを`virtual_storages`から削除することはできません。[GitLab requires it](../gitaly/configure_gitaly.md#gitlab-requires-a-default-repository-storage)。
...
- 内部ロードバランサーは、TLS接続を処理するように設定する必要があります。ロードバランサーがTLSパススルーをサポートするように設定します。これは、ロードバランサーが暗号化されたトラフィックを終端せずにバックエンドに転送するものです。パススルー/ダイレクトサーバーリターン（DSR）ロードバランサーは使用しないでください。ロードバランサーは、適切なロードバランシングとヘルスチェックを維持するために、接続をアクティブにプロキシする必要があります。
...
> [!note]
> 
> データオブジェクトにはNFSの代わりに[Object storage](../object_storage.md)の使用が推奨されるため、以下の例にはオブジェクトストレージの設定が含まれています。
...
Linux packageを使用して、[Prometheus](../monitoring/prometheus/_index.md)を実行するスタンドアロンのモニタリングノードを設定できます。
...
> [!note]
> 
> これは**advanced**な設定です。Kubernetesでサービスを実行することは、複雑であることがよく知られています。Kubernetesに関する十分な実務知識と経験がある場合にのみ、**この設定が推奨**されます。このセクションの残りの部分では、このことを前提としています。

Kubernetes上のGitalyの可用性、制限、およびデプロイに関する考慮事項については、[Gitaly on Kubernetes](../gitaly/kubernetes.md)を参照してください。
...
3. 定評のあるサードパーティの外部PaaS Redisソリューションでオプションで実行できます。詳細については、[独自のRedisインスタンスを提供する](#provide-your-own-redis-instances)および[推奨されるクラウドプロバイダーとサービス](_index.md#recommended-cloud-providers-and-services)を参照してください。
   - Redisは主にシングルスレッドであり、CPUコアを増やしても大きなメリットは得られません。この規模のアーキテクチャでは、最適なパフォーマンスを達成するために、指定されたように個別のキャッシュインスタンスと永続インスタンスを設けることが強く推奨されます。
4. HA機能を提供できる定評のあるサードパーティのロードバランサーまたはサービス（LB PaaS）で実行することをおすすめします。また、サイジングは選択したロードバランサーや、ネットワーク帯域幅などの追加要因によって異なります。詳細については、[ロードバランサー](_index.md#load-balancers)を参照してください。
5. 定評のあるクラウドプロバイダーまたはSelf-Managedソリューションで実行する必要があります。詳細については、[オブジェクトストレージを設定する](#configure-the-object-storage)を参照してください。
6. Gitaly Cluster (Praefect)は、耐障害性の利点を提供しますが、セットアップと管理がさらに複雑になります。[Gitaly Cluster (Praefect)をデプロイする前に、既存の技術的な制限事項と考慮事項](../gitaly/praefect/_index.md#before-deploying-gitaly-cluster-praefect)を確認してください。シャーディングされたGitalyが必要な場合は、上記の表にリストされている`Gitaly`と同じ仕様を使用してください。
7. Gitalyの仕様は、正常に稼働する環境での利用パターンとリポジトリサイズの上位パーセンタイルに基づいています。ただし、（数ギガバイトを超える）[大規模なモノレポ](_index.md#large-monorepos)または[追加のワークロード](_index.md#additional-workloads)がある場合、これらはGitおよびGitalyのパフォーマンスに大幅に影響を与えることがあり、さらなる調整が必要になる場合があります。
<!-- markdownlint-enable MD029 -->
...
200 RPSまたは10,000ユーザーの場合、合計Pumaワーカー数は約80が推奨されるため、少なくとも20のWebserviceポッドを実行することが推奨されます。
...
NGINXコントローラーポッドをWebserviceノード全体にDaemonSetとしてデプロイすることも推奨されます。これにより、コントローラーがサービスを提供するWebserviceポッドと動的にスケールすることができるだけでなく、より大きなマシンタイプが一般的に持つ高いnetwork bandwidthを活用できます。
...
以前に文書化された標準的なデプロイと同様に、ここではSidekiqワーカーの初期目標を14に設定しています。特定のワークフローによっては、追加のワーカーが必要になる場合があります。
...
200 RPSまたは10,000ユーザーのリファレンスアーキテクチャ設定をターゲットとするGitLab Helm Chartsの例は、[Chartsプロジェクト](https://gitlab.com/gitlab-org/charts/gitlab/-/blob/master/examples/ref/10k.yaml)で見つけることができます。
...
> [!note]
> 
> 環境と要件に応じて、必要に応じて追加機能を設定するために、追加のハードウェア要件または調整が必要になる場合があります。詳細については、個別のページを参照してください。
