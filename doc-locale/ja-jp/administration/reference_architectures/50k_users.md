...
title: 'リファレンスアーキテクチャ: 最大1000 RPSまたは50,000ユーザー'
---
...
このページでは、実際のデータに基づいて、手動および自動で最大50,000ユーザーの一般的なピーク負荷である、1000 RPS（1秒あたりのリクエスト数）のピーク負荷をターゲットとするように設計されたGitLabのリファレンスアーキテクチャについて説明します。

リファレンスアーキテクチャの完全なリストについては、[利用可能なリファレンスアーキテクチャ](_index.md#available-reference-architectures)を参照してください。

{{< alert type="note" >}}

このアーキテクチャをデプロイする前に、まず[メインドキュメント](_index.md)をよく読み、特に[開始する前に](_index.md#before-you-start)および[使用するアーキテクチャの決定](_index.md#deciding-which-architecture-to-start-with)のセクションをお読みになることをお勧めします。

{{< /alert >}}

- **Target load**（目標負荷）: API: 1000 RPS、Web: 100 RPS、Git（プル）: 100 RPS、Git（プッシュ）: 20 RPS
- **High Availability**: はい（[Praefect](#configure-praefect-postgresql)には、高可用性に対応したサードパーティのPostgreSQLソリューションが必要です）。
...
|------------------------------------------|-------|-------------------------|------------------|---------------|-----------|
| 外部ロードバランサー<sup>4</sup>       | 1     | 16 vCPU、14.4 GBメモリ | `n1-highcpu-16`  | `c5.4xlarge`  | `F16s v2` |
| Consul<sup>2</sup>                       | 3     | 2 vCPU、1.8 GBメモリ   | `n1-highcpu-2`   | `c5.large`    | `F2s v2`  |
| PostgreSQL<sup>2</sup>                   | 3     | 32 vCPU、120 GBメモリ  | `n1-standard-32` | `m5.8xlarge`  | `D32s v3` |
| PgBouncer<sup>2</sup>                    | 3     | 2 vCPU、1.8 GBメモリ   | `n1-highcpu-2`   | `c5.large`    | `F2s v2`  |
| 内部ロードバランサー<sup>4</sup>       | 1     | 16 vCPU、14.4 GBメモリ | `n1-highcpu-16`  | `c5.4xlarge`  | `F16s v2` |
...
| Gitaly<sup>6</sup><sup>7</sup>           | 3     | 64 vCPU、240 GBメモリ  | `n1-standard-64` | `m5.16xlarge` | `D64s v3` |
...
| GitLab Rails<sup>8</sup>                 | 12    | 32 vCPU、28.8 GBメモリ | `n1-highcpu-32`  | `c5.9xlarge`  | `F32s v2` |
...
3. 定評のあるサードパーティの外部PaaS Redisソリューションでオプションで実行できます。詳細については、[独自のRedisインスタンスを提供する](#provide-your-own-redis-instances)を参照してください。
    - Redisは主にシングルスレッドであり、CPUコアを増やしても大きなメリットは得られません。このサイズのアーキテクチャでは、最適なパフォーマンスを実現するために、指定されたように個別のキャッシュインスタンスと永続インスタンスを用意することを強くお勧めします。
...
開始する前に、[要件](_index.md#requirements)参照アーキテクチャについて確認してください。

## テスト手法 {#testing-methodology}

1000 RPS / 50,000ユーザーのリファレンスアーキテクチャは、もっとも一般的なワークフローに対応するように設計されています。GitLabは、次のエンドポイントスループットの目標に対して、定期的にスモークテストとパフォーマンステストを実施しています:
...
GitLabとそのコンポーネントをセットアップして、最大1000 RPSまたは50,000ユーザーに対応するには、次の手順に従います:
...
- `10.6.0.116`: GitLabアプリケーション6
- `10.6.0.117`: GitLabアプリケーション7
- `10.6.0.118`: GitLabアプリケーション8
- `10.6.0.119`: GitLabアプリケーション9
- `10.6.0.120`: GitLabアプリケーション10
- `10.6.0.121`: GitLabアプリケーション11
- `10.6.0.122`: GitLabアプリケーション12
...
まず、**on each node**（各ノードに）Linuxパッケージを[デプロイ](../../install/package/_index.md#supported-platforms)してください。必ずGitLabパッケージリポジトリのみを追加し、選択したオペレーティングシステム用にGitLabをインストールしてください。ただし、`EXTERNAL_URL`値は**not**（指定しないでください）。
...
1. PostgreSQLレプリケーションのユーザー名/パスワードのペアのパスワードハッシュを生成します。これは、`gitlab_replicator`のデフォルトのユーザー名を使用することを前提としています（推奨）。コマンドは、パスワードと確認をリクエストします。次のステップで、このコマンドによって出力された値を`<postgresql_replication_password_hash>`の値として使用します:
...
PostgreSQLでは、Patroniがフェイルオーバーを管理している場合、`pg_rewind`を使用して競合を処理することがデフォルトになっています。ほとんどのフェイルオーバー処理方法と同様に、これによりデータ損失が発生する可能性がわずかにあります。詳細については、さまざまな[Patroniレプリケーション方法](../postgresql/replication_and_failover.md#selecting-the-appropriate-patroni-replication-method)を参照してください。
...
1. 変更を有効にするには、[GitLabを再設定します](../restart_gitlab.md#reconfigure-a-linux-package-installation)。
   ...
   エラー`execute[generate databases.ini]`が発生した場合、既存の[既知の問題](https://gitlab.com/gitlab-org/omnibus-gitlab/-/issues/4713)が原因です。次の手順の後、2回目の`reconfigure`を実行すると解決されます。
...
Redisは主にシングルスレッドであり、vCPUを増やしても大きなメリットは得られません。このサイズのアーキテクチャでは、この規模で最適なパフォーマンスを実現するために、指定されたように個別のキャッシュインスタンスと永続インスタンスを用意することを強くお勧めします。詳細については、[スケーリングに関するドキュメント](_index.md#scaling-an-environment)を参照してください。{{< /alert >}}
...
オプションで、次のガイダンスに従って[Redisキャッシュおよび永続インスタンス用のサードパーティの外部サービス](../redis/replication_and_failover_external.md#redis-as-a-managed-service-in-a-cloud-provider)を使用できます:
...
このセクションでは、新しいRedisキャッシュクラスターをインストールしてセットアップします。
...
1. 利用したいLinuxパッケージを[ダウンロードしてインストール](../../install/package/_index.md#supported-platforms)します。必ずGitLabパッケージリポジトリのみを追加し、選択したオペレーティングシステム用にGitLabをインストールしてください。現在のインストールと同じバージョンとタイプ（Community EditionまたはEnterprise Edition）を選択します。
1. `/etc/gitlab/gitlab.rb`を編集し、`redis_master_node`を`redis_replica_node`に置き換えることで、前のセクションのプライマリノードと同じコンテンツを追加します:
...
### Redisの永続クラスターを設定する {#configure-the-redis-persistent-cluster}

このセクションでは、新しいRedisクラスターをインストールしてセットアップします。
...
1. **replica**（レプリカ） Redisの永続サーバーにSSHでログインします。
1. 利用したいLinuxパッケージを[ダウンロードしてインストール](../../install/package/_index.md#supported-platforms)します。必ずGitLabパッケージリポジトリのみを追加し、選択したオペレーティングシステム用にGitLabをインストールしてください。現在のインストールと同じバージョンとタイプ（Community EditionまたはEnterprise Edition）を選択します。
1. `/etc/gitlab/gitlab.rb`を編集し、次の内容を追加します:
...
Gitalyの仕様は、正常に稼働する環境での利用パターンとリポジトリサイズの上位パーセンタイルに基づいています。ただし、[大規模なmonorepos](_index.md#large-monorepos) (数ギガバイトを超える) または[追加のワークロード](_index.md#additional-workloads)がある場合、これらは環境のパフォーマンスに大きく影響することがあり、さらなる調整が必要になる場合があります。これがあてはまると思われる場合は、必要に応じて追加のガイダンスについてお問い合わせください。
...
サードパーティのセットアップでは、Geoを使用している場合を除き、メインの[GitLab](#provide-your-own-postgresql-instance)データベースと同じサーバー上にPraefectのデータベースを配置できます。Geoでは、レプリケーションを正しく処理するために個別のデータベースインスタンスが必要です。この設定では、影響は最小限であるため、メインデータベースの設定の仕様を変更する必要はありません。
...
- `<praefect_internal_token>`: Gitaly Cluster (Praefect)内のレプリケーションのトラフィックに使用されます。`praefect_external_token`とは異なり、GitalyクライアントはGitalyクラスター（Praefect）の内部ノードに直接アクセスできません。データ損失につながる可能性があります。
...
|----------------------|-------------------------|-----------------|--------------|
| Webservice           | 308 vCPU<br/>385 GBメモリ（リクエスト）<br/>539 GBメモリ（制限） | 11 x `n1-standard-32` | 11 x `c5.9xlarge` |
...
| PostgreSQL<sup>2</sup>                   | 3     | 32 vCPU、120 GBメモリ | `n1-standard-32` | `m5.8xlarge`  |
| PgBouncer<sup>2</sup>                    | 3     | 2 vCPU、1.8 GBメモリ  | `n1-highcpu-2`   | `c5.large`    |
| 内部ロードバランサー<sup>4</sup>       | 1     | 16 vCPU、14.4 GBメモリ | `n1-highcpu-16` | `c5.4xlarge`  |
...
| Gitaly<sup>6</sup><sup>7</sup>           | 3     | 64 vCPU、240 GBメモリ | `n1-standard-64` | `m5.16xlarge` |
...
1000 RPSまたは50,000ユーザーの場合、合計Pumaワーカー数を約308にすることをおすすめしているので、少なくとも77つのWebserviceポッドを実行することを推奨します。
...
前述の標準デプロイと同様に、ここでは14つのSidekiqワーカーの初期ターゲットが使用されています。特定のワークフローによっては、追加のワーカーが必要になる場合があります。
...
上記の1000 RPSまたは50,000ユーザーリファレンスアーキテクチャ設定のGitLab Helmチャートの例は、[クラスタープロジェクトにあります](https://gitlab.com/gitlab-org/charts/gitlab/-/blob/master/examples/ref/50k.yaml)。
