...
description: "最大50,000ユーザーまたは1000 RPSのGitLab Self-Managedインスタンスを、高可用性、データベースレプリケーション、およびインフラストラクチャのサイジングでデプロイします。"
...
このページでは、実際のデータに基づき、ピーク負荷1000 RPS、および最大50,000ユーザーの一般的なピーク負荷（手動および自動）をターゲットとするように設計されたGitLabのリファレンスアーキテクチャについて説明します。
...
- **Target load**: API: 1000 RPS、Web: 100 RPS、Git（プル）: 100 RPS、Git（プッシュ）: 20 RPS
...
1000 RPS / 50kユーザーリファレンスアーキテクチャは、最も一般的なワークフローに対応するように設計されています。GitLabは、次のエンドポイントスループットの目標に対して、定期的にスモークテストとパフォーマンステストを実施しています。
...
GitLabとそのコンポーネントを最大1000 RPSまたは50,000ユーザーに対応するように設定するには:
...
これは、新しいRedisキューインスタンスをインストールして設定するセクションです。
...
このセクションでは、推奨される標準セットアップを順番に設定する方法について詳しく説明します。より高度なセットアップについては、[スタンドアロンのGitalyクラスター (Praefect) ドキュメント](../gitaly/praefect/_index.md)を参照してください。
...
- `<praefect_internal_token>`: Gitaly Cluster (Praefect)内のレプリケーションのトラフィックに使用されます。これは、GitalyクライアントがGitalyクラスター（Praefect）の内部ノードに直接アクセスできないようにする必要があるため`praefect_external_token`とは異なります。直接アクセスできると、データ損失につながる可能性があります。
...
1000 RPSまたは50,000ユーザーの場合、合計Pumaワーカー数は約308が推奨されるため、少なくとも77のWebserviceポッドを実行することが推奨されます。
...
これらのサポートデプロイには、クラウドプロバイダーの実装に関連するさまざまなデプロイと、[GitLab Shell](https://docs.gitlab.com/charts/charts/gitlab/gitlab-shell/)などのGitLabデプロイが含まれます。
...
1000 RPSまたは50,000ユーザーリファレンスアーキテクチャ構成をターゲットとするGitLabHelm Chartの例は、[Chartsプロジェクトで見つけることができます](https://gitlab.com/gitlab-org/charts/gitlab/-/blob/master/examples/ref/50k.yaml)。
