...
description: GitLabのすべてのコンポーネントを、4つの標準化されたサイズのCloud Nativeリファレンスアーキテクチャと外部のPostgreSQL、Redis、オブジェクトストレージを使用してKubernetesにデプロイします。
title: 'リファレンスアーキテクチャ: Cloud Native'
---
...
Cloud Nativeリファレンスアーキテクチャは、ワークロードの特性に基づいた4つの標準化されたサイズ (S/M/L/XL) を持ち、最新のクラウドネイティブなデプロイパターン向けに設計されています。これらのアーキテクチャは、すべてのGitLabコンポーネントをKubernetesにデプロイし、PostgreSQL、Redis、およびオブジェクトストレージには、マネージドサービスやオンプレミスオプションを含む外部のサードパーティソリューションを使用します。

## アーキテクチャの概要 {#architecture-overview}

Cloud Nativeアーキテクチャは、GitLabコンポーネントをKubernetesと外部サービスにデプロイします:
...
- **Supporting services** \- Envoy Gateway、Toolbox、およびモニタリングコンポーネント

> [!note]
> GitalyをKubernetesにデプロイする場合、Gitalyはシャード化された (クラスターではない)設定のみをサポートします。Gitalyは、[クライアントのリトライ](../../administration/settings/gitaly_timeouts.md)を通じてダウンタイムなしでアップグレードできます。各Gitalyポッドは、そのポッドが提供するリポジトリにとって単一障害点となります。Gitalyクラスター (Praefect) はKubernetesではサポートされていません。
>
...
推奨されるマネージドサービスプロバイダー (GCP Cloud SQL、AWS RDS、Azure Databaseなど) については、[インフラストラクチャとサービス](_index.md#infrastructure-and-services)を参照してください。
...
- **運用オーバーヘッドの削減** \- PostgreSQL、Redis、およびオブジェクトストレージの外部サービスにより、データベースとキャッシュのメンテナンスが不要になります。
- **組み込みの高可用性** \- すべてのコンポーネントに対して自動フェイルオーバーを備えたマルチゾーンデプロイを実現する
- **コスト効率の向上** \- ピーク時のキャパシティを維持しつつ、需要の低い期間はリソースをスケールダウンする

## 要件 {#requirements}

Cloud Nativeアーキテクチャをデプロイする前に、次のことを確認してください:
...
|-----------|------------------|------------------|------------------|---------------------------|
| Webservice | 4 vCPU、5 GB (リクエスト)、7 GB (制限) | 6ポッド (24ワーカー) | 9ポッド (36ワーカー) | GCP: 3 × n2-standard-16 (16 vCPU、64 GB)<br/>AWS: 3 × c6i.4xlarge (16 vCPU、32 GB) |
| Sidekiq | 900m vCPU、2 GB（リクエスト）、4 GB（制限） | 8人のワーカー | 12人のワーカー | GCP: 3 × n2-standard-4 (4 vCPU、16 GB)<br/>AWS: 3 × m6i.xlarge (4 vCPU、16 GB) |
| Gitaly | 7 vCPU、30 GB（リクエストと制限） | 3個のポッド | 3個のポッド | GCP: 3 × n2-standard-8 (8 vCPU、32 GB)<br/>AWS: 3 × m6i.2xlarge (8 vCPU、32 GB) |
| サポート用ノードプール | サービスごとに異なる | 12 vCPU、48 GB | 12 vCPU、48 GB | GCP: 3 × n2-standard-4 (4 vCPU、16 GB)<br/>AWS: 3 × c6i.xlarge (4 vCPU、8 GB) |

### ポッドのスケーリング構成 {#pod-scaling-configuration}

| コンポーネント | 最小 → 最大ポッド数 | 最小 → 最大ワーカー数 | ポッドあたりのリソース | ポッドあたりのワーカー数 |
|-----------|----------------|-------------------|-------------------|-----------------|
| Webservice | 6 → 9 | 24 → 36 | 4 vCPU、5 GB (リクエスト)、7 GB (制限) | 4 |
...
- **リポジトリサイズ:** 低頻度で使用されるモノレポをサポート。大規模または頻繁に使用されるモノレポでは、より大きなアーキテクチャサイズが必要になる場合があります。
...
| コンポーネント | ポッドあたりのリソース | 最小ポッド数/ワーカー数 | 最大ポッド数/ワーカー数 | ノード構成例 |
|-----------|------------------|------------------|------------------|---------------------------|
| Webservice | 4 vCPU、5 GB (リクエスト)、7 GB (制限) | 14ポッド (56ワーカー) | 21ポッド (84ワーカー) | GCP: 3 × n2-standard-32 (32 vCPU、128 GB)<br/>AWS: 3 × c6i.8xlarge (32 vCPU、64 GB) |
| Sidekiq | 900m vCPU、2 GB（リクエスト）、4 GB（制限） | 16人のワーカー | 24人のワーカー | GCP: 3 × n2-standard-8 (8 vCPU、32 GB)<br/>AWS: 3 × m6i.2xlarge (8 vCPU、32 GB) |
| Gitaly | 15 vCPU、62 GB（リクエストと制限） | 3個のポッド | 3個のポッド | GCP: 3 × n2-standard-16 (16 vCPU、64 GB)<br/>AWS: 3 × m6i.4xlarge (16 vCPU、64 GB) |
| サポート用ノードプール | サービスごとに異なる | 12 vCPU、48 GB | 12 vCPU、48 GB | GCP: 3 × n2-standard-4 (4 vCPU、16 GB)<br/>AWS: 3 × c6i.xlarge (4 vCPU、8 GB) |
...
| コンポーネント | 最小 → 最大ポッド数 | 最小 → 最大ワーカー数 | ポッドあたりのリソース | ポッドあたりのワーカー数 |
|-----------|----------------|-------------------|-------------------|-----------------|
| Webservice | 14 → 21 | 56 → 84 | 4 vCPU、5 GB (リクエスト)、7 GB (制限) | 4 |
...
- **リポジトリサイズ:** 適度に使用されるモノレポをサポート。大規模または頻繁に使用されるモノレポでは、より大きなアーキテクチャサイズが必要になる場合があります。
...
| コンポーネント | ポッドあたりのリソース | 最小ポッド数/ワーカー数 | 最大ポッド数/ワーカー数 | ノード構成例 |
|-----------|------------------|------------------|------------------|---------------------------|
| Webservice | 4 vCPU、5 GB (リクエスト)、7 GB (制限) | 28ポッド (112ワーカー) | 42ポッド (168ワーカー) | GCP: 6 × n2-standard-32 (32 vCPU、128 GB)<br/>AWS: 6 × c6i.8xlarge (32 vCPU、64 GB) |
| Sidekiq | 900m vCPU、2 GB（リクエスト）、4 GB（制限） | 32人のワーカー | 48人のワーカー | GCP: 6 × n2-standard-8 (8 vCPU、32 GB)<br/>AWS: 6 × m6i.2xlarge (8 vCPU、32 GB) |
| Gitaly | 31 vCPU、126 GB（リクエストと制限） | 3個のポッド | 3個のポッド | GCP: 3 × n2-standard-32 (32 vCPU、128 GB)<br/>AWS: 3 × m6i.8xlarge (32 vCPU、128 GB) |
| サポート用ノードプール | サービスごとに異なる | 12 vCPU、48 GB | 12 vCPU、48 GB | GCP: 3 × n2-standard-4 (4 vCPU、16 GB)<br/>AWS: 3 × c6i.xlarge (4 vCPU、8 GB) |
...
| コンポーネント | 最小 → 最大ポッド数 | 最小 → 最大ワーカー数 | ポッドあたりのリソース | ポッドあたりのワーカー数 |
|-----------|----------------|-------------------|-------------------|-----------------|
| Webservice | 28 → 42 | 112 → 168 | 4 vCPU、5 GB (リクエスト)、7 GB (制限) | 4 |
...
- **リポジトリサイズ:** 頻繁に使用されるモノレポをサポート。集中的に使用されるモノレポでは、より大きなアーキテクチャサイズが必要になる場合があります。
...
| コンポーネント | ポッドあたりのリソース | 最小ポッド数/ワーカー数 | 最大ポッド数/ワーカー数 | ノード構成例 |
|-----------|------------------|------------------|------------------|---------------------------|
| Webservice | 4 vCPU、5 GB (リクエスト)、7 GB (制限) | 56ポッド (224ワーカー) | 84ポッド (336ワーカー) | GCP: 6 × n2-standard-64 (64 vCPU、256 GB)<br/>AWS: 6 × c6i.16xlarge (64 vCPU、128 GB) |
| Sidekiq | 900m vCPU、2 GB（リクエスト）、4 GB（制限） | 64人のワーカー | 96人のワーカー | GCP: 6 × n2-standard-16 (16 vCPU、64 GB)<br/>AWS: 6 × m6i.4xlarge (16 vCPU、64 GB) |
| Gitaly | 63 vCPU、254 GB（リクエストと制限） | 3個のポッド | 3個のポッド | GCP: 3 × n2-standard-64 (64 vCPU、256 GB)<br/>AWS: 3 × m6i.16xlarge (64 vCPU、256 GB) |
| サポート用ノードプール | サービスごとに異なる | 24 vCPU、96 GB | 24 vCPU、96 GB | GCP: 3 × n2-standard-8 (8 vCPU、32 GB)<br/>AWS: 3 × c6i.2xlarge (8 vCPU、16 GB) |

### ポッドのスケーリング構成 {#pod-scaling-configuration-3}

| コンポーネント | 最小 → 最大ポッド数 | 最小 → 最大ワーカー数 | ポッドあたりのリソース | ポッドあたりのワーカー数 |
|-----------|----------------|-------------------|-------------------|-----------------|
| Webservice | 56 → 84 | 224 → 336 | 4 vCPU、5 GB (リクエスト)、7 GB (制限) | 4 |
...
このセクションでは、Cloud Nativeアーキテクチャのデプロイと運用に関する補足的なガイダンスを提供します。これには、マシンタイプの選択、コンポーネント固有の考慮事項、スケール戦略が含まれます。
...
KubernetesのCloud NativeアーキテクチャにおけるGitalyは、次の仕様のStatefulSetsを使用します:
...
Gitalyクラスター (Praefect) はCloud Nativeアーキテクチャではサポートされていません。KubernetesにおけるGitalyデプロイの制限については、[Kubernetes上のGitaly](../gitaly/kubernetes.md)を参照してください。
...
Cloud Nativeアーキテクチャは、基本仕様を超えてスケールするように設計されています。環境に次の要素がある場合、キャパシティの調整が必要になることがあります:
...
- **PostgreSQL and Redis** \- サービスプロバイダーを通じて、より大きなインスタンスタイプにアップグレードしてください。
...
詳細なデプロイステップについては、[KubernetesへのGitLabのインストール](https://docs.gitlab.com/charts/installation/)を参照してください。
...
> [!note]
> オプション機能には追加のキャパシティが必要な場合があります。要件については、機能別のドキュメントを参照してください。
