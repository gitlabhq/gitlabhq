...
> [!note]
> 
> サードパーティ製の設定を使用する場合、Praefectのデータベースをメインの[GitLab](#provide-your-own-postgresql-instance)データベースと同じサーバーに配置することが可能です。ただしGeoを使用している場合は、レプリケーションを正しく処理するために個別のデータベースインスタンスが必要です。このセットアップでは、影響は最小限であるため、メインデータベースのセットアップの仕様を変更する必要はありません。
...
> [!note]
> 
> 以下に記載されていない場合、これより少ない使用数の場合のその他の変更はサポートされません。
