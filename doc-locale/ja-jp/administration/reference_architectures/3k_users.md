...
description: "3,000ユーザーまたは60RPSまでの高可用性コンポーネントと推奨サイジングを備えたGitLab Self-Managedインスタンスをデプロイします。"
...
> [!note]
> サードパーティのセットアップでは、Praefectのデータベースをメインの[GitLab](#provide-your-own-postgresql-instance)データベースと同じサーバーに配置することが可能です。ただしGeoを使用している場合は、レプリケーションを正しく処理するために個別のデータベースインスタンスが必要です。このセットアップでは、影響は最小限であるため、メインデータベースのセットアップの仕様を変更する必要はありません。
...
> [!note]
> 以下に記載されていない場合、これ以外の変更はより少ない使用数の場合はサポートされません。
