...
このページでは、実際のデータに基づいて、手動および自動で最大25,000ユーザーの一般的なピーク負荷である、500 RPS（1秒あたりのリクエスト数）のピーク負荷をターゲットとするように設計されたGitLabリファレンスアーキテクチャについて説明します。
...
- **Target load**: API: 500 RPS、Web: 50 RPS、Git（プル）: 50 RPS、Git（プッシュ）: 10 RPS
...
6. Gitaly Cluster (Praefect)は、耐障害性の利点を提供しますが、セットアップと管理がさらに複雑になります。[Gitaly Clusterをデプロイする前に、既存の技術的な制限事項と考慮事項](../gitaly/praefect/_index.md#before-deploying-gitaly-cluster-praefect)を確認してください。シャーディングされたGitalyが必要な場合は、上記の表にリストされている`Gitaly`と同じ仕様を使用してください。
...
開始する前に、リファレンスアーキテクチャの[要件](_index.md#requirements)を参照してください。
...
最大500 RPSまたは25,000ユーザーに対応するようにGitLabとそのコンポーネントをセットアップするには:
...
- `10.6.0.114`: GitLabアプリケーション4
- `10.6.0.115`: GitLabアプリケーション5
...
内部ロードバランサーは、GitLab環境が必要とする、[PgBouncer](#configure-pgbouncer)や[Gitaly Cluster (Praefect)](#configure-praefect)への接続などの内部接続の負荷分散に使用されます。

これは外部ロードバランサーとは別のノードであり、外部からアクセスされるべきではありません。
...
まず、**各ノードに**Linuxパッケージを[インストール](../../install/package/_index.md#supported-platforms)してください。必ずGitLabパッケージリポジトリのみを追加し、選択したオペレーティングシステム用にGitLabをインストールしてください。ただし、`EXTERNAL_URL`値は**指定しないでください**。
...
1. ファイアウォールを使用して、外部ネットワーク（インターネット）からのアクセスからノードを保護します。
...
- すべてのデプロイシナリオ（Omnibus、Cloud Native Hybrid）で機能します
- 簡単な設定と運用管理を提供します
...
Gitaly Cluster (Praefect)のルーティングおよびトランザクションマネージャーであるGitalyには、Gitaly Cluster (Praefect)の状態に関するデータを格納するための独自のデータベースサーバーが必要です。
...
> [!note]
> 
> サードパーティのセットアップでは、メインの[GitLab](#provide-your-own-postgresql-instance)データベースと同じサーバーにPraefectのデータベースを併置することが可能です。ただし、Geoを使用している場合は、レプリケーションを正しく処理するために個別のデータベースインスタンスが必要です。このセットアップでは、影響は最小限であるはずなので、メインデータベースセットアップの仕様を変更する必要はありません。
...
> [!note] 
> 
> Praefectは、3ノード以上の奇数でデプロイする必要があります。これは、ノードがクォーラムの一部として投票できるようにするためです。
...
500 RPSまたは25,000ユーザーの場合、合計で約140のPumaワーカー数をお勧めするため、少なくとも35のWebサービスポッドを実行することをお勧めします。
...
NGINXコントローラーポッドをWebserviceノード全体にDaemonSetとしてデプロイすることも推奨されます。これは、コントローラーが提供するWebサービスポッドとともに動的にスケールすることを可能にし、より大きなマシンタイプが通常持つ高いネットワーク帯域幅を活用するためです。
...
500 RPSまたは25,000ユーザーをターゲットとするGitLab Helm Chartのリファレンスアーキテクチャ設定の例は、[チャートプロジェクト](https://gitlab.com/gitlab-org/charts/gitlab/-/blob/master/examples/ref/25k.yaml)で見つけることができます。
