...
description: "GitLab Self-Managedインスタンスを、最大25,000ユーザーまたは500 RPSのHA、データベースレプリケーション、およびインフラストラクチャサイジングでデプロイします。"
...
このページでは、実際のデータに基づき、最大500 RPS（1秒あたりのリクエスト数）のピーク負荷、つまり手動および自動の両方で最大25,000ユーザーの一般的なピーク負荷を対象とするように設計されたGitLabリファレンスアーキテクチャについて説明します。
...
3. 定評のあるサードパーティの外部PaaS Redisソリューションでオプションで実行できます。詳細については、[Provide your own Redis instances](#provide-your-own-redis-instances)を参照してください。
...
開始する前に、リファレンスアーキテクチャの[requirements](_index.md#requirements)を参照してください。

## テスト手法 {#testing-methodology}

500 RPS / 25,000ユーザーリファレンスアーキテクチャは、最も一般的なワークフローに対応するように設計されています。GitLabは、次のエンドポイントスループットの目標に対して、定期的にスモークテストとパフォーマンステストを実施しています。
...
GitLabとそのコンポーネントを、最大500 RPSまたは25,000ユーザーに対応するように設定するには、次の手順を実行します:
...
内部ロードバランサーは、GitLab環境が必要とする内部接続（[PgBouncer](#configure-pgbouncer)や[Gitaly Cluster (Praefect)](#configure-praefect)への接続など）のバランスを取るために使用されます。

これは外部ロードバランサーとは別のノードであり、外部からアクセスできないようにする必要があります。
...
まず、[install](../../install/package/_index.md#supported-platforms)でLinuxパッケージを**on each node**にインストールしてください。必ずGitLabパッケージリポジトリのみを追加し、選択したオペレーティングシステム用にGitLabをインストールしてください。ただし、`EXTERNAL_URL`値は指定しないでください。
...
- すべてのデプロイシナリオ（Omnibus、Cloud Native Hybrid）で動作します。
...
> [!note]
> Praefectは3ノード以上の奇数でデプロイする必要があります。これは、ノードがクォーラムの一部として投票できるようにするためです。
...
500 RPSまたは25,000ユーザーの場合、合計Pumaワーカー数を約140とすることをお勧めします。そのため、少なくとも35個のWebserviceポッドを実行することをお勧めします。
...
ゲートウェイAPIまたはIngressコントローラーポッドをWebserviceノード全体にDaemonSetとしてデプロイすることも推奨されます。これは、コントローラーが提供するWebserviceポッドとともに動的にスケールすることを可能にし、一般的に大型のマシンタイプが持つ高いネットワーク帯域幅を活用するためです。
...
500 RPSまたは25,000ユーザーを対象とするGitLab Helm Chartのリファレンスアーキテクチャ設定の例は、[can be found in the Charts project](https://gitlab.com/gitlab-org/charts/gitlab/-/blob/master/examples/ref/25k.yaml)で確認できます。
