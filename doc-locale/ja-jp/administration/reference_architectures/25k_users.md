...
title: 'リファレンスアーキテクチャ: 最大500 RPSまたは25,000ユーザー'
---
...
このページでは、実際のデータに基づいて、手動および自動による利用を合わせて最大25,000ユーザーの一般的なピーク負荷をターゲットとするように設計されたGitLabのリファレンスアーキテクチャについて説明します。このリファレンスアーキテクチャは、1秒あたり500リクエストを処理するように設計されています。

リファレンスアーキテクチャの完全なリストについては、[利用可能なリファレンスアーキテクチャ](_index.md#available-reference-architectures)を参照してください。

{{< alert type="note" >}}

このアーキテクチャをデプロイする前に、まず[メインドキュメント](_index.md)をよく読み、特に[始める前に](_index.md#before-you-start)と[使用するアーキテクチャの決定](_index.md#deciding-which-architecture-to-start-with)のセクションをお読みください。

{{< /alert >}}

- **Target load**（目標負荷）: API: 500 RPS、Web: 50 RPS、Git（プル）: 50 RPS、Git（プッシュ）: 10 RPS
- **High Availability**: はい（[Praefect](#configure-praefect-postgresql)で高可用性を実現するには、サードパーティのPostgreSQLソリューションが必要です）
...
|------------------------------------------|-------|-------------------------|------------------|--------------|-----------|
| 外部ロードバランサー<sup>4</sup>       | 1     | 8 vCPU、7.2 GBメモリ   | `n1-highcpu-8`   | `c5n.2xlarge` | `F8s v2`  |
| Consul<sup>2</sup>                       | 3     | 2 vCPU、1.8 GBメモリ   | `n1-highcpu-2`   | `c5.large`   | `F2s v2`  |
| PostgreSQL<sup>2</sup>                   | 3     | 16 vCPU、60 GBメモリ   | `n1-standard-16` | `m5.4xlarge` | `D16s v3` |
| PgBouncer<sup>2</sup>                    | 3     | 2 vCPU、1.8 GBメモリ   | `n1-highcpu-2`   | `c5.large`   | `F2s v2`  |
| 内部ロードバランサー<sup>4</sup>       | 1     | 8 vCPU、7.2 GBメモリ   | `n1-highcpu-8`   | `c5n.2xlarge` | `F8s v2`  |
...
| Gitaly<sup>6</sup><sup>7</sup>           | 3     | 32 vCPU、120 GBメモリ  | `n1-standard-32` | `m5.8xlarge` | `D32s v3` |
| Praefect<sup>6</sup>                     | 3     | 4 vCPU、3.6 GBメモリ   | `n1-highcpu-4`   | `c5.xlarge`  | `F4s v2`  |
...
| GitLab Rails<sup>8</sup>                 | 5     | 32 vCPU、28.8 GBメモリ | `n1-highcpu-32`  | `c5.9xlarge` | `F32s v2` |
...
3. 定評のあるサードパーティの外部PaaS Redisソリューションでオプションで実行できます。詳細については、[独自のRedisインスタンスを提供する](#provide-your-own-redis-instances)を参照してください。
    - Redisは主にシングルスレッドであり、CPUコアを増やしても大きなメリットは得られません。この規模のアーキテクチャでは、最適なパフォーマンスを実現するために、指定されたとおりに個別のキャッシュインスタンスと永続インスタンスを用意することを強くお勧めします。
...
6. Gitalyクラスター（Praefect）は、耐障害性の利点を提供しますが、セットアップと管理がさらに複雑になります。[Gitaly Clusterをデプロイする前に、既存の技術的な制限事項と考慮事項](../gitaly/praefect/_index.md#before-deploying-gitaly-cluster-praefect)を確認してください。シャーディングされたGitalyが必要な場合は、上記の表にリストされている`Gitaly`と同じ仕様を使用してください。
...
開始する前に、[参照アーキテクチャの要件](_index.md#requirements)を参照してください。

## テスト手法 {#testing-methodology}

500 RPS / 25,000ユーザーのリファレンスアーキテクチャは、最も一般的なワークフローに対応するように設計されています。GitLabは、次のエンドポイントスループットの目標に対して、定期的にスモークテストとパフォーマンステストを実施しています:
...
GitLabとそのコンポーネントをセットアップして、最大500 RPSまたは25,000ユーザーに対応するには、次の手順に従います:
...
このシナリオでは、ロードバランサーとNGINX間のトラフィックも安全になります。接続は常に安全であるため、プロキシSSLの設定を追加する必要はありません。ただし、SSL証明書を設定するには、GitLabに設定を追加する必要があります。SSL証明書の管理とNGINXの設定の詳細については、[HTTPSのドキュメント](https://docs.gitlab.com/omnibus/settings/ssl/)を参照してください。
...
内部ロードバランサーは、GitLab環境が必要とする[PgBouncer](#configure-pgbouncer)および[Gitalyクラスター（Praefect）](#configure-praefect)への接続など、すべての内部接続のロードバランシングに使用されます。

これは外部ロードバランサーとは別のノードであり、外部からのアクセスは許可されていません。
...
まず、**on each node**（各ノードに）Linuxパッケージを[インストール](../../install/package/_index.md#supported-platforms)してください。必ずGitLabパッケージリポジトリのみを追加し、選択したオペレーティングシステム用にGitLabをインストールしてください。ただし、`EXTERNAL_URL`値は**not**（指定しないでください）。
...
1. 変更を有効にするには、[GitLabを再設定します](../restart_gitlab.md#reconfigure-a-linux-package-installation)。
   ...
   エラー`execute[generate databases.ini]`が発生した場合、これは既存の[既知の問題](https://gitlab.com/gitlab-org/omnibus-gitlab/-/issues/4713)が原因です。これは、次のステップの後に2回目の`reconfigure`を実行すると解決されます。
...
1. 前の手順で発生する可能性のあるエラーを解決するために、もう一度[GitLabを再構成](../restart_gitlab.md#reconfigure-a-linux-package-installation)します。
...
Redisは主にシングルスレッドであり、CPUコアを増やしても大きなメリットは得られません。この規模のアーキテクチャでは、最適なパフォーマンスを実現するために、指定されたとおりに個別のキャッシュインスタンスと永続インスタンスを用意することを強くお勧めします。詳細については、[スケーリングに関するドキュメント](_index.md#scaling-an-environment)を参照してください。{{< /alert >}}
...
1. ファイアウォールなどのオプションを使用して、外部ネットワーキング（インターネット）からのアクセスからノードを保護します。
...
次のガイダンスに従って、[Redisのキャッシュおよび永続インスタンス用のサードパーティの外部サービス](../redis/replication_and_failover_external.md#redis-as-a-managed-service-in-a-cloud-provider)をオプションで使用できます:
...
### Redisキャッシュクラスターを構成する {#configure-the-redis-cache-cluster}

このセクションでは、新しいRedis Cacheインスタンスをインストールしてセットアップします。
...
#### プライマリRedisキャッシュノードを構成する {#configure-the-primary-redis-cache-node}
...
#### レプリカRedisキャッシュノードを構成する {#configure-the-replica-redis-cache-nodes}

1. **replica**（レプリカ）RedisサーバーにSSHで接続します。
1. 利用したいLinuxパッケージを[ダウンロードしてインストール](../../install/package/_index.md#supported-platforms)します。必ずGitLabパッケージリポジトリのみを追加し、選択したオペレーティングシステム用にGitLabをインストールしてください。現在のインストールと同じバージョンとタイプ（Community EditionまたはEnterprise Edition）を選択します。
1. `/etc/gitlab/gitlab.rb`を編集し、前のセクションのプライマリノードと同じコンテンツを追加し、`redis_master_node`を`redis_replica_node`に置き換えます:
...
### Redis永続クラスターを構成する {#configure-the-redis-persistent-cluster}

このセクションでは、新しいRedis Persistentインスタンスをインストールしてセットアップします。

プライマリRedisおよびレプリカRedisノードの両方には、`redis['password']`で定義した同じパスワードが必要です。フェイルオーバー中、Sentinelはノードを再設定し、その状態をプライマリからレプリカ（またはその逆）に変更できます。

#### プライマリRedis永続ノードを構成する {#configure-the-primary-redis-persistent-node}
...
#### レプリカRedis永続ノードを構成する {#configure-the-replica-redis-persistent-nodes}

1. **replica**（レプリカ）Redis永続サーバーにSSHでログインします。
1. 利用したいLinuxパッケージを[ダウンロードしてインストール](../../install/package/_index.md#supported-platforms)します。必ずGitLabパッケージリポジトリのみを追加し、選択したオペレーティングシステム用にGitLabをインストールしてください。現在のインストールと同じバージョンとタイプ（Community EditionまたはEnterprise Edition）を選択します。
1. `/etc/gitlab/gitlab.rb`を編集し、次の内容を追加します:
...
サードパーティのセットアップでは、Geoを使用している場合を除き、メインの[GitLab](#provide-your-own-postgresql-instance)データベースと同じサーバー上にPraefectのデータベースを配置できます。Geoでは、レプリケーションを正しく処理するために個別のデータベースインスタンスが必要です。この設定では、影響は最小限であるため、メインデータベースのセットアップの仕様を変更する必要はありません。
...
NFS経由でジョブログを共有することはサポートされていますが、[増分ログの生成](../cicd/job_logs.md#incremental-logging)を有効にして、NFSの使用が必要となるのを避けてください。増分ログの生成では、ジョブログの一時的なキャッシュのため、ディスク容量の代わりにRedisを使用します。
...
[KubernetesのGitaly](../gitaly/kubernetes.md)の可用性、制限事項、デプロイに関する考慮事項については、こちらをご覧ください。
...
|----------------------|-------------------------|-----------------|--------------|
| Webservice           | 140 vCPU<br/>175 GBメモリ（リクエスト）<br/>245 GBメモリ（制限） | 5 x `n1-standard-32` | 5 x `c5.9xlarge` |
...
| PostgreSQL<sup>2</sup>                   | 3     | 16 vCPU、60 GBメモリ  | `n1-standard-16` | `m5.4xlarge` |
| PgBouncer<sup>2</sup>                    | 3     | 2 vCPU、1.8 GBメモリ  | `n1-highcpu-2`   | `c5.large`   |
| 内部ロードバランサー<sup>4</sup>       | 1     | 8 vCPU、7.2 GBメモリ  | `n1-highcpu-8`   | `c5.2xlarge` |
...
| Gitaly<sup>6</sup><sup>7</sup>           | 3     | 32 vCPU、120 GBメモリ | `n1-standard-32` | `m5.8xlarge` |
| Praefect<sup>6</sup>                     | 3     | 4 vCPU、3.6 GBメモリ  | `n1-highcpu-4`   | `c5.xlarge`  |
...
3. 定評のあるサードパーティの外部PaaS Redisソリューションでオプションで実行できます。詳細については、[独自のRedisインスタンスを提供する](#provide-your-own-redis-instances)を参照してください。
    - Redisは主にシングルスレッドであり、CPUコアを増やしても大きなメリットは得られません。この規模のアーキテクチャでは、最適なパフォーマンスを実現するために、指定されたとおりに個別のキャッシュインスタンスと永続インスタンスを用意することを強くお勧めします。
4. 信頼できるサードパーティのロードバランシングサービス（LB PaaS）でオプションで実行できます。詳細については、[推奨されるクラウドプロバイダーとサービス](_index.md#recommended-cloud-providers-and-services)を参照してください。
5. 定評のあるクラウドプロバイダーまたはSelf-Managedソリューションで実行する必要があります。詳細については、[オブジェクトストレージを設定する](#configure-the-object-storage)を参照してください。
...
500 RPSまたは25,000ユーザーの場合、合計Pumaワーカー数を約140にすることをおすすめしているので、少なくとも35のWebserviceポッドを実行することを推奨します。
...
上記の500 RPSまたは25,000ユーザーのリファレンスアーキテクチャ設定のGitLab Helmチャートの例は、[クラスタープロジェクトにあります](https://gitlab.com/gitlab-org/charts/gitlab/-/blob/master/examples/ref/25k.yaml)。
