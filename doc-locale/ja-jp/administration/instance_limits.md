...
GitLabは、POSTリクエストに対するレート制限をデフォルトで次のパスに適用します:
...
GitLabは、GETリクエストに対するレート制限をデフォルトで次のパスに適用します:
...
これらの設定は、グループとプロジェクトのファイルのインポートとエクスポートを制限します。

| 制限                   | デフォルト（ユーザーごとに毎分） |
|:------------------------|:------------------------------|
| プロジェクトのインポート          | インポートリクエスト6件             |
| プロジェクトのエクスポート          | エクスポートリクエスト6件             |
| プロジェクトのエクスポートのダウンロード | ダウンロードリクエスト1件           |
| グループのインポート            | インポートリクエスト6件             |
| グループのエクスポート            | エクスポートリクエスト6件             |
| グループのエクスポートのダウンロード   | ダウンロードリクエスト1件           |

これらの設定は[構成できます](settings/import_export_rate_limits.md)。

#### 直接転送の移行 {#direct-transfer-migration}

{{< history >}}

- 許可される移行の最大制限はGitLab 15.9で[導入されました](https://gitlab.com/gitlab-org/gitlab/-/issues/386452)。
- 設定はGitLab 16.3で[導入されました](https://gitlab.com/gitlab-org/gitlab/-/issues/384976)。
- GitLab 16.7で、移行にまつわる8時間の制限が[削除](https://gitlab.com/gitlab-org/gitlab/-/issues/429867)されました。

{{< /history >}}

直接転送による移行には、次の制限が適用されます。

| 制限                                                                      | デフォルト     | 設定可能 |
|:---------------------------------------------------------------------------|:------------|:-------------|
| ユーザーあたりの宛先GitLabインスタンスによる1分あたりの移行回数。 | 6           | {{< no >}}   |
| アーカイブファイルを解凍するための待機時間。                            | 210秒 | {{< no >}}   |
| NDJSON行の長さ。                                                   | 50 MB       | {{< no >}}   |
| ソースインスタンスで空のエクスポートステータスが呼び出されるまでの時間。            | 5分   | {{< no >}}   |
| ソースインスタンスからダウンロードできるリレーションサイズ。             | 5 GiB       | {{< yes >}}  |
| 解凍されたアーカイブのサイズ。                                            | 10 GiB      | {{< yes >}}  |

設定可能な制限を変更する方法の詳細については、[インポートとエクスポートの設定](settings/import_and_export_settings.md)を参照してください。
...
トップレベルのネームスペースにあるWebhookが1分間に呼び出される回数を制限します。ネームスペース内のすべてのプロジェクトおよびグループのWebhookは、この制限を共有します。

レート制限を超過した呼び出しは`auth.log`にログされます。

Self-Managedインスタンスのこの制限を設定するには、[プラン制限API](../api/plan_limits.md)を使用するか、[GitLab Railsコンソール](operations/rails_console.md#starting-a-rails-console-session)で次のコマンドを実行します:
...
リベースを行う際、サイズ制限を超過したコミットメッセージは切り詰められます。この制限は、コミットのタイトルと説明のサイズ制限とは別です。
...
この設定は、解凍後のGzip圧縮されたHTTP応答で許可される最大サイズをMiBで制限します。
...
### 送信リクエストからのHTTP応答の最大サイズ {#maximum-http-responses-size-from-outbound-requests}
...
この設定は、解凍されたHTTP応答で許可される最大サイズをMiBで制限します。これは、インテグレーション、インポーター、およびWebhookに適用されます。
...
### 送信リクエストからのJSON HTTP応答で許可されるオブジェクトの最大数 {#maximum-allowed-object-count-in-json-http-responses-from-outbound-requests}
...
この設定は、送信リクエストからのJSON HTTP応答で許可されるオブジェクトの最大数を制限します。オブジェクトの数は、応答中の`:`、`,`、`{`、および`[`の出現回数に基づいて推定されます。
...
### 送信リクエストからのJSON HTTP応答で許可される最大ネスト深度 {#maximum-allowed-nesting-depth-in-json-http-responses-from-outbound-requests}
...
この設定は、送信リクエストからのJSON HTTP応答で許可される最大ネスト深度を制限します。
...
### 送信リクエストからのXML HTTP応答で許可されるオブジェクトの最大数 {#maximum-allowed-object-count-in-xml-http-responses-from-outbound-requests}
...
この設定は、送信リクエストからのXML HTTP応答で許可されるオブジェクトの最大数を制限します。オブジェクトの数は、応答中の`<`、`=`の出現回数に基づいて推定されます。
...
### 送信リクエストからのCSV HTTP応答で許可されるオブジェクトの最大数 {#maximum-allowed-object-count-in-csv-http-responses-from-outbound-requests}
...
この設定は、送信リクエストからのCSV HTTP応答で許可されるオブジェクトの最大数を制限します。オブジェクトの数は、応答中の`,`、`;`、`\t`、および`\n`の出現回数に基づいて推定されます。
...
デフォルトでは、リクエスト内のJSONパラメータは制限されます。詳細については、[JSON検証のエンドポイントごとの制限](#json-validation-limits-by-endpoint)を参照してください。
...
このチェックを無効にするには、`--set gitlab.webservice.extraEnv.GITLAB_JSON_GLOBAL_VALIDATION_MODE="disabled"`を使用するか、次の内容をvaluesファイルに指定します:
...
| エンドポイント                                                                                     | 説明           | メソッド | 最大深度 | 最大配列サイズ | 最大ハッシュサイズ | 総要素数 | 最大JSONサイズ | モード |
|:---------------------------------------------------------------------------------------------|:----------------------|:--------|:----------|:---------------|:--------------|:-------------------|:--------------|:-----|
| その他のすべてのパス                                                                              | デフォルト               | すべて     | 32        | 50,000         | 50,000        | 100,000            | 0（無効）  | 適用済み |
| `/api/v4/projects/{id}/terraform/state/`                                                     | Terraformステート       | POST    | 64        | 50,000         | 50,000        | 250,000            | 50 MB         | ロギング<sup>1</sup> |
| `/api/v4/packages/npm/-/npm/v1/security/`<br/>`{advisories/bulk\|audits/quick}`               | NPMインスタンスパッケージ | POST    | 32        | 50,000         | 50,000        | 250,000            | 50 MB         | 適用済み |
| `/api/v4/groups/{id}/-/packages/npm/-/npm/v1/security/`<br/>`{advisories/bulk\|audits/quick}` | NPMグループパッケージ    | POST    | 32        | 50,000         | 50,000        | 250,000            | 50 MB         | 適用済み |
| `/api/v4/projects/{id}/packages/npm/-/npm/v1/security/`<br/>`{advisories/bulk\|audits/quick}` | NPMプロジェクトパッケージ  | POST    | 32        | 50,000         | 50,000        | 250,000            | 50 MB         | 適用済み |
| `/api/v4/internal/*`                                                                         | 内部API          | POST    | 32        | 50,000         | 50,000        | 0（無効）       | 10 MB         | 適用済み |
| `/api/v4/ai/duo_workflows/workflows/*`                                                        | GitLab DuoワークフローAPI      | POST    | 32        | 5,000          | 5,000         | 0（無効）       | 25 MB         | 適用済み |
...
| 環境変数                 | 目的                     | デフォルト      | スコープ |
|:-------------------------------------|:----------------------------|:-------------|:------|
| `GITLAB_JSON_MAX_DEPTH`              | デフォルトの最大ネスト深度   | 32           | デフォルトの制限のみ |
| `GITLAB_JSON_MAX_ARRAY_SIZE`         | デフォルトの最大配列要素数  | 50,000       | デフォルトの制限のみ |
| `GITLAB_JSON_MAX_HASH_SIZE`          | デフォルトの最大ハッシュキー数       | 50,000       | デフォルトの制限のみ |
| `GITLAB_JSON_MAX_TOTAL_ELEMENTS`     | デフォルトの総要素数  | 100,000      | デフォルトの制限のみ |
| `GITLAB_JSON_MAX_JSON_SIZE_BYTES`    | デフォルトの最大ボディサイズ       | 0（無効） | デフォルトの制限のみ |
| `GITLAB_JSON_VALIDATION_MODE`        | デフォルトの検証モード     | `enforced`   | デフォルトの制限のみ |
...
| モード       | 説明 |
|:-----------|:------------|
| `enforced` | 制限を超過するリクエストを検証し、ブロックします（HTTP 400を返します）。本番環境の保護に使用されます。 |
| `logging`  | 検証し、違反をログに記録しますが、リクエストは許可します。モニタリングとデバッグに使用されます。すべてのエンドポイントはログのみで、`enforced`をオーバーライドします。 |
...
- 適用済みモードで制限を超過すると、応答はJSONエラーメッセージを含むHTTP 400となります。
- 総要素数は、JSON構造全体にわたる配列とハッシュ内のすべての要素を含みます。
...
インポート中に作成される[プレースホルダーユーザー](../user/import/mapping.md#placeholder-users)の数は、トップレベルのネームスペースごとに制限できます。
...
GitLab Self-Managedの場合は、[GitLab Railsコンソール](operations/rails_console.md#starting-a-rails-console-session)を使用することもできます。たとえば、制限を100に設定するには:
...
GitLab Pagesウェブサイトごとのカスタムドメインの総数は、[GitLab.com](../subscriptions/manage_users_and_seats.md#gitlabcom-billing-and-usage)では`150`に制限されています。
...
プロジェクトで[ユニークドメイン](../user/project/pages/_index.md#unique-domains)が有効になっている場合、そのプロジェクトのユニークドメインは、1000デプロイという個別の制限を持つ独自のトップレベルネームスペースとして扱われます。
...
## プロジェクトのプッシュミラーの最大数 {#maximum-number-of-project-push-mirrors}
...
各プロジェクトで有効にできるプッシュミラーの最大数は10個です。この制限は、過剰な同時同期ジョブによるパフォーマンスの問題を防ぎます。

より多くのミラーが必要な場合は、次のことができます:

- 未使用のミラーを無効にします。
- 複数の宛先を1つのミラーに結合して、ミラーを統合します。
...
## 依存関係スキャン（SBOMを使用）の制限 {#dependency-scanning-using-sbom-limits}

The [依存関係スキャン（SBOMを使用する機能）](../user/application_security/dependency_scanning/dependency_scanning_sbom/_index.md)は、次の制限を持つ内部APIを使用します:

- プロジェクトごとの1時間あたりの最大アップロードリクエスト数: 400
- プロジェクトごとの1時間あたりの最大ダウンロードリクエスト数: 6000

これらの制限は、GitLab Self-Managedインスタンスの[依存関係スキャンの設定](settings/security_and_compliance.md#sbom-scan-api-limits)を使用して設定できます。
...
コミットとファイルAPIは、次のエンドポイントで最大サイズとレート制限を適用します:

- `POST /projects/:id/repository/commits` - [コミットを作成](../api/commits.md#create-a-commit)
- `POST /projects/:id/repository/files/:file_path` - [リポジトリ内にファイルを作成](../api/repository_files.md#create-a-file-in-a-repository)
- `PUT /projects/:id/repository/files/:file_path` - [リポジトリ内のファイルを更新](../api/repository_files.md#update-a-file-in-a-repository)

- **Maximum request size**: この制限を超過するリクエストは、`413 Request Entity Too Large`エラーとともに次のメッセージを受け取ります: `RequestBody: upload failed: the upload size <size> is over maximum of 314572800 bytes: entity is too large`。デフォルトは300 MB（314,572,800バイト）です。
- **レート制限**: 20 MBを超えるリクエストの場合、30秒あたり3リクエスト。

最大リクエストサイズは、GitLab Self-Managedで`GITLAB_COMMITS_MAX_REQUEST_SIZE_BYTES`環境変数を設定することで設定可能です。この変数は、最大リクエストサイズをバイト単位で設定します。環境変数の設定方法については、[HTTPリクエスト制限](#http-request-limits)を参照してください。
