...
## GitLab Duoの使用データに関するロギングを有効にする {#turn-on-logging-for-gitlab-duo-usage-data}

前提条件: 

- 管理者である必要があります。

AI関連のアクティビティとリクエストに関するデータを収集するには、AIログをオンにします。このデータは、お使いのGitLabインストール環境に保存され、GitLabと共有されることはありません。
...
ログのサブセットを制御するには、GitLab Duoの設定ページからLLMログをオンまたはオフにします。LLMログをオフにすると、特定の操作のロギングが無効になります。

### `llm.log`ファイル {#llmlog-file}

AIログが有効になっている場合、GitLabSelf-Managedインスタンスを介して発生するコード生成およびチャットイベントは、[`llm.log`ファイル](../logs/_index.md#llmlog)にキャプチャされます。ログファイルは、有効になっていない場合は何もキャプチャしません。コード補完のログは、AIゲートウェイに直接キャプチャされます。これらのログはGitLabに送信されず、GitLab Self-Managedインスタンスのインフラストラクチャでのみ表示されます。
...
- AIログが有効になっている場合、追加のデバッグ情報が以下のようにログに記録されます:
  - 前処理されたプロンプト。
  - 最終的なプロンプト。
  - 追加のコンテキスト。
- **プライバシー**: GitLabとAIゲートウェイの両方がセルフホストされているため:
  ...
  > [!note]
  > AI機能がGitLabのAIサードパーティベンダーモデルを使用する場合、AIログが有効になっている場合でも、GitLabホスト型のAIゲートウェイでは詳細なログは生成されません。これにより、機密情報の意図しない漏洩を防ぎます。
...
- 拡張ロギング: AIログが有効になっている場合でも、機密情報の意図しない漏洩を防ぐため、GitLab管理のAIゲートウェイでは詳細なログは生成されません。
  - この設定ではロギングは最小限に抑えられ、拡張ロギング機能はデフォルトで無効になっています。
- プライバシー: この設定は、クラウド環境で機密情報がログに記録されないように設計されています。

## AIログ {#ai-logs}

AIログは、プロンプトや入力を含む追加のデバッグ情報がログに記録されるかどうかを制御します。この設定は、AI関連のアクティビティをモニタリングおよびデバッグするために不可欠です。
