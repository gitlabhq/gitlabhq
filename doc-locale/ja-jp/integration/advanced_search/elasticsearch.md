...
description: GitLabで高度検索を使用するようにElasticsearchをセットアップして設定します。
title: Elasticsearch
---
...
> [!note]
>
> 高度検索は、すべてのプロジェクトを同じElasticsearchのインデックスに保存します。ただし、非公開プロジェクトは、アクセス権を持つユーザーの検索結果だけに表示されます。
...
> [!note]
> 
> 新しいElasticsearchのクラスターを本番環境で使用する前に、[重要なElasticsearchの設定](https://www.elastic.co/guide/en/elasticsearch/reference/current/important-settings.html)を参照してください。
...
> [!note]
> 
> アカウント間で[AWS `AssumeRole`](https://docs.aws.amazon.com/STS/latest/APIReference/API_AssumeRole.html)を使用する場合は、`aws_role_arn`を指定する必要があります。ARNは、OpenSearchにアクセスする権限を持つロールである必要があります。
...
1. **AWS OpenSearch IAMの認証情報**セクションで、次の手順を実行します。
   ...
   1. **AWSアクセスキー**と**AWSシークレットアクセスキー**に、認証用のアクセスキーを入力します。
      ...
      > [!note] 
      > 
      > GitLabのデプロイがEC2インスタンスで直接実行されている場合（コンテナ内ではない場合）、アクセスキーを入力する必要はありません。GitLabインスタンスは、[AWSインスタンスメタデータサービス（IMDS）](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/configuring-instance-metadata-service.html)からこれらのキーを自動的に取得します。
...
### 新しいElasticsearchバージョンへのアップグレード {#upgrade-to-a-new-elasticsearch-version}
...
Elasticsearchを新しいマイナーまたはメジャーバージョンにアップグレードしても、GitLabの設定を変更する必要はありません。Elasticsearchクラスターが完全にアップグレードされ、アクティブになった場合: 
...
1. （オプション）[インデックス作成状態を確認します](#check-indexing-status)。正しい検索結果を得るには、インデックス作成が完了していることを確認してください。特にElasticsearchインスタンスがしばらくオフラインだった場合は重要です。
...
このプロジェクトは、テキストのエンコードに[Unicode用国際コンポーネント](https://icu.unicode.org/) (ICU) に依存しているため、`make`を実行する前にプラットフォームの開発パッケージがインストールされていることを確認してください。
...
> [!note]
> まず[Homebrewをインストール](https://brew.sh/)する必要があります。
...
1. 右上隅で、**管理者**を選択します。
1. **設定** > **検索**を選択します。
1. Elasticsearchクラスターの[高度な検索設定](#advanced-search-configuration)を構成します。まだ**高度な検索による検索**チェックボックスを選択しないでください。
...
1. 右上隅で、**管理者**を選択します。
1. **設定** > **検索**を選択します。
1. **高度な検索のためのインデックス作成を有効にする**チェックボックスを選択し、次に**変更を保存**を選択します。
...
インデックス作成の進捗をモニタリングするには、バックグラウンドジョブのステータスも確認できます:

1. 右上隅で、**管理者**を選択します。
1. **モニタリング** > **バックグラウンドジョブ**を選択します。
1. Sidekiqダッシュボードで、**ビジー**を選択し、これらのインデックス作成ジョブに注目してください:
   - `Search::Elastic::CommitIndexerWorker`コードとコミット用。
   - `ElasticWikiIndexerWorker` Wikiデータ用。
...
1. 右上隅で、**管理者**を選択します。
1. **設定** > **検索**を選択します。
1. **高度な検索による検索**チェックボックスを選択します。
1. **変更を保存**を選択します。

### 高度検索によるコード検索を有効にする {#enable-code-search-with-advanced-search}

前提条件: 

- インスタンスへの管理者アクセス権が必要です。

GitLabで高度検索によるコード検索を有効にするには:

1. 右上隅で、**管理者**を選択します。
1. **設定** > **検索**を選択します。
1. **高度な検索によるコード検索**チェックボックスを選択します。
1. **変更を保存**を選択します。
...
| パラメータ                                                   | 説明 |
|-------------------------------------------------------------|-------------|
| **高度な検索のためのインデックス作成を有効にする**                    | インデックス作成を有効または無効にし、まだ存在しない場合は空のインデックスを作成します。例えば、インデックス作成を有効にしても検索を無効にして、インデックスが完全に完了する時間を与えることができます。また、このオプションは、既存のデータには影響を与えないことに加えて、データ変更を追跡して、新しいデータにインデックスが作成されるようにするバックグラウンドインデクサーのみを有効/無効にすることに注意してください。 |
| **高度な検索のためのインデックス作成を一時停止**                      | 高度検索のインデックス作成を一時停止します。これは、クラスターの移行/インデックス再作成に役立ちます。すべての変更は引き続き追跡されますが、再開されるまでインデックスにはコミットされません。 |
| **高度な検索による検索**                             | 検索および[高度な脆弱性管理](../../user/application_security/vulnerability_report/_index.md#advanced-vulnerability-management)における高度検索機能を有効または無効にします。 |
| **高度な検索によるコード検索**                        | 高度検索によるコード検索を有効または無効にします。この設定が無効になっている場合、すべてのコードはElasticsearchインスタンスから削除されます。この設定を再度有効にするには、コードを完全に再インデックスします。完全一致コードの検索が有効になっている場合は、リソースを節約するためにこの設定を無効にする必要があります。 |
...
| **インデックスが作成されるファイルの最大サイズ**                               | [インスタンス制限の説明](../../administration/instance_limits.md#maximum-file-size-indexed)を参照してください。 |
| **最大フィールド長**                                    | [インスタンス制限の説明](../../administration/instance_limits.md#maximum-field-length)を参照してください。 |
| **インデックス化のタイムアウト(分)**                              | プロジェクトごとのインデックス作成のタイムアウト (分)。 |
...
| **Index prefix**                                            | Elasticsearchインデックス名のカスタムプレフィックス。デフォルトは`gitlab`です。変更すると、すべてのインデックスは、`gitlab`の代わりにこのプレフィックスを使用します（たとえば、`custom-production-issues`の代わりに`gitlab-production-issues`）。1～100文字で、小文字の英数字、ハイフン、アンダースコアのみを含める必要があり、ハイフンまたはアンダースコアで開始または終了することはできません。[Introduced](https://gitlab.com/gitlab-org/gitlab/-/issues/3421) in GitLab 18.2. |

> [!warning] 
> 
> **最大一括リクエストサイズ (MiB)**と**バルクリクエストの並列実行**の値を増やすと、Sidekiqのパフォーマンスに悪影響を与える可能性があります。Sidekiqログで`scheduling_latency_s`の時間が増加している場合は、デフォルト値に戻してください。詳細については、[イシュー322147](https://gitlab.com/gitlab-org/gitlab/-/issues/322147)を参照してください。
...
> [!warning]
> 
> この設定を有効にした後でネームスペースまたはプロジェクトを指定しない場合、プロジェクトレコードのみがインデックス化され、関連データは検索できません。
...
1. 右上隅で、**管理者**を選択します。
1. **設定** > **検索**を選択します。
1. **高度な検索のためのインデックス作成を有効にする**と**高度な検索による検索**のチェックボックスをオフにします。
1. **変更を保存**を選択します。
...
1. 右上隅で、**管理者**を選択します。
1. **設定** > **検索**を選択します。
1. **高度な検索による検索**チェックボックスをオフにします。
1. **変更を保存**を選択します。

### 高度検索によるコード検索を無効にする {#disable-code-search-with-advanced-search}

前提条件: 

- インスタンスへの管理者アクセス権が必要です。

GitLabで高度検索によるコード検索を無効にするには:

1. 右上隅で、**管理者**を選択します。
1. **設定** > **検索**を選択します。
1. **高度な検索によるコード検索**チェックボックスをオフにします。
1. **変更を保存**を選択します。
...
この再インデックス方法の背後にある考え方は、[Elasticsearchの再インデックスAPI](https://www.elastic.co/guide/en/elasticsearch/reference/current/docs-reindex.html)とElasticsearchインデックスエイリアス機能を使用して操作を実行することです。インデックスエイリアスは、GitLabが読み取りと書き込みに使用する`primary`インデックスに接続します。再インデックス処理が開始されると、`primary`インデックスへの書き込みは一時的に一時停止されます。その後、別のインデックスが作成され、再インデックスAPIが実行されてインデックスデータが新しいインデックスに移行されます。再インデックスジョブが完了すると、インデックスエイリアスが新しいインデックスに切り替わり、それが新しい`primary`インデックスになります。最後に、書き込みが再開され、通常の操作が続行されます。
...
1. 右上隅で、**管理者**を選択します。
1. **設定** > **検索**を選択します。
1. **高度検索のゼロダウンタイムインデックス再作成**を展開する。
...
1. 右上隅で、**管理者**を選択します。
1. **設定** > **検索**を選択します。
1. **高度検索のゼロダウンタイムインデックス再作成**を展開する。次の設定を使用できます:
...
| [`sudo gitlab-rake gitlab:elastic:index`](https://gitlab.com/gitlab-org/gitlab/-/blob/master/ee/lib/tasks/gitlab/elastic.rake)                             | GitLab 17.0以前では、高度検索のインデックス作成を有効にし、`gitlab:elastic:recreate_index`、`gitlab:elastic:clear_index_status`、`gitlab:elastic:index_group_entities`、`gitlab:elastic:index_projects`、`gitlab:elastic:index_snippets`、および`gitlab:elastic:index_users`を実行します。<br>GitLab 17.1以降では、バックグラウンドでSidekiqジョブをキューに入れます。まず、ジョブは高度検索のインデックス作成を有効にし、すべてのインデックスが作成されるようにインデックス作成を一時停止します。次に、ジョブはすべてのインデックスを再作成し、インデックス作成状態をクリアし、追加のSidekiqジョブをキューに入れ、プロジェクトデータ、グループデータ、スニペット、ユーザーにインデックスを作成します。最後に、高度検索のインデックス作成が再開され、完了します。GitLab 17.1で`elastic_index_use_trigger_indexing`[フラグ](../../administration/feature_flags/_index.md)とともに[導入](https://gitlab.com/gitlab-org/gitlab/-/issues/421298)されました。デフォルトでは有効になっています。GitLab 17.3で[一般提供](https://gitlab.com/gitlab-org/gitlab/-/issues/434580)になりました。機能フラグ`elastic_index_use_trigger_indexing`は削除されました。 |
| [`sudo gitlab-rake gitlab:elastic:pause_indexing`](https://gitlab.com/gitlab-org/gitlab/-/blob/master/ee/lib/tasks/gitlab/elastic.rake)                    | 高度検索のインデックス作成を一時停止します。変更は引き続き追跡されます。クラスター/インデックスの移行に役立ちます。 |
| [`sudo gitlab-rake gitlab:elastic:resume_indexing`](https://gitlab.com/gitlab-org/gitlab/-/blob/master/ee/lib/tasks/gitlab/elastic.rake)                   | 高度検索のインデックス作成を再開します。 |
...
| [`sudo gitlab-rake gitlab:elastic:index_work_items`](https://gitlab.com/gitlab-org/gitlab/-/blob/master/ee/lib/tasks/gitlab/elastic.rake)                  | Elasticsearchが有効になっているグループから、すべての作業アイテムをインデックスします。 |
| [`sudo gitlab-rake gitlab:elastic:index_namespaces`](https://gitlab.com/gitlab-org/gitlab/-/blob/master/ee/lib/tasks/gitlab/elastic.rake)                  | すべてのルートネームスペースをインデックスします。 |
...
| [`sudo gitlab-rake gitlab:elastic:index_vulnerabilities`](https://gitlab.com/gitlab-org/gitlab/-/blob/master/ee/lib/tasks/gitlab/elastic.rake)             | すべての脆弱性をインデックスします。 |
...
| [`sudo gitlab-rake gitlab:elastic:index_groups_status`](https://gitlab.com/gitlab-org/gitlab/-/blob/master/ee/lib/tasks/gitlab/elastic.rake)               | すべてのグループリポジトリデータ（グループウィキ）の全体的なインデックス作成ステータスを決定します。ステータスは、インデックス化されたグループの数を総グループ数で割って100を掛けることで計算されます。このタスクには、エピック、マージリクエスト、またはマイルストーンなどの非リポジトリデータは含まれません。 |
...
| [`sudo gitlab-rake gitlab:elastic:groups_not_indexed`](https://gitlab.com/gitlab-org/gitlab/-/blob/master/ee/lib/tasks/gitlab/elastic.rake)                | リポジトリデータがインデックス化されていないグループを表示します。このタスクには、イシュー、マージリクエスト、マイルストーンなど、リポジトリ以外のデータは含まれていません。 |
...
> [!warning]
> 
> 大規模なインスタンスをインデックス作成すると、大量のSidekiqジョブが生成されます。[スケーラブルなセットアップ](../../administration/reference_architectures/_index.md)を用意するか、[追加のSidekiqプロセス](../../administration/sidekiq/extra_sidekiq_processes.md)を作成して、このタスクに備えてください。
>
> Geoのプライマリノードとセカンダリノードは両方とも同じElasticsearchクラスターを指します。しかし、Elasticsearchのインデックス作成ワーカーは、プライマリサイトのSidekiqノードでのみ実行されます。
>
> このため、プライマリサイトのSidekiqノードで[追加のSidekiqプロセス](../../administration/sidekiq/extra_sidekiq_processes.md)を設定する必要があります。
...
1. [**高度な検索のためのインデックス作成を有効にする**チェックボックスを選択](#enable-advanced-search)します。
...
1. プロジェクトとそれに関連付けられたデータにインデックスを作成します。
   ...
   これにより、インデックスを作成する必要がある各プロジェクトに対してSidekiqジョブがエンキューされます。インデックス作成ステータスをRakeタスクでクエリできます:
   ...
   > [!note]
   > 
   > プロジェクトのインデックス作成ジョブが`gitlab:elastic:index_projects`によってキューに入れられた後、中断されることがあります。これには多くの理由が考えられますが、インデックス作成タスクを再度実行することは常に安全です。

   また、`gitlab:elastic:clear_index_status` Rakeタスクを使用して、インデクサーにすべての進捗を「忘れ」させ、インデックス作成プロセスを最初から再試行させることもできます。

1. 作業アイテム、グループウィキ、パーソナルスニペット、およびユーザーはプロジェクトに関連付けられていないため、個別にインデックスする必要があります:
...
1. インデックス作成が完了したら、[**高度な検索による検索**チェックボックスを選択](#enable-advanced-search)します。
...
GitLabでは複数のSidekiqプロセスを開始できるため、一連のキュー（またはキューグループ）にインデックスを作成する専用の追加プロセスを作成できます。これにより、インデックス作成キューに常に専任のワーカーを配置し、残りのキューには別の専任のワーカーを配置して競合を回避できるようになります。
...
> [!note]
> 
> ルーティングルール (`sidekiq['routing_rules']`) は、すべてのGitLabノード（特にGitLab RailsおよびSidekiqノード）で同じである必要があります。

これを処理するには、次の2つのオプションのいずれかを選択できます:
...
> [!warning]
> 
> 複数のプロセスを開始する場合、プロセスの数はSidekiqに割り当てるCPUコアの数を超えることはできません。各Sidekiqプロセスは、利用可能なワークロードと並行処理設定に応じて、1つのCPUコアのみを使用できます。詳細については、[複数のSidekiqプロセスを実行する](../../administration/sidekiq/extra_sidekiq_processes.md)方法を参照してください。
...
> [!note] 
> 
> GitLabを再設定した後、すぐにRakeタスクを実行することが重要です。GitLabを再設定した後、Rakeタスクがジョブの移行を開始するまで、既存のジョブは処理されません。
...
通常、Elasticsearchにデフォルトの設定でスペースを自動的にマージおよび再利用させるべきです。[Luceneの削除済みの処理](https://www.elastic.co/blog/lucenes-handling-of-deleted-documents "ドキュメント")」では、_「全体として、おそらく最大セグメントサイズを縮小することに加えて、Luceneのをそのままにして、削除がいつ再利用されるかをあまり気にする必要はありません。_
...
- また、削除がどれだけ積極的にターゲットにされるかを制御する`index.merge.policy.reclaim_deletes_weight`を調整することもできます。しかし、これは高コストなマージ決定につながる可能性があるため、トレードオフを理解していない限り、これを変更すべきではありません。
