...
- [ユーザー所有のアプリケーション](#create-a-user-owned-application)。
- [グループ所有のアプリケーション](#create-a-group-owned-application)。
- [インスタンス全体のアプリケーション](#create-an-instance-wide-application)。
...
- 関連付けられたグループにSAMLが設定されている場合、ユーザーが[SAML SSO](../user/group/saml_sso/_index.md)を使用してアプリケーションにサインインできるようにします。
...
1. **アプリケーションを保存**を選択します。GitLabは以下を提供します。
   ...
   - **シークレットを更新**機能。この機能を使用すると、このアプリケーションの新しいシークレットを生成し、コピーできます。シークレットを更新すると、認証情報が更新されるまで、既存のアプリケーションは機能しなくなります。
...
| `read_user`              | `/user` APIエンドポイントを介して、認証済みユーザーのプロファイルへの読み取り専用アクセスを許可します。これには、ユーザー名、公開メール、および氏名が含まれます。`/users`の下の読み取り専用APIエンドポイントへのアクセスも許可します。 |
| `create_runner`          | Runnerを作成するアクセスを許可します。 |
| `manage_runner`          | Runnerを管理するアクセスを許可します。 |
...
| `write_registry`         | プライベートプロジェクト上のコンテナレジストリイメージへの書き込みアクセスを許可します。イメージをプッシュするには、読み取りアクセスと書き込みアクセスの両方が必要です。 |
| `read_virtual_registry`  | プライベートプロジェクトおよび仮想レジストリ内の依存プロキシを介したコンテナイメージへの読み取り専用アクセスを許可します。 |
| `write_virtual_registry` | プライベートプロジェクト内の依存プロキシを介したコンテナイメージへの読み取り、書き込み、および削除アクセスを許可します。 |
| `read_observability`     | GitLab可観測性への読み取り専用アクセスを許可します。 |
| `write_observability`    | GitLab可観測性への書き込みアクセスを許可します。 |
| `ai_features`            | GitLab Duo関連のAPIエンドポイントへのアクセスを許可します。 |
| `sudo`                   | 管理者ユーザーとして認証されている場合に、システム内の任意のユーザーとしてAPIアクションを実行する権限を許可します。 |
| `admin_mode`             | 管理者モードが有効になっている場合、管理者としてAPIアクションを実行する権限を許可します |
| `read_service_ping`      | 管理者ユーザーとして認証された場合に、APIを介してService Pingペイロードをダウンロードするアクセスを許可します。 |
