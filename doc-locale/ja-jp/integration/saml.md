...
description: エンタープライズ認証をSAMLインテグレーションによるシングルサインオンアクセスで設定します。
---
...
> [!note]
> 
> GitLab.comについては、[グループ向けSAML SSO](../user/group/saml_sso/_index.md)を参照してください。
...
> [!note]
> 
> 複数のSAML IdPを設定する場合、SAML Group Linksが機能するように、すべてのSAML IdPでSAMLレスポンスにグループ属性が含まれるように設定する必要があります。詳細については、[SAMLグループのリンク](../user/group/saml_sso/group_sync.md)を参照してください。
...
> [!note]
> 
> `groups_attribute`で指定された属性が正しくないか、または欠落している場合、ユーザーは標準ユーザーとしてアクセスします。
...
- GitLab 18.3で[有効](https://gitlab.com/gitlab-org/gitlab/-/work_items/553931)になりました。
...
> [!note]
> 
> 自動サインイン設定をバイパスするには、サインインURLに`?auto_sign_in=false`を追記します。例: `https://gitlab.example.com/users/sign_in?auto_sign_in=false`。
...
> [!note]
> 
> このインテグレーションは、アサーションの暗号化とリクエストの署名の両方に`certificate`と`private_key`の設定を使用します。
