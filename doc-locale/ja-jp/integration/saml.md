...
description: SAMLインテグレーションと認証を利用して、エンタープライズのSSOアクセスを設定します。
---
...
- 一般的に使用される用語については、[用語集](../auth/auth_glossary.md)を参照してください。
...
1. GitLabで[HTTPSが設定されている](https://docs.gitlab.com/charts/installation/tls/)ことを確認してください。
1. [共通設定](omniauth.md#configure-common-settings)で、`saml`をシングルサインオンプロバイダーとして追加します。これにより、既存のGitLabアカウントを持たないユーザーに対して、Just-In-Timeアカウントプロビジョニングが有効になります。
1. Helm値をエクスポートします。
...
1. 次の属性を設定し、SAMLユーザーがこれらを変更できないようにします。
   ...
   ユーザーがこれらの属性を変更できる場合、他の認証済みユーザーとしてサインインできてしまいます。これらの属性を変更不可にする方法については、SAML IdPのドキュメントを参照してください。
1. 次の内容を`saml.yaml`ファイルに記述し、[Kubernetes Secret](https://docs.gitlab.com/charts/charts/globals/#providers)として使用します。
...
1. IdPに設定情報を提供するには、アプリケーションのメタデータURLを作成します。GitLabのメタデータURLを作成するには、GitLabインストールのHTTPS URLに`users/auth/saml/metadata`を付加します。例: 
...
> [!note]
> 複数のSAML IdPを設定する場合、SAML Group Linksが機能するように、すべてのSAML IdPをSAMLレスポンスにグループ属性を含めるように設定する必要があります。詳細については、[SAMLグループのリンク](../user/group/saml_sso/group_sync.md)を参照してください。
...
1. 次の内容を`saml.yaml`ファイルに記述し、最初のSAMLプロバイダーの[Kubernetes Secret](https://docs.gitlab.com/charts/charts/globals/#providers)として使用します。
...
1. 次の内容を`saml_2.yaml`ファイルに記述し、2番目のSAMLプロバイダーの[Kubernetes Secret](https://docs.gitlab.com/charts/charts/globals/#providers)として使用します。
...
1. **Identity Provider Single Sign-On URL**をメモします。このURLは、GitLab設定ファイルの`idp_sso_target_url`に使用します。
...
GitLabは、SAMLサインインのたびにこれらのグループをチェックし、必要に応じてユーザー属性を更新します。この機能では、GitLab [グループ](../user/group/_index.md)にユーザーを自動的に追加することはできません。
...
- グループ情報がどこにあるかを、GitLabに指示する必要があります。そのためには、IdPサーバーが標準のSAML応答とともに特定の`AttributeStatement`を送信するように設定してください。例: 
...
> [!note]
> `groups_attribute`で指定された属性が間違っているか、存在しない場合、ユーザーは標準ユーザーとしてアクセスします。
...
- GitLab 18.3で[有効化](https://gitlab.com/gitlab-org/gitlab/-/work_items/553931)されました。
...
1. IdPが`AuthnContext`を返すようになっていることを確認してください。例: 
...
> [!note]
> 自動サインイン設定をバイパスするには、サインインURLに`?auto_sign_in=false`を追加します。例: `https://gitlab.example.com/users/sign_in?auto_sign_in=false`。
...
> [!note]
> この設定は、OmniAuth `info`ハッシュスキーマの一部である属性をマップするためにのみ使用してください。
...
1. [目的の属性を同期するようにOmniAuthを設定](omniauth.md#keep-omniauth-user-profiles-up-to-date)します。
1. 次のYAMLの内容を`saml.yaml`ファイルに記述して保存し、[Kubernetes Secret](https://docs.gitlab.com/charts/charts/globals/#providers)として使用します。
...
> [!note]
> このインテグレーションは、アサーションの暗号化とリクエストの署名の両方に`certificate`および`private_key`の設定を使用します。
...
1. 設定の`security`セクションで署名に関する設定を行います。例: 
...
> [!note]
> SAMLリダイレクトバインディングは、SAML POSTバインディングとは異なります。POSTバインディングでは、仲介者によってリクエストが改ざんされるのを防ぐために署名が必須となります。

## SAMLを通じて作成されたユーザーのパスワードを生成する {#password-generation-for-users-created-through-saml}

GitLabは、[SAMLを通じて作成されたユーザーに対して、パスワードを生成して設定](../user/profile/user_passwords.md)します。
...
1. GitLabで[HTTPSが設定されている](https://docs.gitlab.com/charts/installation/tls/)ことを確認してください。
1. 次の内容を`group_saml.yaml`ファイルに記述し、[Kubernetes Secret](https://docs.gitlab.com/charts/charts/globals/#providers)として使用します。
