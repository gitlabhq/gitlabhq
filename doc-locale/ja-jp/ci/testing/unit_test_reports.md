...
description: ジョブログを検索することなく、単体テストの結果を表示し、デバッグします。
title: 単体テストレポート
---
...
GitLabは、JUnit XMLの要素と属性のサブセットを解析し、UIにテスト結果を表示します。

| XML要素  | XML属性   | 説明 |
| ------------ | --------------- | ----------- |
| `testsuites` | `time`          | すべてのテストスイートの合計実行時間。テスト実行時間の計算に使用されます。 |
| `testsuite`  | `name`          | テストスイート名。内部グループ化のために解析されます。 |
| `testsuite`  | `time`          | 個々のテストスイートの実行時間。テスト実行時間の計算に使用されます。 |
| `testcase`   | `classname`     | テストクラス名またはカテゴリ名。UIにスイート名として表示されます。 |
| `testcase`   | `name`          | 個々のテスト名。 |
| `testcase`   | `file`          | テストが定義されているファイルパス。 |
| `testcase`   | `time`          | テスト実行時間（秒）。 |
| `failure`    | 要素の内容 | 失敗メッセージとスタックトレース。 |
| `error`      | 要素の内容 | エラーメッセージとスタックトレース。 |
| `skipped`    | 要素の内容 | テストをスキップする理由。 |
| `system-out` | 要素の内容 | システム出力とアタッチメントタグ。`testcase`要素からのみ解析されます。 |
| `system-err` | 要素の内容 | システムエラー出力。`testcase`要素からのみ解析されます。 |

次の要素と属性は解析されません:

- `testsuite`属性（テスト、失敗、エラー、タイムスタンプ）
- `testcase`属性（アサーション、行、ステータス）
- `properties`要素
- `testsuite`レベルでの`system-out`および`system-err`
...
1. XMLテストレポートファイルのパスを指定します。`junit`プロパティは以下を受け入れます:

   - 単一のファイル名: `junit: report.xml`
   - ファイル名パターン: `junit: test-results/**/*.xml`
   - ファイル名の配列: `junit: [rspec-1.xml, rspec-2.xml, rspec-3.xml]`
   - 両方の組み合わせ: `junit: [rspec.xml, test-results/TEST-*.xml]`

   ディレクトリはサポートされていません（例: `junit: test-results`または`junit: test-results/**`）。
...
パイプラインの詳細で、子パイプラインからの結果を含む、すべてのテストスイートとケースを表示します。
...
[パイプラインAPI](../../api/pipelines.md#retrieve-a-test-report-for-a-pipeline)を使用してテストレポートを取得することもできます。
