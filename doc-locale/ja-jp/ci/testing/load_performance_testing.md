...
> [!note]大規模なk6テストの場合、実際のテストを実行するGitLab Runnerインスタンスがテストの実行を処理できることを確認する必要があります。仕様の詳細については、[k6のガイダンス](https://k6.io/docs/testing-guides/running-large-tests#hardware-considerations)を参照してください。The [デフォルトの共有GitLab.com Runner](../runners/hosted_runners/linux.md)は、ほとんどの大規模なk6テストを処理するには仕様が不十分である可能性があります。
