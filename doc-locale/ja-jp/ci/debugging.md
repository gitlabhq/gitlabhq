...
[`workflow:name`](yaml/_index.md#workflowname)を使用してすべてのパイプラインタイプに名前を付けると、パイプラインリストでパイプラインを簡単に識別できます。例: 
...
標準のパイプライン実行では使用されないが、オンデマンドでデバッグに使用できるCI/CD変数を定義できます。次の例のように変数を追加すると、[パイプライン](pipelines/_index.md#run-a-pipeline-manually)または[個々のジョブ](jobs/job_control.md#run-a-manual-job)の手動実行中に変数を追加して、コマンドの動作を変更できます。例: 
...
ジョブで正しいバージョンの依存関係が使用されていることを検証するには、メインスクリプトコマンドを実行する前に依存関係を出力します。例: 
...
依存関係やイメージの最新バージョンを常に使用したいと思うかもしれませんが、アップデートには予期しない破壊的な変更が含まれる可能性があります。予期しない変更を避けるために、主要な依存関係とイメージを固定することを検討してください。例: 
...
> [!warning]ト
> 
> ークン、パスワード、またはその他の機密情報は、パイプラインにアクセスできるユーザーから表示される可能性があるため、アーティファクトに保存しないでください。
...
### エラー: `Identity verification is required in order to run CI jobs` {#error-identity-verification-is-required-in-order-to-run-ci-jobs}
...
GitLab.comでFreeプランのGitLabホスト型Runnerを使用している場合、`Identity verification is required in order to run CI jobs`というエラーメッセージが表示されたら、本人確認を完了する必要があります。

この要件は、Freeのコンピューティングリソースの悪用を防ぐのに役立ちます。リスクスコアに応じて、メール、電話番号を確認するか、支払い方法を追加する必要がある場合があります。詳細については、[本人確認](../security/identity_verification.md)を参照してください。

検証を完了するには:

1. アラートバナーで、**アカウントを認証**を選択します。
1. 促されたら、本人確認の手順に従ってください。電話番号を確認するか、支払い方法を追加する必要がある場合があります。
1. 新しいコミットを作成するか、手動で新しいパイプラインをトリガーしてください。

あるいは、次のこともできます:

- 有料プランにアップグレードします。
- ネームスペースに追加のコンピューティング時間を購入します。
- GitLabホスト型Runnerの代わりに、プロジェクトまたはグループRunnerを使用します。
- あなたのオーナーに、セルフマネージドRunnerを設定するように依頼してください。
...
- マージリクエストページを更新します。
- マージリクエストを閉じて再度開きます。
- マージリクエストを[`/rebase`クイックアクション](../user/project/quick_actions.md#rebase)でリベースします。
...
- パイプラインを実行しているユーザーが、含まれているファイルを含む[プロジェクトのメンバー](../user/project/members/_index.md#add-users-to-a-project)である。ユーザーは、同じプロジェクトでCI/CDジョブを実行するための[権限](../user/permissions.md#project-cicd)も必要です。
...
配列内で複数の[`!reference`タグ](yaml/yaml_optimization.md#reference-tags)を使用すると、次のようなエラーが表示されることがあります:

```plaintext
This GitLab CI configuration is invalid: jobs:my_job_name:parallel:matrix config should be an array of hashes.
```

`script`、`rules`、および`stages`キーワードは複数の参照タグの使用をサポートしていますが、配列を期待する他のキーワードはサポートしていません。[この制限を回避するためにネストを使用する](https://gitlab.com/gitlab-org/gitlab/-/issues/439828#note_1918858137)か、代わりに[YAMLアンカー](yaml/yaml_optimization.md#anchors)を使用できます。
