...
1. 右上隅で、**管理者**を選択します。
1. 左サイドバーで、**CI/CD** > **Runners**を選択します。
1. **インスタンスRunnerを作成**を選択します。
...
> [!note]
> Runner認証トークンは、登録時、限られた期間のみUIに表示されます。Runnerを登録すると、認証トークンは`config.toml`に保存されます。
...
1. 登録トークンをコピーします。
1. [Runnerを登録します](https://docs.gitlab.com/runner/register/#register-with-a-runner-registration-token-legacy)。
...
1. 最後に、残っている唯一のジョブであるため、ジョブ3が次になります。
...
または、登録トークンをコピーし、[Runnerを登録](https://docs.gitlab.com/runner/register/#register-with-a-runner-registration-token-legacy)する方法のドキュメントに従うこともできます。
...
7日間以上非アクティブなグループRunnerをクリーンアップできます。
...
- 他のRunnerから分離することでメリットが得られる、多くのCIアクティビティがあるプロジェクトを保持している。
...
> [!note]
> プロジェクトRunnerは、フォークしたプロジェクトで自動的にインスタンスを取得しません。フォークは、複製されたリポジトリのCI/CD設定をコピーします。
...
1. 左サイドバーで、**設定** > **CI/CD**を選択します。
1. **Runner**を展開します。
1. **割り当てられたプロジェクトのRunner**セクションで、Runnerを見つけます。
...
- インスタンスRunnerを表示するには、管理者アクセスが必要です。
- グループRunnerを表示するには、メンテナーまたはオーナーロールが必要です。
...
1. プロジェクトの**設定** > **CI/CD**に移動し、**Runners**セクションを展開します。
...
- GitLab 18.2で[グループとプロジェクト向けに導入](https://gitlab.com/gitlab-org/gitlab/-/issues/422621)されました。
