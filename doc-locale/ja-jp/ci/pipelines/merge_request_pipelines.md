...
マージリクエストのソースブランチに変更を加えるたびに実行されるよう、パイプラインを設定できます。この種のパイプラインは、マージリクエストパイプラインと呼ばれます。

これらのパイプラインは、以下の場合に実行されます:
...
- ソースブランチのコンテンツのみで実行され、ターゲットブランチのコンテンツは無視されます。
- `merge request`ラベルをパイプラインリストに表示します。
...
- プロジェクトの`.gitlab-ci.yml`ファイルには、`CI_PIPELINE_SOURCE == "merge_request_event"`に一致するジョブルールまたはワークフロールールを含める必要があります。
- マージリクエストパイプラインを実行するには、ソースプロジェクトでデベロッパー、メンテナー、またはオーナーロールが必要です。
- リポジトリは、[外部リポジトリ](../ci_cd_for_external_repos/_index.md)ではなく、GitLabリポジトリである必要があります。

## マージリクエストパイプラインを構成する {#configure-merge-request-pipelines}

マージリクエストパイプラインを構成するには、`.gitlab-ci.yml`ファイル内のジョブを`CI_PIPELINE_SOURCE`が`merge_request_event`と等しい場合に実行するように構成する必要があります。

> [!note] 
> 
> `include:`で定義されたルール（たとえば`include:component`を使用した場合など）は、この要件を満たしません。`rules:`または`workflow: rules`に一致するものを`.gitlab-ci.yml`に直接定義する必要があります。

個々のジョブを`rules`で構成するか、`workflow: rules`を使用してパイプライン全体を制御できます。

### 個々のジョブを構成する {#configure-individual-jobs}

[`rules`](../yaml/_index.md#rules)キーワードを使用して、個々のジョブがマージリクエストパイプラインで実行するように構成します。例: 
...
ファイルの変更に基づいてジョブがいつ実行するかを制御することもできます:
...
### パイプライン全体を構成する {#configure-the-entire-pipeline}

[`workflow: rules`](../yaml/_index.md#workflowrules)キーワードを使用して、パイプライン内のすべてのジョブがマージリクエストパイプラインで実行するように構成します。例: 
...
> [!warning]
> 
> フォークマージリクエストには、パイプラインの実行時、マージ前であっても、親プロジェクトのシークレットを盗もうとする悪意のあるコードが含まれている可能性があります。レビュアーは、パイプラインをトリガーする前に、マージリクエストの変更を慎重に確認してください。APIまたは[`/rebase`クイックアクション](../../user/project/quick_actions.md#rebase)でパイプラインをトリガーした場合を除き、GitLabは警告を表示し、ユーザーはパイプラインを実行する前に承認する必要があります。それ以外の場合、**警告は表示されません**。
...
> [!warning]
> 
> 設定が無効になる前に作成されたパイプラインは影響を受けず、実行を継続します。古いパイプラインでジョブを再実行すると、ジョブはパイプラインの初回作成時と同じコンテキストを使用します。
...
マージリクエストパイプラインは、以下の場合にのみこれらの保護されたリソースにアクセスできます:

- ソースブランチとターゲットブランチの両方が[保護されている](../../user/project/repository/branches/protected.md)場合。
- パイプラインをトリガーするユーザーが、ターゲットブランチへのプッシュ/マージアクセス権を持っている場合。
- ソースブランチとターゲットブランチの両方が同じプロジェクトに属している場合。

フォークしたリポジトリからのマージリクエストパイプラインは、これらの保護されたリソースにアクセスできません。

前提条件: 

- プロジェクトでメンテナーまたはオーナーロールを持っている。
...
1. **設定** > **CI/CD**を選択します。
1. **変数**を展開します。
1. **マージリクエストパイプライン内で保護されたリソースへのアクセス**で、**マージリクエストパイプラインが保護された変数やRunnerにアクセスすることを許可する**チェックボックスをオンまたはオフにします。
