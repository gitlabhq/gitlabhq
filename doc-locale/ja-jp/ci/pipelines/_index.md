...
VS Codeを使用してGitLab CI/CD設定を編集する場合、[VS Code用GitLab拡張機能](../../editor_extensions/visual_studio_code/_index.md)は[設定を検証する](../../editor_extensions/visual_studio_code/cicd.md#test-gitlab-cicd-configuration)のに役立ち、[パイプラインステータスを表示する](../../editor_extensions/visual_studio_code/cicd.md#view-pipeline-information)ことができます。
...
> [!warning]
> この設定をオンにすると、デベロッパーロールを持つユーザーは、手動パイプライン実行からの変数値で、機密情報を含む可能性のあるものを表示できます。認証情報やトークンなどの機密データについては、マニュアルパイプライン変数ではなく、[保護された変数](../variables/_index.md#protect-a-cicd-variable)または[外部シークレット管理](../secrets/_index.md)を使用してください。
...
変数の値は、デフォルトでマスクされます。デベロッパー、メンテナー、またはオーナーロールをお持ちの場合、目のアイコンを選択して値を表示できます。
...
> [!note]
> [既知のイシュー](https://gitlab.com/gitlab-org/gitlab/-/issues/382857)のため、[コンプライアンスパイプライン](../../user/compliance/compliance_pipelines.md)を使用するプロジェクトでは、パイプラインを手動で実行する際に事前入力変数が表示されない場合があります。この問題を回避するには、[コンプライアンスパイプライン設定を変更](../../user/compliance/compliance_pipelines.md#prefilled-variables-are-not-shown)します。
...
クエリ文字列を使用して、**パイプラインを新規作成**ページに自動入力できます。たとえば、クエリ文字列が`.../pipelines/new?ref=my_branch&var[foo]=bar&file_var[file_foo]=file_bar`なら、**パイプラインを新規作成**ページに次の内容が自動入力されます。
...
- ジョブもステージも含まれない空のパイプラインが、GitLabに作成されます。そのパイプラインはUIに表示され、API応答で返されることがあります。
- パイプラインステータスはUIでは**スキップ済み**になり、APIでは`skipped`になります。

> [!note]
> パイプライン実行ポリシーとスキャン実行ポリシーは、`[skip ci]`ディレクティブを制限または無効にすることができます。詳細については、以下を参照してください: 
>
> - パイプライン実行ポリシーにおける[`skip_ci`タイプ](../../user/application_security/policies/pipeline_execution_policies.md#skip_ci-type)。
> - スキャン実行ポリシーにおける[`skip_ci`タイプ](../../user/application_security/policies/scan_execution_policies.md#skip_ci-type)。
...
> [!warning]
> パイプラインを削除すると、すべてのパイプラインキャッシュの有効期限が切れ、ジョブ、ジョブログ、アーティファクト、トリガーなど、直接関連するすべてのオブジェクトが削除されます。**この操作は元に戻すことができません。**
...
保護された変数とRunnerへのアクセスが[マージリクエストパイプラインのコンテキスト](merge_request_pipelines.md#control-access-to-protected-variables-and-runners)でどのように機能するかをレビューします。

パイプラインを保護するための追加のセキュリティに関する推奨事項については、[デプロイの安全性](../environments/deployment_safety.md)ページを確認してください。

## アップストリームプロジェクトが再ビルドされたときにパイプラインをトリガーする {#trigger-a-pipeline-when-an-upstream-project-is-rebuilt}
...
代替案として、[CI/CDジョブでパイプライントリガートークン](../triggers/_index.md#use-a-cicd-job)を使用して、別のパイプラインが実行されたときにパイプラインをトリガーできます。この方法は、パイプラインサブスクリプションよりも信頼性が高く柔軟であり、推奨されるアプローチです。
...
Aがリトライされるため、それは無視され、ジョブA'のみがカウントされます。B、A'、およびCの結合は(1, 4)および(6, 7)です。したがって、合計実行時間は次のようになります。
...
右上のドロップダウンリストで**パイプラインID**を選択すると、パイプラインID（インスタンス全体で一意のID）が表示されます。**パイプラインIID**を選択すると、パイプラインIID（内部ID、プロジェクト内でのみ一意）が表示されます。
...
[`needs`](../yaml/_index.md#needs)キーワードを使用してジョブを設定する場合、パイプライン詳細ページでジョブをグループ化する方法は2通りあります。ステージ設定でジョブをグループ化するには、**ジョブをグループ化**セクションで**ステージ**を選択します。
...
## パイプラインの成功と実行時間チャート {#pipeline-success-and-duration-charts}
...
ユーザーが[GitLab.comアカウントを削除](../../user/profile/account/delete_account.md#delete-your-own-account)しても、削除は7日間行われません。この期間中、そのユーザーによって作成されたすべてのパイプラインサブスクリプションは、ユーザーの元の権限で実行され続けます。不正なパイプライン実行を防ぐため、削除されたユーザーのパイプラインサブスクリプション設定は、すぐに更新してください。

### 事前入力変数は**New Pipeline**ページに表示されません {#pre-filled-variables-do-not-show-up-in-new-pipeline-page}

パイプラインの定義済み変数が[別のファイルで定義されている](../yaml/includes.md)場合、**New Pipeline**ページに表示されないことがあります。別のファイルにアクセスする権限がないと、定義済み変数を表示できません。
