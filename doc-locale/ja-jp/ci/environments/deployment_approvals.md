...
- [ユーザービリティの問題](https://gitlab.com/gitlab-org/gitlab/-/issues/391258)により、GitLab 16.2で自動承認は[削除](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/124638)されました。
...
1. **設定** > **CI/CD**を選択します。
1. **保護された環境**を展開します。
1. **承認のオプション**で、**Allow pipeline triggerer to approve deployment**（パイプラインのトリガー元にデプロイの承認を許可する）チェックボックスをオンにします。
...
1. **操作** > **環境**を選択します。
1. 環境名を選択します。
1. デプロイを見つけて、その**Status badge**（ステータスバッジ）を選択します。
1. （オプション）デプロイの承認または却下の理由を説明するコメントを追加します。
1. **承認する**または**拒否**を選択します。
