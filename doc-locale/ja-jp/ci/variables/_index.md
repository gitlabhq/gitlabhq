...
> [!warning] CI/CD変数をマスクすることは、悪意のあるユーザーが変数値にアクセスするのを防ぐための確実な方法ではありません。機密情報のセキュリティを確保するため、[外部シークレット](../secrets/_index.md)と[ファイルタイプの変数](#use-file-type-cicd-variables)を使用して、`env`や`printenv`などのコマンドがシークレット変数を出力するのを防ぐことを検討してください。
