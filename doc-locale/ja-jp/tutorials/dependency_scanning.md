...
description: 依存関係スキャンを設定し、プロジェクトの依存関係の脆弱性を検出し、実践的なステップごとのガイダンスを通じてそれらを解決する方法を学びます。
title: 'チュートリアル: 依存関係スキャンをセットアップする'
---
...
依存関係スキャンは、アプリケーションの開発およびテスト中に、ソフトウェア依存関係のセキュリティ脆弱性を自動的に検出できます。たとえば、依存関係スキャンを使用すると、アプリケーションが脆弱であることがわかっている外部（オープンソース）ライブラリを使用しているかどうかを知ることができます。その後、アプリケーションを保護するための措置を講じることができます。

このチュートリアルでは、サンプルとなる脆弱なアプリケーションを作成する方法を説明します:

- アプリケーションの依存関係にある脆弱性を検出、トリアージ、解決する方法。
- マージリクエストで脆弱性を検出する方法。

依存関係スキャンを設定するには、以下を実行します:

- [アプリケーションのサンプルファイルを作成する](#create-example-application-files)。
- [脆弱性のトリアージを行います](#triage-the-vulnerabilities)。
- [重大度の高い脆弱性を解決します](#resolve-the-high-severity-vulnerability)。
- [マージリクエストで脆弱性の検出をテストする](#test-vulnerability-detection-in-a-merge-request)。

## はじめる前 {#before-you-begin}

Gitpodが有効になっていることを確認してください。Gitpodは、オンデマンドのクラウド開発環境です。詳細については、[Gitpod](../integration/gitpod.md)を参照してください。または、独自の開発環境を使用することもできます。この場合、YarnとNode.jsがインストールされている必要があります。

## アプリケーションのサンプルファイルを作成する {#create-example-application-files}

まず、新しいプロジェクトで、パイプラインを構成するためのファイルを作成し、脆弱性をスキャンできる依存関係を追加します。

1. デフォルト値を使用して、空白のプロジェクトを作成します。

1. 次のファイルを`main`ブランチに作成します。
   ...
   [Yarnロックファイル](#yarn-lock-file-content)セクションに示されているコンテンツを使用します。

1. **ビルド** > **パイプライン**に移動し、最新のパイプラインが正常に完了したことを確認します。

パイプラインでは、依存関係スキャンが実行され、脆弱性が自動的に検出されます。

## 脆弱性のトリアージを行います {#triage-the-vulnerabilities}

脆弱性レポートには、脆弱性に関する重要な情報が記載されています。通常は、組織のポリシーに従って脆弱性をトリアージします。このチュートリアルでは、中程度の重大度の脆弱性を無視し、重大度の高い脆弱性のみを確認します。

脆弱性をトリアージするには、次の手順に従います:

1. **セキュリティ** > **脆弱性レポート**に移動します。
1. 各行のチェックボックスをオンにして、中程度の重大度の脆弱性をそれぞれ選択します。
1. **ステータスの設定**ドロップダウンリストから**やめる**を選択します。**Dismissal reason**（却下理由）ドロップダウンリストから**テストで使用**を選択し、「テストで使用」というコメントを追加して、**ステータスの変更**を選択します。

   中程度の重大度の脆弱性はビューから除外され、重大度の高い脆弱性のみが残ります。

1. **高い**脆弱性の説明を選択します。

   推奨される解決策は、`fastify`パッケージをアップグレードすることです。通常はこれについてさらに調査しますが、このチュートリアルでは、この脆弱性が確認されたと見なすことができます。

1. **ステータス**ドロップダウンリストから**確認**を選択し、**ステータスの変更**を選択します。

## 重大度の高い脆弱性を解決します {#resolve-the-high-severity-vulnerability}

重大度の高い脆弱性のみが解決されるのを待っています。トリアージの手順から、`fastify`パッケージをアップグレードする必要があることがわかります。

脆弱性を修正するには、次の手順に従います:

1. 左側のサイドバーで、**検索または移動先**を選択して、プロジェクトを見つけます。
1. 右上にある**コード** > **Gitpod**を選択し、新しいタブでGitpodを開きます。
1. プロンプトが表示されたら、**Continue with GitLab**（GitLabで続行）を選択し、**許可する**を選択します。
1. **New Workspace**（新しいワークスペース）ページで、**次に進む**を選択します。
1. **ターミナル**ペインで、次のコマンドを入力して新しいブランチを作成します。

   ```plaintext
   git checkout -b update_packages main
   ```

1. **ターミナル**ペインで、コマンド`yarn upgrade --latest`を実行します。これにより、プロジェクトの依存関係と`yarn.lock`ファイルが更新されます。
1. **ターミナル**ペインで、次のコマンドを実行して変更をコミットします。これにより、CI/CDパイプラインがトリガーされます。
...
1. GitLabブラウザータブに切り替えます。
1. **コード** > **マージリクエスト**に移動し、**マージリクエストを作成**を選択します。
1. **新しいマージリクエスト**ページで、一番下までスクロールし、**マージリクエストを作成**を選択します。マージリクエストパイプラインが完了するまで待ちます。
1. ページを更新し、**マージ**を選択します。
1. パイプラインが正常に完了するまで待ちます。
1. **セキュリティ** > **脆弱性レポート**に移動します。
1. **高**い脆弱性の説明を選択します。

   バナーは、`main`ブランチで脆弱性が解決されたことを確認します。通常は、`fastify`パッケージのバージョンが`yarn.lock`ファイルに指定されていることを確認して、手動で確認します。このチュートリアルでは、確認ステップを省略できます。

1. **ステータス**ドロップダウンリストで**解決する**を選択し、**ステータスの変更**を選択します。
1. **セキュリティ** > **脆弱性レポート**に移動します。

   これで、脆弱性レポートに脆弱性がリストされなくなります。

## マージリクエストで脆弱性の検出をテストする {#test-vulnerability-detection-in-a-merge-request}

脆弱性をトリアージして解決する方法がわかりました。次に、マージリクエストで新しい潜在的な脆弱性を検出する方法を確認するために、脆弱性を持つことがわかっている依存関係を追加します。

新しい脆弱性を追加するには、次の手順に従います:

1. Gitpodタブに切り替えます。タイムアウトした場合は、**Open Workspace**（ワークスペースを開く）を選択します。
1. **ターミナル**ペインで、次のコマンドを実行してローカルの`main`ブランチを更新します:
...
1. **ターミナル**ペインで、次のコマンドを実行して新しいブランチを作成します:

   ```plaintext
   git checkout -b add_dependency main
   ```

1. ファイルエクスプローラーのサイドバーで、`package.json`ファイルを選択します。
1. `package.json`ファイルの`dependencies`セクションに次の行を追加します。

   ```json
   "axios": "0.21.0",
   ```

1. **ターミナル**ペインで、次のコマンドを実行して、`package.json`ファイルに追加された依存関係をインストールします。

   ```plaintext
   yarn install
   ```

1. **ターミナル**ペインで、次のコマンドを実行して変更をコミットします。これにより、GitLabでCI/CDパイプラインがトリガーされます。
...
マージリクエストパイプラインが完了するまで待ってから、ページを更新します。マージリクエストセキュリティウィジェットは、新しい潜在的な脆弱性について警告します。これらの脆弱性は、`add_dependency`ブランチにのみ存在し、`main`ブランチには存在しません。

これで、次の方法がわかりました:

- アプリケーションの依存関係にある脆弱性を検出します。
- 脆弱性をトリアージして解決します。
- マージリクエストで新しい脆弱性を検出します。

### Yarn lockfileのコンテンツ {#yarn-lock-file-content}
