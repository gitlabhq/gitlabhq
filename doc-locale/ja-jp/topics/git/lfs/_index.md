...
- インスタンスでGit LFSを構成する方法（リモートオブジェクトストレージの設定など）については、[Git Large File Storage（LFS）の管理](../../../administration/lfs/_index.md)を参照してください。
...
Git LFSで追跡されているファイルを削除するには、[ファイルを削除](../undo.md#remove-a-file-from-a-repository)を参照してください。

ファイルの過去および現在のすべての履歴を完全に消去するには、[機密情報を扱う](../undo.md#handle-sensitive-information)を参照してください。

> [!warning]
> 
> ファイル履歴の消去には、Git履歴の書き換えが必要です。このアクションは破壊的であり、元に戻すことはできません。

## 大きなファイルを削除した後にリポジトリのサイズを縮小する {#reduce-repository-size-after-removing-large-files}

リポジトリの履歴から大きなファイルを削除して、リポジトリの合計サイズを削減する必要がある場合は、[リポジトリサイズを削減](../../../user/project/repository/repository_size.md#methods-to-reduce-repository-size)を参照してください。

## 関連トピック {#related-topics}

- Git LFSを使用して[排他的なファイルのロック](../file_management.md#configure-file-locks)を設定します。
- ブログ投稿: [Git LFS入門](https://about.gitlab.com/blog/getting-started-with-git-lfs-tutorial/)
