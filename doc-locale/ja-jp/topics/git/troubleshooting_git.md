...
description: 一般的なGitエラーや接続の問題をトラブルシューティングし、解決します。
---
...
Gitの問題をデバッグする際は、GitLabサーバーで、古い可能性があるシステム提供の`git`バイナリではなく、`/opt/gitlab/embedded/bin/git`を使用してください。
...
`<command>`を実行したいGitコマンドに置き換えます。

### デバッグSSH経由のGit {#debug-git-over-ssh}
...
`<git@url>`をお使いのリポジトリのSSH URLに置き換えます。出力は`/tmp/gitlab-clone-test.log`に保存されます。

### デバッグHTTPS経由のGit {#debug-git-over-https}
...
`<url>`をお使いのリポジトリのHTTPS URLに置き換えます。出力は`/tmp/gitlab-clone-test.log`に保存されます。
...
## `git push`での`Broken pipe`エラー {#broken-pipe-errors-on-git-push}

`Broken pipe`エラーは、リモートリポジトリにプッシュしようとすると発生する可能性があります。通常、プッシュする時に次のエラーが表示されます。
...
### エラー: `stream 0 was not closed cleanly` {#error-stream-0-was-not-closed-cleanly}
...
SSH経由でプッシュする場合、はじめにSSH設定を確認してください。「パイプ破損」エラーは、認証などのSSHの根本的な問題が原因で発生することがあります。[SSHのトラブルシューティング](../../user/ssh_troubleshooting.md#password-prompt-with-git-clone)ドキュメントの手順に従って、SSHが正しく設定されていることを確認します。
...
- WindowsでPuTTYを使用している場合は、セッションプロパティに移動し、次に**接続**に移動し、**Sending of null packets to keep session active**の下で、`Seconds between keepalives (0 to turn off)`を`60`に設定します。
...
古いバージョンのGit（2.9未満）を使用している場合は、2.9以上にアップグレードすることを検討してください。詳細については、[Gitリポジトリへのプッシュ時のパイプ破損](https://stackoverflow.com/questions/19120120/broken-pipe-when-pushing-to-git-repository/36971469#36971469)を参照してください。

## エラー: `ssh_exchange_identification` {#ssh_exchange_identification-error}
...
### エラー: `Operation timed out` {#error-operation-timed-out}
...
- 以下のBashコマンドを実行して、`traceroute`と`ping`の情報を収集します: `mtr -T -P 22 <gitlab_server>.com`。MTRについて、またその出力の読み方については、Cloudflareの記事[My Traceroute (MTR)](https://www.cloudflare.com/en-gb/learning/network-layer/what-is-mtr/)を参照してください。

## エラー: `transfer closed with outstanding read data remaining` {#error-transfer-closed-with-outstanding-read-data-remaining}
...
この問題は、Gitそのものが大容量ファイルや大量のファイルを処理できない場合にたびたび発生します。この問題を回避するため、[Git LFS](https://about.gitlab.com/blog/getting-started-with-git-lfs-tutorial/)が作成されましたが、それでも制限事項があります。これは通常、次のいずれかの理由によるものです。

- リポジトリ内のファイルの数。
- 履歴内のリビジョンの数。
- リポジトリに大容量ファイルが存在。

大きなリポジトリをクローンするときにこのエラーが発生した場合は、`1`の値に[クローンの深さを減らす](../../user/project/repository/monorepos/_index.md#use-shallow-clones-and-filters-in-cicd-processes)とよいでしょう。例: 
...
## LDAPユーザー向けのSSHによるGitフェッチ時の`Your password expired`エラー {#your-password-expired-error-on-git-fetch-with-ssh-for-ldap-user}
...
このバグは、[イシュー332455](https://gitlab.com/gitlab-org/gitlab/-/issues/332455)で報告されました。

## Gitフェッチ時のエラー: `HTTP Basic: Access Denied` {#error-on-git-fetch-http-basic-access-denied}
...
これを解決するには、ユーザー名の文字列を指定します。次のいずれかの方法を使用して、`<USERNAME>`をお使いのGitLabユーザー名に置き換えます:
...
[HTTP基本アクセス認証](https://en.wikipedia.org/wiki/Basic_access_authentication)の仕組みにより、HTTP経由で実行される各Git操作の最初の`401`ログエントリは想定されるものです。
...
認証のBAN制限は、[GitLab Self-Managed](../../security/rate_limits.md#failed-authentication-ban-for-git-and-container-registry)または[GitLab.com](../../user/gitlab_com/_index.md#ip-blocks)を使用しているかどうかによって異なります。

### 失敗した認証のログを確認する {#check-logs-for-failed-authentications}
