...
> [!warning] [GitLab 16.0](https://gitlab.com/gitlab-org/gitlab/-/issues/343988)以降、`POSTGRES_ENABLED`はデフォルトで設定されなくなりました。
...
1. アプリケーションをスケールするには、再デプロイします。
   ...
   > [!warning] Kubernetesを直接使用してアプリケーションをスケールすることは避けてください。Helmが変更を検出しない可能性があり、その後のAuto DevOpsによるデプロイで変更が元に戻される可能性があります。
