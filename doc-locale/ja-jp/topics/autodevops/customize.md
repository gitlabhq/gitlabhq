...
> [!warning]シークレットをDockerビルド引数として渡さないでください。シークレットがイメージに残る可能性があります。詳細については、[シークレットに関するベストプラクティスの議論](https://github.com/moby/moby/issues/13490)を参照してください。
...
> [!warning]デフォルトでのPostgreSQLデータベースのプロビジョニングは、GitLab 15.8で[非推奨](https://gitlab.com/gitlab-org/gitlab/-/issues/387766)となり、16.0以降ではデフォルトではなくなります。データベースのプロビジョニングを有効にするには、関連する[CI/CD変数](cicd_variables.md#database-variables)を設定します。
