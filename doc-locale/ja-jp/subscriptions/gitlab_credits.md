...
FreeティアのGitLab.comユーザーは、グループネームスペース向けの月間コミットメントプールのGitLabクレジットを購入できます。これにより、PremiumまたはUltimateサブスクリプションを必要とせずに、[GitLab Duo Agent Platform](../user/duo_agent_platform/_index.md)機能のセットにアクセスできます。
...
次の表に、さまざまな機能で1GitLabクレジットあたりに行えるリクエストまたはモデル呼び出しの回数を示します。この価格設定は、機能で利用可能なすべてのモデル (セルフホストモデルを含む) に適用されます。

| 機能 | 1クレジットあたりのリクエストまたは呼び出し |
|---------|---------------------------|
| [GitLab Duoコード提案](../user/duo_agent_platform/code_suggestions/_index.md) | 50リクエスト |
| コードレビューフロー | 4回の呼び出し |
| SAST誤検出判定フロー | 1回の呼び出し |
| SAST脆弱性の修正フロー | 0.25回の呼び出し |

GitLab Duo Agentic Chatに送信される各メッセージは、1つの請求対象リクエストとしてカウントされます。1つの会話ウィンドウには複数のメッセージを含めることができ、そのため複数の請求対象リクエストを含めることができます。
