...
> [!flag]
> これらのエンドポイントの可用性は、機能フラグによって制御されます。詳細については、履歴を参照してください。
...
## Maven仮想レジストリを管理する {#manage-maven-virtual-registries}
...
- `downloads_count`および`downloaded_at`はGitLab 18.4で[導入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/201790)されました。

{{< /history >}}

指定されたグループのすべてのMaven仮想レジストリをリストします。
...
> [!warning]
> 仮想レジストリを削除すると、他の仮想レジストリと共有されていないすべての関連アップストリームレジストリと、それらのキャッシュエントリもすべて削除されます。

指定されたMaven仮想レジストリを削除します。
...
Maven仮想レジストリのすべての排他的アップストリームレジストリ内のすべてのキャッシュエントリを削除対象としてスケジュールします。キャッシュエントリは、他の仮想レジストリに関連付けられているアップストリームレジストリに対しては削除対象としてスケジュールされません。キャッシュエントリは、他の仮想レジストリに関連付けられているアップストリームレジストリに対して削除するようにスケジュールされていません。
...
### すべてのアップストリームレジストリをリストする {#list-all-upstream-registries}
...
指定されたトップレベルグループのすべてのアップストリームMavenレジストリをリストします。
...
### 作成前にアップストリームレジストリの接続をテストする {#test-upstream-registry-connection-before-creation}
...
> [!note]
> `username`と`password`はリクエストに両方含めるか、どちらも含まないようにしてください。設定されていない場合、パブリック（匿名）リクエストを使用して接続をテストします。
...
指定された仮想レジストリのすべてのアップストリームMavenレジストリをリストします。
...
指定されたMaven仮想レジストリのアップストリームレジストリを作成します。
...
> [!note]
> `username`と`password`はリクエストに両方含めるか、どちらも含まないようにしてください。設定されていない場合、パブリック（匿名）リクエストを使用してアップストリームにアクセスします。
>
...
> - 複数の仮想レジストリに[アップストリームを関連付ける](#associate-an-upstream-registry-with-a-virtual-registry)。
...
### 仮想レジストリにアップストリームレジストリを関連付ける {#associate-an-upstream-registry-with-a-virtual-registry}
...
既存のアップストリームレジストリを指定されたMaven仮想レジストリに関連付けます。
...
### 仮想レジストリからアップストリームレジストリの関連付けを解除する {#disassociate-an-upstream-registry-from-a-virtual-registry}
...
指定されたMaven仮想レジストリからアップストリームレジストリの関連付けを解除します。
...
### アップストリームレジストリの接続をテストする {#test-upstream-registry-connection}
...
指定されたMavenアップストリームレジストリへの接続をテストします。
...
> [!note]
> `2XX`（見つかった）と`404`（見つからなかった）の両方の応答は、アップストリームレジストリへの接続と認証が成功したことを示します。このテストは、GitLabがアップストリームに到達して認証できるかどうかを検証するもので、特定のアーティファクトが存在するかどうかは検証しません。
...
### オーバーライドパラメータを指定してアップストリームレジストリ接続をテストする {#test-upstream-registry-connection-with-override-parameters}
...
オプションのパラメータオーバーライドを指定して、指定されたMavenアップストリームレジストリへの接続をテストします。
...
> [!note]
> `2XX`（見つかった）と`404`（見つからなかった）の両方の応答は、アップストリームレジストリへの接続と認証が成功したことを示します。このテストでは、特定のアーティファクトが存在するかどうかは検証されません。
...
### すべてのアップストリームレジストリキャッシュエントリをリストする {#list-all-upstream-registry-cache-entries}

指定されたMavenアップストリームレジストリのすべてのキャッシュエントリをリストします。
...
指定されたMavenアップストリームレジストリのキャッシュエントリを削除します。
...
## パッケージ操作を管理する {#manage-package-operations}

Maven仮想レジストリのパッケージ操作を管理するには、次のエンドポイントを使用します。

> [!warning]
> これらのエンドポイントはGitLabによる内部使用を目的としており、通常は手動での利用を想定していません。
...
指定されたMaven仮想レジストリからパッケージをダウンロードします。このリソースにアクセスするには、[レジストリで認証する](../user/packages/package_registry/supported_functionality.md#authenticate-with-the-registry)必要があります。
...
指定されたMaven仮想レジストリにパッケージをアップロードします。このエンドポイントは、[GitLab Workhorse](../development/workhorse/_index.md)でのみアクセスできます。
