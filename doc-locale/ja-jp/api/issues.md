...
| `scope`                         | 文字列        | いいえ         | 指定されたスコープ（`created_by_me`、`assigned_to_me`、または`all`）のイシューを返します。`created_by_me`がデフォルトです。                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `scope`             | 文字列           | いいえ         | 指定されたスコープ（`created_by_me`、`assigned_to_me`、または`all`）のイシューを返します。`all`がデフォルトです。 |
...
| `severity`                                | 文字列         | いいえ       | イシューの重大度。インシデントにのみ適用されます。`unknown`、`low`、`medium`、`high`、`critical`のいずれか |
| `start_date`                              | 文字列         | いいえ       | 開始日。`YYYY-MM-DD`形式の日時文字列。たとえば、`2016-03-11`などです。GitLab 19.1で[導入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/238041)されました。 |
...
| `labels`       | 文字列  | いいえ       | イシューのラベル名のカンマ区切りリスト。すべてのラベルの割り当てを解除するには、空の文字列を設定します。ラベルがまだ存在しない場合、新しいプロジェクトラベルが作成され、イシューに割り当てられます。 |
...
- GitLab 18.10で[導入](https://gitlab.com/gitlab-org/gitlab/-/issues/371104)されました。ユーザーは、自分が作成したイシューを削除できます。
