...
- [監査レポートを作成する](audit_report.md)
- [イシューボードを特定する](sample_issue_boards.md)
- [ユーザーのクエリ](users_example.md)
- [カスタム絵文字を使用する](custom_emoji.md)
...
> [!warning]
> 
> GitLabは、[非推奨および削除プロセス](#deprecation-and-removal-process)に従うようあらゆる試みを行います。非推奨のプロセスが重大なリスクをもたらす場合、GitLabは致命的なセキュリティまたはパフォーマンスの問題を修正するために、GraphQL APIに破壊的な変更を即座に加える可能性があります。
...
| [データ制限](#data-limits)                           | 複数のblobのパスが指定されている場合、blobのリクエストは20 MBに制限されます。 |
| リクエストタイムアウト                                       | 30秒。 |
...
データ制限内に収めるには、リクエスト内のパスの数を制限する必要がある場合があります。データフィールドを除外し、`size`フィールドにリクエストを行ってください:
...
- CAPTCHAサービスが設定されていない場合、[GraphQLトップレベルエラー](https://spec.graphql.org/June2018/#sec-Errors)が発生します。例: 
...
> [!note]
> 
> GitLab GraphiQLの実装ではヘッダーの引き渡しが許可されていないため、リクエストはcURLクエリである必要があります。`--data-binary`は、JSON埋め込みクエリ内のエスケープされた二重引用符を適切に処理するために使用されます。
