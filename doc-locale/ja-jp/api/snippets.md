...
このAPIを使用して[スニペット](../user/snippets.md)を管理します。関連APIは、[プロジェクトスニペット](project_snippets.md)と[スニペットのストレージ間移動](snippet_repository_storage_moves.md)に存在します。
...
| `created_before` | 日時 | いいえ       | 指定された時間以前に作成されたスニペットを返します。ISO 8601形式（`2019-03-15T08:00:00Z`）で指定します。 |
| `page`           | 整数  | いいえ       | 取得するページ。 |
| `per_page`       | 整数  | いいえ       | 1ページあたりに返すスニペットの数。 |

成功した場合、[`200 OK`](rest/troubleshooting.md#status-codes)と次のレスポンス属性を返します: 

| 属性       | 型    | 説明 |
|-----------------|---------|-------------|
| `author`        | オブジェクト  | スニペット作成者を表すユーザーオブジェクト。 |
...
| `project_id`    | 整数 | 関連プロジェクトのID。個人スニペットの場合、`null`。 |
...
| `created_at`       | 文字列  | スニペットが作成された日時。 |
| `description`      | 文字列  | スニペットの説明。 |
| `expires_at`       | 文字列  | スニペットの有効期限日時。 |
| `file_name`        | 文字列  | スニペットファイルの名前。 |
| `http_url_to_repo` | 文字列  | スニペットリポジトリへのHTTP URL。 |
| `id`               | 整数 | スニペットのID。 |
| `imported`         | ブール値 | `true`の場合、スニペットがインポートされました。 |
| `imported_from`    | 文字列  | インポート元。 |
| `project_id`       | 整数 | 関連プロジェクトのID。個人スニペットの場合、`null`。 |
| `raw_url`          | 文字列  | rawスニペットコンテンツへのURL。 |
| `ssh_url_to_repo`  | 文字列  | スニペットリポジトリへのSSH URL。 |
| `title`            | 文字列  | スニペットのタイトル。 |
| `updated_at`       | 文字列  | スニペットが最後に更新された日時。 |
| `visibility`       | 文字列  | スニペットの表示レベル。 |
| `web_url`          | 文字列  | GitLab UIのスニペットへのURL。 |
...
## 単一のスニペットコンテンツ {#single-snippet-contents}

単一のスニペットのrawコンテンツを取得します。
...
成功した場合、[`200 OK`](rest/troubleshooting.md#status-codes)とスニペットのrawコンテンツが返されます。
...
成功した場合、[`200 OK`](rest/troubleshooting.md#status-codes)とrawファイルコンテンツが返されます。
...
> [!note]
> ユーザーは新しいスニペットを作成する権限を持っている必要があります。
...
前述の例のリクエストで使用されている`snippet.json`:
...
## スニペットを更新 {#update-snippet}

既存のスニペットを更新します。

> [!note]
> ユーザーは既存のスニペットを変更する権限を持っている必要があります。
...
| `files:action`        | 文字列          | はい           | ファイルで実行するアクションのタイプ。`create`、`update`、`delete`、`move`のいずれか。 |
| `content`             | 文字列          | いいえ            | 非推奨: 代わりに`files`を使用してください。スニペットの内容。 |
| `description`         | 文字列          | いいえ            | スニペットの説明。 |
| `file_name`           | 文字列          | いいえ            | 非推奨: 代わりに`files`を使用してください。スニペットファイルの名前。 |
| `files`               | ハッシュの配列 | 条件付き | スニペットファイルの配列。複数のファイルを持つスニペットを更新する場合に必須です。 |
...
現在のユーザーがアクセスできるすべてのスニペットをリスト表示します。管理者または監査担当者のアクセスレベルを持つユーザーは、すべてのスニペット（個人およびプロジェクトの両方）を表示できます。
...
| `page`               | 整数  | いいえ       | 取得するページ。 |
| `per_page`           | 整数  | いいえ       | 1ページあたりに返すスニペットの数。 |
| `repository_storage` | 文字列   | いいえ       | スニペットで使用されるリポジトリストレージでフィルタリングします_（管理者のみ）_。GitLab 16.3で[導入](https://gitlab.com/gitlab-org/gitlab/-/issues/419640)されました。 |
