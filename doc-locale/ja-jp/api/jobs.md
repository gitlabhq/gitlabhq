...
description: REST APIを使用してCI/CDジョブの詳細を取得する、ジョブの再試行とキャンセル、手動ジョブの実行、およびジョブログファイルへのアクセスを行います。
title: ジョブAPI
---
...
## プロジェクトのすべてのジョブをリスト {#list-all-jobs-for-a-project}

指定されたプロジェクトのすべてのジョブをリストします。

デフォルトでは、APIの結果は[ページネーション](rest/_index.md#pagination)されるため、このリクエストは一度に20件の結果を返します。

> [!note]
> このエンドポイントはオフセットベースと[keyset-based](rest/_index.md#keyset-based-pagination)のページネーションの両方をサポートしていますが、結果の連続するページをリクエストする際には、keyset-basedのページネーションが強く推奨されます。
...
- `waiting_for_callback`: ジョブが外部サービスからのコールバックを待機しています。
- `waiting_for_resource`: ジョブはリソースが利用可能になるまで待機しています。

## パイプライン別にすべてのジョブをリスト {#list-all-jobs-by-pipeline}

指定されたパイプラインのすべてのジョブをリストします。

デフォルトでは、APIの結果は[ページネーション](rest/_index.md#pagination)されるため、このリクエストは一度に20件の結果を返します。

このエンドポイントは、次のように動作します。

- [子パイプライン](../ci/pipelines/downstream_pipelines.md#parent-child-pipelines)を含む[任意のパイプラインのデータを返します](pipelines.md#retrieve-a-single-pipeline)。
...
## ジョブトークン別にジョブを取得する {#retrieve-a-job-by-job-token}

指定されたジョブトークンによって生成されたジョブを取得する。
...
## `CI_JOB_TOKEN`を使用してKubernetes向けGitLabエージェントを取得する {#retrieve-gitlab-agent-for-kubernetes-by-ci_job_token}
...
## ジョブID別にジョブを取得する {#retrieve-a-job-by-job-id}

指定されたジョブIDを持つジョブを取得する。
...
## ジョブのログファイルを取得する {#retrieve-a-log-file-for-a-job}

指定されたジョブIDのジョブログ（トレース）を取得する。
...
- `job_inputs`属性がGitLab 18.10で[導入](https://gitlab.com/groups/gitlab-org/-/work_items/17833)されました。
...
| `id`         | 整数または文字列 | はい      | プロジェクトのIDまたは[URLエンコードされたパス](rest/_index.md#namespaced-paths)。 |
| `job_id`     | 整数           | はい      | ジョブのID。 |
| `job_inputs` | ハッシュ              | いいえ       | ジョブの再試行時に使用する[job input](../ci/jobs/job_inputs.md)値のハッシュ。 |
...
> [!note]
> アーカイブされたジョブをAPIで削除することはできませんが、[特定の日付より前に完了したジョブのアーティファクトとジョブログを削除する](../administration/cicd/job_artifacts_troubleshooting.md#delete-old-builds-and-artifacts)ことができます。
...
| `id`                       | 整数または文字列 | はい      | プロジェクトのIDまたは[URLエンコードされたパス](rest/_index.md#namespaced-paths)。 |
| `job_id`                   | 整数           | はい      | ジョブのID。 |
| `job_inputs`               | ハッシュ              | いいえ       | ジョブの実行時に使用する[job input](../ci/jobs/job_inputs.md)値のハッシュ。 |
