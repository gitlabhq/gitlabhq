...
> [!note] `name`、`api_url`、`ca_cert`、および`token`は、クラスターが[既存のKubernetesクラスターを追加](../user/project/clusters/add_existing_cluster.md)オプションまたは[インスタンスクラスターを作成](#create-an-instance-cluster)エンドポイントを介して追加された場合にのみ更新できます。
