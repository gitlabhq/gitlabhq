...
## トリガートークンの詳細を取得 {#retrieve-trigger-token-details}
...
## パイプライントリガートークンを削除 {#delete-a-pipeline-trigger-token}
...
認証のために[パイプライントリガートークン](../ci/triggers/_index.md#create-a-pipeline-trigger-token)または[CI/CDジョブトークン](../ci/jobs/ci_job_token.md)を使用してパイプラインをトリガーします。
...
| `inputs`    | ハッシュ           | いいえ       | パイプラインの作成時に使用する入力のマップ（キーと値のペア）。 |
