...
> [!note]
> `name`、`api_url`、`ca_cert`、および`token`は、[「Add existing Kubernetes cluster」](../user/project/clusters/add_existing_cluster.md)オプションまたは[「Add existing cluster to project」](#add-existing-cluster-to-project)エンドポイントを使用してクラスターが追加された場合にのみ更新できます。
