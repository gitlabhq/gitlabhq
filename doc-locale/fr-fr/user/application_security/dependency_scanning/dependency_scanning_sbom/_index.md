...
title: "Analyse des dépendances à l'aide de la SBOM"
---
...
- [Introduite](https://gitlab.com/groups/gitlab-org/-/work_items/8026) dans GitLab 17.4 en tant qu'[expérience](../../../../policy/development_stages_support.md#experiment) pour la branche par défaut uniquement [avec un feature flag](../../../../administration/feature_flags/_index.md) nommé `dependency_scanning_using_sbom_reports`. Désactivée par défaut.
- [Activée sur GitLab Self-Managed](https://gitlab.com/gitlab-org/gitlab/-/issues/395692) dans GitLab 17.5.
- [Modifiée](https://gitlab.com/groups/gitlab-org/-/work_items/15960) de la version expérimentale à la version bêta avec la prise en charge de toutes les branches et [activée par défaut avec les derniers templates CI/CD d'analyse des dépendances](https://gitlab.com/gitlab-org/gitlab/-/issues/519597) pour Cargo, Conda, Cocoapods et Swift dans GitLab 17.9.
- Feature flag `dependency_scanning_using_sbom_reports` supprimé dans GitLab 17.10.
- [Modifiée](https://gitlab.com/groups/gitlab-org/-/work_items/15960) de la version bêta à disponibilité limitée pour GitLab.com uniquement avec un nouveau [template CI/CD d'analyse des dépendances V2](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/201175/) dans GitLab 18.5 [avec un feature flag](../../../../administration/feature_flags/_index.md) nommé `dependency_scanning_sbom_scan_api`. Désactivée par défaut.
...
L'analyse des dépendances à l'aide de la nomenclature logicielle (SBOM) CycloneDX analyse les dépendances de votre application à la recherche de vulnérabilités connues. Toutes les dépendances sont analysées, [y compris les dépendances transitives](../_index.md).
...
L'analyse des dépendances peut s'exécuter pendant la phase de développement du cycle de vie de votre application. Lorsque vous utilisez le nouvel analyseur d'analyse des dépendances dans des pipelines CI/CD, les dépendances du projet sont détectées et signalées dans des rapports SBOM CycloneDX. Les résultats de sécurité sont identifiés et comparés entre les branches source et cible. Les résultats et leur gravité sont listés dans la merge request, vous permettant de traiter proactivement le risque pour votre application, avant que la modification du code ne fasse l'objet d'un commit. Les résultats de sécurité pour les composants SBOM signalés sont également identifiés par [l'analyse continue des vulnérabilités](../../continuous_vulnerability_scanning/_index.md) lorsque de nouveaux avis de sécurité sont publiés, indépendamment des pipelines CI/CD.

GitLab propose à la fois l'analyse des dépendances et l'[analyse des conteneurs](../../container_scanning/_index.md) pour assurer la couverture de tous ces types de dépendances. Pour couvrir autant que possible votre zone de risque, nous vous encourageons à utiliser tous nos scanners de sécurité. Pour une comparaison de ces fonctionnalités, consultez [Comparaison de l'analyse des dépendances et de l'analyse des conteneurs](../../comparison_dependency_and_container_scanning.md).
...
Activez l'analyse des dépendances dans votre projet.
...
- Un [fichier de verrouillage ou un export de graphe de dépendances pris en charge](#supported-languages-and-files), soit qu'il fasse l'objet d'un commit dans le dépôt, soit qu'il soit créé dans le pipeline CI/CD et transmis en tant qu'artefact au job `dependency-scanning`. Alternativement, la [résolution des dépendances](#dependency-resolution) peut générer les fichiers requis pour les écosystèmes pris en charge, ou un [fichier manifeste](#manifest-fallback) peut être utilisé comme option de repli.
...
1. Ajoutez le template CI/CD `Dependency-Scanning.v2` :
...
Cette fonctionnalité repose sur des images de conteneur pour exécuter des jobs CI. Les définitions de jobs CI par défaut référencent ces images par leur tag de version majeure (par exemple, `dependency-scanning:2`), de sorte que vous recevez automatiquement les mises à jour correctives et mineures sans modifier votre configuration CI/CD.

### Politique de maintenance {#maintenance-policy}

GitLab suit la [politique de release et de maintenance](../../../../policy/maintenance.md) pour fournir des correctifs de bugs pour la release stable actuelle et des correctifs de sécurité pour les deux releases mensuelles précédentes.
...
L'image de l'analyseur des dépendances et toutes les [images de résolution des dépendances](#dependency-resolution) sont basées sur [Red Hat UBI](https://www.redhat.com/en/blog/introducing-red-hat-universal-base-image) qui utilise un module cryptographique validé FIPS 140. Aucune configuration supplémentaire n'est requise pour les environnements FIPS activés.

## Comprendre les résultats {#understanding-the-results}

Les sorties de l'analyseur des dépendances :

- Une SBOM CycloneDX pour chaque fichier de verrouillage ou export de graphe de dépendances pris en charge détecté.
- Un rapport d'analyse des dépendances unique pour tous les documents SBOM analysés.

> [!note]
> Si l'analyseur ne trouve aucun [fichier pris en charge](#supported-languages-and-files), le job d'analyse des dépendances se termine avec succès et imprime un avertissement dans le job log CI/CD. Aucune SBOM CycloneDX ni aucun rapport d'analyse des dépendances n'est généré dans ce cas.

### Nomenclature logicielle CycloneDX {#cyclonedx-software-bill-of-materials}

L'analyseur des dépendances génère une nomenclature logicielle (SBOM) [CycloneDX](https://cyclonedx.org/) pour chaque répertoire où un fichier de verrouillage, un graphe de dépendances ou un fichier manifeste pris en charge est détecté. Les SBOM CycloneDX sont créées en tant qu'artefacts de job.
...
- Importées en tant que rapports `cyclonedx`.
- Enregistrées dans le même répertoire que les fichiers de verrouillage ou de graphe de dépendances détectés.
...
Les SBOM CycloneDX suivants sont créées en tant qu'artefacts de job :
...
L'analyseur des dépendances génère un rapport d'analyse des dépendances qui documente toutes les vulnérabilités identifiées dans les dépendances identifiées dans les fichiers SBOM CycloneDX.

Le rapport d'analyse des dépendances est :

- Nommé `gl-dependency-scanning-report.json`.
- Disponible en tant qu'artefact de job du job d'analyse des dépendances.
- Importé en tant que rapport `dependency_scanning`.
- Enregistré dans le répertoire racine du projet.

## Améliorer les performances d'analyse {#improve-scanning-performance}

Le principal facteur influençant les performances de l'analyse des dépendances est le nombre de dépendances à analyser. Par défaut, l'analyse des dépendances fonctionne avec la portée suivante :

- Tous les langages et fichiers pris en charge sont inclus.
- Le répertoire racine et ses sous-répertoires immédiats sont inclus.
- Les répertoires masqués sont exclus.

Pour améliorer les performances, vous pouvez exclure des chemins spécifiques, limiter la profondeur du répertoire d'analyse et exclure les dépendances de développement et de test.

### Exclure des chemins de l'analyse {#exclude-paths-from-scanning}

Pour améliorer les performances d'analyse, excluez des chemins. Par exemple, vous pouvez exclure les chemins contenant de la documentation. Lorsque vous excluez des chemins, soyez sélectif pour éviter de masquer des vulnérabilités.

Indiquez les chemins exclus dans le fichier `.gitlab-ci.yml` :

- Pour le modèle d'analyse des dépendances, utilisez la variable CI/CD `DS_EXCLUDED_PATHS`.
- Pour le composant CI/CD d'analyse des dépendances, utilisez l'entrée spec `excluded_paths`.

Les modèles d'exclusion suivent ces règles :

- Les motifs sans barre oblique correspondent aux noms de fichiers ou de répertoires à n'importe quelle profondeur dans le projet. Par exemple, `test` correspond à `./test` et `src/test`.
- Les motifs avec des barres obliques correspondent aux chemins qui commencent par le motif. Par exemple, `a/b` correspond à `a/b` et `a/b/c`, mais pas à `c/a/b`.
- Les caractères génériques glob standard sont pris en charge. Par exemple, `a/**/b` correspond à `a/b`, `a/x/b` et `a/x/y/b`.
- Les barres obliques en début et en fin de chaîne sont ignorées. Par exemple, `/build` et `build/` correspondent aux mêmes résultats que `build`.

### Limiter la profondeur du répertoire d'analyse {#limit-scan-directory-depth}

Par défaut, l'analyse recherche uniquement les fichiers du gestionnaire de paquets dans le répertoire racine du dépôt et ses sous-répertoires immédiats. Pour améliorer les performances d'analyse, définissez la profondeur maximale du répertoire sur `1` afin de limiter la recherche au seul répertoire racine du dépôt. Veillez à inclure tous les fichiers de gestionnaire de paquets pertinents dans votre dépôt.

Pour spécifier la profondeur maximale du répertoire d'analyse dans le fichier `.gitlab-ci.yml` :

- Pour le modèle d'analyse des dépendances, utilisez la variable CI/CD `DS_MAX_DEPTH`.
- Pour le composant CI/CD d'analyse des dépendances, utilisez l'entrée spec `max_scan_depth`.

Dans l'exemple suivant, avec `DS_MAX_DEPTH` défini à `1`, l'analyse des dépendances recherche les fichiers du gestionnaire de paquets uniquement dans le répertoire `timer`. Elle n'analyse pas les sous-répertoires.
...
### Exclure les dépendances de développement et de test {#exclude-development-and-test-dependencies}

Par défaut, les dépendances de développement et de test sont incluses dans l'analyse. Pour améliorer les performances d'analyse, excluez-les en définissant l'un des paramètres suivants :

- Variable CI/CD `DS_INCLUDE_DEV_DEPENDENCIES` à `"false"`
- Input du composant CI/CD `include_dev_dependencies` à false

Seuls les projets utilisant les gestionnaires de paquets suivants sont pris en charge : Composer, Conda, Gradle, Maven, npm, pnpm, Pipenv, Poetry et uv.

## Déploiement {#roll-out}

Après avoir validé les résultats de l'analyse des dépendances avec la SBOM pour un seul projet, vous pouvez étendre sa mise en œuvre à plusieurs projets et groupes. Pour plus de détails, consultez [Appliquer le scan à plusieurs projets](#enforce-scanning-on-multiple-projects).
...
Pour que l'analyse de sécurité soit efficace, les composants listés dans votre rapport de SBOM doivent avoir des entrées correspondantes dans la [base de données d'avis GitLab](../../gitlab_advisory_database/_index.md).
...
| Langage                  | Gestionnaire de paquets | Fichier(s)                                         | Description                                                                                                                                                                           | Prise en charge de l'export de graphe de dépendances | Prise en charge de l'atteignabilité statique |
| ------------------------- | --------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- | --------------------------- |
...
| Java                      | Maven           | `pom.xml`                                       | Fichiers manifeste Maven utilisés par la [résolution des dépendances](#dependency-resolution), ou comme [solution de repli basée sur le manifeste](#manifest-fallback) lorsqu'aucun export de graphe de dépendances n'est disponible.           | {{< no >}}                      | {{< yes >}}                 |
...
| Java/Kotlin               | Gradle          | `build.gradle`, `build.gradle.kts`              | Fichiers de build Gradle utilisés par la [résolution des dépendances](#dependency-resolution), ou comme [solution de repli basée sur le manifeste](#manifest-fallback) lorsqu'aucun fichier de verrouillage ou export de graphe de dépendances n'est disponible. | {{< no >}}                      | {{< yes >}}                 |
| JavaScript/TypeScript     | Bun             | `bun.lock`                                      | Fichiers de verrouillage générés par `bun`.                                                                                                                                                          | {{< yes >}}                     | {{< yes >}}                 |
| JavaScript/TypeScript     | npm             | `package-lock.json`, `npm-shrinkwrap.json`      | Fichiers de verrouillage générés par `npm` v5 ou versions ultérieures (les versions antérieures, qui ne génèrent pas d'attribut `lockfileVersion`, ne sont pas prises en charge).                                                  | {{< yes >}}                     | {{< yes >}}                 |
...
| Python                    | pip             | `requirements.txt`                              | Fichiers manifeste utilisés par la [résolution des dépendances](#dependency-resolution), ou comme [solution de repli basée sur le manifeste](#manifest-fallback) lorsqu'aucun fichier de verrouillage ou export de graphe de dépendances n'est disponible.     | {{< no >}}                      | {{< no >}}                  |
...
1. Si un fichier de verrouillage contient plusieurs entrées pour le même paquet avec des marqueurs d'environnement différents (par exemple, numpy==2.2.6 pour Python <3.11 et numpy==2.4.1 pour Python ≥3.11), seule la première entrée est analysée et signalée.

### Informations de hachage des paquets {#package-hash-information}

Les SBOM d'analyse des dépendances incluent des informations de hachage des paquets lorsqu'elles sont disponibles. Ces informations sont fournies uniquement pour les paquets NuGet. Les hachages de paquets apparaissent aux emplacements suivants dans le SBOM, vous permettant de vérifier l'intégrité et l'authenticité des paquets :
...
### Personnalisation du comportement avec le template CI/CD {#customizing-behavior-with-the-cicd-template}

#### Entrées spec disponibles {#available-spec-inputs}

Les entrées spec suivants peuvent être utilisés en combinaison avec le template `Dependency-Scanning.v2.gitlab-ci.yml`.

| Entrée spec                                  | Type    | Valeur par défaut                                                                                                   | Description                                                                                                                                                                                                                                                           |
| ------------------------------------------- | ------- | --------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `job_name`                                  | chaîne  | `"dependency-scanning"`                                                                                   | Le nom du job d'analyse des dépendances.                                                                                                                                                                                                                              |
| `stage`                                     | chaîne  | `test`                                                                                                    | L'étape du job d'analyse des dépendances.                                                                                                                                                                                                                             |
| `allow_failure`                             | booléen | `true`                                                                                                    | Indique si l'échec du job d'analyse des dépendances doit faire échouer le pipeline.                                                                                                                                                                                                 |
| `analyzer_image_prefix`                     | chaîne  | `"$CI_TEMPLATE_REGISTRY_HOST/security-products"`                                                          | Le préfixe d'URL de registre pointant vers le dépôt de l'analyseur.                                                                                                                                                                                                   |
| `analyzer_image_name`                       | chaîne  | `"dependency-scanning"`                                                                                   | Le dépôt de l'image de l'analyseur utilisé par le job d'analyse des dépendances.                                                                                                                                                                                             |
| `analyzer_image_version`                    | chaîne  | `"2"`                                                                                                     | La version de l'image de l'analyseur utilisée par le job d'analyse des dépendances.                                                                                                                                                                                                |
| `additional_ca_cert_bundle`                 | chaîne  |                                                                                                           | Ensemble de certificats CA à considérer comme fiables. L'ensemble CA fourni ici est ajouté aux certificats du système et également utilisé par d'autres outils pendant le processus d'analyse. Pour plus de détails, consultez [Autorité de certification TLS personnalisée](#custom-tls-certificate-authority).              |
| `pip_manifest_file_name_pattern`            | chaîne  |                                                                                                           | Modèle de nom de fichier manifeste pip personnalisé à utiliser pour la résolution des dépendances et l'analyse de manifeste. Le modèle doit correspondre aux noms de fichiers uniquement, pas aux chemins de répertoires. Consultez la [bibliothèque doublestar](https://www.github.com/bmatcuk/doublestar/tree/v1#patterns) pour les détails de syntaxe. |
| `pipcompile_lockfile_file_name_pattern`     | chaîne  |                                                                                                           | Modèle de nom de fichier de verrouillage pip-compile personnalisé à utiliser lors de l'analyse. Le modèle doit correspondre aux noms de fichiers uniquement, pas aux chemins de répertoires. Consultez la [bibliothèque doublestar](https://www.github.com/bmatcuk/doublestar/tree/v1#patterns) pour les détails de syntaxe.                          |
| `pipcompile_requirements_file_name_pattern` | chaîne  |                                                                                                           | [Déprécié](https://gitlab.com/gitlab-org/gitlab/-/work_items/598796) dans GitLab 19.0 : utilisez `pipcompile_lockfile_file_name_pattern` à la place.                                                                                                                           |
| `max_scan_depth`                            | nombre  | `2`                                                                                                       | Définit le nombre de niveaux de répertoires que l'analyseur doit rechercher pour les fichiers pris en charge. Une valeur de -1 signifie que l'analyseur recherchera tous les répertoires quelle que soit la profondeur.                                                                                                       |
| `excluded_paths`                            | chaîne  | `"**/spec,**/test,**/tests,**/tmp"`                                                                       | Une liste de chemins séparés par des virgules (globs pris en charge) à exclure de l'analyse.                                                                                                                                                                                           |
| `include_dev_dependencies`                  | booléen | `true`                                                                                                    | Inclure les dépendances de développement/test lors de l'analyse d'un fichier pris en charge.                                                                                                                                                                                                 |
| `enable_static_reachability`                | booléen | `false`                                                                                                   | Activer l'[atteignabilité statique](../static_reachability.md).                                                                                                                                                                                                              |
| `enable_manifest_fallback`                  | booléen | `true`                                                                                                    | Activer la [solution de repli basée sur le manifeste](#manifest-fallback).                                                                                                                                                                                                                       |
| `analyzer_log_level`                        | chaîne  | `"info"`                                                                                                  | Niveau de journalisation pour l'analyse des dépendances. Les options sont : fatal, error, warn, info, debug.                                                                                                                                                                               |
| `enable_vulnerability_scan`                 | booléen | `true`                                                                                                    | Activer l'analyse de vulnérabilités des SBOM générés                                                                                                                                                                                                                  |
| `api_timeout`                               | nombre  | `10`                                                                                                      | Délai d'expiration de la requête API SBOM d'analyse des dépendances en secondes.                                                                                                                                                                                                              |
| `api_scan_download_delay`                   | nombre  | `3`                                                                                                       | Délai initial en secondes avant le téléchargement des résultats d'analyse de l'API SBOM d'analyse des dépendances.                                                                                                                                                                                |
| `resolution_jobs_stage`                     | chaîne  | `.pre`                                                                                                    | L'étape pour les jobs de résolution des dépendances.                                                                                                                                                                                                                         |
| `resolution_jobs_allow_failure`             | booléen | `true`                                                                                                    | Lorsque la valeur est `true`, un job de résolution en échec ne fait pas échouer le pipeline. Lorsque la valeur est `false`, un échec de résolution bloque le pipeline.                                                                                                                                              |
| `disabled_resolution_jobs`                  | chaîne  | `""`                                                                                                      | Liste de jobs de résolution à désactiver, séparés par des virgules (par exemple, `"maven, python"`). Par défaut, tous les jobs de résolution disponibles sont activés. Les valeurs possibles sont : `maven`,`gradle`,`python`. Consultez [la résolution des dépendances](#dependency-resolution)                       |
| `resolution_jobs_checkout_timeout`          | nombre  | `60`                                                                                                      | Délai en secondes pendant lequel le service de résolution attend l'extraction du dépôt. Doit être un entier positif.                                                                                                                                                         |
| `resolution_jobs_script_timeout`            | nombre  | `60`                                                                                                      | Délai en secondes pour attendre que le service de résolution des dépendances génère le script de résolution. Doit être un entier positif. Si le service attend toujours l'extraction du dépôt, augmentez plutôt la valeur de `resolution_jobs_checkout_timeout`.                     |
| `maven_resolution_job_name`                 | chaîne  | `"dependency-scanning:maven-resolution"`                                                                  | Le nom du job pour la résolution des dépendances Maven.                                                                                                                                                                                                                  |
| `maven_resolution_image`                    | chaîne  | `"registry.gitlab.com/security-products/dependency-resolution/ubi9/openjdk-21:1"`                         | L'image utilisée par le job de résolution des dépendances Maven.                                                                                                                                                                                                                |
| `maven_dependency_plugin_version`           | chaîne  | `"3.7.0"`                                                                                                 | La version de `maven-dependency-plugin` utilisée lors de la résolution des dépendances Maven. Doit être `3.7.0` ou une version ultérieure.                                                                                                                                                           |
| `python_resolution_job_name`                | chaîne  | `"dependency-scanning:python-resolution"`                                                                 | Le nom du job pour la résolution des dépendances Python.                                                                                                                                                                                                                 |
| `python_resolution_image`                   | chaîne  | `"registry.gitlab.com/security-products/dependency-resolution/ubi9/python-312-minimal-with-piptools-7:9"` | L'image utilisée par le job de résolution des dépendances Python.                                                                                                                                                                                                               |
| `gradle_resolution_job_name`                | chaîne  | `"dependency-scanning:gradle-resolution"`                                                                 | Le nom du job pour la résolution des dépendances Gradle.                                                                                                                                                                                                                 |
| `gradle_resolution_image`                   | chaîne  | `"registry.gitlab.com/security-products/dependency-resolution/ubi9/openjdk-17-with-gradle-8:1"`           | L'image utilisée par le job de résolution des dépendances Gradle.                                                                                                                                                                                                               |

#### Variables CI/CD disponibles {#available-cicd-variables}

Ces variables peuvent remplacer les entrées spec et sont également compatibles avec le template bêta `latest`.

| Variables CI/CD                                | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `AST_ENABLE_MR_PIPELINES`                      | Contrôler si le job d'analyse des dépendances s'exécute dans le pipeline de merge request ou dans le pipeline de branche. Par défaut : `"true"`. Si votre projet n'utilise pas les pipelines de merge request, désactivez cette option pour éviter les pipelines en double.                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `ADDITIONAL_CA_CERT_BUNDLE`                    | Ensemble de certificats CA à considérer comme fiables. L'ensemble CA fourni ici est ajouté aux certificats du système et également utilisé par d'autres outils pendant le processus d'analyse. Pour plus de détails, consultez [Autorité de certification TLS personnalisée](#custom-tls-certificate-authority).                                                                                                                                                                                                                                                                                                                                         |
| `ANALYZER_ARTIFACT_DIR`                        | Répertoire où les rapports CycloneDX (SBOM) sont enregistrés. Par défaut `${CI_PROJECT_DIR}/sca-artifacts`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `DS_EXCLUDED_PATHS`                            | Exclure les fichiers et répertoires de l'analyse en fonction des chemins. Une liste de modèles séparés par des virgules. Les modèles peuvent être des globs (voir [`doublestar.Match`](https://pkg.go.dev/github.com/bmatcuk/doublestar/v4@v4.0.2#Match) pour les modèles pris en charge), ou des chemins de fichiers ou de dossiers (par exemple, `doc,spec`). Consultez [Exclure des chemins de l'analyse](#exclude-paths-from-scanning) pour connaître les règles de correspondance. Il s'agit d'un pré-filtre, appliqué avant l'exécution de l'analyse. S'applique à la fois pour la détection des dépendances et l'atteignabilité statique. Par défaut : `"**/spec,**/test,**/tests,**/tmp,**/node_modules,**/.bundle,**/vendor,**/.git"`. |
...
| `DS_FF_LINK_COMPONENTS_TO_GIT_FILES`           | Lier les composants dans la liste des dépendances aux fichiers ayant fait l'objet d'un commit dans le dépôt plutôt qu'aux fichiers de verrouillage et aux fichiers de graphe générés dynamiquement dans un pipeline CI/CD. Cela garantit que tous les composants sont liés à un fichier source dans le dépôt. Par défaut : `"false"`.                                                                                                                                                                                                                                                                                                                                      |
| `SEARCH_IGNORE_HIDDEN_DIRS`                    | Ignorer les répertoires cachés. Fonctionne à la fois pour l'analyse des dépendances et l'atteignabilité statique. Par défaut : `"true"`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `DS_STATIC_REACHABILITY_ENABLED`               | Active l'[atteignabilité statique](../static_reachability.md). Par défaut : `"false"`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `DS_ENABLE_MANIFEST_FALLBACK`                  | Activer la solution de repli basée sur le manifeste lorsqu'aucun fichier de verrouillage ou export de graphe de dépendances n'est disponible. Consultez [solution de repli basée sur le manifeste](#manifest-fallback). Par défaut : `"true"`.                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `DS_MAVEN_DEPENDENCY_PLUGIN_VERSION`           | La version de `maven-dependency-plugin` utilisée lors de la résolution des dépendances Maven. Doit être `3.7.0` ou une version ultérieure. Par défaut : `3.7.0`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `MAVEN_ARGS`                                   | Arguments supplémentaires à transmettre à la commande `mvn` lors de la résolution des dépendances Maven. Remplace la variable CI/CD héritée `MAVEN_CLI_OPTS`.                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `DS_PYTHON_RESOLUTION_IMAGE`                   | L'image utilisée par le job de résolution des dépendances Python.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `PIP_INDEX_URL`                                | URL de base de l'index de paquets Python utilisée lors de la résolution des dépendances Python. Par défaut : `https://pypi.org/simple`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `PIP_EXTRA_INDEX_URL`                          | URL d'index de paquets Python supplémentaires à utiliser avec `PIP_INDEX_URL` lors de la résolution des dépendances Python.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `DS_GRADLE_RESOLUTION_IMAGE`                   | L'image utilisée par le job de résolution des dépendances Gradle.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `GRADLE_CLI_OPTS`                              | Arguments supplémentaires à transmettre à la commande `gradle` ou `gradlew` lors de la résolution des dépendances Gradle.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
Par défaut, le template `Dependency-Scanning.v2.gitlab-ci.yml` exécute le job d'analyse des dépendances dans les pipelines de merge request. Si votre projet n'utilise pas les pipelines de merge request pour d'autres jobs, cela peut entraîner l'exécution de deux pipelines pour chaque merge request, avec d'autres jobs s'exécutant dans un pipeline de branche séparé. Pour désactiver ce comportement, définissez l'entrée spec `enable_mr_pipelines: false` ou la variable CI/CD `AST_ENABLE_MR_PIPELINES: "false"`.

### Ignorer le job lorsqu'aucun fichier pris en charge n'est présent {#skip-the-job-when-no-supported-file-is-present}

Par défaut, le job d'analyse des dépendances s'exécute dans chaque pipeline qui inclut le template, même lorsque le projet ne contient pas de [fichier pris en charge](#supported-languages-and-files). Pour ignorer le job lorsqu'aucun fichier pris en charge n'est détecté, définissez `DS_SKIP_IF_NO_SUPPORTED_FILES` sur `"true"` :
...
| Langage    | Gestionnaire de paquets | Fichiers manifestes détectés                                                                                                           | Commande de résolution    | Artefact de sortie       |
| ----------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------- | --------------------- | --------------------- |
...
Pour toutes les options disponibles, consultez [les entrées spec disponibles](#available-spec-inputs) et [les variables CI/CD disponibles](#available-cicd-variables).

#### Utiliser une image de résolution des dépendances personnalisée {#use-a-custom-dependency-resolution-image}

Pour utiliser votre propre image, vous pouvez définir les entrées suivantes :
...
Pour désactiver la résolution des dépendances pour un écosystème spécifique, utilisez la variable CI/CD `DS_DISABLED_RESOLUTION_JOBS` ou l'entrée `disabled_resolution_jobs`. Les valeurs possibles sont : `maven`,`gradle`,`python`.
...
Les jobs de résolution des dépendances exécutent des outils de build natifs de l'écosystème (`mvn`, `gradle`, `pip-compile`) dans le conteneur du job CI/CD. Ces outils respectent nativement les variables d'environnement et les fichiers de configuration qui peuvent charger des extensions ou exécuter du code arbitraire au démarrage, notamment :

- Maven : `MAVEN_ARGS`, `MAVEN_CLI_OPTS` (legacy), `MAVEN_OPTS`, `JAVA_TOOL_OPTIONS`, tout `settings.xml` référencé via `-s` ou `--settings`, et `<extensions>` déclaré dans `pom.xml` ou `settings.xml`.
...
Toute personne pouvant définir ces variables CI/CD ou modifier les fichiers de build du projet peut provoquer l'exécution de code arbitraire dans le job de résolution. Le job de résolution s'exécute avec `CI_JOB_TOKEN`, accède aux variables CI/CD masquées dans la portée et lit ou écrit dans le dépôt du projet pendant la durée du job.

Cette propriété est inhérente aux outils de build natifs de l'écosystème, et n'est pas spécifique à l'analyse des dépendances. Traitez le job de résolution comme un contexte d'exécution sensible.

Contrôles recommandés :

- Restreignez les personnes pouvant définir ou remplacer les variables listées précédemment. Utilisez des [variables CI/CD protégées](../../../../ci/variables/_index.md#for-a-project) limitées aux branches et tags protégés. Ne les définissez pas dans les blocs `.gitlab-ci.yml` `variables:` que tout développeur peut modifier.
- Auditez toutes les utilisations de `MAVEN_ARGS`, `MAVEN_CLI_OPTS`, `GRADLE_OPTS`, `--init-script`, `settings.xml` personnalisés et `<extensions>` dans `pom.xml` dans le cadre de votre processus standard de revue de code.
- Lorsque vous utilisez des [politiques d'exécution des scans](../../policies/scan_execution_policies.md) pour appliquer l'analyse des dépendances, les `variables:` rédigées par les développeurs du projet cible sont transmis au job de résolution injecté. Examinez quelles variables votre framework de politique transmet, et annulez ou remplacez les variables d'outils de build dans la politique.
...
La résolution des dépendances exécute des outils de build natifs de l'écosystème dans des images vanilla ou personnalisées avec une version d'exécution fixe unique et un outil de build par écosystème.

Le succès de la résolution dépend de la compatibilité du projet avec cet environnement, de sa capacité à atteindre les registres de paquets et de l'absence d'exigences au moment du build qui dépassent la collecte des dépendances.
...
Pour les projets avec des builds hautement personnalisés qui ne sont pas suffisamment couverts par les workflows de résolution des dépendances, vous devez fournir un fichier de verrouillage ou un export du graphe de dépendances généré dans votre propre environnement de build, comme décrit dans [Créer manuellement un fichier de verrouillage ou un export du graphe de dépendances](#create-lockfile-or-dependency-graph-export-manually).
...
- Plug-in Maven enforcer : les projets utilisant des règles de version Java strictes dans le plug-in Maven Enforcer peuvent échouer. La commande de résolution passe `-Denforcer.skip=true` pour atténuer ce problème, mais toutes les règles d'application ne sont pas ignorées.
...
- Plug-ins dans les premières phases du cycle de vie : les plug-ins liés aux phases de validation ou d'initialisation incompatibles avec la version Java de l'image de résolution peuvent entraîner des échecs.

#### Problèmes connus de la résolution Gradle {#gradle-resolution-known-issues}

Environnement par défaut : Java 17, Gradle 8

Le job de résolution exécute `./gradlew dependencies` lorsqu'un wrapper Gradle est présent, ou `gradle dependencies` dans le cas contraire. Pour les projets multi-modules, chaque sous-projet est résolu individuellement en utilisant `:<subproject>:dependencies`. Le job écrit la sortie dans `gradle.graph.txt` dans le répertoire de projet correspondant.
...
- Compatibilité des plug-ins et des versions : Les projets nécessitant des plug-ins Gradle spécifiques, des chaînes d'outils personnalisées ou des versions Java autres que Java 17 peuvent échouer. Remplacez l'image de résolution (`spec:inputs:gradle_resolution_image`) par une image contenant l'environnement de build requis.
...
- Dépendances Git/VCS : Les dépendances spécifiées en tant qu'URL Git ou VCS (`git+https://...`) ne peuvent pas être résolues. La commande de résolution échouera pour ce fichier manifeste spécifique mais continuera à traiter les autres, le cas échéant.
- Installations locales/modifiables : Les entrées utilisant `-e .`, `file:`, ou des références de chemins locaux sont supprimées avant la résolution et un avertissement est émis. Ces paquets n'apparaissent pas dans la sortie.
...
Si votre projet ne dispose pas d'un [fichier de verrouillage](../../terminology/_index.md#lockfile) ou d'un [export de graphe de dépendances](../../terminology/_index.md#dependency-graph-export) pris en charge et ayant l'objet d'un commit dans son dépôt et que la résolution des dépendances ne le prend pas en charge, vous devez en fournir un.
...
Les exemples ci-dessous montrent comment créer un fichier pris en charge par l'analyseur GitLab pour les langages et gestionnaires de paquets populaires. Consultez également la liste complète des [langages et fichiers pris en charge](#supported-languages-and-files).
...
- Plug-in Nebula Gradle Dependency Lock
- Gradle `HtmlDependencyReportTask`

#### Tâche de gestion des dépendances Gradle {#gradle-dependencies-task}
...
#### Plug-in de verrouillage des dépendances {#dependency-lock-plugin}

> [!warning]
> Le `gradle-dependency-lock-plugin` n'est pas compatible avec Gradle 9 ou version ultérieure. Lorsque vous essayez de générer des fichiers `dependencies.lock` avec ces versions, le build échoue car le plugin dépend d'API Gradle internes qui ont été supprimées dans Gradle 9. Pour les projets utilisant Gradle 9 ou version ultérieure, utilisez plutôt la [tâche dependencies de Gradle](#gradle-dependencies-task) ou [HtmlDependencyReportTask](#htmldependencyreporttask).
...
Cette méthode utilise [`HtmlDependencyReportTask`](https://docs.gradle.org/current/dsl/org.gradle.api.reporting.dependencies.HtmlDependencyReportTask.html) pour produire un fichier `gradle-html-dependency-report.js`, qui contient les dépendances directes et transitives. Cette méthode a été testée avec les versions 4 à 8 de Gradle et est recommandée pour les projets utilisant Gradle 9 ou version ultérieure.
...
En raison d'un [problème connu](https://github.com/tox-dev/pipdeptree/issues/107), `pipdeptree` ne marque pas les [dépendances optionnelles](https://setuptools.pypa.io/en/latest/userguide/dependency_management.html#optional-dependencies) comme dépendances du paquet parent. Par conséquent, l'analyse des dépendances les marque comme dépendances directes du projet, plutôt que comme dépendances transitives.
...
Cette méthode utilise le plug-in [`sbt-dependency-graph`](https://github.com/sbt/sbt-dependency-graph/blob/master/README.md#usage-instructions) pour générer un fichier `dependencies-compile.dot` contenant toutes les informations dont l'analyseur a besoin, notamment les dépendances directes et transitives.

Pour activer l'analyseur sur un projet `sbt` :

1. Ajoutez le template CI/CD `Dependency-Scanning.v2`.
1. Modifiez `plugins.sbt` pour ajouter le plug-in [`sbt-dependency-graph`](https://github.com/sbt/sbt-dependency-graph/blob/master/README.md#usage-instructions).
...
## Solution de repli basée sur le manifeste {#manifest-fallback}
...
- [Mis à jour](https://gitlab.com/gitlab-org/gitlab/-/work_items/586921) dans GitLab 18.9. Prise en charge du fichier de configuration Python ajoutée ; désactivée par défaut.
...
Lorsqu'un fichier de verrouillage ou un export de graphe de dépendances pris en charge n'est pas disponible, l'analyseur des dépendances peut extraire les dépendances à partir de fichiers manifeste pris en charge en tant que solution de repli.

Les fichiers manifeste suivants sont pris en charge :

| Langage | Gestionnaire de paquets | Fichier manifeste                      |
| -------- | --------------- | ---------------------------------- |
| Java     | Maven           | `pom.xml`                          |
...
> [!warning]
>
> La solution de repli basée sur le manifeste offre une précision réduite par rapport à l'analyse des fichiers de verrouillage :
>
...
### Désactiver la solution de repli basée sur le manifeste {#disable-manifest-fallback}

Pour désactiver la solution de repli basée sur le manifeste, utilisez la variable CI/CD `DS_ENABLE_MANIFEST_FALLBACK` ou l'entrée `enable_manifest_fallback`.
...
La fonctionnalité d'analyse des dépendances utilisant SBOM repose sur une approche d'analyse des dépendances décomposée qui sépare la détection des dépendances des autres analyses, comme l'atteignabilité statique ou l'analyse des vulnérabilités.

Cette séparation des préoccupations et la modularité de cette architecture nous permettent de mieux accompagner les clients grâce à l'élargissement de la prise en charge des langages, une intégration et une expérience plus étroites au sein de la plateforme GitLab, ainsi qu'une évolution vers des types de rapports conformes aux standards du secteur.
...
Le flow global de l'analyse des dépendances est illustré ci-dessous.
...
Lors de la phase de détection des dépendances, l'analyseur parse les fichiers de verrouillage disponibles pour constituer un inventaire complet des dépendances de votre projet et de leurs relations (graphe de dépendances). Cet inventaire est capturé dans un document SBOM CycloneDX.

Lors de la phase d'accessibilité statique, l'analyseur parse les fichiers sources pour identifier les composants SBOM activement utilisés et les signale en conséquence dans le fichier SBOM. Cela permet aux utilisateurs de prioriser les vulnérabilités en fonction de l'atteignabilité du composant vulnérable. Pour plus d'informations, consultez la [page d'atteignabilité statique](../static_reachability.md).

Les documents SBOM sont temporairement importés vers l'instance GitLab via l'API SBOM d'analyse des dépendances. Le moteur du scanner de vulnérabilités SBOM de GitLab fait correspondre les composants SBOM aux avis de sécurité pour générer une liste de résultats, qui est renvoyée à l'analyseur pour être incluse dans le rapport d'analyse des dépendances.

L'API utilise le `CI_JOB_TOKEN` par défaut pour l'authentification. Le remplacement de la valeur de `CI_JOB_TOKEN` par un jeton différent peut entraîner des réponses 403 Forbidden de l'API.
...
Pour plus d'informations, consultez [les spec inputs disponibles](#available-spec-inputs) et [les variables CI/CD disponibles](#available-cicd-variables).

Les rapports générés sont importés vers l'instance GitLab à la fin du job CI et sont généralement traités après la fin du pipeline.

Les rapports SBOM sont utilisés pour prendre en charge d'autres fonctionnalités basées sur les SBOM, telles que la [liste des dépendances](../../dependency_list/_index.md), l'[analyse des licences](../../../compliance/license_scanning_of_cyclonedx_files/_index.md) ou l'[analyse continue des vulnérabilités](../../continuous_vulnerability_scanning/_index.md).

Le rapport d'analyse des dépendances suit le processus générique applicable aux [résultats de l'analyse de sécurité](../../detect/security_scanning_results.md) :
...
- Si le rapport d'analyse des dépendances est déclaré par un job CI/CD sur une branche non par défaut : des résultats de sécurité sont créés et peuvent être consultés dans l'[onglet sécurité de la vue pipeline](../../detect/security_scanning_results.md) et les [rapports de merge request](../../../project/merge_requests/reports.md).
...
## Appliquer l'analyse à plusieurs projets {#enforce-scanning-on-multiple-projects}

Appliquez l'analyse des dépendances à plusieurs projets en utilisant une politique de sécurité. L'analyse des dépendances nécessite un artefact analysable, soit un fichier de verrouillage, soit un export de graphe de dépendances. Le fait que l'artefact analysable fasse l'objet ou non d'un commit dans le dépôt du projet détermine le choix de la politique.

- Si l'artefact analysable fait l'objet d'un commit dans le dépôt, utilisez une [politique d'exécution des scans](../../policies/scan_execution_policies.md).

  Pour les projets dont les artefacts analysables sont commis dans leurs dépôts, ou pris en charge par la [résolution des dépendances](#dependency-resolution), une politique d'exécution des scans constitue le moyen le plus direct d'appliquer l'analyse des dépendances.

- Si l'artefact analysable ne fait pas l'objet d'un commit dans le dépôt et n'est pas pris en charge par la [résolution des dépendances](#dependency-resolution), utilisez une [politique d'exécution des pipelines](../../policies/pipeline_execution_policies.md).

  Pour les projets dont les artefacts analysables ne sont pas commis dans leurs dépôts, vous devez utiliser une politique d'exécution des pipelines. La politique doit définir un job CI/CD personnalisé pour générer des artefacts analysables avant d'invoquer l'analyse des dépendances.

  La politique d'exécution des pipelines doit :
...
L'exemple suivant utilise le plug-in Gradle `nebula` pour générer des fichiers de verrouillage. Pour les autres langages, voir [Créer un fichier de verrouillage ou un export de graphe de dépendances manuellement](#create-lockfile-or-dependency-graph-export-manually).

### Exemple : politique d'exécution des pipelines pour un projet Gradle {#example-pipeline-execution-policy-for-a-gradle-project}

Pour un projet Gradle sans artefact analysable ayant fait l'objet d'un commit dans le dépôt, vous devez définir une étape de génération d'artefact dans la politique d'exécution des pipelines. L'exemple suivant utilise le plug-in `nebula`.
...
Cette approche garantit les éléments suivants :
...
Nous vous recommandons vivement d'activer la fonctionnalité d'analyse des dépendances en utilisant le template `v2`. Si cela n'est pas possible, vous pouvez choisir l'une des méthodes suivantes.
...
- Pour activer le nouvel analyseur des dépendances, définissez la variable CI/CD `DS_ENFORCE_NEW_ANALYZER` sur `true`.
- Un [fichier de verrouillage ou un export de graphe de dépendances pris en charge](#create-lockfile-or-dependency-graph-export-manually), ou un [fichier déclencheur](#trigger-files-for-the-latest-template) doit exister dans le dépôt pour créer le job `dependency-scanning` dans les pipelines.
...
Vous pouvez également activer la fonctionnalité en utilisant les [Scan Execution Policies](../../policies/scan_execution_policies.md) avec le modèle `latest` et imposer le nouvel analyseur d'analyse des dépendances en définissant la variable CI/CD `DS_ENFORCE_NEW_ANALYZER` sur `true`.
...
Les fichiers déclencheurs créent un job CI/CD `dependency-scanning` lors de l'utilisation du [template CI/CD d'analyse des dépendances le plus récent](https://gitlab.com/gitlab-org/gitlab/-/blob/master/lib/gitlab/ci/templates/Jobs/Dependency-Scanning.latest.gitlab-ci.yml). L'analyseur ne scanne pas ces fichiers. Votre projet peut être pris en charge si vous utilisez un fichier déclencheur pour [créer un fichier de verrouillage ou un export de graphe de dépendances manuellement](#create-lockfile-or-dependency-graph-export-manually).
...
- [En disponibilité générale](https://gitlab.com/gitlab-org/gitlab/-/issues/578686) dans GitLab 18.8. [Composant CI/CD d'analyse des dépendances](https://gitlab.com/explore/catalog/components/dependency-scanning) version [`1.0.0`](https://gitlab.com/components/dependency-scanning/-/tags/1.0.0).

{{< /history >}}

Utilisez le [composant CI/CD d'analyse des dépendances](https://gitlab.com/explore/catalog/components/dependency-scanning) pour activer le nouvel analyseur des dépendances. Avant de choisir cette approche, consultez les [limitations](../../../../ci/components/_index.md#use-a-gitlabcom-component-on-gitlab-self-managed) actuelles pour GitLab Self-Managed.
...
> [!warning]
> La prise en charge des SBOM tierces est techniquement possible, mais très susceptible d'évoluer pendant que nous finalisons la prise en charge officielle avec cet [epic](https://www.gitlab.com/groups/gitlab-org/-/epics/14760).

Utilisez votre propre document SBOM CycloneDX généré avec un générateur de SBOM CycloneDX tiers ou un outil personnalisé en tant que [rapport d'artifact CI/CD](../../../../ci/yaml/artifacts_reports.md#artifactsreportscyclonedx) dans un job CI personnalisé.

Pour activer l'analyse des dépendances à l'aide de SBOM, le document SBOM CycloneDX fourni doit :
...
- Être importé en tant que [rapport d'artefact CI/CD](../../../../ci/yaml/artifacts_reports.md#artifactsreportscyclonedx) depuis un job CI réussi.
