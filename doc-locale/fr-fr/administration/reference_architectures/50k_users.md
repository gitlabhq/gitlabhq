...
description: "Déployez une instance GitLab Auto-géré pour jusqu'à 50 000 utilisateurs ou 1 000 RPS avec haute disponibilité, réplication de base de données et dimensionnement de l'infrastructure."
title: 'Architecture de référence : Jusqu''à 1 000 RPS ou 50 000 utilisateurs'
---
...
Cette page décrit l'architecture de référence GitLab conçue pour cibler une charge de pointe de 1 000 requêtes par seconde (RPS), la charge de pointe typique de jusqu'à 50 000 utilisateurs, manuels et automatisés, basée sur des données réelles.
...
- **Target load** :  API :  1 000 RPS, Web :  100 RPS, Git (Pull) :  100 RPS, Git (Push) :  20 RPS
...
|------------------------------------------|-------|-------------------------|------------------|---------------|-----------|
| Équilibreur de charge externe<sup>4</sup>       | 1     | 16 vCPU, 14,4 Go de mémoire | `n1-highcpu-16`  | `c5.4xlarge`  | `F16s v2` |
| Consul<sup>2</sup>                       | 3     | 2 vCPU, 1,8 Go de mémoire   | `n1-highcpu-2`   | `c5.large`    | `F2s v2`  |
| PostgreSQL<sup>2</sup>                   | 3     | 32 vCPU, 120 Go de mémoire  | `n1-standard-32` | `m5.8xlarge`  | `D32s v3` |
| PgBouncer<sup>2</sup>                    | 3     | 2 vCPU, 1,8 Go de mémoire   | `n1-highcpu-2`   | `c5.large`    | `F2s v2`  |
| Équilibreur de charge interne<sup>4</sup>       | 1     | 16 vCPU, 14,4 Go de mémoire | `n1-highcpu-16`  | `c5.4xlarge`  | `F16s v2` |
...
| Gitaly<sup>6</sup><sup>7</sup>           | 3     | 64 vCPU, 240 Go de mémoire  | `n1-standard-64` | `m5.16xlarge` | `D64s v3` |
...
| GitLab Rails<sup>8</sup>                 | 12    | 32 vCPU, 28,8 Go de mémoire | `n1-highcpu-32`  | `c5.9xlarge`  | `F32s v2` |
...
1. Les exemples de types de machines sont donnés à titre d'illustration. Ces types sont utilisés pour la [validation et les tests](_index.md#validation-and-test-results) mais ne sont pas destinés à être des valeurs par défaut prescriptives. Le passage à d'autres types de machines répondant aux exigences listées est pris en charge, y compris les variantes ARM si disponibles. Voir [Types de machines pris en charge](_index.md#supported-machine-types) pour plus d'informations.
2. Peut être exécuté optionnellement sur des solutions PaaS PostgreSQL externes tierces réputées. Voir [Fournir votre propre instance PostgreSQL](#provide-your-own-postgresql-instance) pour plus d'informations.
3. Peut être exécuté optionnellement sur des solutions PaaS Redis externes tierces réputées. Voir [Fournir vos propres instances Redis](#provide-your-own-redis-instances) pour plus d'informations.
   - Redis est principalement monothread et ne bénéficie pas significativement d'une augmentation du nombre de cœurs CPU. Pour cette taille d'architecture, il est fortement recommandé d'avoir des instances Cache et Persistant séparées comme spécifié pour atteindre des performances optimales.
4. Peut être exécuté optionnellement sur des services d'équilibrage de charge tiers réputés (LB PaaS). Voir [Fournisseurs cloud et services recommandés](_index.md#recommended-cloud-providers-and-services) pour plus d'informations.
...
6. Gitaly Cluster (Praefect) offre les avantages de la tolérance aux pannes, mais implique une complexité supplémentaire de configuration et de gestion. Consultez les [limitations techniques et considérations existantes avant de déployer Gitaly Cluster (Praefect)](../gitaly/praefect/_index.md#before-deploying-gitaly-cluster-praefect). Si vous souhaitez un Gitaly en mode fragmenté, utilisez les mêmes spécifications listées dans le tableau précédent pour `Gitaly`.
7. Les spécifications Gitaly sont basées sur des percentiles élevés des modèles d'utilisation et des tailles de dépôts en bonne santé. Cependant, si vous avez des [monodépôts volumineux](_index.md#large-monorepos) (plus grands que plusieurs gigaoctets) ou des [charges de travail supplémentaires](_index.md#additional-workloads), ceux-ci peuvent avoir un impact significatif sur les performances de Git et Gitaly et des ajustements supplémentaires seront probablement nécessaires.
8. Peut être placé dans des groupes de mise à l'échelle automatique (ASGs) car le composant ne stocke aucune [donnée avec état](_index.md#autoscaling-of-stateful-nodes). Cependant, les [configurations Cloud Native Hybrid](#cloud-native-hybrid-reference-architecture-with-helm-charts-alternative) sont généralement préférées car certains composants tels que les [migrations](#gitlab-rails-post-configuration) et [Mailroom](../incoming_email.md) ne peuvent être exécutés que sur un seul nœud, ce qui est mieux géré dans Kubernetes.
...
L'architecture de référence 1 000 RPS / 50k utilisateurs est conçue pour s'adapter aux workflows les plus courants. GitLab effectue régulièrement des tests de smoke et de performance selon les objectifs de débit de points de terminaison suivants :
...
- Des équivalents de fournisseur cloud avec une implémentation Gateway API ou Ingress pour les Cloud Native Hybrids

## Configurer les composants {#set-up-components}

Pour configurer GitLab et ses composants pour accueillir jusqu'à 1 000 RPS ou 50 000 utilisateurs :
...
La liste suivante inclut les descriptions de chaque serveur et son IP assignée :
...
- `10.6.0.52` :  Redis - Cache Réplique 1
- `10.6.0.53` :  Redis - Cache Réplique 2
- `10.6.0.61` :  Redis - Persistant Principal
- `10.6.0.62` :  Redis - Persistant Réplique 1
- `10.6.0.63` :  Redis - Persistant Réplique 2
- `10.6.0.91` :  Gitaly 1
- `10.6.0.92` :  Gitaly 2
- `10.6.0.93` :  Gitaly 3
...
- `10.6.0.116` :  Application GitLab 6
- `10.6.0.117` :  Application GitLab 7
- `10.6.0.118` :  Application GitLab 8
- `10.6.0.119` :  Application GitLab 9
- `10.6.0.120` :  Application GitLab 10
- `10.6.0.121` :  Application GitLab 11
- `10.6.0.122` :  Application GitLab 12
...
Les spécificités sur quel équilibreur de charge utiliser, ou sa configuration exacte, dépassent le cadre de la documentation GitLab, mais référez-vous aux [Équilibreurs de charge](_index.md) pour plus d'informations sur les exigences générales. Cette section se concentrera sur les spécificités de ce qu'il faut configurer pour l'équilibreur de charge de votre choix.
...
Si vous utilisez GitLab Pages avec la prise en charge de domaines personnalisés, vous aurez besoin de quelques configurations de ports supplémentaires. GitLab Pages nécessite une adresse IP virtuelle séparée. Configurez le DNS pour pointer le `pages_external_url` depuis `/etc/gitlab/gitlab.rb` vers la nouvelle adresse IP virtuelle. Consultez la [documentation GitLab Pages](../pages/_index.md) pour plus d'informations.
...
La prochaine question est de savoir comment vous allez gérer le SSL dans votre environnement. Il existe plusieurs options différentes :
...
Configurez votre équilibreur de charge pour transmettre les connexions sur le port 443 en tant que `TCP` plutôt que le protocole `HTTP(S)`. Cela transmettra la connexion au service NGINX du nœud d'application sans modification. NGINX disposera du certificat SSL et écoutera sur le port 443.
...
Référez-vous à la documentation de votre équilibreur de charge préféré pour plus de conseils.
...
> [!note]
> Consul doit être déployé en nombre impair de 3 nœuds ou plus. Cela permet de s'assurer que les nœuds peuvent prendre des votes dans le cadre d'un quorum.
...
1. [Téléchargez et installez](../../install/package/_index.md#supported-platforms) le package Linux de votre choix. Assurez-vous d'ajouter uniquement le dépôt de packages GitLab et d'installer GitLab pour le système d'exploitation de votre choix. Sélectionnez la même version et le même type (éditions Community ou Enterprise) que votre installation actuelle.
...
Dans cette section, vous serez guidé à travers la configuration d'un cluster PostgreSQL hautement disponible à utiliser avec GitLab.

### Fournir votre propre instance PostgreSQL {#provide-your-own-postgresql-instance}

Au lieu des composants PostgreSQL, PgBouncer et Consul de découverte de services inclus dans le package Linux, vous pouvez utiliser un [service externe tiers pour PostgreSQL](../postgresql/external.md).
...
Pour plus d'informations, y compris des conseils sur la haute disponibilité et l'équilibrage de charge de la base de données, voir :
...
- [Équilibrage de charge de base de données](../postgresql/database_load_balancing.md) activé.

  Un service PgBouncer local à configurer sur chaque nœud PostgreSQL. Il est distinct du cluster PgBouncer principal qui suit le nœud principal.
...
Tout d'abord, assurez-vous d'[installer](../../install/package/_index.md#supported-platforms) le package Linux **on each node**. Assurez-vous d'ajouter uniquement le dépôt de packages GitLab et d'installer GitLab pour le système d'exploitation de votre choix, mais ne fournissez **not** la valeur `EXTERNAL_URL`.
...
PostgreSQL, avec Patroni gérant son basculement, utilisera par défaut `pg_rewind` pour gérer les conflits. Comme la plupart des méthodes de gestion du basculement, cela a une faible probabilité d'entraîner une perte de données. Pour plus d'informations, consultez les différentes [méthodes de réplication Patroni](../postgresql/replication_and_failover.md#selecting-the-appropriate-patroni-replication-method).
...
> [!note]
> PgBouncer est monothread et ne bénéficie pas significativement d'une augmentation du nombre de cœurs CPU. Référez-vous à la [documentation sur la mise à l'échelle](_index.md#scaling-an-environment) pour plus d'informations.
...
1. [Reconfigurez GitLab](../restart_gitlab.md#reconfigure-a-linux-package-installation) une nouvelle fois pour résoudre les éventuelles erreurs des étapes précédentes.
...
> [!note]
>
> - Les clusters Redis doivent chacun être déployés en nombre impair de 3 nœuds ou plus. Cela permet de s'assurer que Redis Sentinel peut prendre des votes dans le cadre d'un quorum. Cela ne s'applique pas lors de la configuration de Redis en externe, comme avec un service de fournisseur cloud.
> - Redis est principalement monothread et ne bénéficie pas significativement d'une augmentation du nombre de cœurs CPU. Pour cette taille d'architecture, il est fortement recommandé d'avoir des instances Cache et Persistant séparées comme spécifié pour atteindre des performances optimales. Référez-vous à la [documentation sur la mise à l'échelle](_index.md#scaling-an-environment) pour plus d'informations.

Redis nécessite une authentification s'il est utilisé avec Sentinel. Consultez la documentation [Sécurité Redis](https://redis.io/docs/latest/operate/rc/security/) pour plus d'informations. Nous recommandons d'utiliser une combinaison d'un mot de passe Redis et de règles de pare-feu strictes pour sécuriser votre service Redis. Vous êtes vivement encouragé à lire la documentation de [Redis Sentinel](https://redis.io/docs/latest/operate/oss_and_stack/management/sentinel/) avant de configurer Redis avec GitLab pour bien comprendre la topologie et l'architecture.
...
1. Protégez les nœuds de l'accès depuis des réseaux externes (Internet) à l'aide d'un pare-feu.

Dans cette section, vous serez guidé à travers la configuration de deux clusters Redis externes à utiliser avec GitLab. Les IPs suivantes seront utilisées comme exemple :
...
- Le mode Redis Cluster n'est pas spécifiquement pris en charge, mais Redis Standalone avec HA l'est.
...
Les nœuds Redis principal et réplique ont tous les deux besoin du même mot de passe défini dans `redis['password']`. À tout moment lors d'un basculement, les Sentinels peuvent reconfigurer un nœud et modifier son statut de principal à réplique (et vice versa).
...
C'est la section où nous installons et configurons les nouvelles instances Redis Queues.
...
- Pour implémenter Gitaly en mode fragmenté à la place, suivez la [documentation Gitaly séparée](../gitaly/configure_gitaly.md) au lieu de cette section. Utilisez les mêmes spécifications Gitaly.
- Pour la migration de dépôts existants non gérés par Gitaly Cluster (Praefect), voir [migrer vers Gitaly Cluster (Praefect)](../gitaly/praefect/_index.md#migrate-to-gitaly-cluster-praefect).

La configuration de cluster recommandée inclut les composants suivants :
...
Cette section détaille comment configurer la configuration standard recommandée dans l'ordre. Pour des configurations plus avancées, référez-vous à la [documentation autonome de Gitaly Cluster (Praefect)](../gitaly/praefect/_index.md).
...
- Disponible uniquement pour les installations du package Linux (Omnibus)

Pour les instructions de configuration, consultez [Service discovery](../gitaly/praefect/configure.md#service-discovery).

### Configurer PostgreSQL pour Praefect {#configure-praefect-postgresql}
...
#### PostgreSQL autonome non-HA pour Praefect à l'aide du package Linux {#praefect-non-ha-postgresql-standalone-using-the-linux-package}

Les IPs suivantes seront utilisées comme exemple :

- `10.6.0.141` :  Praefect PostgreSQL

Tout d'abord, assurez-vous d'[installer](../../install/package/_index.md#supported-platforms) le package Linux sur le nœud Praefect PostgreSQL. Assurez-vous d'ajouter uniquement le dépôt de packages GitLab et d'installer GitLab pour le système d'exploitation de votre choix, mais ne fournissez **not** la valeur `EXTERNAL_URL`.

1. Connectez-vous en SSH au nœud Praefect PostgreSQL.
1. Créez un mot de passe robuste à utiliser pour l'utilisateur Praefect PostgreSQL. Notez ce mot de passe en tant que `<praefect_postgresql_password>`.
...
#### Solution tierce PostgreSQL HA pour Praefect {#praefect-ha-postgresql-third-party-solution}
...
Il existe de nombreuses solutions tierces pour PostgreSQL HA. La solution sélectionnée doit présenter les caractéristiques suivantes pour fonctionner avec Praefect :
...
> [!note]
> Avec une configuration tierce, il est possible de colocaliser la base de données de Praefect sur le même serveur que la base de données principale de [GitLab](#provide-your-own-postgresql-instance) pour plus de commodité, à moins que vous n'utilisiez Geo, où des instances de base de données séparées sont nécessaires pour gérer correctement la réplication. Dans cette configuration, les spécifications de la configuration de la base de données principale ne devraient pas avoir à être modifiées, car l'impact devrait être minimal.
...
Nous recommandons de nommer l'utilisateur `praefect` et la base de données `praefect_production`, qui peuvent être configurés de manière standard dans PostgreSQL. Le mot de passe de l'utilisateur est le même que celui que vous avez configuré précédemment en tant que `<praefect_postgresql_password>`.
...
Praefect est le routeur et gestionnaire de transactions pour Gitaly Cluster (Praefect) et toutes les connexions à Gitaly transitent par lui. Cette section explique en détail comment le configurer.

> [!note]
> Praefect doit être déployé en nombre impair de 3 nœuds ou plus. Cela permet de s'assurer que les nœuds peuvent prendre des votes dans le cadre d'un quorum.
...
- `<praefect_internal_token>` :  Utilisé pour le trafic de réplication au sein de Gitaly Cluster (Praefect). Ce jeton est distinct de `praefect_external_token` car les clients Gitaly ne doivent pas pouvoir accéder directement aux nœuds internes du Gitaly Cluster (Praefect) ; cela pourrait entraîner une perte de données.
- `<praefect_postgresql_password>` :  Le mot de passe Praefect PostgreSQL défini dans la section précédente est également requis dans le cadre de cette configuration.

Les nœuds Gitaly Cluster (Praefect) sont configurés dans Praefect via un `virtual storage`. Chaque stockage contient les détails de chaque nœud Gitaly qui compose le cluster. Chaque stockage reçoit également un nom, et ce nom est utilisé dans plusieurs zones de la configuration. Dans ce guide, le nom du stockage sera `default`. De plus, ce guide est destiné aux nouvelles installations ; si vous mettez à niveau un environnement existant pour utiliser Gitaly Cluster (Praefect), vous devrez peut-être utiliser un nom différent. Consultez la [documentation de Gitaly Cluster (Praefect)](../gitaly/praefect/configure.md#praefect) pour plus d'informations.
...
1. [Téléchargez et installez](../../install/package/_index.md#supported-platforms) le package Linux de votre choix. Assurez-vous d'ajouter uniquement le dépôt de packages GitLab et d'installer GitLab pour le système d'exploitation de votre choix.
...
1. Copiez le fichier `/etc/gitlab/gitlab-secrets.json` depuis le premier nœud de package Linux que vous avez configuré et ajoutez-le ou remplacez le fichier du même nom sur ce serveur. Si c'est le premier nœud de package Linux que vous configurez, vous pouvez ignorer cette étape.
1. Praefect doit exécuter des migrations de base de données, tout comme l'application GitLab principale. Pour cela, vous devez sélectionner **one Praefect node only to run the migrations**, alias le _nœud de déploiement_. Ce nœud doit être configuré en premier, avant les autres, comme suit :
   ...
   1. [Reconfigurer GitLab](../restart_gitlab.md#reconfigure-a-linux-package-installation) pour que les modifications prennent effet et pour exécuter les migrations de base de données Praefect.
...
Pour configurer Gitaly, vous devez noter ce qui suit :
...
1. [Téléchargez et installez](../../install/package/_index.md#supported-platforms) le package Linux de votre choix. Assurez-vous d'ajouter uniquement le dépôt de packages GitLab et d'installer GitLab pour le système d'exploitation de votre choix, mais ne fournissez **not** la valeur `EXTERNAL_URL`.
...
### Prise en charge TLS pour Gitaly Cluster (Praefect) {#gitaly-cluster-praefect-tls-support}
...
- Vous pouvez configurer les serveurs Praefect avec une adresse d'écoute non chiffrée `listen_addr` et une adresse d'écoute chiffrée `tls_listen_addr` en même temps. Cela vous permet d'effectuer une transition progressive du trafic non chiffré vers le trafic chiffré, si nécessaire. Pour désactiver l'écouteur non chiffré, définissez `praefect['configuration'][:listen_addr] = nil`.
- L'équilibreur de charge interne doit être configuré pour gérer les connexions TLS. Configurez l'équilibreur de charge pour prendre en charge le passthrough TLS, c'est-à-dire que l'équilibreur de charge transfère le trafic chiffré vers le backend sans le terminer. N'utilisez pas un équilibreur de charge passthrough/retour direct au serveur (DSR). L'équilibreur de charge doit activement proxy les connexions pour maintenir un équilibrage de charge et une vérification de l'état appropriés.
...
> [!note]
> [Étant donné qu'il est recommandé d'utiliser Object storage](../object_storage.md) plutôt que NFS pour les objets de données, les exemples suivants incluent la configuration d'Object storage.
...
Lors de la configuration de fonctionnalités GitLab supplémentaires telles que Container Registry, SAML ou LDAP, mettez à jour la configuration Sidekiq en plus de la configuration Rails. Consultez la [documentation externe sur Sidekiq](../sidekiq/_index.md) pour plus d'informations.
...
1. Copiez le fichier `/etc/gitlab/gitlab-secrets.json` depuis le premier nœud de package Linux que vous avez configuré et ajoutez-le ou remplacez le fichier du même nom sur ce serveur. Si c'est le premier nœud de package Linux que vous configurez, vous pouvez ignorer cette étape.
1. Copiez les clés d'hôte SSH (toutes au format de nom `/etc/ssh/ssh_host_*_key*`) depuis le premier nœud Rails que vous avez configuré et ajoutez-les ou remplacez les fichiers du même nom sur ce serveur. Cela garantit que les erreurs de non-correspondance d'hôte ne sont pas générées pour vos utilisateurs lorsqu'ils accèdent aux nœuds Rails avec équilibrage de charge. S'il s'agit du premier nœud du package Linux que vous configurez, vous pouvez ignorer cette étape.
1. Pour vous assurer que les migrations de base de données ne sont exécutées que lors de la reconfiguration et non automatiquement lors de la mise à niveau, exécutez :
   ...
   Seul un nœud désigné doit gérer les migrations, comme indiqué dans la section [Post-configuration de GitLab Rails](#gitlab-rails-post-configuration).
1. [Reconfigurez GitLab](../restart_gitlab.md#reconfigure-a-linux-package-installation) pour que les modifications prennent effet.
...
1. Confirmez que le nœud peut se connecter à Gitaly :
   ...
   Ensuite, consultez les journaux pour voir les requêtes :

   ```shell
   sudo gitlab-ctl tail gitaly
   ```

1. Facultativement, depuis les serveurs Gitaly, confirmez que Gitaly peut effectuer des rappels vers l'API interne :
...
1. Désignez un nœud d'application pour exécuter les migrations de base de données lors de l'installation et des mises à jour. Initialisez la base de données GitLab et assurez-vous que toutes les migrations ont été exécutées :
   ...
   Cette opération nécessite de configurer le nœud Rails pour se connecter directement à la base de données principale, en [contournant PgBouncer](../postgresql/pgbouncer.md#procedure-for-bypassing-pgbouncer). Une fois les migrations terminées, vous devez configurer le nœud pour passer à nouveau par PgBouncer.
...
GitLab Runner renvoie les journaux de jobs par blocs, que le package Linux met temporairement en cache sur le disque dans `/var/opt/gitlab/gitlab-ci/builds` par défaut, même lors de l'utilisation d'un stockage d'objets consolidé. Avec la configuration par défaut, ce répertoire doit être partagé via NFS sur tous les nœuds GitLab Rails et Sidekiq.

Bien que le partage des journaux de jobs via NFS soit pris en charge, évitez l'obligation d'utiliser NFS en activant la [journalisation incrémentielle](../cicd/job_logs.md#incremental-logging) (obligatoire lorsqu'aucun nœud NFS n'a été déployé). La journalisation incrémentielle utilise Redis plutôt que l'espace disque pour la mise en cache temporaire des journaux de jobs.
...
Consultez la documentation de configuration [avancée](https://docs.gitlab.com/charts/advanced/) des Charts Helm pour les instructions de configuration, notamment des conseils sur les secrets GitLab à synchroniser entre Kubernetes et les composants backend.

> [!note]
> Il s'agit d'une configuration **advanced**. L'exécution de services dans Kubernetes est connue pour être complexe. **This setup is only recommended** si vous avez de solides connaissances et une expérience pratique de Kubernetes. Le reste de cette section part de ce principe.

Pour des informations sur la disponibilité de Gitaly sur Kubernetes, ses limitations et les considérations de déploiement, consultez [Gitaly sur Kubernetes](../gitaly/kubernetes.md).

### Topologie du cluster {#cluster-topology}

Les tableaux et diagrammes suivants décrivent en détail l'environnement hybride en utilisant les mêmes formats que l'environnement typique documenté précédemment.

Voici d'abord les composants qui s'exécutent dans Kubernetes. Ceux-ci s'exécutent sur plusieurs groupes de nœuds, bien que vous puissiez modifier la composition globale comme vous le souhaitez, tant que les exigences minimales en matière de CPU et de mémoire sont respectées.

| Groupe de nœuds de composants | Totaux cibles du pool de nœuds | Exemple GCP     | Exemple AWS  |
|----------------------|-------------------------|-----------------|--------------|
| Webservice           | 308 vCPU<br/>385 Go de mémoire (requête)<br/>539 Go de mémoire (limite) | 11 x `n1-standard-32` | 11 x `c5.9xlarge` |
...
- Pour cette configuration, nous effectuons régulièrement des [tests](_index.md#validation-and-test-results) et recommandons [Google Kubernetes Engine (GKE)](https://cloud.google.com/kubernetes-engine) et [Amazon Elastic Kubernetes Service (EKS)](https://aws.amazon.com/eks/). D'autres services Kubernetes peuvent également fonctionner, mais les résultats peuvent varier.
- Les exemples de types de machines sont donnés à titre d'illustration. Ces types sont utilisés pour la [validation et les tests](_index.md#validation-and-test-results) mais ne sont pas destinés à être des valeurs par défaut prescriptives. Le passage à d'autres types de machines répondant aux exigences listées est pris en charge. Consultez [Types de machines pris en charge](_index.md#supported-machine-types) pour plus d'informations.
- Les totaux du pool de nœuds cibles [Webservice](#webservice) et [Sidekiq](#sidekiq) sont donnés uniquement pour les composants GitLab. Des ressources supplémentaires sont nécessaires pour les processus système du fournisseur Kubernetes choisi. Les exemples donnés en tiennent compte.
- Le total du pool de nœuds cibles [Supporting](#supporting) est généralement donné pour accueillir plusieurs ressources pour soutenir le déploiement GitLab ainsi que tout déploiement supplémentaire que vous pourriez souhaiter effectuer en fonction de vos besoins. Comme pour les autres pools de nœuds, les processus système du fournisseur Kubernetes choisi nécessitent également des ressources. Les exemples donnés en tiennent compte.
- Dans les déploiements en production, il n'est pas obligatoire d'assigner des pods à des nœuds spécifiques. Cependant, il est recommandé d'avoir plusieurs nœuds dans chaque pool répartis sur différentes zones de disponibilité pour s'aligner sur les pratiques d'architecture cloud résiliente.
...
Voici ensuite les composants backend qui s'exécutent sur des VM de calcul statiques à l'aide du package Linux (ou de services PaaS externes le cas échéant) :
...
| PostgreSQL<sup>2</sup>                   | 3     | 32 vCPU, 120 Go de mémoire | `n1-standard-32` | `m5.8xlarge`  |
| PgBouncer<sup>2</sup>                    | 3     | 2 vCPU, 1,8 Go de mémoire  | `n1-highcpu-2`   | `c5.large`    |
| Équilibreur de charge interne<sup>4</sup>       | 1     | 16 vCPU, 14,4 Go de mémoire | `n1-highcpu-16` | `c5.4xlarge`  |
...
| Gitaly<sup>6</sup><sup>7</sup>           | 3     | 64 vCPU, 240 Go de mémoire | `n1-standard-64` | `m5.16xlarge` |
...
La section suivante décrit en détail les cibles utilisées pour les composants GitLab déployés dans Kubernetes.
...
Pour 1 000 RPS ou 50 000 utilisateurs, nous recommandons un nombre total de workers Puma d'environ 308, ce qui implique à son tour l'exécution d'au moins 77 pods Webservice.

Pour plus d'informations sur l'utilisation des ressources par Webservice, consultez la documentation Charts sur les [ressources Webservice](https://docs.gitlab.com/charts/charts/gitlab/webservice/#resources).

##### Gateway API / Ingress {#gateway-api--ingress}

Il est également recommandé de déployer les pods du contrôleur Gateway API ou Ingress sur les nœuds Webservice en tant que DaemonSet. Cela permet aux contrôleurs de s'adapter dynamiquement avec les pods Webservice qu'ils servent et de tirer parti de la bande passante réseau plus élevée que les types de machines plus grandes offrent généralement.
...
Pour plus d'informations sur l'utilisation des ressources par Sidekiq, consultez la documentation Charts sur les [ressources Sidekiq](https://docs.gitlab.com/charts/charts/gitlab/sidekiq/#resources).

### Supporting {#supporting}

Le pool de nœuds Supporting est conçu pour héberger tous les déploiements de support qui ne sont pas nécessaires dans les pools Webservice et Sidekiq.

Ces déploiements de support comprennent divers déploiements liés à l'implémentation du fournisseur cloud et des déploiements GitLab de support tels que [GitLab Shell](https://docs.gitlab.com/charts/charts/gitlab/gitlab-shell/).

Pour tout déploiement supplémentaire tel que Container Registry, Pages ou Monitoring, déployez-les dans le pool de nœuds Supporting dans la mesure du possible et non dans les pools Webservice ou Sidekiq. Le pool de nœuds Supporting a été conçu pour accueillir plusieurs déploiements supplémentaires. Cependant, si vos déploiements ne tiennent pas dans le pool tel que défini, vous pouvez augmenter le pool de nœuds en conséquence. À l'inverse, si le pool est sur-provisionné dans votre cas d'utilisation, vous pouvez le réduire en conséquence.

### Exemple de fichier de configuration {#example-config-file}

Un exemple pour les Charts Helm GitLab ciblant la configuration d'architecture de référence pour 1 000 RPS ou 50 000 utilisateurs [est disponible dans le projet Charts](https://gitlab.com/gitlab-org/charts/gitlab/-/blob/master/examples/ref/50k.yaml).
...
Après avoir suivi ce guide, vous devriez maintenant disposer d'un environnement GitLab neuf avec les fonctionnalités principales configurées en conséquence.
