...
title: Versions de PostgreSQL livrées avec le paquet Linux
---
...
> [!note]
> Ce tableau répertorie uniquement les versions de GitLab où un changement significatif s'est produit dans le paquet concernant les versions de PostgreSQL.

En général, les versions de PostgreSQL changent avec les releases majeures ou mineures de GitLab. Cependant, les versions de correctifs du paquet Linux mettent parfois à jour le niveau de correctif de PostgreSQL. Nous avons établi une cadence annuelle pour les mises à niveau de PostgreSQL et déclenchons des mises à niveau automatiques de la base de données dans la release précédant celle pour laquelle la nouvelle version est requise.

Par exemple :

- Le paquet Linux 12.7.6 était livré avec PostgreSQL 9.6.14 et 10.9.
- Le paquet Linux 12.7.7 était livré avec PostgreSQL 9.6.17 et 10.12.

Découvrez [quelles versions de PostgreSQL (et d'autres composants) sont livrées](https://gitlab-org.gitlab.io/omnibus-gitlab/licenses.html) avec chaque release du paquet Linux.

Les versions minimales de PostgreSQL prises en charge sont répertoriées dans les [exigences d'installation](../../install/requirements.md#postgresql).
...
| -------------- | ------------------- | ---------------------------------- | ---------------------------- | ----- |
| 19.3.0 | 17.10, 18.4 | 17.10 | 17.10 | PostgreSQL 18.4 est disponible en option à la demande pour les nouvelles installations. |
| 19.0.0 | 17.8 | 17.8 | 17.8 | Les mises à niveau du package sont abandonnées si PostgreSQL n'a pas déjà été mis à niveau vers la version 17. |
| 18.11.0 | 16.11, 17.7 | 17.7 | 17.7 | Les nouvelles installations utilisent PostgreSQL 17 par défaut. Sauf en cas de [désactivation](https://docs.gitlab.com/omnibus/settings/database/#opt-out-of-automatic-postgresql-upgrades), les mises à niveau des instances du paquet Linux effectuent automatiquement une mise à niveau vers PostgreSQL 17 pour les nœuds qui ne font pas partie d'un cluster Geo ou HA. |
| 18.4.1, 18.3.3, 18.2.7 | 16.10 | 16.10 | 16.10 | |
| 18.0.0 | 16.8 | 16.8 | 16.8 | Les mises à niveau du paquet sont abandonnées si PostgreSQL n'a pas déjà été mis à niveau vers la version 16. |
| 17.11.0 | 14.17, 16.8 | 16.8 | 16.8 | Les mises à niveau du paquet effectuent automatiquement une mise à niveau vers PostgreSQL 16 pour les nœuds qui ne font pas partie d'un cluster Geo ou HA, sauf [désactivation](https://docs.gitlab.com/omnibus/settings/database/#opt-out-of-automatic-postgresql-upgrades). |
...
| 17.0.0 | 14.11 | 14.11 | 14.11 | Les mises à niveau du paquet sont abandonnées si PostgreSQL n'a pas déjà été mis à niveau vers la version 14. |
...
| 14.0 | 12.7       | 12.7 | 12.7 | Les installations HA avec repmgr ne sont plus prises en charge et ne peuvent pas être mises à niveau vers le paquet Linux 14.0. |
| 13.8 | 11.9, 12.4 | 12.4 | 12.4 | Les mises à niveau du package ont effectué automatiquement une mise à niveau de PostgreSQL pour les nœuds qui ne font pas partie d'un cluster Geo ou HA. |
...
| 13.4 | 11.9, 12.4 | 11.9 | 11.9 | Les mises à niveau du package sont abandonnées si les utilisateurs n'exécutent pas déjà PostgreSQL 11. |
| 13.3 | 11.7, 12.3 | 11.7 | 11.7 | Les mises à niveau du package sont abandonnées si les utilisateurs n'exécutent pas déjà PostgreSQL 11. |
| 13.0 | 11.7 | 11.7 | 11.7 | Les mises à niveau du package sont abandonnées si les utilisateurs n'exécutent pas déjà PostgreSQL 11. |
| 12.10 | 9.6.17, 10.12 et 11.7 | 11.7 | 11.7 | Les mises à niveau du package ont effectué automatiquement une mise à niveau de PostgreSQL pour les nœuds qui ne font pas partie d'un cluster Geo ou repmgr. |
...
| 12.0 | 9.6.11 et 10.7 | 10.7 | 10.7 | Les mises à niveau du package ont effectué automatiquement une mise à niveau de PostgreSQL. |
...
| 9.0 | 9.2.18 et 9.6.1 | 9.6.1 | 9.6.1 | Les mises à niveau du package ont effectué automatiquement une mise à niveau de PostgreSQL. |
