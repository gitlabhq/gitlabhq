...
description: Installer le package Linux sur Debian
title: Installer le package Linux sur Debian
---
...
- Avant de commencer, assurez-vous d'avoir correctement [configuré votre DNS](https://docs.gitlab.com/omnibus/settings/dns/). Remplacez `https://gitlab.example.com` dans les commandes suivantes par votre URL GitLab préférée. GitLab est automatiquement configuré et démarré à cette adresse.
...
- Les packages Linux et autres fichiers de métadonnées associés sont stockés et servis depuis Google Cloud Storage. Si vous utilisez un pare-feu, vous devez autoriser l'accès aux préfixes d'URL suivants :
...
1. Avec `ufw` installé, ouvrez les ports du pare-feu :
...
GitLab génère un mot de passe et une adresse e-mail aléatoires pour le compte administrateur root, stockés dans `/etc/gitlab/initial_root_password` pendant 24 heures. Au bout de 24 heures, ce fichier est automatiquement supprimé pour des raisons de sécurité.
...
Vous pouvez personnaliser votre installation GitLab en définissant les variables d'environnement facultatives suivantes avant l'installation. **Ces variables ne fonctionnent que lors de la première installation** et n'ont aucun effet sur les exécutions ultérieures de reconfigure. Pour les installations existantes, utilisez le mot de passe de `/etc/gitlab/initial_root_password` ou [réinitialisez le mot de passe root](../../security/reset_user_password.md).
...
| `GITLAB_ROOT_EMAIL` | Adresse e-mail personnalisée pour le compte administrateur root | Facultatif | `GITLAB_ROOT_EMAIL="admin@example.com"` |
...
Si GitLab ne parvient pas à détecter un nom d'hôte valide lors de l'installation, reconfigure ne s'exécutera pas automatiquement. Dans ce cas, transmettez les variables d'environnement nécessaires à votre première commande `gitlab-ctl reconfigure`.

> [!warning]
> Bien que vous puissiez également définir le mot de passe initial dans `/etc/gitlab/gitlab.rb` en configurant `gitlab_rails['initial_root_password']`, cela n'est pas recommandé. Cela représente un risque de sécurité, car le mot de passe est en texte clair. Si vous avez configuré cela, assurez-vous de le supprimer après l'installation.
...
Visitez notre [centre de préférences d'abonnement aux e-mails](https://about.gitlab.com/company/preference-center/) pour nous indiquer quand vous contacter. Nous appliquons une politique d'opt-in explicite par e-mail, afin que vous ayez un contrôle total sur ce que nous vous envoyons et à quelle fréquence.
...
> [!note]
> Si vous ne vous abonnez pas à la newsletter de sécurité, vous ne recevrez pas d'alertes de sécurité.

## Prochaines étapes recommandées {#recommended-next-steps}

Une fois votre installation terminée, consultez les [prochaines étapes recommandées, incluant les options d'authentification et les restrictions pour les nouveaux comptes utilisateurs](../next_steps.md).
