...
description: Installer le package Linux sur Amazon Linux 2
title: Installer le package Linux sur Amazon Linux 2
---
...
- Les packages Linux et les autres fichiers de métadonnées associés sont stockés et servis depuis Google Cloud Storage. Si vous utilisez un pare-feu, vous devrez autoriser l'accès aux préfixes d'URL suivants : - `https://packages.gitlab.com/*` - `https://storage.googleapis.com/packages-ops/*`
...
1. Utilisez le script suivant pour ajouter le dépôt GitLab (vous pouvez coller l'URL du script dans votre navigateur pour voir ce qu'il fait avant de le passer à `bash` via un pipe) :
...
Après avoir terminé votre installation, consultez les [prochaines étapes recommandées, notamment les options d'authentification et les restrictions sur les nouveaux comptes utilisateurs](../next_steps.md).
