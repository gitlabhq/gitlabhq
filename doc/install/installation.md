...
edited by hand. You can use any editor you like instead.
...
```bash
$ sudo apt-get install build-essential zlib1g-dev libyaml-dev libssl-dev libgdbm-dev libreadline-dev libncurses5-dev libffi-dev curl git-core openssh-server redis-server postfix checkinstall libxml2-dev libxslt-dev libcurl4-openssl-dev libicu-dev
```
...
Python 2.5+ required (3.x is not supported at the moment).
...
If it is Python 3, you might need to install Python 2 separately:
...
If you get a "command not found" error, create a symlink to the python binary:
...
Edit `config.yml` and replace gitlab_url with your FQDN.
...
We will install GitLab into home directory of the user "git":
...
Throughout this section we will be working inside GitLab's default directory.
Make sure you cd into it:
...
Edit `gitlab.yml`, and where "localhost" change to the fully-qualified domain name of your host serving GitLab:
...
For MySQL run (note, the option says "without"):
...
The init script will be downloaded in `/etc/init.d/gitlab`:
...
Change **YOUR_SERVER_IP** and **YOUR_SERVER_FQDN** to the IP address 
and fully-qualified domain name of your host serving GitLab:
...
If you are running SSH on a non-standard port, you must change the git user's SSH config.
...
for alternative web servers in our [recipes collection](https://github.com/gitlabhq/gitlab-recipes/).
