...
workload sizing. See
[scaling for GitLab Duo Agent Platform](../administration/reference_architectures/_index.md#scaling-for-gitlab-duo-agent-platform)
...
If autoscaling is a requirement, a [cloud-native deployment](../administration/reference_architectures/_index.md#cloud-native) is
generally preferred over VM-based autoscaling groups. Components that must run on a single node,
such as database migrations and [Mailroom](../administration/incoming_email.md), are handled more reliably by
...
straightforward, the [reference architectures](../administration/reference_architectures/_index.md)
...
1. Configure monitoring with [Prometheus](../administration/monitoring/prometheus/_index.md). Reference architecture documentation
...
1. Identify anticipated [additional workloads](../administration/reference_architectures/_index.md#additional-workloads).
1. Assess number of large repositories
1. Incorporate growth projections.
1. Select a reference architecture with
   [appropriate buffer](../administration/reference_architectures/_index.md#if-in-doubt-start-large-monitor-and-then-scale-down).

### Running PromQL queries

Running PromQL queries depends on the monitoring solution you use. As noted in
[Prometheus monitoring documentation](../administration/monitoring/prometheus/_index.md#how-prometheus-works), monitoring data can be
...
1. Consult the [available reference architectures](../administration/reference_architectures/_index.md#available-reference-architectures) to see which reference architecture each traffic
   type suggests.
...
1. Compare your peak and sustained overall RPS against the available sizes:
   ...
   |:------------------|:----------------------------------------------------|:-----------|
   | Small             | [Small (S)](../administration/reference_architectures/cloud_native.md#small-s)               | ≤100 RPS   |
   | Medium            | [Medium (M)](../administration/reference_architectures/cloud_native.md#medium-m)             | ≤200 RPS   |
   | Large             | [Large (L)](../administration/reference_architectures/cloud_native.md#large-l)               | ≤500 RPS   |
   | Extra Large       | [Extra Large (XL)](../administration/reference_architectures/cloud_native.md#extra-large-xl) | ≤1000 RPS  |
...
[high availability section](../administration/reference_architectures/_index.md#high-availability-ha) to identify whether switching to the 60 RPS / 3,000 user
...
|:---------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:--------------------|
| Database-intensive         | <ul><li>Heavy API usage for non-Git traffic (webhooks, issues, groups, and projects)</li><li>Known [extensive automation or integration workloads](../administration/reference_architectures/_index.md#additional-workloads)</li></ul> | <ul><li>Increase Rails resources</li><li>[Database scaling](#database-scaling)</li></ul> |
| Sidekiq/Gitaly-intensive** | <ul><li>Heavy Git operations, CI/CD jobs, security scanning, import operations, and Git server hooks</li><li>Known CI/CD-heavy usage patterns</li></ul>                                      | <ul><li>Increase Sidekiq specifications</li><li>Gitaly vertical scaling</li><li>[Database scaling](#database-scaling)</li><li>Advanced: Configure specific [job classes](../administration/sidekiq/processing_specific_job_classes.md)</li></ul> |
...
1. [Database load balancing](../administration/postgresql/database_load_balancing.md) (recommended) with read replicas, which:
...
| Very High    | >10 Gbps  | Exceeds most standard instance capabilities                        | <ul><li>Network-enhanced VMs required across all providers</li><li>For large artifacts, disable [object proxy download](../administration/object_storage.md#proxy-download)</li></ul> |
...
1. Compare the current environment to [available reference architectures](../administration/reference_architectures/_index.md). Consider the following:
...
- Has no performance issues and current resources > recommended RA:
  - Current environment might be over-provisioned or has valid reasons for additional resources that need to be analysed.
    Check CPU/memory [resource utilization](../administration/monitoring/prometheus/_index.md#sample-prometheus-queries) on Rails,
