...
| `ENABLE_BOOTSNAP`                            | string  | Toggles [Bootsnap](https://github.com/rails/bootsnap) for speeding up initial Rails boot. Enabled by default for non-production environments. Set to `0` to disable.                                                                                                                                                           |
| `EXTERNAL_URL`                               | string  | Specify the external URL at the [time of installation](https://docs.gitlab.com/omnibus/settings/configuration/#specify-the-external-url-at-the-time-of-installation).                                                                                                                                                     |
...
| `GITLAB_ROOT_EMAIL` | string | Sets the email address for the administrator on installation. |
| `GITLAB_ROOT_ORG_NAME` | string | Sets the name of the organization created for the administrator on installation. |
| `GITLAB_ROOT_ORG_PATH` | string | Sets the path of the organization created for the administrator on installation. |
| `GITLAB_ROOT_PASSWORD`                       | string  | Sets the password for the administrator on installation.                                                                                                                                                                                                                                                                           |
| `GITLAB_ROOT_USERNAME` | string | Sets the username for the administrator on installation. |
...
| `GITLAB_CI_CONFIG_GITALY_TIMEOUT_SECONDS`    | integer | Per-request timeout in seconds for Gitaly calls when fetching CI configuration files (local, project, and component includes). Default is `10`.                                                              |
| `GITLAB_CI_CONFIG_HTTP_OPEN_TIMEOUT_SECONDS`  | integer | Per-request open (connection) timeout in seconds for HTTP calls when fetching remote CI configuration files. Must be between `1` and `60`. Default is `10`.                                                                                   |
| `GITLAB_CI_CONFIG_HTTP_READ_TIMEOUT_SECONDS`  | integer | Per-request read timeout in seconds for HTTP calls when fetching remote CI configuration files. Must be between `1` and `60`. Default is `30`.                                                                                                |
...
To set environment variables, follow [these instructions](https://docs.gitlab.com/omnibus/settings/environment-variables/).
