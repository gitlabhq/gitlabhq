...
| `GITLAB_CI_CONFIG_HTTP_OPEN_TIMEOUT_SECONDS`  | integer | Per-request open (connection) timeout in seconds for HTTP calls when fetching remote CI configuration files. Must be between `1` and `60`. Default is `10`. Requires the `ci_config_http_timeout` feature flag.                                                                                   |
| `GITLAB_CI_CONFIG_HTTP_READ_TIMEOUT_SECONDS`  | integer | Per-request read timeout in seconds for HTTP calls when fetching remote CI configuration files. Must be between `1` and `60`. Default is `30`. Requires the `ci_config_http_timeout` feature flag.                                                                                                |
