...
| `_netdev`              | Wait to mount file system until network is online. See also the [`high_availability['mountpoint']`](https://docs.gitlab.com/omnibus/settings/configuration/#start-linux-package-installation-services-only-after-a-given-file-system-is-mounted) option. |
...
provides configuration for [UDP log shipping](https://docs.gitlab.com/omnibus/settings/logs/#udp-log-forwarding).
