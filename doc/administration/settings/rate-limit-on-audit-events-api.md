...
You can configure rate limits for requests to two APIs:
...
Requests over either rate limit return a `429 Too Many Requests` status code and are logged into
the `auth.log` file.

The instance audit events API rate limit applies to both `GET /audit_events` and
`GET /audit_events/:id`. The project audit events API rate limit applies to both
`GET /projects/:id/audit_events` and `GET /projects/:id/audit_events/:audit_event_id`. For example,
if you set a limit of 200, requests to these endpoints that exceed a rate of 200 within one minute
are blocked. Access is restored after one minute.
