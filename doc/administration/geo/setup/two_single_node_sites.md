...
1. Execute the command below to define the site as the primary site:
...
1. Define a password for the database [replication user](https://wiki.postgresql.org/wiki/Streaming_Replication).
   ...
   If you are using an external database not managed by Linux package GitLab, you need
...
1. Define a password for the database [replication user](https://wiki.postgresql.org/wiki/Streaming_Replication).
   ...
   1. Optional. If you use an external database not managed by Linux pacakge, you must
