...
There are 3 ways to authenticate to the Container Registry via GitLab CI/CD:
...
- **Using a personal access token**: You can create and use a
  ...
  in case your project is private. For read (pull) access, the scope
  should be `read_registry`. For read/write (pull/push) access, use `api`.
