...
| `the merge train pipeline could not be prepared: merging commits: merge: there are conflicting files. Conflicts in: ...` | The changes conflict with those in earlier merge requests on the train. The system note names up to 10 of the conflicting files and adds a count of any additional files. | Rebase the source branch on the target branch, resolve the conflicts, then add the merge request to the merge train again. |
