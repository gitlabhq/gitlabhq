...
ID tokens are [JSON web tokens (JWT)](https://www.rfc-editor.org/info/rfc7519/) generated by GitLab CI/CD.
...
| `user_access_level`     | Always                                     | Access level of the user executing the job. |
...
| `ref_path`              | Always                                     | Fully qualified ref for the job. For example, `refs/heads/main`. In a merge request pipeline, this is the source branch ref path. |
| `ref_protected`         | Always                                     | `true` if the Git ref is protected, `false` otherwise. |
| `groups_direct`         | User is a direct member of 0 to 200 groups | The paths of the user's direct membership groups. Omitted if the user is a direct member of more than 200 groups. [Feature flag](../../administration/feature_flags/_index.md) `ci_jwt_groups_direct` [added](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/146881) in GitLab 17.3. Disabled by default. |
...
| `deployment_tier`       | Job specifies an environment               | [Deployment tier](../environments/_index.md#deployment-tier-of-environments) of the environment the job specifies. |
| `environment_action`    | Job specifies an environment               | [Environment action (`environment:action`)](../environments/_index.md) specified in the job. |
| `runner_id`             | Always                                     | ID of the runner executing the job. |
| `runner_environment`    | Always                                     | The type of runner used by the job. Can be either `gitlab-hosted` or `self-hosted`. |
| `sha`                   | Always                                     | The commit SHA for the job. |
| `ci_config_ref_uri`     | Always                                     | The ref path to the top-level pipeline definition, for example, `gitlab.example.com/my-group/my-project//.gitlab-ci.yml@refs/heads/main`. This claim is `null` unless the pipeline definition is located in the same project. |
| `ci_config_sha`         | Always                                     | Git commit SHA for the `ci_config_ref_uri`. This claim is `null` unless the pipeline definition is located in the same project. |
| `project_visibility`    | Always                                     | The [visibility](../../user/public_access.md) of the project where the pipeline is running. Can be `internal`, `private`, or `public`. |
