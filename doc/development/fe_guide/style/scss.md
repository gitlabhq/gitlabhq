...
- [Tailwind CSS config viewer](https://gitlab-org.gitlab.io/gitlab-ui/tailwind-config-viewer/): A visual view of Tailwind CSS classes specific to our design system, like spacing, colors, or sizing. Does not show all available Tailwind CSS classes.
...
> utility classes, note that both the classes for margin and padding differ. The size scale used at
...
The available breakpoints are configured in [tailwind.defaults.js#L361](https://gitlab.com/gitlab-org/gitlab-services/design.gitlab.com/-/blob/769a616ce89d6f7c0e8d7fb6909771c8e2c2db41/packages/gitlab-ui/tailwind.defaults.js#L361)
...
[src/tokens/build/tailwind/tokens.cjs](https://gitlab.com/gitlab-org/gitlab-services/design.gitlab.com/-/blob/769a616ce89d6f7c0e8d7fb6909771c8e2c2db41/packages/gitlab-ui/src/tokens/build/tailwind/tokens.cjs). See <https://design.gitlab.com/tailwind-documentation/margin> for available CSS utility classes.

**Example**: `gl-mt-5` is `margin-top: 1rem;`

##### Color CSS utility classes

Color CSS utility classes (for example, `color` and `background-color`) use colors defined in
[src/tokens/build/tailwind/tokens.cjs](https://gitlab.com/gitlab-org/gitlab-services/design.gitlab.com/-/blob/769a616ce89d6f7c0e8d7fb6909771c8e2c2db41/packages/gitlab-ui/src/tokens/build/tailwind/tokens.cjs).
...
[Arbitrary values](https://v3.tailwindcss.com/docs/adding-custom-styles#using-arbitrary-values) should be avoided for a few reasons:

- They are likely only needed on one page, but are added to the global CSS bundle which increases its size. [Page specific CSS](../create_new_page.md#page-bundle-styles) should be used instead so that CSS is only included where it is needed.
- A pre-defined CSS class likely already exists. Use [GitLab Tailwind CSS documentation](https://design.gitlab.com/tailwind-documentation) to view available Tailwind CSS classes.
- They do not enforce our design system.
...
update [tailwind.defaults.js](https://gitlab.com/gitlab-org/gitlab-services/design.gitlab.com/-/blob/769a616ce89d6f7c0e8d7fb6909771c8e2c2db41/packages/gitlab-ui/tailwind.defaults.js) in GitLab UI.
...
maintain consistency with the design system and keep the CSS bundles small.
...
to the utility-first philosophy. By avoiding combining utility classes with custom styles in one component class
...
Sometimes it might not be feasible to use utility classes directly in HTML and you need to include them in our
...
For CSS properties that are unit-less (for example, `display: flex`) it is okay to use CSS properties directly.
...
Our current Tailwind configuration does not support max-width container queries, so if such a workaround
...
and so might its styling. We need to be able to move these roles and attributes
