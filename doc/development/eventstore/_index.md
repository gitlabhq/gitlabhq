...
```ruby
{
  ...
  'properties' => {
    ...
    'source' => { 'type' => 'string' },
    'id' => { 'type' => 'string' },
    'gitlab_user_id' => { 'type' => 'number' },       # optional
    'gitlab_user_username' => { 'type' => 'string' },  # optional
    'gitlab_organization_id' => { 'type' => 'number' }, # optional
...
1. Outer envelope payload which adheres to the CloudEvents specification. This is generated by the `CloudEvent` class.
   The `gitlab_user_id`, `gitlab_user_username`, and `gitlab_organization_id` fields are optional extension attributes.
   They should be included when a user or organization is associated with the event, but can be omitted for
   system-originated events that have no user actor or organization context.
...
The `current_user` and `organization` parameters of `build_cloud_event` are optional.
For system-originated events that have no user actor or organization context, omit them:

```ruby
build_cloud_event(
  source: "registries/#{registry.id}",
  subject: "artifacts/#{artifact.id}",
  event_data: { image: image, digest: digest }
)
```
...
module MergeRequests
  class ProcessAssignedReviewersWorker
    ...
    def handle_event(event)
      # Access envelope fields (may be nil for system-originated events)
