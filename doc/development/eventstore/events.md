...
| `Ai::DuoWorkflows::WorkflowFailedEvent` | `duo_agent_platform` | EE | Published when a Duo workflow is dropped or stopped. Carries the status event (drop or stop). Only emitted for messaging-triggered workflows. Deferred via run_after_commit. |
