...
If these guidelines differ from the [documentation style guide](styleguide/_index.md),
follow this page for CLI-specific content such as page structure, Synopsis wording,
flag descriptions, and deprecation messages.

For conventions not addressed here, such as capitalization, voice, and general
terminology, follow the documentation style guide.
...
> [!note]
> Short descriptions should use verbs consistent with the command grammar
> defined in [CONTRIBUTING.md](https://gitlab.com/gitlab-org/cli/-/blob/main/CONTRIBUTING.md#grammar).
> For example, use `Create` only for commands that create a single object, and
> `List` only for commands that return more than one object.
...
- Requires a minimum GitLab version. For more information, see [version support](#version-support).
- Is experimental.
...
- Annotate non-obvious examples with an inline comment on the line above the command,
  using `# Comment text` format. An example is non-obvious if it does any of the following:
  - Combines multiple flags in a way that is not clear from the flag descriptions alone.
  - Uses a value format not shown in the Options section. For example, a specific
    date format or ID pattern.
  - Produces a result that depends on context, such as the current branch or
    repository state.
...
Flags appear in the order they are registered in the Go source
(`cmd.Flags()` and `cmd.PersistentFlags()`). Do not manually reorder them in the
...
If a command or flag requires a specific version of `glab` itself instead, document this in the Synopsis using the same pattern.
For example:
...
If a feature has both a `glab` version requirement and a GitLab server version
requirement, state both.
