...
To seed the database with data, the tool uses Data Seeder with [`bulk_data.rb`](https://gitlab.com/gitlab-org/gitlab/-/blob/master/ee/db/seeds/data_seeder/bulk_data.rb)
configuration to seed all factories and uses `db:seed_fu` to seed all [`db/fixtures`](../development_seed_files.md).
The latest dump is generated automatically in scheduled pipelines for the latest patch
...
guidance. It outlines the steps to import the latest PostgreSQL dump
...
If the job is failing on the setup stage prior to `gitlab:db:configure`
