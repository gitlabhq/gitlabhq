...
Sometimes certain bits of code might only work on a certain database
version. While we try to avoid such code as much as possible, sometimes it is
necessary to add version-specific database behavior.
...
You can use this to write code such as:
...
guard, to make sure it doesn't write to read-only databases.

We have a Rails middleware that filters any potentially writing
operations (the `CUD` operations of CRUD) and prevents the user from trying
