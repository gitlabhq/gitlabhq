...
- Prefixes are preferred over suffixes because they make it easier to identify the type of a given constraint quickly, as well as group them alphabetically.
- The `_and_` that joins column names can be omitted to keep the identifiers under the 63-character length limit defined by PostgreSQL. Additionally, the notation might be abbreviated to the best of our ability if struggling to keep under this limit.
- For indexes added to solve a very specific problem, it might make sense for the name to reflect their use.
