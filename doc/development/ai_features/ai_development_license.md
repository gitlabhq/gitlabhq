...
- Run the [GitLab Duo health check](../../administration/gitlab_duo/configure/_index.md#run-a-health-check-for-gitlab-duo) to identify specific issues. Note that if you have GitLab Duo licenses that were generated from a setup script locally, this will show "Cloud Connector access token is missing" but that is OK.
...
- Consider end-user context. Remember that features should work well for both GitLab.com users and GitLab Self-Managed/dedicated customers.
