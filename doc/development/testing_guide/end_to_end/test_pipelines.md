...
The `e2e:test-on-gdk` pipeline is retired and no longer runs.
It ran end-to-end tests against a
[GitLab Development Kit](https://gitlab.com/gitlab-org/gitlab-development-kit) (GDK) instance.

[`e2e:test-on-cng`](#e2etest-on-cng) covers merge requests, and `e2e:test-on-omnibus-ee` covers Linux
package installs.
Both are preferred because they run against production-like environments.
For more information, see
[Stop running E2E tests against GDK](https://gitlab.com/groups/gitlab-org/quality/test-governance/-/work_items/5).

### Setup

This E2E test child pipeline was triggered by the `e2e:test-on-gdk` job using dynamically generated
CI/CD YAML file stored as artifacts in the `e2e-test-pipeline-generate` CI/CD job.
The CI/CD YAML file was generated by using a [template](https://gitlab.com/gitlab-org/gitlab/-/blob/master/.gitlab/ci/test-on-gdk/main.gitlab-ci.yml).

#### child pipeline jobs

The child pipeline consisted of several stages that supported E2E test execution.

##### test

Jobs in the `test` stage performed the following actions:

1. Started an instance of GDK by using the Docker image built by the `build-gdk-image` job.
1. Ran E2E tests against the running instance of GDK.

#### report

This stage was responsible for [allure test report](_index.md#allure-report) generation.
