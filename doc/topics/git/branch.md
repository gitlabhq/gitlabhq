...
- To [rebase](git_rebase.md) your branch against the default branch, run:

  ```shell
  git rebase
  ```

  Use this command when you want your changes to be listed in Git logs after the changes from the default branch.

- To merge the default branch into your branch, run:

  ```shell
  git pull <remote-name> <default-branch-name>
  ```

  Use this command when you want your changes to appear in Git logs in chronological order with the changes from the
  default branch, or if you're sharing your branch with others. If you're unsure of the correct value for
  `<remote-name>`, run: `git remote`.
