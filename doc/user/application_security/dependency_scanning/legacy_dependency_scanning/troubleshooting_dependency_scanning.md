...
Support for [2to3](https://docs.python.org/3.12/library/2to3.html)
...
If using a `Pipfile.lock` generated by [`pipenv`](https://pipenv.pypa.io/en/latest/), run `pipenv verify` to verify that the lockfile contains the latest package hashes.
...
## Vulnerability scanning produces no results for custom or merged CycloneDX SBOMs

The dependency scanning CI/CD job succeeds and SBOM components appear in the dependency list,
but no vulnerabilities are reported in the pipeline security tab.
...
- Multiple SBOMs are merged using `cyclonedx merge`, which strips metadata properties.
- A third-party SBOM generator does not include GitLab-specific properties.
- The `gitlab:meta:schema_version` property (must be `1`) is missing from `metadata.properties`.

### Required properties for vulnerability scanning

| Property | Location | Description |
|---|---|---|
| `gitlab:meta:schema_version` | `metadata.properties` | Must be set to `1`. |
| `gitlab:dependency_scanning:input_file:path` | `metadata.properties` or each component's `properties` | Path to the lockfile analyzed to produce the dependency. If neither is present, no vulnerability findings are produced for those components. On GitLab 18.10 and later, an error is displayed in the pipeline security tab. |

To resolve this problem, choose one of the following approaches:
...
- Add properties to third-party SBOMs.
  ...
  SBOMs generated by third-party tools typically
  don't include GitLab-specific properties. To enable vulnerability scanning, ensure your
  SBOM includes the following in `metadata.properties`:

  - `gitlab:meta:schema_version` set to `1`
  - `gitlab:dependency_scanning:input_file:path` set to the repository-relative path
    of the lockfile (for example, `package-lock.json` or `src/Gemfile.lock`)
...
## Vulnerability scanning shows wrong input file for all dependencies

All dependencies in the vulnerability report or dependency list show the same input file path,
even though they originate from different lockfiles.
...
top-level metadata. The `input_file:path` property is especially important for merged SBOMs containing components
