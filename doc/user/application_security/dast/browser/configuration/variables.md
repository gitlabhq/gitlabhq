...
| `DAST_CRAWL_GROUPED_URLS` | string | `https://example.com/hello/*,https://example.com/world/*/details` | Set to a comma-separated list of wildcard URL patterns with at least one `*`. To reduce scan time, the scanner groups and analyzes only one matched URL per pattern. |
