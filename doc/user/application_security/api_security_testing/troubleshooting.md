...
- In GitLab 15.5 and earlier, `Error waiting for DAST API 'http://127.0.0.1:5000' to become available`.
...
- In GitLab 13.10 and earlier, `API Security version header not found. Are you sure that you are connecting to the API security testing server?`.

**Solution**

- Remove the `DAST_API` variable from the `.gitlab-ci.yml` file. The value inherits from the API security testing CI/CD template. We recommend this method instead of manually setting a value.
