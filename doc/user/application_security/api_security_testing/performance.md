...
If the API security testing testing job still takes longer than expected reach after following the advice in this performance guide, reach out to support for further assistance.
...
- [Exclude this operation](#excluding-slow-operations) from the API security testing test. While this is the simplest, it has the downside of a gap in security test coverage.
- [Exclude the operation from feature branch API security testing tests, but include it in the default branch test](#excluding-operations-in-feature-branches-but-not-default-branch).
- [Split up the API security testing testing into multiple jobs](#splitting-a-test-into-multiple-jobs).
