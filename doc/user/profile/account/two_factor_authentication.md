...
You can use Duo as a one-time password (OTP) provider in GitLab.

#### Prerequisites

You must:

- Exist in both Duo and GitLab, with the same username in both applications.
- Have configured Duo and have an integration key, secret key, and API hostname for Duo. Access this information in the [Duo Admin Panel](https://admin.duosecurity.com/).
