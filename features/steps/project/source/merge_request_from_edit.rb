class Spinach::Features::ProjectSourceMergeRequestFromEdit < Spinach::FeatureSteps
  ...
  step 'I uncheck "Create merge request"' do
  ...
  step 'I should be redirected to the new merge request page '\
       'from origin to itself' do
  ...
  step 'I should be redirected to the new merge request page '\
       'from origin to fork' do
    page.should have_content(
      "From #{@user.fork_of(@project).namespace.path}:"\
      "#{valid_new_branch_name} "\
      "into #{@project.namespace.path}:#{@project.default_branch}"
    )
  end

  step 'I should be redirected to the new merge request page '\
       'from fork to itself' do
    page.should have_content(
      "From #{@user.fork_of(@project).namespace.path}:"\
      "#{valid_new_branch_name} "\
      "into #{@user.fork_of(@project).namespace.path}:"\
      "#{@project.default_branch}"
    )
  end

  step 'I see the "New branch name" input' do
    expect(page).to have_field(:new_branch_name, visible: true)
  end

  step 'I don\'t see the "New branch name" input' do
    expect(page).not_to have_field(:new_branch_name, visible: true)
  end

  step 'I see the "On my fork" checkbox' do
    expect(page).to have_field(:on_my_fork, visible: true)
  end

  step 'I don\'t see the "On my fork" checkbox' do
    expect(page).not_to have_field(:on_my_fork, visible: true)
  end

  step 'I see the current project path as the source project' do
    expect(page).to have_css('#source_project', text: @project.namespace.path,
                                                visible: true)
  end

  step "I see my fork's path as the source project" do
    expect(page).to have_css('#source_project', text: @user.namespace.path,
                                                visible: true)
  end

  step 'I see the current branch name as the source branch' do
    expect(page).to have_css('#source_branch', text: default_branch_name,
                                               visible: true)
  end

  step 'I don\'t see the current branch name as the source branch' do
    expect(page).not_to have_css('#source_branch', text: default_branch_name,
                                                   visible: true)
  end
