...
class Spinach::Features::ProjectSourceBrowseFiles < Spinach::FeatureSteps
  ...
  step 'I should see files from repository for "6d39438"' do
    expect(current_path).to eq namespace_project_tree_path(*@project, "6d39438")
    expect(page).to have_content ".gitignore"
    expect(page).to have_content "LICENSE"
  end
  ...
  step 'I am redirected to the files URL' do
    expect(current_path).to eq namespace_project_tree_path(*@project, 'master')
  end

  step 'I am redirected to the ".gitignore"' do
    expect(current_path).to eq(namespace_project_blob_path(*@project, 'master/.gitignore'))
  end

  step 'I am redirected to the permalink URL' do
    expect(current_path).to(
      eq(namespace_project_blob_path(*@project,
  ...
  step 'I am redirected to the new file' do
    expect(current_path).to eq(
      namespace_project_blob_path(*@project, 'master/' + new_file_name))
  end

  step 'I am redirected to the new file with directory' do
    expect(current_path).to eq(
      namespace_project_blob_path(*@project, 'master/' + new_file_name_with_directory))
  end
  ...
  step "I am redirected to the fork's new merge request page" do
    ...
    expect(current_path).to eq(new_namespace_project_merge_request_path(*fork))
  end

  step 'I am redirected to the root directory' do
    expect(current_path).to eq(
      namespace_project_tree_path(*@project, 'master'))
  end
  ...
  step "I visit the 'test' tree" do
    visit namespace_project_tree_path(*@project, "'test'")
  end

  step "I visit the fix tree" do
    visit namespace_project_tree_path(*@project, "fix/.testdir")
  end
  ...
  step 'I click on "files/lfs/lfs_object.iso" file in repo' do
    ...
    visit namespace_project_tree_path(*@project, "lfs")
    click_link 'files'
    click_link "lfs"
  ...
  step 'I visit the SVG file' do
    visit namespace_project_blob_path(*@project, 'new_branch_name/logo_sample.svg')
  end
