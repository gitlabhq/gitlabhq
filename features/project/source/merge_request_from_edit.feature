Feature: Project Source Merge Request From Edit
  ...
  # Only a single test for new file since it is well factored with file edit.
  @javascript
  Scenario: I can create a merge request on new file from the current project to itself
    Given I visit the new file page
  ...
  @javascript
  Scenario: The source project shown gets updated when the user interacts with the form
    Given I visit ".gitignore" file edit page
    Then I see the current project path as the source project
    Given I check "Create merge request"
    Then I see my fork's path as the source project
    Given I uncheck "On my fork"
    Then I see the current project path as the source project
    Given I uncheck "Create merge request"
    Then I see the current project path as the source project
    Given I check "Create merge request"
    Then I see the current project path as the source project
    Given I check "On my fork"
    Then I see my fork's path as the source project
    Given I uncheck "Create merge request"
    Then I see the current project path as the source project

  @javascript
  Scenario: If I don't click on "Create merge request", then I don't see the merge request inputs
    Given I visit ".gitignore" file edit page
    Then I don't see the "New branch name" input
    Then I don't see the "On my fork" checkbox
    Then I see the current branch name as the source branch
    Given I check "Create merge request"
    Then I see the "New branch name" input
    Then I see the "On my fork" checkbox
    Then I don't see the current branch name as the source branch
