...
RSpec.describe BackfillProjectIntegrationsOrganizationId, feature_category: :integrations do
  ...
  let(:integration_to_backfill) do
    integrations.create!(project_id: project.id, type_new: 'Integrations::Zentao', organization_id: organization.id)
  end

  let(:another_integration_to_backfill) do
    integrations.create!(project_id: project.id, type_new: 'Integrations::Telegram', organization_id: organization.id)
  end
  ...
  describe "#up" do
    it 'sets organization_id to nil for group integrations that have it' do
      expect(integration_to_backfill.project_id).to eq(project.id)
      expect(integration_to_backfill.organization_id).to eq(organization.id)

      expect(another_integration_to_backfill.project_id).to eq(project.id)
      expect(another_integration_to_backfill.organization_id).to eq(organization.id)

      expect(valid_integration.project_id).to eq(project.id)
      expect(valid_integration.organization_id).to be_nil

      expect(group_integration.group_id).to eq(group.id)
      expect(group_integration.organization_id).to be_nil
      ...
      expect(integration_to_backfill.reload.project_id).to eq(project.id)
      expect(integration_to_backfill.reload.organization_id).to be_nil

      expect(another_integration_to_backfill.reload.project_id).to eq(project.id)
      expect(another_integration_to_backfill.reload.organization_id).to be_nil

      expect(valid_integration.reload.project_id).to eq(project.id)
      expect(valid_integration.reload.organization_id).to be_nil

      expect(group_integration.reload.group_id).to eq(group.id)
      expect(group_integration.reload.organization_id).to be_nil
