...
RSpec.describe AddShardingKeyTriggerOnClusterProvidersAws, feature_category: :deployment_management do
  let(:organizations) { table(:organizations) }
  let(:namespaces) { table(:namespaces) }
  let(:projects) { table(:projects) }
  let(:clusters) { table(:clusters) }
  let(:cluster_providers_aws) { table(:cluster_providers_aws) }
  ...
  describe '#up' do
    ...
    context 'when inserting new records' do
      it 'sets sharding key columns from the parent clusters table' do
        provider1 = cluster_providers_aws.create!(cluster_id: cluster_with_organization_id.id, key_name: 'key1',
          organization_id: nil, group_id: nil, project_id: nil, num_nodes: 0, status: 0, role_arn: 0, region: 0,
          vpc_id: 'a', security_group_id: 'b', instance_type: 'c')
        provider2 = cluster_providers_aws.create!(cluster_id: cluster_with_group_id.id, key_name: 'key2',
          organization_id: nil, group_id: nil, project_id: nil, num_nodes: 0, status: 0, role_arn: 0, region: 0,
          vpc_id: 'a', security_group_id: 'b', instance_type: 'c')
        provider3 = cluster_providers_aws.create!(cluster_id: cluster_with_project_id.id, key_name: 'key3',
          organization_id: nil, group_id: nil, project_id: nil, num_nodes: 0, status: 0, role_arn: 0, region: 0,
          vpc_id: 'a', security_group_id: 'b', instance_type: 'c')
    ...
    context 'when updating existing records' do
      it 'sets sharding key columns when updating a record without them' do
        ActiveRecord::Base.connection.execute(
          <<~SQL
            DROP TRIGGER IF EXISTS trigger_cluster_providers_aws_sharding_key ON cluster_providers_aws;

            ALTER TABLE cluster_providers_aws DROP CONSTRAINT IF EXISTS check_fca4a4cb60;
          SQL
        )

        provider1 = cluster_providers_aws.create!(cluster_id: cluster_with_organization_id.id,
          key_name: 'key1', organization_id: nil, group_id: nil, project_id: nil, num_nodes: 0,
          status: 0, role_arn: 0, region: 0, vpc_id: 'a', security_group_id: 'b', instance_type: 'c')
        provider1.reload
        ...
        provider2 = cluster_providers_aws.create!(cluster_id: cluster_with_group_id.id, key_name: 'key2',
          organization_id: nil, group_id: nil, project_id: nil, num_nodes: 0, status: 0, role_arn: 0,
          region: 0, vpc_id: 'a', security_group_id: 'b', instance_type: 'c')
        provider2.reload
        expect(provider2.organization_id).to be_nil
        ...
        ActiveRecord::Base.connection.execute(
          <<~SQL
            ALTER TABLE cluster_providers_aws ADD CONSTRAINT check_fca4a4cb60 CHECK (num_nonnulls(group_id, organization_id, project_id) = 1) NOT VALID;
        ...
        provider1.update!(key_name: 'key1_updated')
        provider1.reload
        ...
        # Update the record to trigger the function
        provider2.update!(key_name: 'key2_updated')
        provider2.reload
        ...
        # Update the record to trigger the function
        provider3.update!(key_name: 'key3_updated')
        provider3.reload
