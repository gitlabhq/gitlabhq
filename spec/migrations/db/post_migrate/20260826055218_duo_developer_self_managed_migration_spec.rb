...
RSpec.describe DuoDeveloperSelfManagedMigration,
  migration: :gitlab_main_cell_setting,
  feature_category: :ai_abstraction_layer,
  migration_version: 20260826055218 do
  ...
  describe '#up' do
    context 'when a record with feature=16 exists' do
      let!(:source_record) do
        instance_feature_settings.create!(
          ...
          model_allowlist_gitlab_model_refs: ['claude_sonnet_4_20250514_vertex'],
          created_at: 1.day.ago,
          updated_at: 1.hour.ago
        )
      end

      it 'creates a new record with feature=26' do
      ...
      it 'copies all attributes except feature and timestamps' do
        ...
        expect(new_record).to have_attributes(
          ...
          model_allowlist_enabled: source_record.model_allowlist_enabled,
          model_allowlist_gitlab_model_refs: source_record.model_allowlist_gitlab_model_refs
        )
    ...
    context 'when source record has model allowlist attributes set' do
      let!(:source_record) do
        instance_feature_settings.create!(
          feature: described_class::SOURCE_FEATURE,
          offered_model_ref: 'claude_sonnet_4_20250514_vertex',
          offered_model_name: 'Claude Sonnet 4',
          model_allowlist_enabled: true,
          model_allowlist_gitlab_model_refs: %w[claude_sonnet_4_20250514_vertex claude_sonnet_3_7_20250219]
        )
      end

      it 'copies the model allowlist attributes correctly' do
        migration.up

        new_record = instance_feature_settings.find_by(feature: described_class::TARGET_FEATURE)
        expect(new_record.model_allowlist_enabled).to be(true)
        expect(new_record.model_allowlist_gitlab_model_refs).to eq(
          %w[claude_sonnet_4_20250514_vertex claude_sonnet_3_7_20250219]
        )
      end
    end
  end
  ...
  describe '#down' do
    ...
    it 'deletes the record with feature=26' do
