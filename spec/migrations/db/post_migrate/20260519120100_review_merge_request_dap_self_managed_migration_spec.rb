...
RSpec.describe ReviewMergeRequestDapSelfManagedMigration,
  migration: :gitlab_main_cell_setting,
  feature_category: :ai_abstraction_layer,
  migration_version: 20260519120100 do
  let(:migration) { described_class.new }
  let(:instance_feature_settings) { table(:instance_model_selection_feature_settings) }

  describe '#up' do
    context 'when a record with feature=15 exists' do
      let!(:source_record) do
        instance_feature_settings.create!(
          feature: described_class::SOURCE_FEATURE,
          offered_model_ref: 'claude_sonnet_4_20250514_vertex',
          offered_model_name: 'Claude Sonnet 4',
          created_at: 1.day.ago,
          updated_at: 1.hour.ago
        )
      end

      it 'creates a new record with feature=21' do
    ...
    context 'when no record with feature=15 exists' do
  ...
  describe '#down' do
    ...
    it 'deletes the record with feature=21' do
