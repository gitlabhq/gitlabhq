...
RSpec.describe TrimViolatingInstanceStreamingDestinations, migration: :gitlab_main_org, feature_category: :audit_events do
  ...
  before do
    ActiveRecord::Base.connection.execute(<<~SQL)
      ALTER TABLE audit_events_instance_external_streaming_destinations
      DROP CONSTRAINT IF EXISTS check_audit_event_streams_instance_secret_token_max_length
    SQL
  end
  ...
  describe '#up' do
    context 'when secret_token is too long (exceeds PLAINTEXT_LIMIT)' do
      let!(:long_token_record) do
        record = described_class::InstanceExternalStreamingDestination.new(
          category: 1,
      ...
      it 'trims the secret_token to PLAINTEXT_LIMIT' do
        ...
        reloaded = described_class::InstanceExternalStreamingDestination.find(long_token_record.id)
      ...
      context 'when the name with prefix would exceed MAX_NAME_LENGTH' do
        ...
        it 'truncates the prefixed name to MAX_NAME_LENGTH' do
          ...
          reloaded = described_class::InstanceExternalStreamingDestination.find(long_name_record.id)
    ...
    context 'when multiple records violate in the same batch' do
      ...
      it 'updates all violating records in a single batch' do
        ...
        violating_records.each do |original|
          reloaded = described_class::InstanceExternalStreamingDestination.find(original.id)
          expect(reloaded.secret_token.length).to eq(4096)
          expect(reloaded.name).to start_with('[INVALID]')
          expect(reloaded.active).to be false
        end
    ...
    context 'when secret_token is under PLAINTEXT_LIMIT' do
      ...
      it 'does not modify the secret_token' do
        original_token = described_class::InstanceExternalStreamingDestination.find(short_token_record.id).secret_token

        migrate!

        reloaded = described_class::InstanceExternalStreamingDestination.find(short_token_record.id)
    ...
    context 'when migration is run multiple times' do
      ...
      it 'is idempotent' do
        ...
        first_run_state = {
          name: described_class::InstanceExternalStreamingDestination.find(violating_record.id).name,
          active: described_class::InstanceExternalStreamingDestination.find(violating_record.id).active,
          token_length: described_class::InstanceExternalStreamingDestination.find(violating_record.id)
            .secret_token.length
        }
