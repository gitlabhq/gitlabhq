...
RSpec.describe RequeueMigrateProjectAuthorizations, migration: :gitlab_main, feature_category: :user_management do
    ...
    reversible_migration do |migration|
      ...
      migration.after -> {
        expect(batched_migration).to have_scheduled_batched_migration(
          ...
          column_name: :user_id,
          min_cursor: [user_a.id, project.id, access_level],
  ...
  context 'when a record from the previous run exists' do
    it 'deletes it before scheduling the new batched migration' do
      Gitlab::Database::BackgroundMigration::BatchedMigration.create!(
        ...
        column_name: :user_id,
        min_cursor: [0, 0, 0],
        max_cursor: [user_a.id, project.id, access_level],
        interval: batch_min_delay,
        batch_class_name: batch_class_name,
        batch_size: batch_size,
        sub_batch_size: sub_batch_size,
      ...
      expect(
        Gitlab::Database::BackgroundMigration::BatchedMigration
          .where(job_class_name: described_class::MIGRATION).count
      ).to eq(1)
