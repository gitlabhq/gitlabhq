...
RSpec.describe BackfillProjectJiraTrackerDataOrganizationId, feature_category: :integrations do
  ...
  let(:tracker_data_to_backfill) do
    jira_tracker_data.create!(project_id: project.id, integration_id: integration.id, organization_id: organization.id)
  end
  ...
  let(:group_tracker_data) do
    group_integration = integrations.create!(group_id: group.id, type_new: 'Integrations::Jira')
    jira_tracker_data.create!(group_id: group.id, integration_id: group_integration.id)
  end
  ...
  describe "#up" do
    it 'sets organization_id to nil for project jira_tracker_data that have it' do
