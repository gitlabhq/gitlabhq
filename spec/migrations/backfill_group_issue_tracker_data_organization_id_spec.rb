...
RSpec.describe BackfillGroupIssueTrackerDataOrganizationId, feature_category: :team_planning do
  ...
  let(:tracker_data_to_backfill) do
    issue_tracker_data.create!(group_id: group.id, integration_id: integration.id, organization_id: organization.id)
  end
  ...
  let(:valid_tracker_data) do
    issue_tracker_data.create!(group_id: group.id, integration_id: integration.id)
  end
  ...
  describe "#up" do
    it 'sets organization_id to nil for group issue_tracker_data that have it' do
