...
RSpec.describe QueueUpdateDuoSastFpDetectionEnabledToFalse, migration: :gitlab_main, feature_category: :vulnerability_management do
  let(:migration) { described_class.new }

  describe '#up' do
    it 'queues the background migration' do
      expect(migration).to receive(:queue_batched_background_migration).with(
        'UpdateDuoSastFpDetectionEnabledToFalse',
        :project_settings,
        :project_id,
        batch_size: 50_000,
