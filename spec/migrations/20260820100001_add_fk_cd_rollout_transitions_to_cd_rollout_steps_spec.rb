...
RSpec.describe AddFkCdRolloutTransitionsToCdRolloutSteps, migration: :gitlab_main,
  feature_category: :continuous_delivery do
  let(:connection) { described_class.new.connection }
  let(:table_name) { :cd_rollout_transitions }
  ...
  describe '#up', :aggregate_failures do
    ...
    it 'nullifies rollout_step_id on delete, rather than cascading' do
      ...
      # The journal is append-only audit history: deleting the step a gate
      # was opened for must not delete the gate's own journal row.
