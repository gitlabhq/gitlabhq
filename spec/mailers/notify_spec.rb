...
describe Notify do
  ...
  context 'for a project' do
        ...
        describe 'that are new' do
          ...
          it 'has the correct subject and body' do
            aggregate_failures do
              ...
              is_expected.to have_body_text(namespace_project_issue_path(*project, issue))
            end
          end
        ...
        describe 'that are new' do
          ...
          it 'has the correct subject and body' do
            aggregate_failures do
              ...
              is_expected.to have_body_text(namespace_project_merge_request_path(*project, merge_request))
    ...
    describe 'project access requested' do
      context 'for a project in a user namespace' do
        ...
        it 'contains all the useful information' do
          ...
          is_expected.to have_html_escaped_body_text project.name_with_namespace
          is_expected.to have_body_text namespace_project_project_members_url(*project)
    ...
    context 'items that are noteable, the email for a note' do
      ...
      describe 'on a merge request' do
        ...
        let(:note_on_merge_request_path) { namespace_project_merge_request_path(*project, merge_request, anchor: "note_#{note.id}") }
      ...
      describe 'on an issue' do
        ...
        let(:note_on_issue_path) { namespace_project_issue_path(*project, issue, anchor: "note_#{note.id}") }
  ...
  describe 'email on push for a created branch' do
    let(:example_site_path) { root_path }
    let(:user) { create(:user) }
    let(:tree_path) { namespace_project_tree_path(*project, "empty-branch") }
  ...
  describe 'email on push for a created tag' do
    let(:example_site_path) { root_path }
    let(:user) { create(:user) }
    let(:tree_path) { namespace_project_tree_path(*project, "v1.0") }
  ...
  describe 'email on push with multiple commits' do
    ...
    let(:diff_path) { namespace_project_compare_path(*project, from: Commit.new(compare.base, project), to: Commit.new(compare.head, project)) }
  ...
  describe 'email on push with a single commit' do
    ...
    let(:compare) { Compare.decorate(raw_compare, project) }
    let(:commits) { compare.commits }
    let(:diff_path) { namespace_project_commit_path(*project, commits.first) }
    let(:diff_refs) { Gitlab::Diff::DiffRefs.new(base_sha: project.merge_base_commit(sample_image_commit.id, sample_commit.id).id, head_sha: sample_commit.id) }
