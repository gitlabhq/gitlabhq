...
RSpec.describe 'AiHarness::Doctor integration', :aggregate_failures, feature_category: :tooling do
  ...
  describe 'forbidden files check' do
    ...
    it 'allows files under the .claude/skills/split-mr/ directory committed' do
      setup_valid_repo
      add_tracked_file('.claude/skills/split-mr/SKILL.md')
    ...
    it 'allows files under the .claude/skills/ai-principles-review-feedback/ directory committed' do
      setup_valid_repo
      add_tracked_file('.claude/skills/ai-principles-review-feedback/SKILL.md')

      result = run_doctor

      expect(result[:exit_code]).to eq(0)
      expect(result[:stdout]).not_to include('.claude/skills/ai-principles-review-feedback/')
    end
  ...
  describe 'error cases' do
    it 'reports multiple failures and all three checks run' do
      write_file('AGENTS.md', "# Instructions\nRead .ai/missing.md")
  ...
  describe '--fix mode' do
    it 'fixes gitignore issues with exit code 0' do
