...
RSpec.describe Tooling::Danger::PreventColumnAdditionSuggestion, feature_category: :tooling do
  ...
  let(:filename) { 'db/migrate/20260000000000_add_column_to_users.rb' }
  ...
  context 'when cop is disabled without a URL' do
    let(:changed_lines) { ["+# rubocop:disable Migration/PreventAddingColumns"] }

    it 'adds a suggestion comment' do
      expect(suggestion).to receive(:markdown).with(
        a_string_including('Migration/PreventAddingColumns'),
  ...
  context 'when cop is disabled with a comment but no URL' do
    let(:changed_lines) { ["+# rubocop:disable Migration/PreventAddingColumns -- We need this column"] }
  ...
  context 'when cop is marked as todo without a URL' do
    let(:changed_lines) { ["+# rubocop:todo Migration/PreventAddingColumns"] }
  ...
  context 'when multiple cops are disabled without a URL' do
    let(:changed_lines) { ["+# rubocop:disable Migration/PreventAddingColumns, Migration/AnotherCop"] }
  ...
  context 'when multiple cops are disabled with a valid URL' do
    let(:changed_lines) do
      ["+# rubocop:disable Migration/PreventAddingColumns, Migration/AnotherCop -- https://gitlab.com/gitlab-org/database-team/team-tasks/-/work_items/789"]
    end
  ...
  context 'when file does not contain Migration/PreventAddingColumns disable' do
