...
RSpec.describe Tooling::Danger::AiLogging, feature_category: :service_ping do
  ...
  let(:modified_files) { ['app/services/ai_service.rb'] }
  let(:file_content) { '@logger = Gitlab::Llm::Logger.build; @logger.info("Some AI log")' }
  ...
  before do
    ...
    allow(fake_helper).to receive(:markdown_list).and_return("app/services/ai_service.rb")
  end
  ...
  describe '#check_ai_logging' do
    ...
    context 'when there is appropriate AI logging with expanded_ai_logging feature flag' do
      let(:modified_files) { ['app/services/ai_service.rb'] }
      let(:file_content) { '@logger = Gitlab::Llm::Logger.build; @logger.info("Some AI log with expanded_ai_logging")' }
