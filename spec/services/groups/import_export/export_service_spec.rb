...
RSpec.describe Groups::ImportExport::ExportService, feature_category: :importers do
  ...
  describe '#execute' do
    ...
    it 'compresses and removes tmp files' do
      ...
      expect(Dir.exist?(shared.archive_path)).to be false
      expect(File.exist?(group.import_export_upload_by_user(user).export_file.path)).to be true
    end
    ...
    context 'when saver succeeds' do
      it 'saves the group in the file system' do
        ...
        expect(File.directory?(archive_path)).to be(false)
