# frozen_string_literal: true

require 'spec_helper'

RSpec.describe Namespaces::UnlinkProjectForksService, feature_category: :source_code_management do
  include ProjectForksHelper

  let_it_be(:user) { create(:user) }
  let_it_be(:group) { create(:group) }
  let_it_be(:source_project) { create(:project, :repository) }

  let_it_be(:fork_project1) do
    source_project.add_developer(user)
    fork_project(source_project, user, { repository: true, namespace: group })
  end

  let_it_be(:fork_project2) { fork_project(source_project, user, { repository: true, namespace: group }) }
  let_it_be(:non_forked_project) { create(:project, namespace: group) }

  subject(:service) { described_class.new(group, user) }

  describe '#execute' do
    it 'unlinks fork relationships for projects in the group' do
      expect(fork_project1.forked?).to be_truthy
      expect(fork_project2.forked?).to be_truthy

      service.execute

      expect(fork_project1.reload.forked?).to be_falsey
      expect(fork_project2.reload.forked?).to be_falsey
    end

    it 'does not affect non-forked projects' do
      expect(non_forked_project.forked?).to be_falsey

      service.execute

      expect(non_forked_project.reload.forked?).to be_falsey
    end

    it 'calls UnlinkForkService for each forked project in the group' do
      expect(Projects::UnlinkForkService).to receive(:new).with(fork_project1, user).and_call_original
      expect(Projects::UnlinkForkService).to receive(:new).with(fork_project2, user).and_call_original
      expect(Projects::UnlinkForkService).not_to receive(:new).with(non_forked_project, user)

      service.execute
    end

    context 'when group has no projects' do
      let(:empty_group) { create(:group) }

      subject(:service) { described_class.new(empty_group, user) }

      it 'does not call UnlinkForkService' do
        expect(Projects::UnlinkForkService).not_to receive(:new)

        service.execute
      end
    end

    context 'with subgroups' do
      let_it_be(:subgroup) { create(:group, parent: group) }
      let_it_be(:fork_subgroup_project) do
        fork_project(source_project, user, { repository: true, namespace: subgroup })
      end

      it 'calls UnlinkForkService for projects in subgroups too' do
        expect(Projects::UnlinkForkService).to receive(:new).with(fork_project1, user).and_call_original
        expect(Projects::UnlinkForkService).to receive(:new).with(fork_project2, user).and_call_original
        expect(Projects::UnlinkForkService).to receive(:new).with(fork_subgroup_project, user).and_call_original

        service.execute
      end
    end
  end
end
