...
RSpec.describe Authn::IamService::AcceptConsentChallengeService, feature_category: :system_access do
  ...
  let(:challenge) { 'a' * 64 }
  let(:granted_scope) { %w[openid profile] }
  let(:redirect_url) { "#{iam_service_url}/oauth2/authorize?consent_verifier=#{'b' * 64}" }

  let(:service) do
    described_class.new(challenge: challenge, user: user, granted_scope: granted_scope)
  end
  ...
  describe '#execute' do
    ...
    context 'when the IAM service accepts the consent' do
