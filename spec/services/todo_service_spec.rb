...
RSpec.describe TodoService, feature_category: :notifications do
  ...
  let_it_be_with_reload(:author) { create(:user, developer_of: project) }
  let_it_be_with_reload(:assignee) { create(:user, developer_of: project) }
  let_it_be_with_reload(:non_member) { create(:user) }
  let_it_be_with_reload(:member) { create(:user, developer_of: project) }
  let_it_be_with_reload(:guest) { create(:user, guest_of: project) }
  let_it_be_with_reload(:admin) { create(:admin) }
  let_it_be_with_reload(:john_doe) { create(:user, developer_of: project) }
  let_it_be_with_reload(:skipped) { create(:user, developer_of: project) }
    ...
    describe '#new_note' do
      ...
      context 'commits' do
        ...
        context 'leaving a note on a commit in a public project with private code' do
          let_it_be_with_reload(:project) { create(:project, :repository, :public, :repository_private, guests: guest, developers: [author, assignee, member, john_doe, skipped]) }
        ...
        context 'leaving a note on a commit in a private project' do
          let_it_be_with_reload(:project) { create(:project, :repository, :private, guests: guest, developers: [author, assignee, member, john_doe, skipped]) }
  ...
  describe 'Merge Requests' do
    ...
    describe '#ssh_key_expiring_soon' do
      let_it_be_with_reload(:ssh_key) { create(:key, user: author) }
      ...
      context 'when given an array of keys' do
        let_it_be_with_reload(:ssh_key_of_member) { create(:key, user: member) }
        let_it_be_with_reload(:ssh_key_of_guest) { create(:key, user: guest) }
    ...
    describe '#ssh_key_expired' do
      ...
      describe 'auto-resolve behavior' do
        let_it_be_with_reload(:ssh_key_2) { create(:key, user: author) }
        let_it_be_with_reload(:todo_for_expiring_key_1) { create(:todo, target: ssh_key, action: Todo::SSH_KEY_EXPIRING_SOON, user: author) }
        let_it_be_with_reload(:todo_for_expiring_key_2) { create(:todo, target: ssh_key_2, action: Todo::SSH_KEY_EXPIRING_SOON, user: author) }
    ...
    describe '#new_note' do
      let_it_be_with_reload(:project) { create(:project, :repository, guests: guest, developers: [author, assignee, member, john_doe, skipped]) }
  ...
  describe '#resolve_access_request_todos' do
    ...
    let_it_be_with_reload(:group_requester) { create(:group_member, :access_request, group: group, user: assignee) }
    let_it_be_with_reload(:project_requester) { create(:project_member, :access_request, project: project, user: non_member) }
    let_it_be_with_reload(:another_pending_todo) { create(:todo, state: :pending, user: john_doe) }
    # access request by another user
    let_it_be_with_reload(:another_group_todo) do
      create(:todo, state: :pending, target: group, action: Todo::MEMBER_ACCESS_REQUESTED)
    end

    let_it_be_with_reload(:another_project_todo) do
  ...
  describe '#create_member_access_request_todos' do
    ...
    context 'when request is raised for group' do
      it_behaves_like 'member access request is raised' do
        ...
        let_it_be_with_reload(:requester1) { create(:group_member, :access_request, group: source, user: assignee) }
        let_it_be_with_reload(:requester2) { create(:group_member, :access_request, group: source, user: non_member) }
      end
    end
  ...
  describe 'composite identity attribution', :request_store do
    ...
    let_it_be_with_reload(:assignee_user) { create(:user, developer_of: project) }
