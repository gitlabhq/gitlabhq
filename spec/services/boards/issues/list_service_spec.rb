...
describe Boards::Issues::ListService, services: true do
  describe '#execute' do
    ...
    let!(:closed_issue5) { create(:labeled_issue, :closed, project: project, title: 'Closed 5', labels: [development]) }
    ...
    it 'delegates search to IssuesFinder' do
      params = { id: list1.id, state: 'opened' }
    ...
    context 'sets default order to priority' do
      ...
      it 'returns closed issues that have label list applied when listing issues from a label list' do
        params = { id: list1.id, state: 'closed' }

        issues = described_class.new(project, user, params).execute

        expect(issues).to eq [closed_issue4, closed_issue5]
      end

      it 'returns opened/closed issues that have label list applied when listing issues from a label list' do
        params = { id: list1.id, state: 'all' }

        issues = described_class.new(project, user, params).execute

        expect(issues).to eq [closed_issue4, list1_issue3, list1_issue1, closed_issue5, list1_issue2]
      end
    end
  end
end
