...
describe 'gitlab:app namespace rake task' do
  ...
  def reenable_backup_sub_tasks
    %w{db repo uploads builds artifacts lfs composer}.each do |subtask|
  ...
  describe 'backup_restore' do
    ...
    context 'gitlab version' do
      ...
      it 'should invoke restoration on match' do
        ...
        expect(Rake::Task["gitlab:backup:composer:restore"]).to receive(:invoke)
  ...
  describe 'backup_create' do
    ...
    it 'should set correct permissions on the tar contents' do
      tar_contents, exit_status = Gitlab::Popen.popen(
        %W{tar -tvf #{@backup_tar} db uploads.tar.gz repositories builds.tar.gz artifacts.tar.gz lfs.tar.gz composer.tar.gz}
      )
      ...
      expect(tar_contents).to match('composer.tar.gz')
      expect(tar_contents).not_to match(/^.{4,9}[rwx].* (database.sql.gz|uploads.tar.gz|repositories|builds.tar.gz|artifacts.tar.gz|composer.tar.gz)\/$/)
    end

    it 'should delete temp directories' do
      temp_dirs = Dir.glob(
        File.join(Gitlab.config.backup.path, '{db,repositories,uploads,builds,artifacts,lfs,composer}')
      )
