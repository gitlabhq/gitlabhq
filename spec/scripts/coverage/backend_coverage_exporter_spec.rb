...
require 'tempfile'
require_relative '../../../scripts/coverage/backend_coverage_exporter'

RSpec.describe BackendCoverageExporter, feature_category: :tooling do
  ...
  before do
    ...
    stub_const('BackendCoverageExporter::COVERAGE_REPORT', File.join(temp_dir, 'coverage.lcov'))
    stub_const('BackendCoverageExporter::TEST_MAP', File.join(temp_dir, 'merged-mapping.json.gz'))
    stub_const('BackendCoverageExporter::RSPEC_REPORTS_GLOB', File.join(temp_dir, 'rspec-*.json'))
    stub_const('BackendCoverageExporter::E2E_REPORTS_GLOB', File.join(temp_dir, 'e2e-*.json'))
  end
  ...
  def create_coverage_report
    File.write(File.join(temp_dir, 'coverage.lcov'), 'SF:app/models/user.rb')
  end

  def create_test_map
    File.write(File.join(temp_dir, 'merged-mapping.json.gz'), '{}')
  end

  def create_rspec_report(name = 'rspec-1.json')
  ...
  describe '#run' do
    ...
    context 'when only E2E reports exist (no RSpec reports)' do
      before do
        create_coverage_report
        create_test_map
        create_e2e_report
  ...
  describe '#print_summary' do
    context 'when all artifacts exist' do
      before do
        ...
        create_test_map
        create_rspec_report
        create_rspec_report('rspec-2.json')
        create_e2e_report
      end
      ...
      it 'prints status with checkmarks' do
        ...
        expect(exporter).to have_received(:puts).with(/RSpec test reports: 2 files/)
