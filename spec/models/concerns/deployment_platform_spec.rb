...
describe DeploymentPlatform do
  ...
  describe '#deployment_cluster' do
    subject { project.deployment_cluster }
    ...
    context 'with no Kubernetes configuration on CI/CD, no Kubernetes Service and a Kubernetes template configured' do
      ...
      it { is_expected.to be_a_kind_of(Clusters::Cluster) }
      ...
      it 'includes appropriate attributes for Cluster' do
        expect(subject.name).to eq('kubernetes-template')
        expect(subject.project).to eq(project)
        expect(subject.provider_type).to eq('user')
        expect(subject.platform_type).to eq('kubernetes')
      end
    end
