...
describe Issue, "Issuable" do
  ...
  describe "#sort" do
    ...
    context "by milestone due date" do
      ...
      it "sorts desc" do
        issues = project.issues.sort2('milestone_due_desc')
        expect(issues).to match_array([issue2, issue1, issue, issue3])
      end

      it "sorts asc" do
        issues = project.issues.sort2('milestone_due_asc')
    ...
    context 'when all of the results are level on the sort key' do
      ...
      it 'has no duplicates across pages' do
        sorted_issue_ids = 1.upto(10).map do |i|
          project.issues.sort2('milestone_due_desc').page(i).per(1).first.id
        end
