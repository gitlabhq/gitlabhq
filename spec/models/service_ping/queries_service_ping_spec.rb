...
RSpec.describe ServicePing::QueriesServicePing, feature_category: :service_ping do
  ...
  describe 'scopes' do
    describe '.for_current_reporting_cycle' do
      ...
      before_all do
        create(:queries_service_ping, created_at: (described_class::REPORTING_CADENCE + 1.day).ago)
      end
      ...
      context 'with records matching filtering criteria' do
        let_it_be(:fresh_record) { create(:queries_service_ping) }
        let_it_be(:record_at_edge_of_time_range) do
          create(:queries_service_ping, created_at: described_class::REPORTING_CADENCE.ago)
        end
  ...
  describe 'validations' do
    ...
    describe 'uniqueness validation' do
      let!(:existing_record) { create(:queries_service_ping) }
