...
RSpec.describe LooseForeignKeys::OrganizationDeletedRecord, feature_category: :database do
  ...
  it_behaves_like 'a loose foreign key record that includes DeletedRecordConcern class methods' do
    let_it_be(:deleted_record_1) do
      described_class.create!(
        ...
        organization_id: organization.id, consume_after: 3.hours.ago
      )
    end
    ...
    let_it_be(:deleted_record_2) do
      described_class.create!(
        ...
        organization_id: organization.id, consume_after: 2.hours.ago
      )
    end
    ...
    let_it_be(:deleted_record_3) do
      described_class.create!(
        ...
        organization_id: organization.id, consume_after: 1.hour.ago
      )
    end
    ...
    let_it_be(:deleted_record_4) do
      described_class.create!(
        ...
        organization_id: organization.id, consume_after: Time.current
      )
    end
  end

  it_behaves_like 'a loose foreign key record that includes DeletedRecordConcern sliding list partitioning' do
    let(:sharding_key) { { organization_id: organization.id } }
  end
end
