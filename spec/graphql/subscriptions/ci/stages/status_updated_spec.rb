...
RSpec.describe Subscriptions::Ci::Stages::StatusUpdated, feature_category: :continuous_integration do
  include GraphqlHelpers

  it { expect(described_class).to have_graphql_arguments(:stage_id) }
  it { expect(described_class.payload_type).to eq(Types::Ci::StageType) }

  describe '#resolve' do
    let_it_be(:project) { create(:project, :private) }
    let_it_be(:pipeline) { create(:ci_pipeline, project: project) }
    let_it_be(:stage) { create(:ci_stage, pipeline: pipeline, project: project, name: 'build') }
    ...
    context 'with subscription updates' do
      let(:resolver) do
        resolver_instance(described_class, obj: stage, ctx: query_context, subscription_update: true)
      end

      it 'returns the stage' do
        expect(subscription).to eq(stage)
      end

      context 'when stage belongs to a different subscription' do
        let(:other_stage) { create(:ci_stage, pipeline: pipeline, project: project, name: 'test') }
        let(:stage_id) { other_stage.to_gid }
