...
RSpec.describe GitlabSchema.types['MlCandidate'], feature_category: :mlops do
  let_it_be_with_reload(:model_version) { create(:ml_model_versions, :with_package) }
  let_it_be(:project) { model_version.project }
  let_it_be(:current_user) { project.owner }
  let_it_be_with_reload(:pipeline) { create(:ci_pipeline, project: project, user: current_user) }
  let_it_be_with_reload(:candidate) do
