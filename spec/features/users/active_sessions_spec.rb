...
RSpec.describe 'Active user sessions', :clean_gitlab_redis_sessions, feature_category: :system_access do
  ...
  it 'successful login cleans up obsolete entries', :js do
    ...
    Gitlab::Redis::Sessions.with do |redis|
      expect(redis.smembers("session:lookup:user:gitlab:#{user.id}"))
        .not_to include '59822c7d9fcdfa03725eff41782ad97d'
    end
  end

  it 'sessionless login does not clean up obsolete entries', :js do
    ...
    expect(page).to have_field 'Username or primary email'
