...
feature 'Resolving all open discussions in a merge request from an issue', feature: true, js: true do
  ...
  describe 'as a user with access to the project' do
    ...
    it 'shows a button to resolve all discussions by creating a new issue' do
      within('#resolve-count-app') do
        expect(page).to have_link "Resolve all discussions in new issue", href: new_namespace_project_issue_path(*project, merge_request_to_resolve_discussions_of: merge_request.iid)
      end
    end
    ...
    context 'resolving the discussion' do
      ...
      it 'hides the link for creating a new issue' do
        expect(page).not_to have_link "Resolve all discussions in new issue", href: new_namespace_project_issue_path(*project, merge_request_to_resolve_discussions_of: merge_request.iid)
      end
    end

    context 'creating an issue for discussions' do
      before do
        click_link "Resolve all discussions in new issue", href: new_namespace_project_issue_path(*project, merge_request_to_resolve_discussions_of: merge_request.iid)
      end
      ...
      context 'merge request has discussions that need to be resolved' do
        ...
        it 'has a link to resolve all discussions by creating an issue' do
          page.within '.mr-widget-body' do
            expect(page).to have_link 'Create an issue to resolve them later', href: new_namespace_project_issue_path(*project, merge_request_to_resolve_discussions_of: merge_request.iid)
          end
        end

        context 'creating an issue for discussions' do
          before do
            page.click_link 'Create an issue to resolve them later', href: new_namespace_project_issue_path(*project, merge_request_to_resolve_discussions_of: merge_request.iid)
          end
  ...
  describe 'as a reporter' do
    before do
      ...
      gitlab_sign_in user
      visit new_namespace_project_issue_path(*project, merge_request_to_resolve_discussions_of: merge_request.iid)
    end
