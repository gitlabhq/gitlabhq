...
feature 'Create Branch/Merge Request Dropdown on issue page', feature: true, js: true do
  ...
  context 'for team members' do
    ...
    it 'allows creating a merge request from the issue page' do
      ...
      visit namespace_project_merge_request_path(*project, MergeRequest.first)

      expect(page).to have_content('WIP: Resolve "Cherry-Coloured Funk"')
      expect(current_path).to eq(namespace_project_merge_request_path(*project, MergeRequest.first))
    end
    ...
    it 'allows creating a branch from the issue page' do
      ...
      expect(current_path).to eq namespace_project_tree_path(*project, '1-cherry-coloured-funk')
    end
