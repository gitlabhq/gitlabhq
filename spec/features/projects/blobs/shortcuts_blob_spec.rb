...
feature 'Blob shortcuts', feature: true do
  ...
  describe 'On a file(blob)', js: true do
    ...
    def visit_blob(fragment = nil)
      visit namespace_project_blob_path(*project, tree_join('master', path), anchor: fragment)
    end
    ...
    describe 'pressing "y"' do
      it 'redirects to permalink with commit sha' do
        ...
        expect(page).to have_current_path(get_absolute_url(namespace_project_blob_path(*project, tree_join(sha, path))), url: true)
      end
      ...
      it 'maintains fragment hash when redirecting' do
        ...
        expect(page).to have_current_path(get_absolute_url(namespace_project_blob_path(*project, tree_join(sha, path), anchor: fragment)), url: true)
      end
    end
  end
end
