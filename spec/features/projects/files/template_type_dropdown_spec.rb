...
feature 'Template type dropdown selector', js: true do
  ...
  context 'creating a matching file' do
    before do
      visit namespace_project_new_blob_path(*project, 'master', file_name: '.gitignore')
    end
  ...
  context 'creating a file' do
    before do
      visit namespace_project_new_blob_path(*project, project.default_branch)
    end
...
def create_and_edit_file(file_name)
  visit namespace_project_new_blob_path(*project, 'master', file_name: file_name)
  click_button "Commit changes"
  visit namespace_project_edit_blob_path(*project, File.join(project.default_branch, file_name))
end
