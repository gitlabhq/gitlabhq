...
feature 'Integration settings', feature: true do
  ...
  let(:role) { :developer }
  let(:integrations_path) { namespace_project_settings_integrations_path(*project) }
  ...
  context 'for master' do
    ...
    context 'Webhook logs' do
      ...
      scenario 'show list of hook logs' do
        hook_log
        visit edit_namespace_project_hook_path(*project, hook)
      ...
      scenario 'retry hook log' do
        ...
        expect(current_path).to eq(edit_namespace_project_hook_path(*project, hook))
      end
    end
  end
end
