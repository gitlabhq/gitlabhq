...
feature 'Merge Request button', feature: true do
  ...
  context 'on branches page' do
    it_behaves_like 'Merge request button only shown when allowed' do
      ...
      let(:url) { namespace_project_branches_path(*project, search: 'feature') }
      let(:fork_url) { namespace_project_branches_path(*forked_project, search: 'feature') }
    end
  end
  ...
  context 'on compare page' do
    it_behaves_like 'Merge request button only shown when allowed' do
      ...
      let(:url) { namespace_project_compare_path(*project, from: 'master', to: 'feature') }
      let(:fork_url) { namespace_project_compare_path(*forked_project, from: 'master', to: 'feature') }
    end
  end
  ...
  context 'on commits page' do
    it_behaves_like 'Merge request button only shown when allowed' do
      let(:label) { 'Create merge request' }
      let(:url) { namespace_project_commits_path(*project, 'feature') }
      let(:fork_url) { namespace_project_commits_path(*forked_project, 'feature') }
    end
  end
end
