...
feature 'Create New Merge Request', feature: true, js: true do
  ...
  context 'when target project cannot be viewed by the current user' do
    it 'does not leak the private project name & namespace' do
      ...
      visit new_namespace_project_merge_request_path(*project, merge_request: { target_project_id: private_project.id })
  ...
  context 'when source project cannot be viewed by the current user' do
    it 'does not leak the private project name & namespace' do
      ...
      visit new_namespace_project_merge_request_path(*project, merge_request: { source_project_id: private_project.id })
  ...
  it 'populates source branch button' do
    visit new_namespace_project_merge_request_path(*project, change_branches: true, merge_request: { target_branch: 'master', source_branch: 'fix' })

    expect(find('.js-source-branch')).to have_content('fix')
  end

  it 'allows to change the diff view' do
    visit new_namespace_project_merge_request_path(*project, merge_request: { target_branch: 'master', source_branch: 'fix' })
  ...
  it 'does not allow non-existing branches' do
    visit new_namespace_project_merge_request_path(*project, merge_request: { target_branch: 'non-exist-target', source_branch: 'non-exist-source' })
  ...
  context 'when a branch contains commits that both delete and add the same image' do
    it 'renders the diff successfully' do
      visit new_namespace_project_merge_request_path(*project, merge_request: { target_branch: 'master', source_branch: 'deleted-image-test' })
  ...
  it 'does not show error messages on initial form' do
    visit new_namespace_project_merge_request_path(*project)
