...
feature 'Merge requests > User posts diff notes', :js do
  ...
  context 'when hovering over a parallel view diff file' do
    before do
      visit diffs_namespace_project_merge_request_path(*project, merge_request, view: 'parallel')
    end
  ...
  context 'when hovering over an inline view diff file' do
    before do
      visit diffs_namespace_project_merge_request_path(*project, merge_request, view: 'inline')
    end
    ...
    context 'when hovering over a diff discussion' do
      before do
        visit diffs_namespace_project_merge_request_path(*project, merge_request, view: 'inline')
        should_allow_commenting(find('[id="2f6fcd96b88b36ce98c38da085c795a27d92a3dd_7_7"]'))
        visit namespace_project_merge_request_path(*project, merge_request)
      end
