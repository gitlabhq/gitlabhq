...
RSpec.describe 'Merge Requests Feed', feature_category: :devops_reports do
  describe 'GET /merge_requests' do
    ...
    let_it_be_with_reload(:assignee) { create(:user, email: 'private2@example.com') }
    let_it_be(:group) { create(:group) }
    let_it_be_with_reload(:project) { create(:project, :repository) }
    let_it_be_with_reload(:merge_request) { create(:merge_request, source_project: project, assignees: [assignee]) }
