...
feature 'User creates snippet', :js do
  ...
  scenario 'previews a snippet with file' do
    ...
    page.within('#new_personal_snippet .md-preview') do
      ...
      expect(link).to match(%r{/uploads/-/system/temp/\h{32}/banana_sample\.gif\?w=365\&h=360\z})
  ...
  scenario 'uploads a file when dragging into textarea' do
    ...
    link = find('a.no-attachment-icon img[alt="banana_sample"]')['src']
    expect(link).to match(%r{/uploads/-/system/personal_snippet/#{Snippet.last.id}/\h{32}/banana_sample\.gif\?w=365\&h=360\z})
