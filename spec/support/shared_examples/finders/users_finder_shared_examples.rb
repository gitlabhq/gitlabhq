...
RSpec.shared_examples 'executes users finder as normal user' do
  include_context 'with project bot and service account users'

  it 'returns searchable users' do
    users = described_class.new(user).execute

    expect(users).to contain_exactly(user, normal_user, external_user, unconfirmed_user, omniauth_user, internal_user,
      admin_user, project_bot, service_account_user)
  end
  ...
  it 'filters by non external users' do
    ...
    expect(users).to contain_exactly(user, normal_user, unconfirmed_user, omniauth_user, internal_user, admin_user,
      project_bot, service_account_user)
  end
  ...
  it 'filters by active users' do
    ...
    expect(users).to contain_exactly(user, normal_user, unconfirmed_user, external_user, admin_user, omniauth_user,
      project_bot, service_account_user)
  end
  ...
  it 'filters by created_at' do
    ...
    users = described_class.new(
      ...
      created_before: Time.current + 2.days
    ).execute
  ...
  it 'filters by non internal users' do
    ...
    expect(users).to contain_exactly(user, normal_user, unconfirmed_user, external_user, omniauth_user, admin_user,
      project_bot, service_account_user)
  end
  ...
  it 'orders returned results' do
    ...
    expected = [normal_user, admin_user, external_user, unconfirmed_user, omniauth_user, internal_user, project_bot,
      service_account_user, user].sort_by(&:id)
    expect(users).to eq(expected)
  end

  it 'does not filter by admins' do
    users = described_class.new(user, admins: true).execute
    expect(users).to contain_exactly(user, normal_user, external_user, admin_user, unconfirmed_user, omniauth_user,
      internal_user, project_bot, service_account_user)
  end
...
RSpec.shared_examples 'executes users finder as admin' do |user_is_admin: true|
  include_context 'with project bot and service account users'
  ...
  it 'returns all users' do
    ...
    expect(users).to contain_exactly(user, normal_user, blocked_user, unconfirmed_user, banned_user, external_user,
      omniauth_user, internal_user, admin_user, project_bot, service_account_user)
  end
  ...
  it 'returns only admins' do
    ...
    expected = user_is_admin ? [user, admin_user] : [admin_user]
    expect(users).to match_array(expected)
  end
