...
RSpec.shared_examples_for 'a ci_finished_pipelines aggregation model' do |table_name|
  ...
  let(:has_finished_at_optimization) { table_name == :ci_finished_pipelines }

  let(:finished_at_clause_builder) do
    ->(time) do
      next "" unless has_finished_at_optimization

      " AND `#{table_name}`.`finished_at` >= toDateTime64('#{time.utc.strftime('%Y-%m-%d %H:%M:%S')}', 6, 'UTC')"
    end
  end
  ...
  describe '#within_dates' do
    ...
    it 'builds the correct SQL' do
      expected_sql = <<~SQL.lines(chomp: true).join(' ')
        ...
        AND `#{table_name}`.`#{started_at_field}` < toDateTime64('#{to_time.utc.strftime('%Y-%m-%d %H:%M:%S')}', 6, 'UTC')#{finished_at_clause_builder.call(from_time)}
      SQL
    ...
    context 'when only from_date is passed' do
      ...
      it 'builds the correct SQL' do
        expected_sql = <<~SQL.lines(chomp: true).join(' ')
          SELECT * FROM `#{table_name}`
          WHERE `#{table_name}`.`#{started_at_field}` >= toDateTime64('#{from_time.utc.strftime('%Y-%m-%d %H:%M:%S')}', 6, 'UTC')#{finished_at_clause_builder.call(from_time)}
        SQL
