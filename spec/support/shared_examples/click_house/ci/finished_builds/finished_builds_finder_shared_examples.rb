...
RSpec.shared_context 'with percentile test data' do
  let!(:additional_builds) do
    (1..20).map do |i|
      build_stubbed(:ci_build, :success,
        ...
        finished_at: base_time + ((i * 100.0) / 1000.0))
    end
  end
...
RSpec.shared_examples 'finished builds finder select behavior' do
  describe '#select' do
...
RSpec.shared_examples 'finished builds finder aggregations' do
  describe '#select_aggregations' do
    ...
    context 'with invalid aggregations' do
      ...
      it 'raises ArgumentError' do
        expect do
          result
        end.to raise_error(ArgumentError, /Cannot aggregate columns:.*invalid_aggregation/)
      end
    end
  end
...
RSpec.shared_examples 'finished builds finder ordering' do
...
RSpec.shared_examples 'finished builds finder filters' do
  ...
  describe '#filter_by_job_name' do
    ...
    context 'with exact match' do
      ...
      it 'returns builds with matching name' do
        expect(filter_by_job_name.size).to eq(5)
...
RSpec.shared_examples 'finished builds finder method chaining' do
  ...
  describe 'query examples' do
    it 'finds top 10 slowest builds by p95 duration' do
      ...
      expect(result.first['name']).to eq('compile-slow')
    end
...
RSpec.shared_examples 'finished builds finder execution' do
  describe '#execute' do
  ...
  describe '#to_sql' do
    it 'generates correct SQL for inspection' do
      ...
      expect(sql).to include('ci_finished_builds').or include('finished_builds')
...
RSpec.shared_examples 'percentile duration calculation' do |percentile, method_name, expected_value:|
  subject(:result) do
    instance.for_project(project.id)
      .select(:name)
