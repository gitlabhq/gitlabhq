...
RSpec.shared_examples 'every metric definition' do
  ...
  before do
    ...
    allow(::Gitlab::Usage::Metrics::Instrumentations::BaseMetric).to receive(:available?).and_return(true)
    ...
    def assert_uses_all_nested_classes(parent_module)
      parent_module.constants(false).each do |const_name|
        ...
        when Class
          ...
          allow(metric_class_instance).to receive_messages(available?: true, value: -1)
