...
RSpec.shared_examples 'Composer package index' do |member_role:, expected_status:, package_returned:|
  include_context 'Composer user type', member_role: member_role do
    let_it_be(:expected_packages, freeze: false) { package_returned ? [package] : [] }
    let_it_be(:presenter, freeze: false) { ::Packages::Composer::PackagesPresenter.new(group, expected_packages) }

    it_behaves_like 'Composer package index with version', 'public_api/v4/packages/composer/index', expected_status

    context 'with version 2' do
      let_it_be(:presenter, freeze: false) { ::Packages::Composer::PackagesPresenter.new(group, expected_packages, true) }
