...
RSpec.shared_context 'container registry auth service context' do
  let_it_be_with_reload(:rsa_key) { OpenSSL::PKey::RSA.generate(3072) }
...
RSpec.shared_examples 'a container registry auth service' do
  ...
  context 'user authorization' do
    ...
    context 'for internal project with container_registry `enabled`' do
      let_it_be_with_reload(:project) { create(:project, :internal, :container_registry_enabled) }
      ...
      context 'for external user' do
        context 'disallow anyone to pull or push images' do
          let_it_be_with_reload(:current_user) { create(:user, external: true) }
  ...
  context 'build authorized as user' do
    let_it_be_with_reload(:current_project) { create(:project) }
    let_it_be_with_reload(:current_user) { create(:user) }
    ...
    context 'for other projects' do
      context 'when pulling' do
        ...
        shared_examples 'pullable for being team member' do
          ...
          context 'when you are owner' do
            let_it_be_with_reload(:project) { create(:project, namespace: current_user.namespace) }
      ...
      context 'when pushing' do
        ...
        context 'disallow for all' do
          ...
          context 'when you are owner' do
            let_it_be_with_reload(:project) { create(:project, :public, namespace: current_user.namespace) }
    ...
    context 'for project that disables repository' do
      let_it_be_with_reload(:project) { create(:project, :public, :repository_disabled) }
  ...
  context 'registry catalog browsing authorized as admin' do
    let_it_be_with_reload(:current_user) { create(:user, :admin) }
    let_it_be_with_reload(:project) { create(:project, :public) }
  ...
  context 'support for multiple scopes' do
    let_it_be_with_reload(:internal_project) { create(:project, :internal) }
    let_it_be_with_reload(:private_project) { create(:project, :private) }
    let_it_be_with_reload(:public_project) { create(:project, :public) }
    let_it_be_with_reload(:public_project_private_container_registry) do
      create(:project, :public, :container_registry_private)
    end
  ...
  context 'for deploy tokens' do
    ...
    context 'when deploy token has read and write registry as scopes' do
      ...
      context 'for public project' do
        let_it_be_with_reload(:project) { create(:project, :public) }
        let_it_be_with_reload(:deploy_token) { create(:deploy_token, write_registry: true, projects: [project]) }
    ...
    context 'when deploy token does not have read_registry scope' do
      ...
      context 'for public project with container registry `enabled`' do
        let_it_be_with_reload(:project) { create(:project, :public, :container_registry_enabled) }
        let_it_be_with_reload(:deploy_token) { create(:deploy_token, projects: [project], read_registry: false) }
    ...
    context 'when deploy token is not related to the project' do
      let_it_be_with_reload(:deploy_token) { create(:deploy_token, read_registry: false) }
    ...
    context 'when deploy token has been revoked' do
      context 'for public project with container registry `enabled`' do
        let_it_be_with_reload(:project) { create(:project, :public, :container_registry_enabled) }
        let_it_be_with_reload(:deploy_token) { create(:deploy_token, :revoked, projects: [project]) }
  ...
  context 'when the deploy token is restricted with external_authorization' do
    context 'when the authenticator is a regular user' do
      ...
      let_it_be_with_reload(:project) { create(:project, :private, :container_registry_enabled) }
  ...
  context 'with a project with a path containing special characters' do
    let_it_be_with_reload(:bad_project) { create(:project) }
  ...
  context 'with container registry protection rules' do
    ...
    let_it_be_with_reload(:current_project) { create(:project) }
    let_it_be_with_reload(:project) { current_project }

    let_it_be_with_reload(:container_repository_path) { current_project.full_path }
    let_it_be_with_reload(:container_repository_path_pattern_no_match) { "#{container_repository_path}_no_match" }
    ...
    let_it_be_with_reload(:project_developer) { create(:user, developer_of: current_project) }
    let_it_be_with_reload(:project_maintainer) { create(:user, maintainer_of: current_project) }
    let_it_be_with_reload(:project_owner) { current_project.owner }
    let_it_be_with_reload(:instance_admin) { create(:admin) }
    ...
    context 'with different scopes and actions' do
      let_it_be_with_reload(:current_user) { project_maintainer }
    ...
    context 'for deploy tokens' do
      let_it_be_with_reload(:deploy_token) do
        create(:deploy_token, write_registry: true, projects: [current_project])
      end
  ...
  context 'with protected tags' do
    ...
    let_it_be_with_reload(:rules) do
      [
  ...
  context 'with organization maintenance mode enforcement' do
    ...
    let_it_be_with_reload(:current_user) { create(:user, maintainer_of: project) }
