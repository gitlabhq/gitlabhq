...
module DependencyProxyHelpers
  ...
  def build_jwt(user_or_token = nil, expire_time: nil, service_type: nil)
    JSONWebToken::HMACToken.new(::Auth::ContainerProxyAuthenticationService.secret).tap do |jwt|
      if block_given?
        yield(jwt)
      else
        ...
        jwt['service_type'] = service_type if service_type
