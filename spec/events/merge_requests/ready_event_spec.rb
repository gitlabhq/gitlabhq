...
require 'spec_helper'
require_relative '../../../app/events/merge_requests/ready_event'
...
RSpec.describe MergeRequests::ReadyEvent, feature_category: :code_suggestions do
  let_it_be(:user) { create(:user) }
  let_it_be(:merge_request) { create(:merge_request) }

  describe '.build' do
    let(:event) do
      described_class.build(merge_request: merge_request, current_user: user)
    end

    it_behaves_like 'a merge request base cloud event'

    it 'sets event_type to :ready' do
      expect(event.event_type).to eq(:ready)
    end
  end
