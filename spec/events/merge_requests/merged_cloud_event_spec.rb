...
require 'spec_helper'
require_relative '../../../app/events/merge_requests/merged_cloud_event'
...
RSpec.describe MergeRequests::MergedCloudEvent, feature_category: :code_suggestions do
  ...
  describe '.build' do
    ...
    it 'sets event_type to :merged' do
      expect(event.event_type).to eq(:merged)
    end

    it 'sets the acting user on the event' do
      expect(event.current_user).to eq(user)
    end
  end
