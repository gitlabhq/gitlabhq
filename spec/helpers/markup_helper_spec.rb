  ...
  describe "#markdown" do
    describe "referencing multiple objects" do
      ...
      it "links to the merge request" do
        expected = namespace_project_merge_request_path(*project, merge_request)
        expect(helper.markdown(actual)).to match(expected)
      end

      it "links to the commit" do
        expected = namespace_project_commit_path(*project, commit)
        expect(helper.markdown(actual)).to match(expected)
      end

      it "links to the issue" do
        expected = namespace_project_issue_path(*project, issue)
        expect(helper.markdown(actual)).to match(expected)
      end
    end
    ...
    describe "override default project" do
      ...
      it 'links to the issue' do
        expected = namespace_project_issue_path(*second_project, second_issue)
  ...
  describe '#link_to_gfm' do
    ...
    it 'handles references nested in links with all the text' do
      ...
      expect(doc.css('a')[1].attr('href'))
        .to eq namespace_project_issue_path(*project, issues[0])
      ...
      expect(doc.css('a')[3].attr('href'))
        .to eq namespace_project_issue_path(*project, issues[1])
