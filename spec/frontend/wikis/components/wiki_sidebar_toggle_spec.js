...
describe('WikiSidebarToggle', () => {
  ...
  describe.each`
    action     | icon                        | buttonCategory | title              | cssClass
    ${'open'}  | ${'sidebar'}                | ${'secondary'} | ${'Open sidebar'}  | ${'toggle-action-open'}
    ${'close'} | ${'chevron-double-lg-left'} | ${'tertiary'}  | ${'Close sidebar'} | ${'toggle-action-close'}
  `('when action=$action', ({ action, icon, title, buttonCategory, cssClass }) => {
    beforeEach(() => {
