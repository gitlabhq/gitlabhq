  ...
  describe('toggleResolveNote', () => {
    ...
    describe('as discussion', () => {
      ...
      describe('auto-collapse', () => {
        let legacyDiffsStore;
        let mockToggleLineDiscussions;

        beforeEach(() => {
          legacyDiffsStore = useLegacyDiffs();
          mockToggleLineDiscussions = jest.fn();
          legacyDiffsStore.toggleLineDiscussions = mockToggleLineDiscussions;
        });

        it('collapses line discussions when all discussions on the line are resolved', () => {
          const localRes = {
            resolved: true,
            expanded: true,
            discussion_id: '1',
          ...
          axiosMock.reset();
          axiosMock.onAny().reply(HTTP_STATUS_OK, localRes);
          axiosMock.onPut(`${TEST_HOST}/discussions/1/resolve`).replyOnce(200, localRes);

          return testAction(
            store.toggleResolveNote,
            { endpoint: `${TEST_HOST}/discussions/1/resolve`, isResolved: false, discussion: true },
            {
              discussions: [
                {
                  resolved: true,
            ...
            expect(mockToggleLineDiscussions).toHaveBeenCalledWith({
              lineCode: 'abc123',
              fileHash: 'file123',
              expanded: false,
            });
        ...
        it('does not collapse when there are unresolved discussions on the same line', () => {
          return testAction(
            store.toggleResolveNote,
            { endpoint: `${TEST_HOST}`, isResolved: false, discussion: true },
            {
              discussions: [
                {
                  resolved: true,
        ...
        it('does not collapse when unresolving a discussion', () => {
          return testAction(
            store.toggleResolveNote,
            { endpoint: `${TEST_HOST}`, isResolved: true, discussion: true },
            {
        ...
        it('does not collapse when discussion parameter is false', () => {
          return testAction(
            store.toggleResolveNote,
            { endpoint: `${TEST_HOST}`, isResolved: false, discussion: false },
            {
              discussions: [
                { resolved: true, discussion_id: 1, id: 1, individual_note: true, notes: [] },
              ],
            },
            [
              {
