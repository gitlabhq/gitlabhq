...
describe('AreaChartPresenter', () => {
  let wrapper;

  const createComponent = (props = {}) => {
    wrapper = shallowMountExtended(AreaChartPresenter, {
      propsData: {
  ...
  const findSingleDim = () => wrapper.findComponent(SingleDimensionSeriesChart);
  const findTwoDim = () => wrapper.findComponent(TwoDimensionsAreaChart);
  ...
  describe('routing', () => {
    it('routes to the single-dimension series chart for 1 dimension', () => {
      createComponent();
    ...
    it('forwards the dimension and metrics to the series chart with the area variant', () => {
  ...
  describe('validation', () => {
    ...
    it('emits error when there are no dimensions', () => {
      ...
      expect(findEmittedErrorMessage()).toBe('areaChart requires at least one dimension');
      expect(findSingleDim().exists()).toBe(false);
      expect(findTwoDim().exists()).toBe(false);
    });
    ...
    it('emits error when there are more than 2 dimensions', () => {
      ...
      expect(findEmittedErrorMessage()).toBe('areaChart supports a maximum of 2 dimensions');
    });
    ...
    it('emits error when there are no metrics', () => {
      ...
      expect(findEmittedErrorMessage()).toBe('areaChart requires at least one metric');
    });
    ...
    it('emits error when there are 2 dimensions and more than one metric', () => {
      ...
      expect(findEmittedErrorMessage()).toBe(
        'areaChart with 2 dimensions supports only a single metric',
      );
    });
