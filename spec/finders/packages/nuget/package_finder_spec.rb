...
RSpec.describe Packages::Nuget::PackageFinder, feature_category: :package_registry do
  ...
  # and use ::Packages::Nuget::Package instead
  # https://gitlab.com/gitlab-org/gitlab/-/issues/499602
  let(:packages_class) { package1.class }
  ...
  describe '#execute!' do
    ...
    shared_examples 'handling all the conditions' do |factory: :nuget_package|
      ...
      context 'with limit hit' do
        let_it_be(:package4) { create(factory, name: package1.name, project: project) }
        let_it_be(:package5) { create(factory, name: package1.name, project: project) }
        let_it_be(:package6) { create(factory, name: package1.name, project: project) }
    ...
    context 'when nuget_extract_nuget_package_model is disabled' do
      let_it_be_with_refind(:package1) { create(:nuget_package_legacy, project: project, name: FFaker::Lorem.word) }

      let_it_be(:package2) do
        create(:nuget_package_legacy, :with_metadatum, name: package1.name, version: '2.0.0+ABC', project: project)
      end

      let_it_be(:package3) { create(:nuget_package_legacy, name: FFaker::Lorem.word, project: project) }
      let_it_be(:other_package_1) { create(:nuget_package_legacy, name: package1.name, version: package1.version) }
      let_it_be(:other_package_2) { create(:nuget_package_legacy, name: package1.name, version: package2.version) }
