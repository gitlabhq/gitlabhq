...
RSpec.describe NewIssueWorker, feature_category: :team_planning do
  ...
  describe '#perform' do
    ...
    context 'with a user' do
      ...
      let_it_be_with_reload(:mentioned) { create(:user) }
      let_it_be_with_reload(:user) { nil }
      let_it_be_with_reload(:issue) { create(:issue, project: project, description: "issue for #{mentioned.to_reference}") }
      ...
      context 'when the new issue author is blocked' do
        let_it_be_with_reload(:user) { create_default(:user, :blocked) }
      ...
      context 'when the new issue author is a ghost' do
        let_it_be_with_reload(:user) { create_default(:user, :ghost) }

        it_behaves_like 'a new issue where the current_user cannot trigger notifications'
      end

      context 'when everything is ok' do
        let_it_be_with_reload(:user) { create_default(:user) }
        ...
        context 'when issue has multiple assignees' do
          let_it_be_with_reload(:users) do
