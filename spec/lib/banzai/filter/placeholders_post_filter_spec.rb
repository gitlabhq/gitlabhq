...
RSpec.describe Banzai::Filter::PlaceholdersPostFilter, feature_category: :markdown do
  ...
  context 'when placeholders in link' do
    ...
    it 'works the same when the input originated from inline HTML' do
      ...
      project.update!(title: 'Cool C++ Project')
      markdown = '<a href="https://%%7Bproject_title%7D.%%7Bgitlab_server%7D/foo%23%2fbar/' \
        '%{project_path}/%{project_title}" data-placeholder>Visit %{project_title} at %{project_path}</a>'
      expected_data_placeholder = "https://%%7Bproject_title%7D.%%7Bgitlab_server%7D/foo%23%2fbar/%{project_path}/%{project_title}"
      expected_href = "https://%%7Bproject_title%7D.#{Gitlab.config.gitlab.host}/foo%23%2fbar/#{project.full_path}/Cool%20C%2B%2B%20Project"
