...
RSpec.describe Banzai::Filter::AttributesFilter, feature_category: :markdown do
  ...
  describe 'unrecognized attributes' do
    context 'when attribute matches neither value nor class syntax' do
      where(:text, :result) do
        "#{image}{#myid}"  | '<img src="example.jpg">'
        "#{image}{key}"    | '<img src="example.jpg">'
      end
  ...
  describe 'class attributes' do
    describe 'float classes' do
      context 'when float class is valid' do
        where(:text, :result) do
          "#{image}{.float-right}" | '<img src="example.jpg" class="glfm-float-right">'
          "#{image}{.float-left}"  | '<img src="example.jpg" class="glfm-float-left">'
          "#{image}{.float-none}"  | '<img src="example.jpg" class="glfm-float-none">'
        end
      ...
      context 'when float class is invalid' do
        where(:text, :result) do
          "#{image}{.float-center}" | '<img src="example.jpg">'
        end
