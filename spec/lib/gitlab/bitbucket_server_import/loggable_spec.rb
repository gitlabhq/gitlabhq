...
RSpec.describe Gitlab::BitbucketServerImport::Loggable, feature_category: :importers do
  ...
  let(:loggable_class) do
    Class.new do
      include Gitlab::BitbucketServerImport::Loggable

      attr_reader :project

      def self.name
        'BitbucketServerImportLoggable'
      end
