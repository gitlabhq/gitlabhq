...
RSpec.describe Gitlab::Graphql::Validators::AtLeastOneOfValidator, feature_category: :integrations do
  let(:schema) do
    Class.new(GraphQL::Schema) do
      query(Class.new(GraphQL::Schema::Object) do
        ...
        field :find_user, GraphQL::Types::String, null: true do
          ...
          validates({ Gitlab::Graphql::Validators::AtLeastOneOfValidator => [:username, :user_id] })
        end
  ...
  it 'raises an error when no arguments are provided' do
    ...
    expect(result['errors']).to include(
      a_hash_including('message' => 'At least one of [username, userId] arguments is required.')
    )
  end

  it 'does not raise an error when one argument is provided' do
    query = <<-GRAPHQL
      query {
  ...
  it 'does not raise an error when both arguments are provided' do
    query = <<-GRAPHQL
      query {
