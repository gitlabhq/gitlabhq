...
RSpec.describe Gitlab::SidekiqMiddleware::ConcurrencyLimit::WorkerExecutionTracker,
  ...
  let(:tracking_hash) { "#{redis_key_prefix}:{#{worker_class_name}}:executing" }
  let(:tracking_elem) { "#{sidekiq_pid}:tid:#{sidekiq_tid}" }

  subject(:service) { described_class.new(worker_class_name, redis_key_prefix) }
  ...
  describe '#cleanup_stale_trackers' do
    ...
    it 'only cleans up dangling keys' do
      expect { cleanup_stale_trackers }.to change { service.concurrent_worker_count }.from(3).to(2)
    end
  end
end
