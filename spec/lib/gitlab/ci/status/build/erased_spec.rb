...
RSpec.describe Gitlab::Ci::Status::Build::Erased, feature_category: :continuous_integration do
  ...
  describe '.matches?' do
    ...
    context 'when build is erased' do
      let(:build) { build_stubbed(:ci_build, :success, :erased) }
    ...
    context 'when build is not erased' do
      let(:build) { build_stubbed(:ci_build, :success, :trace_artifact) }
