    ...
    context 'when given pattern has a wildcard' do
      ...
      where(:pattern, :environment, :matches) do
        [
          ...
          ['*', 'anything-goes', true],
          # Gitlab::UntrustedRegexp#match? here only scans non-blank text
          # unless called with allow_empty_string: true, so a pattern of
          # just '*' does not match an empty environment name.
          ['*', '', false],
