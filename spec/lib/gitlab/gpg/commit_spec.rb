...
RSpec.describe Gitlab::Gpg::Commit, feature_category: :source_code_management do
  ...
  shared_examples_for 'gpg commit' do |use_signature_class|
    before do
      stub_feature_flags(gpg_commit_delegate_to_signature: use_signature_class)
    end

    describe '#signature' do
      shared_examples 'returns the cached signature on second call' do |testing_signature_class|
        if testing_signature_class
      ...
      context 'known key' do
        context 'user matches the key uid' do
          context 'user email matches the email committer' do
            ...
            it_behaves_like 'returns the cached signature on second call', use_signature_class
  ...
  context 'when the gpg_commit_delegate_to_signature feature flag is not enabled' do
    it_behaves_like 'gpg commit', false
  end

  it_behaves_like 'gpg commit', true
end
