  ...
  describe '#format_patch' do
    def verify_content(patch)
      sample_compare.commits.each do |commit|
        expect(patch.include?(commit)).to be_true
      end
    end
  ...
  describe '#can_be_merged?' do
    context 'on fork' do
      it { expect(Gitlab::Satellite::MergeAction.new(
        ...
        merge_request_fork).can_be_merged?).to be_true }

      it { expect(Gitlab::Satellite::MergeAction.new(
        merge_request_fork_with_conflict.author,
        merge_request_fork_with_conflict).can_be_merged?).to be_false }
    end

    context 'between branches' do
      it { expect(Gitlab::Satellite::MergeAction.new(
        merge_request.author,
        merge_request).can_be_merged?).to be_true }

      it { expect(Gitlab::Satellite::MergeAction.new(
        merge_request_with_conflict.author,
        merge_request_with_conflict).can_be_merged?).to be_false }
    end
  end
end
