...
RSpec.describe Gitlab::ImportExport::Group::TreeRestorer, feature: :subgroups, feature_category: :importers do
  ...
  context 'when destination parent group has shared runners disabled and unoverridable' do
    let(:parent) { create(:group, :shared_runners_disabled_and_unoverridable) }
    ...
    it 'imports the group while complying with the parent shared runners setting' do
      ...
      group.reload
      expect(group.shared_runners_enabled).to be(false)
      expect(group.allow_descendants_override_disabled_shared_runners).to be(false)
    end
  end
