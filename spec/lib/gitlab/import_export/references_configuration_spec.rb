  ...
  with_them do
    context "where relation #{params[:relation_path]}" do
      it 'does not have prohibited keys' do
        ...
        encrypted_attributes = relation_class.try(:attr_encrypted_encrypted_attributes)&.keys&.map(&:to_s) || []
        relation_attributes = relation_class.new.attributes.keys - encrypted_attributes
