  ...
  describe '#resolve' do
    ...
    context 'when commit_id param is set' do
      ...
      context 'when commit_id is a context commit' do
        ...
        let(:params) { { commit_id: context_commit_sha } }

        before do
          create(:merge_request_context_commit, merge_request: merge_request, sha: context_commit_sha)
        end

        it 'returns the single context commit instead of the full diff', :aggregate_failures do
          commit = merge_request.project.commit(context_commit_sha)
          expect(commit).not_to be_nil
          expect(diff_resolver.resolve).to eq(commit)
        end
      end
    end
  end
