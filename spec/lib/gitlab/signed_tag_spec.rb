...
RSpec.describe Gitlab::SignedTag, feature_category: :source_code_management do
  ...
  describe '#lazy_cached_signature' do
    def build_git_tag(signature_type:, id:)
      instance_double(Gitlab::Git::Tag,
        has_signature?: true,
    ...
    let(:gpg_git_tag) { build_git_tag(signature_type: :PGP, id: 'git_tag_id1') }
    let(:ssh_git_tag) { build_git_tag(signature_type: :SSH, id: 'git_tag_id3') }
    let(:git_tags) do
      [
    ...
    context 'when the type of signature is not cached in a table' do
      let(:x509_git_tag) { build_git_tag(signature_type: :x509, id: 'git_tag_id1') }
    ...
    context 'when called for multiple tags of the same type' do
      let(:gpg_git_tag2) { build_git_tag(signature_type: :PGP, id: 'git_tag_id2') }
      let(:ssh_git_tag2) { build_git_tag(signature_type: :SSH, id: 'git_tag_id4') }
