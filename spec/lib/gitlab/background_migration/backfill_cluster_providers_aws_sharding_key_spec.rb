...
RSpec.describe Gitlab::BackgroundMigration::BackfillClusterProvidersAwsShardingKey, feature_category: :deployment_management do
  ...
  let(:function_name) { 'cluster_providers_aws_sharding_key' }
  let(:trigger_name) { "trigger_#{function_name}" }
  let(:constraint_name) { 'check_6d49cca3b0' }
  ...
  let(:provider_without_sharding_key) do
    ...
    record = cluster_providers_aws.create!(
  ...
  subject(:migration) do
    described_class.new(
      start_id: cluster_providers_aws.minimum(:id),
      end_id: cluster_providers_aws.maximum(:id),
      batch_table: :cluster_providers_aws,
      batch_column: :id,
      sub_batch_size: 1,
      pause_ms: 0,
  ...
  describe "#perform" do
    ...
    it 'does not modify records that already have complete sharding key' do
  ...
  def drop_constraint_and_trigger
    connection.execute(
      <<~SQL
        DROP TRIGGER IF EXISTS #{trigger_name} ON cluster_providers_aws;

        ALTER TABLE cluster_providers_aws DROP CONSTRAINT IF EXISTS #{constraint_name};
      SQL
    )
  end
  ...
  def add_constraint_and_trigger
    connection.execute(
      <<~SQL
        ALTER TABLE cluster_providers_aws ADD CONSTRAINT #{constraint_name} CHECK ((num_nonnulls(group_id, organization_id, project_id) = 1)) NOT VALID;

        CREATE TRIGGER #{trigger_name} BEFORE INSERT OR UPDATE ON cluster_providers_aws FOR EACH ROW EXECUTE FUNCTION #{function_name}();
      SQL
    )
  end
end
