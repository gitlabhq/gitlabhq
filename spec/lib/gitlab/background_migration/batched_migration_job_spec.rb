...
RSpec.describe Gitlab::BackgroundMigration::BatchedMigrationJob, feature_category: :database do
  ...
  describe '#perform' do
    ...
    context 'when the subclass uses cursor-based sub-batching' do
      let(:job_class) do
        Class.new(described_class) do
          cursor :id_a, :id_b
          operation_name :update
          ...
          def perform(*)
            each_sub_batch do |sub_batch|
              sub_batch.update_all('payload = payload')
            end
          end
        end
      end
      ...
      it 'first sub-batch uses inclusive `>= start_cursor`, later sub-batches use strict `> prev_cursor` only' do
        queries = []
        ...
        expect(table_queries.any? { |q| q.match?(tuple_gteq) }).to be(true),
          'expected first sub-batch SQL to use `(id_a, id_b) >=` against start_cursor'
