...
RSpec.describe Gitlab::Database::DeduplicateCiTags, :aggregate_failures, feature_category: :runner do
  ...
  let!(:tagging_ids) do
    create_taggings(
      { taggable_type: 'Ci::Runner', taggable_id: runner1_id, tag_id: tag_ids.first },
      { taggable_type: 'Ci::Runner', taggable_id: runner1_id, tag_id: tag_ids.second },
      { taggable_type: 'Ci::Runner', taggable_id: runner2_id, tag_id: tag_ids.third },
      { taggable_type: 'Ci::Build', taggable_id: build1_id, tag_id: tag_ids.second }
    )
  end
  ...
  describe '#execute' do
    ...
    it 'does not change number of tags or taggings tag_ids' do
      expect { execute }.to not_change { table_count('tags') }
        .and not_change { tagging_relationship_for(tagging_ids.third) }
    ...
    context 'when duplicate tags exist' do
      ...
      let(:build2_tagging_ids) do
        create_taggings(
          { taggable_type: 'Ci::Build', taggable_id: build2_id, tag_id: duplicate_tag_ids.second },
          { taggable_type: 'Ci::Build', taggable_id: build2_id, tag_id: tag_ids.third }
        )
      end

      let(:duplicate_build_tagging_id) { build2_tagging_ids.first }
      ...
      let(:duplicate_runner_tagging_id) do
        create_taggings({ taggable_type: 'Ci::Runner', taggable_id: runner2_id, tag_id: duplicate_tag_ids.first }).sole
      end
      ...
      let(:duplicate_tagging_ids) do
        [
          duplicate_build_tagging_id, duplicate_ci_build_tagging_id, duplicate_runner_tagging_id,
          duplicate_ci_runner_tagging_id, pending_build2_id
        ]
      end
      ...
      it 'deletes duplicate tags and updates taggings' do
        expect { execute }
          ...
          .and not_change { ci_runner_tagging_relationship_for(ci_runner_tagging_ids.second) }
          .and not_change { ci_pending_build_tag_ids_for(pending_build1_id) }
          .and change { tagging_relationship_for(duplicate_build_tagging_id) }
          ...
          .and change { tagging_relationship_for(duplicate_runner_tagging_id) }
      ...
      context 'and dry_run is true' do
        ...
        it 'does not change number of tags or taggings tag_ids' do
          expect { execute }.to not_change { table_count('tags') }
            ...
            .and not_change { tagging_relationship_for(tagging_ids.second) }
            .and not_change { ci_runner_tagging_relationship_for(ci_runner_tagging_ids.second) }
            .and not_change { tagging_relationship_for(duplicate_build_tagging_id) }
            ...
            .and not_change { tagging_relationship_for(duplicate_runner_tagging_id) }
  ...
  def create_taggings(*taggings)
    values = taggings.map { |t| "(#{t[:tag_id]}, #{t[:taggable_id]}, '#{t[:taggable_type]}', 'tags')" }.join(', ')
  ...
  def tagging_relationship_for(tagging_id)
    connection.execute("SELECT taggable_id, tag_id FROM taggings WHERE id = #{tagging_id}").values.to_h
  end
