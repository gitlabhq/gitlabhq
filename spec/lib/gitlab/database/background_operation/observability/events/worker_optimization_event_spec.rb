...
RSpec.describe Gitlab::Database::BackgroundOperation::Observability::Events::WorkerOptimizationEvent, feature_category: :database do
  shared_examples 'logging for the given record type' do
    context 'when job transitions with no error' do
      let(:attributes) { { old_batch_size: 100, new_batch_size: 250 } }
      let(:expected_payload) do
        {
          message: 'background_operation_worker_optimization_event',
          old_batch_size: 100,
          new_batch_size: 250,
