...
RSpec.describe Gitlab::Database::Aggregation::QueryPlan::Dimension, feature_category: :value_stream_management do
  ...
  describe '#name' do
    it 'delegates to definition' do
      dimension = described_class.new(part_definition, part_configuration, query_plan: nil)
  ...
  describe 'validations' do
    ...
    it 'is invalid when definition is nil' do
      dimension = described_class.new(nil, part_configuration, query_plan: nil)
      expect(dimension).not_to be_valid
    end

    it 'includes error message when definition is missing' do
      dimension = described_class.new(nil, { identifier: :missing_dimension }, query_plan: nil)
      dimension.validate
