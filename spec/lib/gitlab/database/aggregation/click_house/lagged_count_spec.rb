...
RSpec.describe Gitlab::Database::Aggregation::ClickHouse::LaggedCount, :click_house,
  ...
  describe 'lagged_count with default lag_offset' do
    ...
    it 'returns the distinct user count from the previous period' do
      ...
      expect(engine).to execute_aggregation(request).and_return([
        { event_date_granularity_daily: Date.parse('2025-03-01'), previous_users_count: 0 },
        { event_date_granularity_daily: Date.parse('2025-03-02'), previous_users_count: 2 },
        { event_date_granularity_daily: Date.parse('2025-03-03'), previous_users_count: 2 }
      ])
    end
  end
  ...
  describe 'lagged_count with custom lag_offset' do
    ...
    it 'returns the distinct user count from two periods ago' do
      ...
      expect(engine).to execute_aggregation(request).and_return([
        { event_date_granularity_daily: Date.parse('2025-03-01'), two_periods_ago_users_count: 0 },
        { event_date_granularity_daily: Date.parse('2025-03-02'), two_periods_ago_users_count: 0 },
        { event_date_granularity_daily: Date.parse('2025-03-03'), two_periods_ago_users_count: 2 }
      ])
    end
  end
  ...
  describe 'lagged_count combined with a regular metric' do
    ...
    it 'returns both the lagged user count and total session count per day' do
      expect(engine).to execute_aggregation(request).and_return([
        { event_date_granularity_daily: Date.parse('2025-03-01'), total_count: 2, previous_users_count: 0 },
        { event_date_granularity_daily: Date.parse('2025-03-02'), total_count: 2, previous_users_count: 2 },
        { event_date_granularity_daily: Date.parse('2025-03-03'), total_count: 1, previous_users_count: 2 }
      ])
    end
  end
  ...
  describe 'lagged_count with filtering' do
    ...
    it 'computes lagged count only over the filtered subset' do
      ...
      expect(engine).to execute_aggregation(request).and_return([
        { event_date_granularity_daily: Date.parse('2025-03-01'), previous_users_count: 0 },
        { event_date_granularity_daily: Date.parse('2025-03-02'), previous_users_count: 2 },
        { event_date_granularity_daily: Date.parse('2025-03-03'), previous_users_count: 1 }
      ])
    end
  end
  ...
  describe 'multiple lag offsets in one query' do
    ...
    it 'returns both lag_offset=1 and lag_offset=2 results together' do
      expect(engine).to execute_aggregation(request).and_return([
        { event_date_granularity_daily: Date.parse('2025-03-01'), previous_users_count: 0,
          two_periods_ago_users_count: 0 },
        { event_date_granularity_daily: Date.parse('2025-03-02'), previous_users_count: 2,
          two_periods_ago_users_count: 0 },
        { event_date_granularity_daily: Date.parse('2025-03-03'), previous_users_count: 2,
          two_periods_ago_users_count: 2 }
      ])
    end
  end
