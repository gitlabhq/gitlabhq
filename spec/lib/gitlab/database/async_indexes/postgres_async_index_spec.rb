...
RSpec.describe Gitlab::Database::AsyncIndexes::PostgresAsyncIndex, type: :model, feature_category: :database do
  ...
  describe 'scopes' do
    let_it_be(:async_index_creation, freeze: false) { create(:postgres_async_index, attempts: 0) }
    let_it_be(:async_index_destruction, freeze: false) { create(:postgres_async_index, :with_drop, attempts: 0) }
