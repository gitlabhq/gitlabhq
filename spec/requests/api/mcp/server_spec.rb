...
RSpec.describe API::Mcp::Server, feature_category: :mcp_server do
  let_it_be(:user) { create(:user) }
  let_it_be(:access_token) { create(:oauth_access_token, user: user, scopes: [:mcp]) }

  describe 'POST /mcp_server' do
    ...
    context 'when authenticated' do
      ...
      context 'when access token is PAT' do
        it 'returns forbidden' do
          post api('/mcp_server', user), params: { jsonrpc: '2.0', method: 'initialize', id: '1' }
      ...
      context 'when access token is OAuth without mcp scope' do
        ...
        it 'returns forbidden' do
          post api('/mcp_server', user, oauth_access_token: insufficient_access_token),
      ...
      context 'when required jsonrpc param is missing' do
        it 'returns JSON-RPC Invalid Request error' do
          post api('/mcp_server', user, oauth_access_token: access_token), params: { id: '1', method: 'initialize' }
      ...
      context 'when required method param is missing' do
        it 'returns JSON-RPC Invalid Request error' do
          post api('/mcp_server', user, oauth_access_token: access_token), params: { jsonrpc: '2.0', id: '1' }
  ...
  describe 'GET /mcp_server' do
    ...
    context 'when authenticated' do
      context 'when feature flag is disabled' do
        ...
        it 'returns not found' do
          get api('/mcp_server', user, oauth_access_token: access_token)
