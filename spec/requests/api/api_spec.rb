...
RSpec.describe API::API, feature_category: :system_access do
  ...
  describe 'Handle Gitaly unavailability' do
    let_it_be(:user) { create(:user) }
    let_it_be(:project) { create(:project, :repository, creator: user) }
    ...
    context 'when GRPC::Unavailable is raised' do
      before do
        allow(Gitlab::GitalyClient).to receive(:call).with(any_args).and_raise(GRPC::Unavailable)
      end

      it 'returns 503 status and logs the error' do
        expect(Gitlab::ErrorTracking).to receive(:track_exception).with(instance_of(GRPC::Unavailable))

        get api("/projects/#{project.id}/repository/commits", user)

        expect(response).to have_gitlab_http_status(:service_unavailable)
        expect(json_response).to eql(
          'message' => 'Gitaly service temporarily unavailable'
        )
      end
    end

    context 'when GRPC::DeadlineExceeded is raised' do
      before do
        allow(Gitlab::GitalyClient).to receive(:call).with(any_args).and_raise(GRPC::DeadlineExceeded)
      end

      it 'returns 503 status and logs the error' do
        expect(Gitlab::ErrorTracking).to receive(:track_exception).with(instance_of(GRPC::DeadlineExceeded))
    ...
    context 'when Gitlab::Git::CommandError wraps GRPC::Unavailable' do
      let(:grpc_error) { GRPC::Unavailable.new }
      let(:command_error) do
        Gitlab::Git::CommandError.new(grpc_error).tap do |error|
          allow(error).to receive(:cause).and_return(grpc_error)
        end
      end

      before do
        allow(Gitlab::GitalyClient).to receive(:call).with(any_args).and_raise(command_error)
      end
    ...
    context 'when Gitlab::Git::CommandError wraps GRPC::DeadlineExceeded' do
    ...
    context 'when Gitlab::Git::CommandError has a non-GRPC cause' do
      let(:non_grpc_error) { StandardError.new('Some other error') }
      let(:command_error) do
        Gitlab::Git::CommandError.new('Command failed').tap do |error|
          allow(error).to receive(:cause).and_return(non_grpc_error)
        end
      end
      ...
      before do
        ...
        allow(Gitlab::ErrorTracking).to receive(:sentry_dsn).and_return(Gitlab.config.sentry.dsn)
        Gitlab::ErrorTracking.configure
      end

      it 'calls handle_api_exception and returns 500' do
        get api("/projects/#{project.id}/repository/commits", user)

        expect(response).to have_gitlab_http_status(:internal_server_error)
        expect(json_response['message']).to include('Gitlab::Git::CommandError')
      end
    end
  end
