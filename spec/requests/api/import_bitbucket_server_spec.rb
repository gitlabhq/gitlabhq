...
RSpec.describe API::ImportBitbucketServer, feature_category: :importers do
  ...
  describe "POST /import/bitbucket_server" do
    ...
    it_behaves_like 'authorizing granular token permissions', :create_bitbucket_server_import do
      before do
        allow(Gitlab::BitbucketServerImport::ProjectCreator).to receive(:new).and_return(double(execute: project))
      end

      let(:boundary_object) { :instance }
      let(:request) do
        post api("/import/bitbucket_server", personal_access_token: pat), params: params
      end
    end
    ...
    context 'with a private inaccessible namespace' do
      ...
      let(:repo) { double(name: repo_slug, full_path: "/private-group/#{repo_slug}") }
