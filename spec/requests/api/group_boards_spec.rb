...
RSpec.describe API::GroupBoards, :with_license, feature_category: :planning_views do
  let_it_be(:user) { create(:user) }
  let_it_be(:non_member) { create(:user) }
  let_it_be(:guest) { create(:user) }
  let_it_be(:admin) { create(:user, :admin) }
  let_it_be_with_reload(:board_parent) { create(:group, :public, owners: user) }
  ...
  let_it_be_with_reload(:board) { create(:board, group: board_parent, lists: [dev_list, test_list]) }
