...
RSpec.describe Projects::ArtifactsController, feature_category: :job_artifacts do
  ...
  describe 'GET file' do
    ...
    context 'when the project is private' do
      ...
      let_it_be(:private_job) { create(:ci_build, :success, :artifacts, pipeline: private_pipeline) }

      before do
        private_project.add_developer(user)
      end

      context 'without access control' do
        before do
          allow(Gitlab.config.pages).to receive(:artifacts_server).and_return(true)
        end

        it 'does not redirect the request' do
          get :file, params: { namespace_id: private_project.namespace, project_id: private_project, job_id: private_job, path: 'ci_artifacts.txt' }
