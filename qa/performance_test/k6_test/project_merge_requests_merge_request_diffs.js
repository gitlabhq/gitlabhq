...
export const TEST_NAME = 'project_merge_requests_merge_request_diffs';
...
export function setup() {
  ...
  const apiUrl = `${apiVersion}/projects/${projectId}/merge_requests/${mrIid}/diffs`;
  return { apiUrl };
}
...
export default function projectMergeRequestDiffsTest(data) {
  const baseUrl = __ENV.GITLAB_URL || `http://gitlab.${__ENV.AI_GATEWAY_IP}.nip.io`;
  const token = __ENV.GITLAB_QA_ADMIN_ACCESS_TOKEN || '';
  ...
  try {
    console.log(
      `Request ${__ITER}: ${res.request.method} ${res.request.url} - Status ${res.status} - Duration ${res.timings.duration}ms - Diffs ${
