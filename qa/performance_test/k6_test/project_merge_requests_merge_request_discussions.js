...
export const TEST_NAME = 'project_merge_requests_merge_request_discussions';
...
export function setup() {
  ...
  const apiUrl = `${apiVersion}/projects/${projectId}/merge_requests/${mrIid}/discussions`;
  return { apiUrl };
}
...
// mr_seed.rb does not currently create discussion threads, so this legitimately
// returns an empty array - the test still measures real endpoint TTFB/RPS.
export default function projectMergeRequestDiscussionsTest(data) {
  const baseUrl = __ENV.GITLAB_URL || `http://gitlab.${__ENV.AI_GATEWAY_IP}.nip.io`;
  const token = __ENV.GITLAB_QA_ADMIN_ACCESS_TOKEN || '';
  ...
  try {
    console.log(
      `Request ${__ITER}: ${res.request.method} ${res.request.url} - Status ${res.status} - Duration ${res.timings.duration}ms - Discussions ${
