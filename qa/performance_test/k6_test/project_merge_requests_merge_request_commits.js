...
export const TEST_NAME = 'project_merge_requests_merge_request_commits';
...
export function setup() {
  ...
  const apiUrl = `${apiVersion}/projects/${projectId}/merge_requests/${mrIid}/commits`;
  return { apiUrl };
}
...
export default function projectMergeRequestCommitsTest(data) {
  const baseUrl = __ENV.GITLAB_URL || `http://gitlab.${__ENV.AI_GATEWAY_IP}.nip.io`;
  const token = __ENV.GITLAB_QA_ADMIN_ACCESS_TOKEN || '';
  ...
  check(res, {
    ...
    'response is array': () => Array.isArray(parsedResponse),
  });

  try {
    console.log(
      `Request ${__ITER}: ${res.request.method} ${res.request.url} - Status ${res.status} - Duration ${res.timings.duration}ms - Commits ${
        Array.isArray(parsedResponse) ? parsedResponse.length : 'n/a'
      }`,
    );
  } catch (e) {
    console.log(
