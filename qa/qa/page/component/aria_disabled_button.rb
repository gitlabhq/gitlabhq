...
module QA
  module Page
    module Component
      ...
      # attribute. Capybara's built-in `Element#disabled?` only checks the native
      # attribute, so it returns `false` for aria-disabled buttons. Use
      # `element_disabled?` wherever a GlButton's disabled state needs to be tested.
