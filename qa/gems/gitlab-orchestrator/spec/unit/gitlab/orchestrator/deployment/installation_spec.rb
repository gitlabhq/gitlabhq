...
RSpec.describe Gitlab::Orchestrator::Deployment::Installation, :aggregate_failures do
  describe "with setup" do
    ...
    let(:kubeclient) do
      instance_double(Gitlab::Orchestrator::Kubectl::Client, create_namespace: "", create_resource: "", execute: "")
    end

    let(:helmclient) do
      instance_double(Gitlab::Orchestrator::Helm::Client, add_gitlab_helm_chart: chart_reference, upgrade: nil)
    end

    let(:configuration) do
      instance_double(
        Gitlab::Orchestrator::Deployment::Configurations::Kind,
    ...
    before do
      ...
      allow(Gitlab::Orchestrator::Kubectl::Client).to receive(:new).with("gitlab").and_return(kubeclient)
      allow(Gitlab::Orchestrator::Helm::Client).to receive(:new).and_return(helmclient)
      allow(Gitlab::Orchestrator::Deployment::Configurations::Kind).to receive(:new).and_return(configuration)
      allow(Gitlab::Orchestrator::Deployment::DefaultValues).to receive(:common_values).with(gitlab_domain).and_return({
        global: { common: "val" }
      })
    ...
    context "with deployment failure" do
      ...
      before do
        allow(helmclient).to receive(:upgrade).and_raise(Gitlab::Orchestrator::Helm::Client::Error, "error")
    ...
    context "with successful deployment" do
      it "runs setup and helm deployment" do
        ...
        expect(kubeclient).to have_received(:create_resource).with(
          Gitlab::Orchestrator::Kubectl::Resources::Secret.new("gitlab-license", "license", "license")
        )
    ...
    context "with successful deployment on CI" do
      ...
      before do
        allow(Gitlab::Orchestrator::Deployment::DefaultValues)
          .to receive(:component_ci_versions)
          .and_return(ci_components)
      end
  ...
  describe "with cleanup" do
    ...
    let(:helm) { instance_double(Gitlab::Orchestrator::Helm::Client, uninstall: nil) }
    let(:kubeclient) { instance_double(Gitlab::Orchestrator::Kubectl::Client, delete_resource: "") }
    let(:cleanup_configuration) do
      instance_double(
        Gitlab::Orchestrator::Deployment::Configurations::Cleanup::Kind,
    ...
    before do
      allow(Gitlab::Orchestrator::Helm::Client).to receive(:new).and_return(helm)
      allow(Gitlab::Orchestrator::Helpers::Spinner).to receive(:spin).and_yield
    end
