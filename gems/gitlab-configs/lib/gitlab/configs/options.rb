...
module Gitlab
  module Configs
    class Options
      ...
      # Recursively build Gitlab::Configs::Options
      def self.build(obj)
        case obj
        when Hash
      ...
      def stringify_keys!
        ...
        report_mutation_warning(error_msg, method: __method__)
      ...
      def method_missing(name, *args, &block)
        ...
        if @options.respond_to?(name)
          ...
          report_mutation_warning(error_msg, method: name)

          # rubocop: disable GitlabSecurity/PublicSend -- Delegates a public Hash method confirmed via respond_to?
          return @options.public_send(name, *args, &block)
          # rubocop: enable GitlabSecurity/PublicSend
        end

        raise ::Gitlab::Configs::MissingConfig, "option '#{name}' not defined"
      end
      ...
      # Delegates mutation warnings to the injectable callback on Gitlab::Configs.
      # The monolith installs a callback that logs in production and raises elsewhere.
      # In standalone/gem usage the default is to always raise.
      #
      # Caller is captured here (frame 0 = options.rb) so the callback receives the
      # real call-site frames regardless of how many dispatch layers sit above it.
      def report_mutation_warning(message, extra = {})
        ::Gitlab::Configs.on_mutation_warning(message, extra.merge(caller: caller[0..10]))
      end
    end
  end
end
