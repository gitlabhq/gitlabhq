...
module OpenbaoClient
  class PkiGenerateIntermediateRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenbaoClient::PkiGenerateIntermediateRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenbaoClient::PkiGenerateIntermediateRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [add_basic_constraints, alt_names, common_name, country, exclude_cn_from_sans, format, ip_sans, key_bits, key_name, key_ref, key_type, locality, not_after, not_before_duration, organization, other_sans, ou, postal_code, private_key_format, province, serial_number, signature_bits, street_address, ttl, uri_sans].hash
    end
