  ...
  class LeasesReadLeaseResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenbaoClient::LeasesReadLeaseResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenbaoClient::LeasesReadLeaseResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'issue_time')
        self.issue_time = attributes[:'issue_time']
      end

      if attributes.key?(:'last_renewal')
        self.last_renewal = attributes[:'last_renewal']
      end
    ...
    def hash
      [expire_time, id, issue_time, last_renewal, renewable, ttl].hash
    end
