...
module OpenbaoClient
  class PkiIssuerIssueWithRoleRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenbaoClient::PkiIssuerIssueWithRoleRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenbaoClient::PkiIssuerIssueWithRoleRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [alt_names, common_name, exclude_cn_from_sans, format, ip_sans, key_bits, key_type, not_after, other_sans, private_key_format, remove_roots_from_chain, serial_number, ttl, uri_sans, user_ids].hash
    end
