  ...
  class PkiRevokeIssuerResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenbaoClient::PkiRevokeIssuerResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenbaoClient::PkiRevokeIssuerResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [ca_chain, certificate, crl_distribution_points, delta_crl_distribution_points, issuer_id, issuer_name, issuing_certificates, key_id, leaf_not_after_behavior, manual_chain, ocsp_servers, revocation_signature_algorithm, revocation_time, revocation_time_rfc3339, revoked, usage].hash
    end
