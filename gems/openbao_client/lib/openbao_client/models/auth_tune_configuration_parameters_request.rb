...
module OpenbaoClient
  class AuthTuneConfigurationParametersRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenbaoClient::AuthTuneConfigurationParametersRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenbaoClient::AuthTuneConfigurationParametersRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [allowed_response_headers, audit_non_hmac_request_keys, audit_non_hmac_response_keys, default_lease_ttl, description, listing_visibility, max_lease_ttl, options, passthrough_request_headers, plugin_version, token_type, user_lockout_config].hash
    end
