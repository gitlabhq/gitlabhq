...
module OpenbaoClient
  class TransitImportKeyRequest
    ...
    attr_accessor :auto_rotate_period
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenbaoClient::TransitImportKeyRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenbaoClient::TransitImportKeyRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'allow_plaintext_backup')
        self.allow_plaintext_backup = attributes[:'allow_plaintext_backup']
      end

      if attributes.key?(:'allow_rotation')
        self.allow_rotation = attributes[:'allow_rotation']
      end

      if attributes.key?(:'auto_rotate_period')
        self.auto_rotate_period = attributes[:'auto_rotate_period']
      else
        self.auto_rotate_period = 0
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          allow_plaintext_backup == o.allow_plaintext_backup &&
          allow_rotation == o.allow_rotation &&
          auto_rotate_period == o.auto_rotate_period &&
    ...
    def hash
      [allow_plaintext_backup, allow_rotation, auto_rotate_period, ciphertext, context, derived, exportable, hash_function, public_key, type].hash
    end
