  ...
  class PkiSignVerbatimRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenbaoClient::PkiSignVerbatimRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenbaoClient::PkiSignVerbatimRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [alt_names, basic_constraints_valid_for_non_ca, common_name, csr, exclude_cn_from_sans, ext_key_usage, ext_key_usage_oids, format, ip_sans, issuer_ref, key_usage, not_after, other_sans, private_key_format, remove_roots_from_chain, role, serial_number, signature_bits, ttl, uri_sans, use_pss, user_ids].hash
    end
