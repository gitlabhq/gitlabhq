...
module OpenbaoClient
  class PkiIssuerSignIntermediateRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenbaoClient::PkiIssuerSignIntermediateRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenbaoClient::PkiIssuerSignIntermediateRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [alt_names, common_name, country, csr, exclude_cn_from_sans, ext_key_usage, ext_key_usage_oids, format, ip_sans, issuer_name, key_usage, locality, max_path_length, not_after, not_before_duration, organization, other_sans, ou, permitted_dns_domains, postal_code, private_key_format, province, serial_number, signature_bits, skid, street_address, ttl, uri_sans, use_csr_values, use_pss].hash
    end
