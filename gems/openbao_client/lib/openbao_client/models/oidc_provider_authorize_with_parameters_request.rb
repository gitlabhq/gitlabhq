...
module OpenbaoClient
  class OidcProviderAuthorizeWithParametersRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenbaoClient::OidcProviderAuthorizeWithParametersRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenbaoClient::OidcProviderAuthorizeWithParametersRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'code_challenge')
        self.code_challenge = attributes[:'code_challenge']
      end

      if attributes.key?(:'code_challenge_method')
        self.code_challenge_method = attributes[:'code_challenge_method']
      else
    ...
    def hash
      [client_id, code_challenge, code_challenge_method, max_age, nonce, redirect_uri, response_type, scope, state].hash
    end
