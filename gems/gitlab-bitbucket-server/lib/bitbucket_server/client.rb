...
module BitbucketServer
  class Client
    ...
    def initialize(options = {}, http_client:)
      @connection = Connection.new(options, http_client: http_client)
    end
    ...
    def delete_branch(project_key, repo, branch_name, _sha)
      payload = {
        name: BRANCH_REF_PREFIX + branch_name,
        dryRun: false
      }
    ...
    def get_collection(path, type, page_offset: 0, limit: nil)
      paginator = BitbucketServer::Paginator.new(connection, Addressable::URI.escape(path), type,
        page_offset: page_offset, limit: limit)
