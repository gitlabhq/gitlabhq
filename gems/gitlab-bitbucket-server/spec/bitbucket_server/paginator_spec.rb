...
RSpec.describe BitbucketServer::Paginator do
  let(:last_page) { instance_double(BitbucketServer::Page, next?: false, items: ['item_2']) }
  let(:first_page) { instance_double(BitbucketServer::Page, next?: true, next: last_page, items: ['item_1']) }
  ...
  describe '#items' do
    ...
    it 'calls the connection with different offsets' do
      page_length = BitbucketServer::Paginator::PAGE_LENGTH
      expect(connection).to receive(:get)
        .with('http://more-data', start: 0, limit: page_length)
        .and_return(page_attrs)

      expect(paginator.items).to eq([])

      expect(connection).to receive(:get)
        .with('http://more-data', start: 1, limit: page_length)
        .and_return({})
