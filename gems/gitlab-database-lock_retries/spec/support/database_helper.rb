...
module DatabaseHelper
  def self.test_database_name
    @test_database_name ||= "gitlab_lock_retries_test_#{SecureRandom.alphanumeric(6)}"
  end
  ...
  def self.setup_database
    ...
    # Load from `gitlab-rails/config/database.yml`, then create a throwaway
    # database. The `postgres` maintenance database is needed to issue DDL.
