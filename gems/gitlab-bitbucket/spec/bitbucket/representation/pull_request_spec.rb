...
RSpec.describe Bitbucket::Representation::PullRequest do
  ...
  describe '#source_branch_name' do
    it 'returns source branch name' do
      raw = { source: { branch: { name: 'feature' } } }.with_indifferent_access
      expect(described_class.new(raw).source_branch_name).to eq('feature')
    end
  ...
  describe '#source_branch_sha' do
    it 'returns source branch sha' do
      raw = { source: { commit: { hash: 'abcd123' } } }.with_indifferent_access
      expect(described_class.new(raw).source_branch_sha).to eq('abcd123')
    end
  ...
  describe '#target_branch_name' do
    it 'returns target branch name' do
      raw = { destination: { branch: { name: 'master' } } }.with_indifferent_access
      expect(described_class.new(raw).target_branch_name).to eq('master')
    end
  ...
  describe '#target_branch_sha' do
    it 'returns target branch sha' do
      raw = { destination: { commit: { hash: 'abcd123' } } }.with_indifferent_access
      expect(described_class.new(raw).target_branch_sha).to eq('abcd123')
    end
