...
RSpec.describe Bitbucket::Representation::Repo do
  ...
  describe '#default_branch' do
    ...
    it { expect(described_class.new({}).default_branch).to be_nil }
  end

  describe '#private?' do
    it 'returns true for private repos' do
      expect(described_class.new({ 'is_private' => true }).private?).to be(true)
    end

    it 'returns false for public repos' do
      expect(described_class.new({ 'is_private' => false }).private?).to be(false)
    end

    it 'returns false when is_private is absent' do
      expect(described_class.new({}).private?).to be(false)
    end
  end
  ...
  describe '#error' do
    ...
    it { expect(described_class.new({}).error).to be_nil }
  end
end
