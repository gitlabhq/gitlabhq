...
RSpec.describe Gitlab::Utils::DelegatorOverride do
  ...
  describe '.delegator_target' do
    subject(:delegator_target) { delegator_class.delegator_target(target_class) }
    ...
    context 'when the class does not inherit SimpleDelegator' do
      ...
      it 'raises an error' do
        expect { delegator_target }.to raise_error(ArgumentError, /not a subclass of 'SimpleDelegator' class/)
      end
    end
  end

  describe '.delegator_override' do
    subject(:delegator_override) { delegator_class.delegator_override(:foo) }
  ...
  describe '.delegator_override_with' do
    subject(:delegator_override_with) { delegator_class.delegator_override_with(dummy_module) }
  ...
  describe '.verify!' do
    subject(:verify) { described_class.verify! }
    ...
    it 'raises an error when there is an override' do
      ...
      expect { verify }.to raise_error(Gitlab::Utils::DelegatorOverride::Validator::UnexpectedDelegatorOverrideError)
    end
  end
end
