  ...
  describe '.safe_parse' do
    ...
    context 'when TOML content is invalid' do
      ...
      it 'raises a ParserError' do
        expect { result }.to raise_error(described_class::ParseError, 'content is not valid TOML')
      end
    end
    ...
    context 'with timeout' do
      ...
      it 'times out when parsing takes too long' do
        ...
        timeout_logger = instance_double(Proc)
        allow(described_class).to receive(:timeout_logger).and_return(timeout_logger)
        expect(timeout_logger).to receive(:call).with(instance_of(Timeout::Error))

        expect { result }.to raise_error(described_class::ParseError, 'timeout while parsing TOML')
      end
    end
    ...
    context 'with error raised by TomlRB' do
      context 'with TomlRB::ValueOverwriteError' do
        ...
        it 'raises a ParserError with the error message' do
          ...
          expect { result }.to raise_error(described_class::ParseError, error_message)
        end
      end
