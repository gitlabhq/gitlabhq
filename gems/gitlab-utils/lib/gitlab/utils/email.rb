...
module Gitlab
  module Utils
    module Email
      ...
      DEFAULT_EMAIL_VALIDATION_REGEXP = /\A[^@\s]+@[^@\s]+\z/

      class << self
        attr_writer :email_validation_regexp

        def email_validation_regexp
          @email_validation_regexp || DEFAULT_EMAIL_VALIDATION_REGEXP
        end
      end
      ...
      def normalize_email(email)
        ...
        return email unless Email.email_validation_regexp.match?(email.strip)
