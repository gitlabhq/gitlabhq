class MergeRequestDiff < ActiveRecord::Base
  ...
  def create_merge_request_diff_files(diffs)
    tuples = diffs.map.with_index do |diff, index|
      MergeRequestDiffFile.keys.map do |key|
        self.class.connection.quote(diff.public_send(key))
      end.unshift(index).unshift(self.id)
    end

    columns = [:merge_request_diff_id, :relative_order] + MergeRequestDiffFile.keys

    columns = columns.map do |column|
      self.class.connection.quote_column_name(column)
    end

    self.class.connection.execute <<-EOF.strip_heredoc
      INSERT INTO merge_request_diff_files (#{columns.join(', ')})
  ...
  def load_diffs(options)
    raw =
      if st_diffs.present? && valid_raw_diff?(st_diffs)
        st_diffs
