module DiffViewer
  class Base
    ...
    # These limit relate to the sum of the old and new blob sizes.
    ...
    def self.can_render?(diff_file, verify_binary: true)
      ...
      can_render = true
      can_render &&= can_render_blob?(old_blob, verify_binary: verify_binary) if old_blob
      can_render &&= can_render_blob?(new_blob, verify_binary: verify_binary) if new_blob
      can_render
    end
