...
class Note < ApplicationRecord
  ...
  SHARDING_KEYS = %i[namespace_id project_id organization_id].freeze
  ...
  validates_with ExactlyOnePresentValidator,
    fields: SHARDING_KEYS,
    if: :validate_sharding_keys?
  ...
  # Legacy rows the cleanup BBM is still draining stay saveable; a key that is
  # present and differs from the stored row was introduced by this write. Read
  # from the database rather than dirty state so repeated validation agrees.
  def validate_sharding_keys?
    new_record? || SHARDING_KEYS.any? do |key|
      self[key].present? && self[key] != attribute_in_database(key)
    end
  end
