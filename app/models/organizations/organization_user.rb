...
module Organizations
  class OrganizationUser < ApplicationRecord
    ...
    def self.create_organization_record_for(user_id, organization_id)
      # Use upsert directly rather than find_by + upsert to avoid an extra SELECT per member.
      # The ON CONFLICT DO NOTHING clause (via on_duplicate: :skip) handles the case where the
      # record already exists, so the preceding find_by is redundant.
      #
