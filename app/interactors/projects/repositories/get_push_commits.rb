  ...
  class GetPushCommits < Projects::Base
    def perform
      ...
      if push_to_new_branch?(ref, oldrev)
        ...
        if default_branch?(ref, project)
          # Initial push to the default branch.
          # Take the full history of that branch as "newly pushed".
          push_commits = repository.commits(newrev)
        else
          ...
          push_commits = repository.commits_between(project.default_branch,
                                                    newrev)
        end
      else
    ...
    def default_branch?(ref, project)
