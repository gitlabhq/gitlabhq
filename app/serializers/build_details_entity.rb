class BuildDetailsEntity < JobEntity
  ...
  expose :erase_path, if: -> (*) { build.erasable? && can?(current_user, :update_build, project) } do |build|
    erase_namespace_project_job_path(*project, build)
  end
  ...
  expose :merge_request, if: -> (*) { can?(current_user, :read_merge_request, build.merge_request) } do
    ...
    expose :path do |build|
      namespace_project_merge_request_path(*project, build.merge_request)
    end
  end

  expose :new_issue_path, if: -> (*) { can?(request.current_user, :create_issue, project) && build.failed? } do |build|
    new_namespace_project_issue_path(*project, issue: build_failed_issue_options)
  end

  expose :raw_path do |build|
    raw_namespace_project_job_path(*project, build)
  end
  ...
  def build_failed_issue_options
    { title: "Build Failed ##{build.id}",
      description: namespace_project_job_path(*project, build) }
  end
