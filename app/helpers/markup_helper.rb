...
module MarkupHelper
  ...
  def link_to_markdown_field(object, field, url, html_options = {}, strip_links: false)
    rendered_field = markdown_field(object, field)

    link_to_html(rendered_field, url, html_options, strip_links: strip_links)
  end
  ...
  #
  # When +strip_links+ is true, all anchors are unwrapped (their inner content is
  # kept) and the whole fragment is wrapped in a single link. This is used
  # where the rendered HTML is itself placed inside a link, and mirrors the
  # frontend `titleInLinkSafeHtmlConfig` DOMPurify config.
  def link_to_html(redacted, url, html_options = {}, strip_links: false)
    fragment = Nokogiri::HTML5::DocumentFragment.parse(redacted)

    if strip_links
      fragment.css('a').each { |node| node.replace(node.children) }
      return link_to(fragment.to_html.html_safe, url, html_options)
    end
