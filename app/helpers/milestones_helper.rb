module MilestonesHelper
  def milestones_filter_path(opts = {})
    if @project
      namespace_project_milestones_path(*@project, opts)
    elsif @group
  ...
  def milestones_label_path(opts = {})
    if @project
      namespace_project_issues_path(*@project, opts)
    elsif @group
  ...
  def milestones_filter_dropdown_path
    ...
    if project
      namespace_project_milestones_path(*project, :json)
    else
  ...
  def milestone_merge_request_tab_path(milestone)
    if @project
      merge_requests_namespace_project_milestone_path(*@project, milestone, format: :json)
    elsif @group
  ...
  def milestone_participants_tab_path(milestone)
    if @project
      participants_namespace_project_milestone_path(*@project, milestone, format: :json)
    elsif @group
  ...
  def milestone_labels_tab_path(milestone)
    if @project
      labels_namespace_project_milestone_path(*@project, milestone, format: :json)
    elsif @group
