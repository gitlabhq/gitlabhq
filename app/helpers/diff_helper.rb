module DiffHelper
  ...
  def diff_file_blob_raw_path(diff_file)
    namespace_project_raw_path(*@project, tree_join(diff_file.content_sha, diff_file.file_path))
  end

  def diff_file_old_blob_raw_path(diff_file)
    sha = diff_file.old_content_sha
    return unless sha
    namespace_project_raw_path(*@project, tree_join(diff_file.old_content_sha, diff_file.old_path))
  end

  def diff_file_html_data(project, diff_file_path, diff_commit_id)
    {
      blob_diff_path: namespace_project_blob_diff_path(*project,
  ...
  def diff_render_error_options(viewer)
    ...
    blob_url = namespace_project_blob_path(*@project, tree_join(diff_file.content_sha, diff_file.file_path))
  ...
  def commit_diff_whitespace_link(project, commit, options)
    url = namespace_project_commit_path(*project, commit.id, params_with_whitespace)
    toggle_whitespace_link(url, options)
  end

  def diff_merge_request_whitespace_link(project, merge_request, options)
    url = diffs_namespace_project_merge_request_path(*project, merge_request, params_with_whitespace)
    toggle_whitespace_link(url, options)
  end

  def diff_compare_whitespace_link(project, from, to, options)
    url = namespace_project_compare_path(*project, from, to, params_with_whitespace)
    toggle_whitespace_link(url, options)
  end
