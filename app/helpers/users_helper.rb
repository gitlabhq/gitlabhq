...
def create_timestamps_by_project(user_project)
    projects = {}
    project_commit = {}
    timestamps_copy = {}
      ...
      if raw_repository.exists?
        ...
        commits_log.each do |k, v|
          if timestamps_copy.has_key?("#{k}")
            timestamps_copy["#{k}"].merge!({raw_repository.path_with_namespace => v.count})
          else
            hash = { "#{k}" => {raw_repository.path_with_namespace => v.count} }
            timestamps_copy.merge!(hash)
          end
        end
        project_commit = project_commits(timestamps_copy, raw_repository.path_with_namespace)
