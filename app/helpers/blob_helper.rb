module BlobHelper
  ...
  def edit_path(project = @project, ref = @ref, path = @path, options = {})
    namespace_project_edit_blob_path(*project,
                                     tree_join(ref, path),
                                     options[:link_opts])
  end
  ...
  def edit_blob_link(project = @project, ref = @ref, path = @path, options = {})
    ...
    # This condition applies to anonymous or users who can edit directly
      ...
      fork_path = namespace_project_forks_path(*project, namespace_key: current_user.namespace.id, continue: continue_params)
  ...
  def blob_raw_url
    if @build && @entry
      raw_namespace_project_job_artifacts_path(*@project, @build, path: @entry.path)
    elsif @snippet
      if @snippet.project_id
        raw_namespace_project_snippet_path(*@project, @snippet)
      else
        raw_snippet_path(@snippet)
      end
    elsif @blob
      namespace_project_raw_path(*@project, @id)
    end
  end
  ...
  def contribution_options(project)
    ...
    if can?(current_user, :create_issue, project)
      options << link_to("submit an issue", new_namespace_project_issue_path(*project))
    end

    merge_project = can?(current_user, :create_merge_request, project) ? project : (current_user && current_user.fork_of(project))
    if merge_project
      options << link_to("create a merge request", new_namespace_project_merge_request_path(*project))
    end
