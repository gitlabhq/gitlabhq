...
(() => {
  class SmartInterval {
    /**
      ...
      * @param { milliseconds } delayStartBy Add delay before 1st iteration
      * @param { boolean } runInBackground Toggle to continue polling when browser tab is hidden
      * @param { boolean } runInCache Toggle to continue polling after page:before-unload
      * @param { boolean } logIterations Logs each iteration of smartInterval.
      */
    constructor({ callback, startingInterval, maxInterval, incrementByFactorOf, lazyStart = false,
      delayStartBy = 0, runInBackground = false, runInCache = false, logIterations = false }) {
      this.cfg = {
        callback,
        startingInterval,
        maxInterval,
