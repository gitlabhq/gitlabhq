...
- **Skip CI when agent pushes draft commits** — push branches with `git push -o ci.skip` on commits to draft MRs, or intermediary commits, never `[ci skip]`/`[skip ci]` in the commit message, since that travels with history and could suppress the pipeline on the default branch if the commit becomes its tip; a full MR pipeline here is roughly 600 jobs across 6 pipelines, wasted on every push while work is still in progress. Combine push options freely, e.g. `git push -o ci.skip -o merge_request.create -o merge_request.draft`. This leaves the MR pipeline `skipped`, which this project won't merge until a full pipeline is run — intentional, so a human decides when it's worth it.
