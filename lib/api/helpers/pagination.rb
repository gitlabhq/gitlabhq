    ...
    module Pagination
      ...
      class KeysetPaginationInfo
        ...
        def keys
          ...
          raise "precondition failed: only one order_value supported, given: #{relation.order_values.map { |v| v.expr.name }}" if relation.order_values.size > 1
      ...
      class KeysetPaginationStrategy
        ...
        def paginate(relation)
          ...
          paged_relation.tap do |data|
            add_default_pagination_headers

            if data.last
              next_page_params = pagination.params_for(data.last)
        ...
        def conditions(pagination)
          fields = pagination.keys.zip(pagination.values).reject { |_, v| v.nil? }.to_h
