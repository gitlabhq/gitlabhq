  ...
  class Runner < Grape::API
    ...
    resource :runners do
      ...
      post '/running' do
        ...
        present current_runner.running_details
      end
    end
    ...
    resource :jobs do
      ...
      get '/pending' do
        ...
        if Feature.enabled?(:ci_pending_jobs_raw_sql)
          ...
          builds = builds.map do |build|
            {
              ...
              condition: {
                and: [{
                    or: [
                      ( { param: :group_ids, contains: build[2].to_s } if build[5] ),
                      { param: :project_ids, contains: build[1].to_s },
                      ( { param: :shared, contains: true.to_s } if build[4] ),
                    ].compact
                  },
                  ( {
