  ...
  class Runner < Grape::API
    ...
    resource :jobs do
      ...
      params do
        ...
        optional 'artifact_type', type: String, default: 'zip', desc: %q(Type of the artifact (`archive`, `junit` or `codeclimate`))
        optional 'artifact_format', type: String, default: 'archive', desc: %q(Format of the artifact (`zip` or `gzip`))
      ...
      post '/:id/artifacts' do
        ...
        bad_request!("Undefined artifact type") unless Ci::JobArtifact.file_types.key?(params['artifact_type'])
        bad_request!("Undefined artifact format") unless Ci::JobArtifact.file_formats.key?(params['artifact_format'])

        artifact_type = params['artifact_type']
        artifact_format = params['artifact_format']

        bad_request!("Already uploaded") if job.public_send("job_artifacts_#{artifact_type}")
        ...
        Ci::JobArtifact.create!(
          ...
          # artifact_format: params['artifact_format'].to_sym, # TODO: Do we persist artifact_format?
          file_sha256: artifacts.sha256,
          expire_in: expire_in) # TODO: Do we respect the archive.zip's expire_in?
