  ...
  class Runner < Grape::API
    ...
    resource :jobs do
      ...
      params do
        ...
        optional 'artifact_format', type: String, desc: %q(Format of the artifact (`zip` or `raw`))
        optional 'artifact_type', type: String, desc: %q(Type of the artifact (`archive`, `junit` or `codeclimate`))
      ...
      post '/:id/artifacts' do
        ...
        if params['artifact_type'] == 'junit'
          bad_request!("Undefined artifact_format") unless Ci::JobArtifact.file_formats.key?(params['artifact_format'])

          job.build_job_artifacts_junit(
            project: job.project,
            file: artifacts,
            file_type: params['artifact_type'].to_sym,
