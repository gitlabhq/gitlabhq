    ...
    module Build
      class Rules::Rule::Clause::Exists < Rules::Rule::Clause
        ...
        def complex_exists_enabled?
          # We can't use `YamlProcessor::FeatureFlags.enabled?(:ci_support_rules_exists_paths_and_project` here
          # because this class can be called in `Ci::Pipeline::Chain::EvaluateWorkflowRules` or
          # `Ci::Pipeline::Chain::Seed`, which do not use a `Ci::YamlProcessor::FeatureFlags.with_actor` block.
          # Instead, we can simply check if the value is a Hash because it can only be a Hash if the FF was
          # enabled when `Ci::YamlProcessor` ran, and it always runs before we reach here.
          @clause.is_a?(Hash)
        end
      end
    end
