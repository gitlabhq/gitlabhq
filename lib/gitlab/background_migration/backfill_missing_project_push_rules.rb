...
module Gitlab
  module BackgroundMigration
    class BackfillMissingProjectPushRules < BatchedMigrationJob
      operation_name :backfill_missing_project_push_rules
      feature_category :source_code_management

      def perform
        each_sub_batch do |sub_batch|
          backfill_missing_project_push_rules(sub_batch)
        end
      end

      def backfill_missing_project_push_rules(sub_batch)
