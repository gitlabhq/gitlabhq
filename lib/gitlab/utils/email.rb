  ...
  module Utils
    module Email
      ...
      def obfuscate_emails_in_text(text)
        ...
        matches = text.scan(EMAIL_REGEXP_WITH_CAPTURING_GROUP).flatten.index_with do |match|
          obfuscated_email(match, deform: true)
        end

        text.gsub(Regexp.union(matches.keys), matches)
      end
