...
module Gitlab
  module Database
    module Aggregation
      module ClickHouse
        class ChurnedCount < BitmapWindow
          def initialize(name, type = :integer, expression = nil, over:, lag_offset: 1, **kwargs)
            super(name, type, expression, operation: :reverse_difference, over: over, lag_offset: lag_offset, **kwargs)
          end
          ...
          # sort by it. GROUP BY ALL adds it to the inner grouping key, and the
          # :reverse_difference operation applies arrayDistinct, so repeated values do not
          # inflate the count.
