...
module Gitlab
  module Database
    module Aggregation
      module ClickHouse
        class Min < MetricDefinition
          def initialize(name, type = :float, expression = nil, **kwargs)
            super
          end

          def identifier
            dotted_name? ? name : :"min_#{name}"
          end
          ...
          def to_outer_arel(context)
            ...
            if secondary_expression
              inner_condition_column = Arel::Table.new(context[:inner_query_name])[context.fetch(:local_secondary_alias,
                name)]
              Arel::Nodes::NamedFunction.new('minIf', [inner_column, inner_condition_column.eq(1)])
            else
              Arel::Nodes::NamedFunction.new('min', [inner_column])
            end
          end
        end
      end
