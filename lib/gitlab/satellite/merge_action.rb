module Gitlab
  module Satellite
    ...
    class MergeAction < Action
      ...
      def diff_in_satellite
        in_locked_and_timed_satellite do |merge_repo|
          ...
          if merge_request.for_fork?
            diff = merge_repo.git.native(:diff, {timeout: 30, raise: true}, "origin/#{merge_request.target_branch}", "source/#{merge_request.source_branch}")
          else
            diff = merge_repo.git.native(:diff, {timeout: 30, raise: true}, "#{merge_request.target_branch}", "#{merge_request.source_branch}")
      ...
      def diffs_in_satellite
        in_locked_and_timed_satellite do |merge_repo|
          prepare_satellite!(merge_repo)
          update_satellite_source_and_target!(merge_repo)
          if merge_request.for_fork?
            common_commit = merge_repo.git.native(:merge_base, {}, ["origin/#{merge_request.target_branch}", "source/#{merge_request.source_branch}"]).strip
            diffs = merge_repo.diff(common_commit, "source/#{merge_request.source_branch}")
          else
            common_commit = merge_repo.git.native(:merge_base, {}, ["#{merge_request.target_branch}", "#{merge_request.source_branch}"]).strip
      ...
      def format_patch
        in_locked_and_timed_satellite do |merge_repo|
          ...
          update_satellite_source_and_target!(merge_repo)
          if (merge_request.for_fork?)
            patch = merge_repo.git.format_patch({timeout: 30, raise: true, stdout: true}, "origin/#{merge_request.target_branch}", "source/#{merge_request.source_branch}")
          else
            patch = merge_repo.git.format_patch({timeout: 30, raise: true, stdout: true}, "#{merge_request.target_branch}..#{merge_request.source_branch}")
          end
          return patch
      ...
      def update_satellite_source_and_target!(repo)
        ...
        if  merge_request.for_fork?
          ...
          if repo.heads.map(&:name).include? "#{merge_request.source_branch}"
            #puts "#{merge_request.source_branch} found heads:#{repo.heads.map(&:name)}"
            repo.git.checkout({raise: true, timeout: true, b: true}, merge_request.source_branch, "origin/#{merge_request.source_branch}")
            repo.git.checkout({raise: true, timeout: true, b: true}, merge_request.target_branch, "origin/#{merge_request.target_branch}")
          else
            #puts "#{merge_request.source_branch} not found heads:#{repo.heads.map(&:name)}"
            repo.git.checkout({raise: true, timeout: true}, "#{merge_request.source_branch}")
            repo.git.checkout({raise: true, timeout: true}, "#{merge_request.target_branch}")
          end
