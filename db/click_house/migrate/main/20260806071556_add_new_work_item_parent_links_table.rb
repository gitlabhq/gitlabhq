...
class AddNewWorkItemParentLinksTable < ClickHouse::Migration
  def up
    execute <<-SQL
      CREATE TABLE IF NOT EXISTS siphon_work_item_parent_links_tmp
      (
        id Int64 CODEC(DoubleDelta, ZSTD),
        work_item_id Int64 CODEC(DoubleDelta, ZSTD),
  ...
  def down
    execute <<-SQL
      DROP TABLE IF EXISTS siphon_work_item_parent_links_tmp
    SQL
  end
end
