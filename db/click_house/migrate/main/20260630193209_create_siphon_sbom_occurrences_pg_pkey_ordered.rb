...
class CreateSiphonSbomOccurrencesPgPkeyOrdered < ClickHouse::Migration
  def up
    execute <<-SQL
  ...
  def down
    execute "DROP VIEW IF EXISTS siphon_sbom_occurrences_pg_pkey_ordered_mv"
    execute "DROP TABLE IF EXISTS siphon_sbom_occurrences_pg_pkey_ordered"
  end
end
