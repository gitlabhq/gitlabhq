...
class CreateMaterializedViewMergeRequestsEnrichedMv < ClickHouse::Migration
  def up
    # APPEND is load-bearing: without it a refresh replaces the whole table.
    #
    # The window is 5x the interval, so four missed refreshes still get caught.
    # Kept short on purpose: every key in the window is re-emitted on every pass,
    # and at production write rates a wider window is mostly redundant writes.
    #
    # _siphon_replicated_at is the source version + 1us, not now64(): enough to
    # beat the unenriched row, never enough to beat a later delete tombstone.
    #
    # _siphon_watermark carries base.seen through, so it keeps meaning "when CDC
    # last saw this row" rather than "when it was last enriched".
    execute <<~SQL
      CREATE MATERIALIZED VIEW IF NOT EXISTS merge_requests_enriched_mv
      REFRESH EVERY 1 MINUTE APPEND TO merge_requests
      AS
      WITH
        base AS (
          SELECT * FROM (
            SELECT * FROM merge_requests_base FINAL
            WHERE seen > now64(6, 'UTC') - INTERVAL 5 MINUTE
          )
        ),
        ...
        siphon_duo_workflows_workflow_merge_requests_cte AS (SELECT
          merge_request_id,
        ...
        FROM (
          ...
          WHERE merge_request_id IN (SELECT id from base)
  ...
  def down
    execute 'DROP VIEW IF EXISTS merge_requests_enriched_mv'
  end
end
