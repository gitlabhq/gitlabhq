...
class CreateSiphonPCiBuildsMetadataPgPkeyOrdered < ClickHouse::Migration
  def up
    execute <<-SQL
    ...
    execute <<-SQL
      CREATE MATERIALIZED VIEW IF NOT EXISTS siphon_p_ci_builds_metadata_pg_pkey_ordered_mv
      TO siphon_p_ci_builds_metadata_pg_pkey_ordered
      AS
      SELECT
        id,
        partition_id,
