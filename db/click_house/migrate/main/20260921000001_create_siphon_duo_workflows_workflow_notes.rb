...
class CreateSiphonDuoWorkflowsWorkflowNotes < ClickHouse::Migration
  def up
    execute <<-SQL
      CREATE TABLE IF NOT EXISTS siphon_duo_workflows_workflow_notes
      (
  ...
  def down
    execute <<-SQL
      DROP TABLE IF EXISTS siphon_duo_workflows_workflow_notes
    SQL
  end
end
