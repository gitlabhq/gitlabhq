...
class AddCreatedByDuoToMergeRequestsMv < ClickHouse::Migration
  DUO_CTE = <<~SQL
    ,
  ...
  def up
    execute modify_query_sql(duo_cte: DUO_CTE, duo_column: DUO_COLUMN, duo_join: DUO_JOIN)
  end

  def down
    execute modify_query_sql(duo_cte: '', duo_column: '', duo_join: '')
  end

  private

  # Query mirrors CreateMaterializedViewMergeRequests; MODIFY QUERY swaps it atomically
  # so no siphon_merge_requests inserts are missed, unlike DROP + CREATE.
  def modify_query_sql(duo_cte:, duo_column:, duo_join:)
    <<~SQL
      ALTER TABLE merge_requests_mv MODIFY QUERY
      WITH
      ...
      SELECT
        ...
        siphon_award_emoji_cte.award_emojis AS award_emojis#{duo_column}
      FROM base
...
base.id = siphon_award_emoji_cte.merge_request_id#{duo_join}
