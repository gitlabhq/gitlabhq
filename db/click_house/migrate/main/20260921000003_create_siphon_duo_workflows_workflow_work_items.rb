...
class CreateSiphonDuoWorkflowsWorkflowWorkItems < ClickHouse::Migration
  def up
    execute <<-SQL
      CREATE TABLE IF NOT EXISTS siphon_duo_workflows_workflow_work_items
      (
  ...
  def down
    execute <<-SQL
      DROP TABLE IF EXISTS siphon_duo_workflows_workflow_work_items
    SQL
  end
end
