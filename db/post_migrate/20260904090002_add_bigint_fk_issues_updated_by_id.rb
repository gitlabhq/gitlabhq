...
class AddBigintFkIssuesUpdatedById < Gitlab::Database::Migration[2.3]
  ...
  BIGINT_COLUMN = :updated_by_id_convert_to_bigint
  FK_NAME = :fk_ffed080f01

  def up
    return if skip_migration?

    # rubocop:disable Migration/ForeignKeysToDestroyServiceTables -- duplicates the existing
    # fk_ffed080f01 on the bigint shadow column, so it introduces no new dependent records
