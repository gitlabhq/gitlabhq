...
class AddBigintFkIssuesMovedToId < Gitlab::Database::Migration[2.3]
  ...
  BIGINT_COLUMN = :moved_to_id_convert_to_bigint
  FK_NAME = :fk_a194299be1

  def up
    return if skip_migration?

    # rubocop:disable Migration/ForeignKeysToDestroyServiceTables -- duplicates the existing
    # fk_a194299be1 on the bigint shadow column, so it introduces no new dependent records
