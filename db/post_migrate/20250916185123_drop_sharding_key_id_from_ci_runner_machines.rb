...
class DropShardingKeyIdFromCiRunnerMachines < Gitlab::Database::Migration[2.3]
  ...
  def down
    ...
    {
      instance_type_ci_runner_machines: 'idx_inst_type_ci_runner_machines_on_sharding_key_when_not_null',
      group_type_ci_runner_machines: 'idx_group_type_ci_runner_machines_on_sharding_key_when_not_null',
      project_type_ci_runner_machines: 'idx_proj_type_ci_runner_machines_on_sharding_key_when_not_null'
    ...
    with_lock_retries do
      add_index( # rubocop:disable Migration/AddIndex -- no need for add_concurrent_index as this is just the routing table
        TABLE_NAME,
        COLUMN_NAME,
        name: 'index_ci_runner_machines_on_sharding_key_id_when_not_null',
        **opts
      )
    end
