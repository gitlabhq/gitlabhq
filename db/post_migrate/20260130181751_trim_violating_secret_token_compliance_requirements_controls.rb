...
class TrimViolatingSecretTokenComplianceRequirementsControls < Gitlab::Database::Migration[2.3]
  milestone '18.10'
  ...
  class ComplianceRequirementsControl < MigrationRecord
    include EachBatch
    ...
    ComplianceRequirementsControl
      ...
      .each_batch(of: BATCH_SIZE) do |batch|
        ...
        batch.each do |record|
          ...
          next unless plaintext && plaintext.bytesize > PLAINTEXT_LIMIT

          record.secret_token = plaintext.byteslice(0, PLAINTEXT_LIMIT)
        ...
        values = updates.map do |row|
          ...
          "(#{row[:id]}, #{token_hex}, #{iv_hex})"
        end.join(",\n  ")

        sql = <<~SQL
          UPDATE compliance_requirements_controls
          SET
            encrypted_secret_token = t.encrypted_secret_token,
            encrypted_secret_token_iv = t.encrypted_secret_token_iv
          FROM (VALUES #{values}) AS t(id, encrypted_secret_token, encrypted_secret_token_iv)
          WHERE compliance_requirements_controls.id = t.id
        SQL
