...
class DropNamespaceGroupPushRulesSyncTriggers < Gitlab::Database::Migration[2.3]
  include Gitlab::Database::SchemaHelpers

  disable_ddl_transaction!
  milestone '19.0'

  NAMESPACE_TRIGGER_NAME = 'trigger_sync_namespace_to_group_push_rules'
  NAMESPACE_FUNCTION_NAME = 'sync_namespace_to_group_push_rules'
  PUSH_RULES_TRIGGER_NAME = 'trigger_sync_push_rules_to_group_push_rules'
  PUSH_RULES_FUNCTION_NAME = 'sync_push_rules_to_group_push_rules'

  def up
    drop_trigger(:namespaces, NAMESPACE_TRIGGER_NAME)
    drop_trigger(:push_rules, PUSH_RULES_TRIGGER_NAME)
    drop_function(NAMESPACE_FUNCTION_NAME)
    drop_function(PUSH_RULES_FUNCTION_NAME)
  end

  def down
    execute(<<~SQL)
      CREATE OR REPLACE FUNCTION #{NAMESPACE_FUNCTION_NAME}() RETURNS trigger
          LANGUAGE plpgsql
          AS $$
      DECLARE
        push_rule RECORD;
    ...
    execute(<<~SQL)
      CREATE OR REPLACE FUNCTION #{PUSH_RULES_FUNCTION_NAME}() RETURNS trigger
          LANGUAGE plpgsql
          AS $$
      BEGIN
    ...
    create_trigger(:namespaces, NAMESPACE_TRIGGER_NAME, NAMESPACE_FUNCTION_NAME, fires: 'AFTER UPDATE',
      replace: true) do
      'WHEN (OLD.push_rule_id IS DISTINCT FROM NEW.push_rule_id)'
    end

    create_trigger(:push_rules, PUSH_RULES_TRIGGER_NAME, PUSH_RULES_FUNCTION_NAME, fires: 'AFTER UPDATE', replace: true)
  end
end
