...
class CreateBackgroundOperationWorkers < Gitlab::Database::Migration[2.3]
  ...
  def down
    drop_table(:background_operation_workers, if_exists: true)
  end
  ...
  def add_indexes
    add_concurrent_partitioned_index(
      ...
      :status,
      name: 'index_background_operation_workers_by_status'
    )
    add_concurrent_partitioned_index(
      :background_operation_workers,
      :organization_id,
      name: 'index_background_operation_workers_by_organization'
    )
    add_concurrent_partitioned_index(
      :background_operation_workers,
      :user_id,
      name: 'index_background_operation_workers_by_user'
    )
    add_concurrent_partitioned_index(
      :background_operation_workers,
      :created_at,
      name: 'index_background_operation_workers_by_created_at'
    )
    add_concurrent_partitioned_index(
      :background_operation_workers,
  ...
  def add_constraints
    add_check_constraint(
      ...
      check_constraint_name(:background_operation_workers, 'batch_size', 'greater_than_sub_batch_size')
    )
    add_check_constraint(
      :background_operation_workers,
      '(sub_batch_size > 0)',
      check_constraint_name(:background_operation_workers, 'sub_batch_size', 'greater_than_zero')
    )
    add_check_constraint(
      :background_operation_workers,
    ...
    add_check_constraint(
      ...
      check_constraint_name(:background_operation_workers, 'cursors', 'not_null')
    )
  end
end
