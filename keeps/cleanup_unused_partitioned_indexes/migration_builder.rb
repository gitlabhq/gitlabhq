...
module Keeps
  class CleanupUnusedPartitionedIndexes < ::Gitlab::Housekeeper::Keep
    class MigrationBuilder
      ...
      # Cop/FilenameLength caps migration filenames at 100 chars including the
      # 15-char timestamp prefix and .rb, so the name part must stay <= 82.
      # The SHA suffix disambiguates names that would otherwise truncate to
      # the same value.
      def unique_migration_name_for(index_name)
        base = "remove_unused_index_#{index_name}"
        return base if base.length <= 82
      ...
      # Dropping a partitioned parent index cascades to every partition and
      # cannot run concurrently; the helper wraps it in with_lock_retries.
