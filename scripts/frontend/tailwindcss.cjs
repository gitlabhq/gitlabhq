...
const { createProcessor } = require('tailwindcss/lib/cli/build/plugin.js');

const ROOT_PATH = path.resolve(__dirname, '../../');

async function build({ shouldWatch = false, content = false } = {}) {
...
function viteTailwindCompilerPlugin({ shouldWatch = true }) {
  return {
...
function webpackTailwindCompilerPlugin({ shouldWatch = true }) {
  return {
    async start() {
      return build({ shouldWatch });
    },
