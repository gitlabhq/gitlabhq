...
def check_category_names(categories, context, valid_names, lower_map, errors)
  categories.each do |cat|
    ...
    errors << if lower_map.key?(cat.downcase)
                "  CASE MISMATCH #{context}\n               '#{cat}' should be '#{lower_map[cat.downcase]}'"
              else
                "  UNKNOWN      #{context}\n               '#{cat}' not found in categories.yml"
              end
  end
end
...
def check_frontmatter_categories(content, valid_names, lower_map)
  ...
  errors = []
  check_category_names(categories, 'frontmatter', valid_names, lower_map, errors)
  errors
end
...
def check_h3_categories(content, valid_names, lower_map)
  ...
  errors.concat(
    features.select { |_heading, cats| cats.nil? }.map do |heading, _cats|
      "  MISSING tag  ### #{heading}\n               All features must have a category."
    end
  )

  tagged.each do |heading, categories|
    check_category_names(categories, "### #{heading}", valid_names, lower_map, errors)
  end
  errors
end
...
def check_body_comment_categories(content, valid_names, lower_map)
  ...
  errors = []
  check_category_names(categories, 'body comment', valid_names, lower_map, errors)
  errors
end
...
def validate_file(file_path, valid_names)
  ...
  errors = check_frontmatter_categories(content, valid_names, lower_map) ||
    ...
    ["  MISSING categories\n               No 'categories' field in frontmatter or body comment."]
...
def load_valid_names(categories_url)
  ...
  valid_names
rescue OpenURI::HTTPError, SocketError, OpenSSL::SSL::SSLError, Net::OpenTimeout, Net::ReadTimeout, ArgumentError => e
...
def main
  ...
  valid_names = load_valid_names(options[:categories_url])

  file_paths = expand_file_paths(options[:release_notes])
